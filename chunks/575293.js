...
function M(e) {
    ...
    i.useEffect(() => {
        _
            ? r.A.timing(M, { toValue: 0, duration: A ? 1 : 350, easing: r.A.Easing.quad, delay: 500 * !E }).start(() =>
                  j(!0),
              )
            : r.A.timing(M, { toValue: 1, duration: A ? 1 : 350, easing: r.A.Easing.quad, delay: 400 }).start();
    }, [_, M, E, A]),
        i.useEffect(() => {
            _ && N && r.A.timing(O, { toValue: 1, duration: 350 * !A, easing: r.A.Easing.quad, delay: 400 }).start();
        }, [_, O, N, A]);
    let P = i.useCallback(() => {
...
function O(e) {
    ...
    return r || a || !s || !c ? null : (0, l.jsx)(M, { guildId: t, channel: n, className: i });
}
