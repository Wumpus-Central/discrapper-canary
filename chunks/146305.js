n.d(t, { Z: () => eE });
var r = n(200651),
    i = n(990547),
    s = n(704215),
    a = n(481060),
    l = n(37234),
    o = n(493544),
    c = n(132871),
    d = n(147890),
    u = n(353093),
    m = n(593128),
    p = n(979264),
    g = n(688465),
    h = n(676317),
    f = n(874993),
    b = n(45966),
    x = n(637853),
    j = n(352981),
    N = n(610403),
    v = n(923726),
    _ = n(303737),
    O = n(15030),
    y = n(220685),
    C = n(536442),
    I = n(84615),
    E = n(575258),
    S = n(276687),
    T = n(594174),
    P = n(585483),
    w = n(823379),
    R = n(434404),
    D = n(360606),
    Z = n(946724),
    A = n(999382),
    k = n(44550),
    W = n(853714),
    L = n(359191),
    M = n(84658),
    U = n(142961),
    G = n(730910),
    B = n(475013),
    F = n(213956),
    z = n(702478),
    H = n(247031),
    V = n(787561),
    Y = n(579648),
    K = n(511480),
    q = n(460461),
    X = n(29605),
    Q = n(365307),
    J = n(497666),
    $ = n(324330),
    ee = n(495300),
    et = n(312397),
    en = n(651780),
    er = n(279542),
    ei = n(413335),
    es = n(665324),
    ea = n(208150),
    el = n(353398),
    eo = n(365774),
    ec = n(803288),
    ed = n(804153),
    eu = n(486358),
    em = n(942478),
    ep = n(890861),
    eg = n(708510),
    eh = n(237245),
    ef = n(658666),
    eb = n(224726),
    ex = n(308619),
    ej = n(254925),
    eN = n(981631),
    ev = n(308083),
    e_ = n(135899),
    eO = n(388032),
    ey = n(964644);
function eC(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            r = Object.keys(n);
        'function' == typeof Object.getOwnPropertySymbols &&
            (r = r.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                })
            )),
            r.forEach(function (t) {
                var r;
                (r = n[t]),
                    t in e
                        ? Object.defineProperty(e, t, {
                              value: r,
                              enumerable: !0,
                              configurable: !0,
                              writable: !0
                          })
                        : (e[t] = r);
            });
    }
    return e;
}
function eI(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : (function (e, t) {
                  var n = Object.keys(e);
                  if (Object.getOwnPropertySymbols) {
                      var r = Object.getOwnPropertySymbols(e);
                      n.push.apply(n, r);
                  }
                  return n;
              })(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
let eE = {
    getSectionDefinition: function (e, t) {
        var eE;
        let eS = (0, j.p)(t.guild.id, 'guild_settings'),
            eT = !1 === t.guild.hasFeature(eN.oNc.CREATOR_MONETIZABLE) && !1 === t.guild.hasFeature(eN.oNc.CREATOR_MONETIZABLE_PROVISIONAL),
            eP =
                eS && eT
                    ? {
                          label: (0, r.jsx)('div', {
                              className: ey.roleSubLabel,
                              children: eO.NW.string(eO.t.Tn7Koq)
                          }),
                          ariaLabel: eO.NW.string(eO.t.Tn7Koq)
                      }
                    : {
                          label: (0, r.jsx)('div', {
                              className: ey.roleSubLabel,
                              children: eO.NW.string(eO.t['KzCF//'])
                          }),
                          ariaLabel: eO.NW.string(eO.t['KzCF//'])
                      },
            ew = t.guild.hasFeature(eN.oNc.COMMUNITY) && t.canViewGuildAnalytics,
            eR = (null === (eE = t.guild.profile) || void 0 === eE ? void 0 : eE.badge) != null ? (0, u.ky)(t.guild.id, t.guild.profile.badge, ev.NC.SIZE_16) : null;
        switch (e) {
            case eN.pNK.PROFILE:
                return {
                    section: eN.pNK.PROFILE,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_PROFILE,
                    label: eO.NW.string(eO.t.txdaxc),
                    ariaLabel: eO.NW.string(eO.t.txdaxc),
                    element: eb.Z,
                    type: o.bT.SCROLLABLE_CUSTOM,
                    notice: {
                        stores: [A.Z],
                        element: ex.Z
                    }
                };
            case eN.pNK.TAG:
                return {
                    section: eN.pNK.TAG,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_TAG,
                    label: eO.NW.string(eO.t['2QmKZ2']),
                    ariaLabel: eO.NW.string(eO.t['2QmKZ2']),
                    element: ej.Z,
                    predicate: () => !1
                };
            case eN.pNK.ENGAGEMENT:
                return {
                    section: eN.pNK.ENGAGEMENT,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_ENGAGEMENT,
                    label: eO.NW.string(eO.t['6TAHb2']),
                    ariaLabel: eO.NW.string(eO.t['6TAHb2']),
                    element: eu.Z,
                    notice: {
                        stores: [A.Z],
                        element: em.R
                    }
                };
            case eN.pNK.INVITES:
                return {
                    section: eN.pNK.INVITES,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_INVITES_V2,
                    label: eO.NW.string(eO.t.YOt3Oj),
                    ariaLabel: eO.NW.string(eO.t.YOt3Oj),
                    element: ep.Z,
                    type: o.bT.CUSTOM,
                    predicate: () => t.canManageGuild
                };
            case eN.pNK.OVERVIEW:
                return {
                    section: eN.pNK.OVERVIEW,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_OVERVIEW,
                    label: eO.NW.string(eO.t['/dp6yc']),
                    ariaLabel: eO.NW.string(eO.t['/dp6yc']),
                    element: et.Z,
                    notice: {
                        stores: [A.Z],
                        element: et.O
                    }
                };
            case eN.pNK.ROLES:
                return {
                    section: eN.pNK.ROLES,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_ROLES,
                    label: eO.NW.string(eO.t.LPJmLy),
                    element: B.Z,
                    type: o.bT.MINIMAL,
                    ariaLabel: eO.NW.string(eO.t.KXcECw),
                    notice: {
                        stores: [Z.Z],
                        element: B._
                    },
                    predicate: () => t.canManageRoles
                };
            case eN.pNK.EMOJI:
                return {
                    section: eN.pNK.EMOJI,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_EMOJI,
                    label: eO.NW.string(eO.t.sMOuub),
                    element: X.ZP,
                    predicate: () => t.canManageGuildExpressions
                };
            case eN.pNK.STICKERS:
                return {
                    section: eN.pNK.STICKERS,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_STICKERS,
                    label: eO.NW.string(eO.t.R5nQkZ),
                    ariaLabel: eO.NW.string(eO.t.R5nQkZ),
                    element: ea.Z,
                    predicate: () => t.canManageGuildExpressions
                };
            case eN.pNK.SOUNDBOARD:
                return {
                    section: eN.pNK.SOUNDBOARD,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_SOUNDBOARD,
                    label: eO.NW.string(eO.t.OWQNYm),
                    ariaLabel: eO.NW.string(eO.t.OWQNYm),
                    element: es.Z,
                    predicate: () => t.canManageGuildExpressions
                };
            case eN.pNK.WIDGET:
                return {
                    section: eN.pNK.WIDGET,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_WIDGET,
                    label: eO.NW.string(eO.t.ACjbTk),
                    element: ed.Z,
                    predicate: () => t.canManageGuild
                };
            case eN.pNK.GUILD_TEMPLATES:
                return {
                    section: eN.pNK.GUILD_TEMPLATES,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_TEMPLATE,
                    label: (0, r.jsx)(eo.Z, { showDirtyIndicator: t.showDirtyGuildTemplateIndicator }),
                    ariaLabel: eO.NW.string(eO.t.KUw7Sk),
                    element: el.ZP,
                    notice: {
                        stores: [el.Sf],
                        element: el.Q5
                    },
                    predicate: () => t.canManageGuild
                };
            case eN.pNK.VANITY_URL:
                return {
                    section: eN.pNK.VANITY_URL,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_VANITY_URL,
                    label: eO.NW.string(eO.t['5XZKy8']),
                    element: ec.Z,
                    notice: {
                        stores: [k.Z],
                        element: ec.T
                    },
                    predicate: () => t.isGuildAdmin
                };
            case eN.pNK.ACCESS:
                return {
                    section: eN.pNK.ACCESS,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_ACCESS,
                    label: eO.NW.string(eO.t['+hFV+P']),
                    element: eg.O,
                    notice: {
                        stores: [ef.Z],
                        element: eh.w
                    },
                    predicate: () => t.memberVerificationRolloutEnabled && t.canManageGuild,
                    ariaLabel: eO.NW.string(eO.t['+hFV+P'])
                };
            case eN.pNK.INTEGRATIONS:
                return {
                    section: eN.pNK.INTEGRATIONS,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_INTEGRATION,
                    label: eO.NW.string(eO.t.CIsNZ2),
                    ariaLabel: eO.NW.string(eO.t.CIsNZ2),
                    element: Q.Z,
                    notice: {
                        stores: [D.default, A.Z],
                        element: Q.j
                    },
                    predicate: () => t.canManageGuild || t.canManageWebhooks || t.canUnlinkChannels
                };
            case eN.pNK.APP_DIRECTORY:
                return {
                    section: eN.pNK.APP_DIRECTORY,
                    label: eO.NW.string(eO.t.AKcFUl),
                    ariaLabel: eO.NW.string(eO.t.AKcFUl),
                    onClick() {
                        (0, d.dx)({
                            guildId: t.guild.id,
                            entrypoint: {
                                name: c.n3.GUILD_SETTINGS,
                                guildSettingsSection: e
                            }
                        }),
                            (0, l.xf)();
                    }
                };
            case eN.pNK.SAFETY:
                return {
                    section: eN.pNK.SAFETY,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_SAFETY,
                    label: eO.NW.string(eO.t['suhY+f']),
                    ariaLabel: eO.NW.string(eO.t['suhY+f']),
                    element: F.Z,
                    newIndicatorDismissibleContentTypes: [s.z.COMMUNITY_GUILD_SETTINGS_SAFETY],
                    predicate: () => t.canManageGuild && t.guild.hasFeature(eN.oNc.COMMUNITY)
                };
            case eN.pNK.MODERATION:
                return {
                    section: eN.pNK.MODERATION,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_MODERATION,
                    label: eO.NW.string(eO.t['suhY+f']),
                    element: er.ZP,
                    predicate: () => t.canManageGuild && !t.guild.hasFeature(eN.oNc.COMMUNITY)
                };
            case eN.pNK.GUILD_AUTOMOD:
                return {
                    section: eN.pNK.GUILD_AUTOMOD,
                    label: (0, r.jsx)('div', {
                        className: ey.landingPageTabLink,
                        children: eO.NW.string(eO.t.uRelg4)
                    }),
                    ariaLabel: eO.NW.string(eO.t.uRelg4),
                    element: f.Z,
                    newIndicatorDismissibleContentTypes: null,
                    predicate: () => !t.guild.hasFeature(eN.oNc.COMMUNITY) && ((0, h.Nb)(t.guild.id) || (0, h.ze)(t.guild.id))
                };
            case eN.pNK.AUDIT_LOG:
                return {
                    section: eN.pNK.AUDIT_LOG,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_AUDIT_LOG,
                    label: eO.NW.string(eO.t.SPWLyc),
                    element: W.Z,
                    type: o.bT.CUSTOM,
                    predicate: () => t.canViewAuditLog
                };
            case eN.pNK.BANS:
                return {
                    section: eN.pNK.BANS,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_BANS,
                    label: eO.NW.string(eO.t.ZbeITU),
                    element: H.Z,
                    predicate: () => t.canManageBans
                };
            case eN.pNK.CLAN:
                return {
                    section: eN.pNK.CLAN,
                    label: eO.NW.string(eO.t['6g8uHB']),
                    ariaLabel: eO.NW.string(eO.t['6g8uHB']),
                    onClick() {
                        (0, I.q4)(
                            (e) => {
                                let { closeLayer: n } = e;
                                return (0, r.jsx)(m.Z, {
                                    onClose: n,
                                    guildId: t.guild.id
                                });
                            },
                            { layerKey: ev.q2 }
                        ),
                            (0, l.xf)();
                    },
                    predicate() {
                        var e;
                        return null !== (e = t.canAccessClanSettings) && void 0 !== e && e;
                    },
                    icon: null != eR ? (0, r.jsx)(p.KQ, { src: eR }) : null
                };
            case eN.pNK.COMMUNITY:
                return {
                    section: eN.pNK.COMMUNITY,
                    impressionName: t.guild.hasFeature(eN.oNc.COMMUNITY) ? i.ImpressionNames.GUILD_SETTINGS_COMMUNITY_OVERVIEW : i.ImpressionNames.GUILD_SETTINGS_ENABLE_COMMUNITY,
                    label: t.guild.hasFeature(eN.oNc.COMMUNITY) ? eO.NW.string(eO.t['/dp6yc']) : eO.NW.string(eO.t.ElKTeX),
                    element: V.Z,
                    ariaLabel: t.guild.hasFeature(eN.oNc.COMMUNITY) ? eO.NW.string(eO.t['/dp6yc']) : eO.NW.string(eO.t.ElKTeX),
                    newIndicator: !t.guild.hasFeature(eN.oNc.COMMUNITY) && null != t.memberCount && t.memberCount >= e_.U3 && C.qc.hasHotspot(C.v6.GUILD_SETTINGS_COMMUNITY_GUILD_UPSELL),
                    notice: {
                        stores: [A.Z],
                        element: V.X
                    }
                };
            case eN.pNK.ONBOARDING:
                return {
                    section: eN.pNK.ONBOARDING,
                    label: eO.NW.string(eO.t.LdxCKy),
                    element: ee.Z,
                    ariaLabel: eO.NW.string(eO.t.LdxCKy),
                    newIndicatorDismissibleContentTypes: t.onboardingEnabled ? null : [s.z.COMMUNITY_ONBOARDING_NEW_BADGE],
                    notice: {
                        stores: [L.Z],
                        element: G.Z
                    },
                    predicate: () => (0, x.wC)(t.guild.id),
                    decoration: (function (e, t) {
                        if (b.Z.getEnabled(e)) return eO.NW.string(eO.t.pPSmxs).toUpperCase();
                        if ((0, U.C)(e)) return eO.NW.string(eO.t.HDF4o6).toUpperCase();
                        let n = M.ug,
                            r = n.findIndex((e) => e === t);
                        if (-1 !== r)
                            return eO.NW.formatToPlainString(eO.t['5r28iY'], {
                                count: r + 1,
                                total: n.length
                            });
                    })(t.guild.id, t.onboardingStep)
                };
            case eN.pNK.ANALYTICS:
                return {
                    section: eN.pNK.ANALYTICS,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_ANALYTICS,
                    label: eO.NW.string(eO.t['0wWfUF']),
                    ariaLabel: eO.NW.string(eO.t['0wWfUF']),
                    element: z.Z,
                    predicate: () => ew
                };
            case eN.pNK.DISCOVERY:
                return {
                    section: eN.pNK.DISCOVERY,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_DISCOVERY,
                    label: eO.NW.string(eO.t.oGaVGx),
                    element: K.Z,
                    notice: {
                        stores: [A.Z],
                        element: K.s
                    },
                    predicate: () => !t.memberVerificationRolloutEnabled && t.canManageGuild && t.guild.hasFeature(eN.oNc.COMMUNITY),
                    ariaLabel: t.guild.hasFeature(eN.oNc.DISCOVERABLE) ? eO.NW.string(eO.t.oGaVGx) : eO.NW.string(eO.t.XU0S6u)
                };
            case eN.pNK.DISCOVERY_LANDING_PAGE:
                return {
                    section: eN.pNK.DISCOVERY_LANDING_PAGE,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_DISCOVERY_LANDING_PAGE,
                    label: (0, r.jsxs)('div', {
                        className: ey.landingPageTabLink,
                        children: [eO.NW.string(eO.t.kGlQGB), ' ', (0, r.jsx)(g.Z, {})]
                    }),
                    ariaLabel: eO.NW.string(eO.t.kGlQGB),
                    element: q.Z,
                    notice: {
                        stores: [A.Z],
                        element: q.J
                    },
                    type: o.bT.CUSTOM,
                    predicate() {
                        var e;
                        return t.canManageGuild && (t.guild.hasFeature(eN.oNc.DISCOVERABLE) || !!(null === (e = t.guildMetadata) || void 0 === e ? void 0 : e.isPublished));
                    }
                };
            case eN.pNK.COMMUNITY_WELCOME:
                return {
                    section: eN.pNK.COMMUNITY_WELCOME,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_COMMUNITY_WELCOME,
                    label: eO.NW.string(eO.t['2rkmDg']),
                    ariaLabel: eO.NW.string(eO.t['2rkmDg']),
                    element: Y.Z,
                    notice: {
                        stores: [E.Z],
                        element: S.Z
                    },
                    predicate: () => t.canManageGuild && t.guild.hasFeature(eN.oNc.COMMUNITY) && !t.welcomeScreenEmpty && !t.guild.hasFeature(eN.oNc.GUILD_SERVER_GUIDE)
                };
            case eN.pNK.ROLE_SUBSCRIPTIONS:
                return eI(eC({}, eP), {
                    section: eN.pNK.ROLE_SUBSCRIPTIONS,
                    element: O.Z,
                    type: o.bT.CUSTOM,
                    predicate: t.monetizationPredicate,
                    newIndicatorDismissibleContentTypes: (0, _.sO)(t.guild) ? [s.z.SERVER_SUBSCRIPTION_SETTINGS_NEW_BADGE_TIER_TEMPLATE] : null
                });
            case eN.pNK.GUILD_PRODUCTS:
                return {
                    section: eN.pNK.GUILD_PRODUCTS,
                    label: eO.NW.string(eO.t.X6h2g4),
                    ariaLabel: eO.NW.string(eO.t.X6h2g4),
                    element: N.Z,
                    predicate: () => t.monetizationPredicate() && (0, j.p)(t.guild.id, 'guild_product_settings_predicate') && (0, v.sy)(t.guild)
                };
            case eN.pNK.ROLE_SUBSCRIPTIONS_STORE_PAGE:
                return {
                    section: eN.pNK.ROLE_SUBSCRIPTIONS_STORE_PAGE,
                    impressionName: i.ImpressionNames.GUILD_ROLE_SUBSCRIPTION_STORE_PAGE_GUILD_SETTINGS,
                    newIndicator: 2022 === new Date().getFullYear(),
                    label: eO.NW.string(eO.t.ipTYsr),
                    ariaLabel: eO.NW.string(eO.t.ipTYsr),
                    element: y.Z,
                    predicate: () => t.monetizationPredicate() && (0, v.sy)(t.guild)
                };
            case eN.pNK.GUILD_PREMIUM:
                return {
                    section: eN.pNK.GUILD_PREMIUM,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_BOOST_STATUS,
                    label: eO.NW.string(eO.t['8+VX4+']),
                    ariaLabel: eO.NW.string(eO.t['8+VX4+']),
                    element: en.Z,
                    className: ey.serverBoostTabItem,
                    icon: (0, r.jsx)(ei.Z, { className: ey.icon })
                };
            case eN.pNK.MEMBERS:
                return {
                    section: eN.pNK.MEMBERS,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_MEMBERS,
                    label: (0, r.jsx)($.$, {}),
                    ariaLabel: eO.NW.string(eO.t['9Oq93t']),
                    element: $.Z,
                    type: o.bT.CUSTOM,
                    predicate: () => t.canAccessMembersPage
                };
            case eN.pNK.INSTANT_INVITES:
                return {
                    section: eN.pNK.INSTANT_INVITES,
                    impressionName: i.ImpressionNames.GUILD_SETTINGS_INVITES,
                    label: eO.NW.string(eO.t['9F90iY']),
                    element: J.Z,
                    type: o.bT.CUSTOM,
                    predicate: () => t.canManageGuild
                };
            case eN.pNK.DELETE:
                return {
                    section: eN.pNK.DELETE,
                    onClick() {
                        let e = T.default.getCurrentUser();
                        if (null == e) return;
                        let i = t.guild.toString(),
                            s = {
                                header: eO.NW.formatToPlainString(eO.t.us7mCw, { name: i }),
                                confirmText: eO.NW.string(eO.t.l3hWPz),
                                cancelText: eO.NW.string(eO.t['ETE/oK']),
                                onConfirm: () => {
                                    P.S.subscribeOnce(eN.CkL.LAYER_POP_COMPLETE, () => {
                                        R.Z.deleteGuild(t.guild.id, i);
                                    }),
                                        (0, l.xf)();
                                }
                            };
                        e.mfaEnabled
                            ? (0, a.h7j)((e) =>
                                  (0, r.jsx)(
                                      a.ConfirmModal,
                                      eI(eC({}, e, s), {
                                          children: (0, r.jsx)(a.Text, {
                                              variant: 'text-md/normal',
                                              children: eO.NW.format(eO.t['Zuo+VV'], { name: i })
                                          })
                                      })
                                  )
                              )
                            : (0, a.ZDy)(async () => {
                                  let { default: e } = await n.e('18259').then(n.bind(n, 1493));
                                  return (t) =>
                                      (0, r.jsx)(
                                          e,
                                          eI(eC({}, t, s), {
                                              validationText: i,
                                              instructionText: eO.NW.string(eO.t.abprOD),
                                              errorText: eO.NW.string(eO.t['c2/DS0']),
                                              children: eO.NW.format(eO.t['Zuo+VV'], { name: i })
                                          })
                                      );
                              });
                    },
                    label: eO.NW.string(eO.t.l3hWPz),
                    ariaLabel: eO.NW.string(eO.t.l3hWPz),
                    icon: (0, r.jsx)(a.XHJ, {
                        size: 'xs',
                        color: 'currentColor'
                    }),
                    predicate: () => t.isOwnerWithRequiredMfaLevel
                };
            case eN.pNK.LANDING:
            case eN.pNK.MEMBER_EDIT:
            case eN.pNK.MEMBER_TIMEOUT:
            case eN.pNK.MEMBER_KICK:
            case eN.pNK.MEMBER_BAN:
            case eN.pNK.CHANNELS:
            case eN.pNK.ROLE_EDIT:
            case eN.pNK.ROLE_PERMISSIONS:
            case eN.pNK.MEMBER_VERIFICATION:
            case eN.pNK.CLYDE:
            case eN.pNK.INTEGRATION_SETTINGS:
            case eN.pNK.SECURITY:
            case eN.pNK.AUDIT_LOG_FILTER:
            case eN.pNK.ROLE_SUBSCRIPTIONS_TIER_EDIT:
            case eN.pNK.WEBHOOKS:
            case eN.pNK.EDIT_WEBHOOK:
            case eN.pNK.INTEGRATION_PLATFORM:
            case eN.pNK.LOBBIES_LINKED:
            case eN.pNK.EDIT_LINKED_LOBBY:
            case eN.pNK.CHANNELS_FOLLOWED:
            case eN.pNK.COMMUNITY_INTRO:
            case eN.pNK.ROLE_EDIT_REFRESH:
            case eN.pNK.ROLE_SUBSCRIPTIONS_ENABLE_MONETIZATION:
            case eN.pNK.ROLE_SUBSCRIPTIONS_BASIC:
            case eN.pNK.ROLE_SUBSCRIPTIONS_TIERS:
            case eN.pNK.ROLE_SUBSCRIPTIONS_PAYMENTS:
            case eN.pNK.ROLE_SUBSCRIPTIONS_EMOJIS:
            case eN.pNK.ROLE_SUBSCRIPTIONS_TIER_TEMPLATE_SELECTION:
                return null;
            default:
                (0, w.vE)(e);
        }
    }
};
