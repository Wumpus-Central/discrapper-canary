n.d(t, { A: () => R, o: () => N });
var a = n(477900),
    l = n(582128),
    r = n(503698),
    s = n.n(r),
    i = n(317097),
    o = n(17928),
    d = n(661531),
    c = n(602853),
    u = n(778712),
    m = n(654107),
    f = n(450373),
    g = n(885386),
    E = n(531685),
    p = n(486020),
    b = n(837529),
    h = n(686189),
    x = n(714719),
    _ = n(859161),
    A = n(207634),
    v = n(565890);
function N(e) {
    let {
            user: t,
            displayProfile: n,
            guildId: r,
            pendingBanner: A,
            overlay: N,
            className: R,
            avatarSize: I,
            avatarOffsetX: j,
            avatarOffsetY: T,
            bannerWidth: C,
            bannerHeight: y,
            themePadding: S,
            pendingAccentColor: D,
            animateOnHoverOrFocusOnly: M = !1,
        } = e,
        [P, O] = l.useState(!1),
        U = (0, o.bG)([E.A], () => E.A.isFocused()),
        B = g.kt.getSetting(),
        k = (0, b.Nx)(),
        { bannerSrc: G, status: w } = (0, h.A)({
            displayProfile: n,
            pendingBanner: A,
            size: C,
            canAnimate: M || !B ? P : U,
        }),
        L = k ? null : (G ?? null),
        H = (0, c.r)(d.A.unsafe_rawColors.PRIMARY_800).hex(),
        V = t.getAvatarURL(r, (0, u.FT)(I)),
        $ = (0, i.LX)((0, m.Ay)(V, H, !1)),
        z = (0, f.A)(D ?? n?.primaryColor ?? $).hex,
        F = { align: "start", insetStart: j - S, insetBottom: T + S, radius: (0, _.A)(I) };
    return (0, a.jsx)(x.A, {
        fillClassName: s()(v.v, R),
        bannerSrc: L,
        backgroundColor: "COMPLETE" === w || k ? z : d.A.unsafe_rawColors.PRIMARY_800.css,
        showGifTag: !B && (0, p.o4)(L),
        height: y,
        cutout: F,
        overlay: N,
        onInteractionStart: () => O(!0),
        onInteractionEnd: () => O(!1),
    });
}
function R(e) {
    let {
            themeType: t,
            user: n,
            displayProfile: l,
            canUsePremiumProfileCustomization: r = !1,
            specOverrides: s,
            ...i
        } = e,
        o = A.T[t],
        d = null != s ? { ...o, ...s } : o,
        c = !(0, b.Qq)() && (r || l?.canUsePremiumProfileCustomization || !1);
    return (0, a.jsx)(N, { ...i, ...d, user: n, displayProfile: l, themePadding: c ? d.themePadding : 0 });
}
