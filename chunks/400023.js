n.d(t, { Z: () => z }), n(388685);
var r = n(255367),
    i = n(73800),
    l = n(120356),
    a = n.n(l),
    o = n(91192),
    s = n(442837),
    c = n(481060),
    u = n(904245),
    d = n(902840),
    p = n(448986),
    h = n(607070),
    f = n(931056),
    m = n(835473),
    g = n(95398),
    b = n(580747),
    _ = n(135938),
    y = n(160404),
    x = n(765104),
    C = n(695346),
    v = n(314897),
    j = n(323873),
    O = n(607744),
    E = n(375954),
    I = n(496675),
    P = n(306680),
    S = n(62817),
    Z = n(594174),
    N = n(459273),
    T = n(255269),
    A = n(47481),
    w = n(977391),
    R = n(959258),
    M = n(73274),
    k = n(419388),
    L = n(406534),
    D = n(981631),
    U = n(388032),
    B = n(977659);
function F(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            r = Object.keys(n);
        'function' == typeof Object.getOwnPropertySymbols &&
            (r = r.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                })
            )),
            r.forEach(function (t) {
                var r;
                (r = n[t]),
                    t in e
                        ? Object.defineProperty(e, t, {
                              value: r,
                              enumerable: !0,
                              configurable: !0,
                              writable: !0
                          })
                        : (e[t] = r);
            });
    }
    return e;
}
function G(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : (function (e, t) {
                  var n = Object.keys(e);
                  if (Object.getOwnPropertySymbols) {
                      var r = Object.getOwnPropertySymbols(e);
                      n.push.apply(n, r);
                  }
                  return n;
              })(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
function H(e, t) {
    if (null == e) return {};
    var n,
        r,
        i = (function (e, t) {
            if (null == e) return {};
            var n,
                r,
                i = {},
                l = Object.keys(e);
            for (r = 0; r < l.length; r++) (n = l[r]), t.indexOf(n) >= 0 || (i[n] = e[n]);
            return i;
        })(e, t);
    if (Object.getOwnPropertySymbols) {
        var l = Object.getOwnPropertySymbols(e);
        for (r = 0; r < l.length; r++) (n = l[r]), !(t.indexOf(n) >= 0) && Object.prototype.propertyIsEnumerable.call(e, n) && (i[n] = e[n]);
    }
    return i;
}
let V = i.memo(
        function (e) {
            var t;
            let { className: n, messageGroupSpacing: l, scrollerClassName: d, channel: f, messages: m, unreadCount: g, showNewMessagesBar: b, messageDisplayCompact: _, channelStream: x, uploads: C, hasUnreads: v, editingMessageId: j, fontSize: O, keyboardModeEnabled: E, filterAfterTimestamp: P, showingQuarantineBanner: S, hideSummaries: Z = !1, jumpBarClassName: T, typingGradient: A } = e,
                [R, V] = i.useState(!1),
                z = i.useMemo(
                    () =>
                        _
                            ? (0, w.aJ)({
                                  compact: !0,
                                  messageGroups: 30,
                                  groupRange: 4,
                                  attachments: 8,
                                  fontSize: O,
                                  groupSpacing: l
                              })
                            : (0, w.aJ)({
                                  compact: !1,
                                  messageGroups: 26,
                                  groupRange: 4,
                                  attachments: 8,
                                  fontSize: O,
                                  groupSpacing: l
                              }),
                    [_, O, l]
                ),
                W = (0, k.ZP)({
                    messages: m,
                    channel: f,
                    compact: _,
                    hasUnreads: v,
                    focusId: j,
                    placeholderHeight: z.totalHeight,
                    canLoadMore: null == P,
                    handleScrollToBottom: i.useCallback(() => V(!0), [V]),
                    handleScrollFromBottom: i.useCallback(() => V(!1), [V])
                }),
                Y = (0, M.Z)({
                    scrollerRef: W.ref,
                    isEditing: null != j,
                    keyboardModeEnabled: E,
                    hasMoreAfter: m.hasMoreAfter
                }),
                q = (0, s.e7)([y.Z], () => (I.Z.can(D.Plq.READ_MESSAGE_HISTORY, f) ? null : y.Z.getViewingRolesTimestamp(f.getGuildId()))),
                {
                    channelStreamMarkup: K,
                    newMessagesBar: X,
                    jumpToPresentBar: Q,
                    forumPostActionBar: J,
                    safetyWarningBanner: $
                } = (0, L.Z)({
                    channel: f,
                    messages: m,
                    unreadCount: g,
                    showNewMessagesBar: b,
                    messageDisplayCompact: _,
                    channelStream: x,
                    uploads: C,
                    loadMore: W.loadMore,
                    scrollManager: W,
                    specs: z,
                    filterAfterTimestamp: null != P ? P : q,
                    showingQuarantineBanner: S,
                    hideSummaries: Z,
                    isAtBottom: R,
                    jumpToPresent: () => {
                        if (m.hasPresent()) {
                            var e;
                            null == (e = W.ref.current) || e.scrollToBottom({ animate: !h.Z.useReducedMotion });
                        } else u.Z.jumpToPresent(f.id, D.AQB);
                    },
                    jumpBarClassName: T
                });
            !(function (e) {
                let t = i.useCallback(() => {
                        var t;
                        return null == (t = e.current) ? void 0 : t.scrollToBottom();
                    }, [e]),
                    n = i.useCallback(() => {
                        var t;
                        return null == (t = e.current) ? void 0 : t.scrollPageUp({ animate: !h.Z.useReducedMotion });
                    }, [e]),
                    r = i.useCallback(() => {
                        var t;
                        return null == (t = e.current) ? void 0 : t.scrollPageDown({ animate: !h.Z.useReducedMotion });
                    }, [e]);
                (0, N.yp)({
                    event: D.CkL.SCROLLTO_PRESENT,
                    handler: t
                }),
                    (0, N.yp)({
                        event: D.CkL.SCROLL_PAGE_UP,
                        handler: n
                    }),
                    (0, N.yp)({
                        event: D.CkL.SCROLL_PAGE_DOWN,
                        handler: r
                    });
            })(W.ref);
            let ee = (0, c.mFp)(),
                et = (0, o.l2)(Y),
                { ref: en } = et,
                er = H(et, ['ref']),
                ei = (0, p.Z)((e) => {
                    var t;
                    (W.ref.current = e), (en.current = null != (t = null == e ? void 0 : e.getScrollerNode()) ? t : null);
                });
            return (0, r.jsxs)(o.bG, {
                navigator: Y,
                children: [
                    null != $ && $,
                    (0, r.jsxs)('div', {
                        className: a()(B.messagesWrapper, n, 'group-spacing-'.concat(l), { [B.typingGradient]: A }),
                        children: [
                            null == $ && X,
                            (0, r.jsxs)(
                                c.eTT,
                                G(
                                    F(
                                        {
                                            ref: ei,
                                            customTheme: !0,
                                            className: a()(d, B.scroller),
                                            contentClassName: B.scrollerContent,
                                            onResize: W.handleResize,
                                            onScroll: W.handleScroll,
                                            onMouseDown: W.handleMouseDown,
                                            onMouseUp: W.handleMouseUp
                                        },
                                        ee
                                    ),
                                    {
                                        tabIndex: -1,
                                        role: 'group',
                                        children: [
                                            J,
                                            (0, r.jsxs)(
                                                'ol',
                                                G(
                                                    F(
                                                        {
                                                            className: a()(B.scrollerInner, { [B.scrollerAllowSticky]: f.isModeratorReportChannel() }),
                                                            'aria-label': U.intl.formatToPlainString(U.t.XarRiI, { channelName: f.name })
                                                        },
                                                        er
                                                    ),
                                                    {
                                                        children: [
                                                            (0, r.jsx)('span', {
                                                                className: B.navigationDescription,
                                                                id: 'messagesNavigationDescription',
                                                                'aria-hidden': !0,
                                                                children: U.intl.string(U.t.Spb3s7)
                                                            }),
                                                            K,
                                                            (0, r.jsx)('div', {
                                                                className: a()({
                                                                    [B.scrollerSpacer]: !S,
                                                                    [B.empty]: 0 === m.length && !m.loadingMore,
                                                                    [B.emptyForum]: 1 === m.length && !m.loadingMore && f.isForumPost() && (null == (t = m.first()) ? void 0 : t.isFirstMessageInForumPost(f))
                                                                })
                                                            })
                                                        ]
                                                    }
                                                )
                                            )
                                        ]
                                    }
                                )
                            ),
                            Q
                        ]
                    })
                ]
            });
        },
        (e, t) => null != e.isHidden && null != t.isHidden && e.isHidden && t.isHidden
    ),
    z = i.memo(function (e) {
        var { channel: t, showingQuarantineBanner: n, hideSummaries: l = !1, forceCompact: a = !1, forceCozy: o = !1, typingGradient: c = !1 } = e,
            u = H(e, ['channel', 'showingQuarantineBanner', 'hideSummaries', 'forceCompact', 'forceCozy', 'typingGradient']);
        let {
                canManageMessages: p,
                permissionVersion: y,
                canChat: N
            } = (function (e) {
                let t = e.getGuildId(),
                    n = (0, s.e7)([O.Z], () => null == t || O.Z.canChatInGuild(t), [t]),
                    { canManageMessages: r, permissionVersion: i } = (0, s.cj)(
                        [I.Z],
                        () => ({
                            canManageMessages: I.Z.can(D.Plq.MANAGE_MESSAGES, e),
                            permissionVersion: null != t ? I.Z.getGuildVersion(t) : null
                        }),
                        [e, t]
                    );
                return {
                    canChat: n,
                    permissionVersion: i,
                    canManageMessages: r
                };
            })(t),
            {
                messageGroupSpacing: w,
                fontSize: M,
                messageDisplayCompact: k,
                renderSpoilers: L,
                keyboardModeEnabled: U
            } = (function () {
                let e = C.jU.useSetting(),
                    t = C.cC.useSetting(),
                    {
                        messageGroupSpacing: n,
                        fontSize: r,
                        keyboardModeEnabled: i
                    } = (0, s.cj)([h.Z], () => {
                        let { messageGroupSpacing: e, fontSize: t, keyboardModeEnabled: n } = h.Z;
                        return {
                            messageGroupSpacing: e,
                            fontSize: t,
                            keyboardModeEnabled: n
                        };
                    });
                return {
                    messageGroupSpacing: n,
                    messageDisplayCompact: e,
                    renderSpoilers: t,
                    fontSize: r,
                    keyboardModeEnabled: i
                };
            })(),
            {
                messages: B,
                channelStream: z,
                oldestUnreadMessageId: W,
                editingMessageId: Y
            } = (function (e) {
                var t, n;
                let r = (0, s.e7)([E.Z], () => E.Z.getMessages(e.id), [e.id]),
                    l = (0, s.e7)(
                        [P.ZP],
                        () => {
                            var t;
                            return null != (t = P.ZP.getOldestUnreadMessageId(e.id)) ? t : null;
                        },
                        [e.id]
                    ),
                    { enabled: a } = _.Z.useExperiment({ location: '41de6d_1' }, { autoTrackExposure: !1 }),
                    o = null != (n = null == (t = Z.default.getUser(v.default.getId())) ? void 0 : t.hasFlag(D.xW$.SPAMMER)) && n,
                    c = (0, d.ts)(e),
                    u = (0, b.Z)('use_topic_dividers_in_chat'),
                    p = (0, s.Wu)(
                        [x.Z],
                        () => {
                            var t;
                            return c && u && null != (t = x.Z.summaries(e.id)) ? t : [];
                        },
                        [c, e.id, u]
                    ),
                    h = (0, s.e7)([x.Z], () => (c ? x.Z.selectedSummary(e.id) : null), [c, e.id]),
                    g = (function (e) {
                        let t = i.useMemo(() => {
                            let t = new Set();
                            return (
                                e.forEach((e) => {
                                    null != e.applicationId && null == e.application && t.add(e.applicationId);
                                }),
                                Array.from(t)
                            );
                        }, [e]);
                        return (0, m.Z)(t);
                    })(r);
                (0, f.Z)(r, e);
                let y = i.useMemo(
                    () =>
                        (0, A.Z)({
                            channel: e,
                            messages: r,
                            oldestUnreadMessageId: l,
                            treatSpam: a && !o,
                            summaries: p,
                            selectedSummary: h
                        }),
                    [r, e, l, a, p, h, g, o]
                );
                return {
                    messages: r,
                    channelStream: y,
                    oldestUnreadMessageId: l,
                    editingMessageId: (0, s.e7)([j.Z], () => {
                        var t;
                        return null == (t = j.Z.getEditingMessage(e.id)) ? void 0 : t.id;
                    })
                };
            })(t);
        return (0, r.jsx)(g.aQ.Provider, {
            value: (0, T.Z)(L, p),
            children: (0, r.jsx)(R.v, {
                children: (0, r.jsx)(
                    V,
                    G(F({}, u), {
                        messageGroupSpacing: w,
                        showNewMessagesBar: !0,
                        channel: t,
                        messageDisplayCompact: !o && (a || k),
                        messages: B,
                        channelStream: z,
                        permissionVersion: y,
                        uploads: (0, s.e7)([S.Z], () => S.Z.getFiles(t.id), [t]),
                        unreadCount: (0, s.e7)([P.ZP], () => P.ZP.getUnreadCount(t.id), [t]),
                        hasUnreads: null != W,
                        canChat: N,
                        editingMessageId: Y,
                        fontSize: M,
                        keyboardModeEnabled: U,
                        showingQuarantineBanner: n,
                        hideSummaries: l,
                        typingGradient: c
                    })
                )
            })
        });
    });
