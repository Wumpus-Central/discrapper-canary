"use strict";
n.d(t, { l: () => h }), n(667532);
var i = n(627968),
    r = n(64700),
    s = n(17928),
    a = n(834730),
    o = n(573435),
    l = n(71393),
    u = n(967198),
    c = n(743790),
    d = n(340879);
function _(e) {
    let { children: t, iconSize: n } = e;
    return (0, i.jsx)(o.Ay, { className: d.Iu, mask: o.Ay.Masks.SQUIRCLE, width: n + 5, height: n + 5, children: t });
}
function h(e) {
    let { guildIds: t, iconSize: n = 20, prioritizeSelectedGuild: o = !1 } = e,
        h = (0, s.bG)([u.A], () => (o ? u.A.getGuildId() : null), [o]),
        f = (0, s.yK)([l.A], () => t.map((e) => l.A.getGuild(e)), [t]),
        E = (0, r.useMemo)(() => f.filter((e) => null != e), [f]),
        { visibleGuilds: p, numTruncated: m } = (0, r.useMemo)(() => {
            let e = [...E];
            if (null != h) {
                let t = e.findIndex((e) => e.id === h);
                t > 0 && (e.splice(t, 1), e.unshift(E[t]));
            }
            return 4 === e.length
                ? { visibleGuilds: e, numTruncated: 0 }
                : { visibleGuilds: e.slice(0, 3), numTruncated: e.length - 3 };
        }, [E, h]);
    return (0, i.jsxs)("div", {
        className: d.WM,
        children: [
            p.map((e) => (0, i.jsx)(_, { iconSize: n, children: (0, i.jsx)(c.K, { guild: e, size: n }) }, e.id)),
            m > 0 &&
                (0, i.jsx)(_, {
                    iconSize: n,
                    children: (0, i.jsxs)(a.E, {
                        className: d.br,
                        color: "text-subtle",
                        variant: "text-xxs/semibold",
                        children: ["+", m],
                    }),
                }),
        ],
    });
}
