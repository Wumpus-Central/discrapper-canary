...
class y {
        ...
        (this.rpcServer.getCurrentUser = () => E.default.getCurrentUser()),
            (this.rpcServer.onConnect = async (e) => {
                a.Z.dispatch({
                    ...
                    C.default.track(x.rMx.AUTHORIZED_APP_CONNECTED, {
    ...
    handleMessage(e) {
        ...
        switch (e.type) {
            case 'MESSAGE_CREATE':
                ...
                (t = x.zMe.MESSAGE_CREATE), (n = e.channelId), (i = e.message), (l = ''.concat(t).concat(e.message.id));
                break;
            case 'MESSAGE_UPDATE':
                (t = x.zMe.MESSAGE_UPDATE), (n = e.message.channel_id), (i = e.message);
                break;
            case 'MESSAGE_DELETE':
                (t = x.zMe.MESSAGE_DELETE), (n = e.channelId), (i = { id: e.id }), (l = ''.concat(t).concat(e.id));
                break;
            default:
                return (0, v.vE)(e);
        }
    ...
    handleActivityJoin(e) {
        ...
        if (0 === this.rpcServer.subscriptions.length) return;
        let r = { secret: n };
        l && (r.intent = i), this.rpcServer.dispatchToSubscriptions(x.zMe.ACTIVITY_JOIN, (e) => e.socket.application.id === t, r), this.rpcServer.dispatchToSubscriptions(x.zMe.GAME_JOIN, (e) => e.socket.application.id === t, r);
    }
    handleActivityLayoutModeUpdate(e) {
        let { applicationId: t, layoutMode: n } = e;
        if (0 === this.rpcServer.subscriptions.length) return;
        let i = n !== A.cE.FOCUSED;
        this.rpcServer.dispatchToSubscriptions(x.zMe.ACTIVITY_PIP_MODE_UPDATE, (e) => e.socket.application.id === t, { is_pip_mode: i }), this.rpcServer.dispatchToSubscriptions(x.zMe.ACTIVITY_LAYOUT_MODE_UPDATE, (e) => e.socket.application.id === t, { layout_mode: n });
    }
    handleThermalStateChange(e) {
        let { applicationId: t } = e;
        if (0 === this.rpcServer.subscriptions.length || null == t) return;
        let n = { thermal_state: (0, o.bY)() };
        this.rpcServer.dispatchToSubscriptions(x.zMe.THERMAL_STATE_UPDATE, (e) => e.socket.application.id === t, n);
    }
    handleScreenOrientationUpdate(e) {
        let { screenOrientation: t } = e;
        0 !== this.rpcServer.subscriptions.length && this.rpcServer.dispatchToSubscriptions(x.zMe.ORIENTATION_UPDATE, {}, { screen_orientation: t });
    }
    handleActivityMessage(e) {
        ...
        let o = r.type === x.mFx.JOIN_REQUEST ? p.Z.getApplicationActivity(s.id, l.id) : p.Z.getApplicationActivity(a.id, l.id);
        ...
        switch (r.type) {
            ...
            case x.mFx.JOIN_REQUEST:
                this.rpcServer.dispatchToSubscriptions(x.zMe.ACTIVITY_JOIN_REQUEST, (e) => e.socket.application.id === c, { user: (0, Z.Z)(a) });
        }
    }
    handleOAuth2TokenRevoke(e) {
        let { accessToken: t } = e;
        this.rpcServer.sockets.forEach((e) => {
            e.authorization.accessToken === t && e.close(x.$VG.TOKEN_REVOKED, 'Token revoked');
        });
    }
    handleGuildCreate(e) {
        let {
    ...
    handleRelationshipRemove(e) {
        ...
        0 !== this.rpcServer.subscriptions.length && this.rpcServer.dispatchToSubscriptions(x.zMe.RELATIONSHIP_UPDATE, {}, (0, T.cD)(x.OGo.NONE, t));
    }
    handlePresenceUpdate(e) {
        ...
        0 !== this.rpcServer.subscriptions.length &&
            t.forEach((e) => {
                ...
                i === x.OGo.NONE && c.Z.getUserAffinitiesUserIds().has(n) && (i = x.OGo.IMPLICIT), i !== x.OGo.NONE && this.rpcServer.dispatchToSubscriptions(x.zMe.RELATIONSHIP_UPDATE, {}, (0, T.cD)(i, n));
            });
    }
    handlePresencesReplace() {
        0 !== this.rpcServer.subscriptions.length &&
            l().forEach(_.Z.getRelationships(), (e, t) => {
                this.rpcServer.dispatchToSubscriptions(x.zMe.RELATIONSHIP_UPDATE, {}, (0, T.cD)(e, t));
            });
    }
    handleEntitlementCreate(e) {
        let { entitlement: t } = e;
        0 !== this.rpcServer.subscriptions.length && this.rpcServer.dispatchToSubscriptions(x.zMe.ENTITLEMENT_CREATE, (e) => e.socket.application.id === t.application_id, { entitlement: t });
    }
    handleEntitlementDelete(e) {
        let { entitlement: t } = e;
        0 !== this.rpcServer.subscriptions.length && this.rpcServer.dispatchToSubscriptions(x.zMe.ENTITLEMENT_DELETE, (e) => e.socket.application.id === t.application_id, { entitlement: t });
    }
    constructor(e) {
