...
function C(e) {
    ...
    for (let n in s) {
        let i = s[n];
        l.A.can(I.xBc.VIEW_CHANNEL, i) && r.push(N(i, e, t));
    }
    for (let n of a.A.getAllThreadsForGuild(e.id)) l.A.can(I.xBc.VIEW_CHANNEL, n) && r.push(N(n, e, t));
...
class D extends i.A {
    ...
    handleInit() {
        R(c.A.getCurrentlySelectedChannelId()),
            ...
            T() &&
                (function () {
                    if (!T()) return;
                    let e = o.A.getGuildsArray().map((e) => C(e)),
                        t = [],
    ...
    handleChannelCreate(e) {
        ...
        let i = v(n);
        g.indexDomains([{ id: n?.id ?? I.ME, items: [N(t, n)], defaultThumbnailURL: i }]);
    }
    ...
    handleGuildCreateOrUpdate(e) {
        ...
        if (T()) {
            ...
            null != e ? g.indexDomains([C(e, "GUILD_UPDATE" === n)]) : g.deleteSearchDomains([t.id]);
        }
    }
    handleGuildDelete(e) {
        let { guild: t } = e;
