...
function m(e, t) {
    let [n, o] = i.useState(0),
        [m, A] = i.useState(!1),
        [f, E] = i.useState({ step: _.b.INITIALIZING }),
        x = i.useRef(null),
        I = i.useMemo(() => new s.A(1500, 3e4), []),
        N = (0, a.A)(() => {
            E({ step: _.b.INITIALIZING }),
                t
                    ? o((e) => e + 1)
    ...
    return (
        ...
        i.useEffect(() => {
            ...
            let T = () => {
                    ...
                    switch (o.op) {
                        ...
                        case "pending_remote_init": {
                            ...
                            i("handshake complete awaiting remote auth."),
                                E({ step: _.b.PENDING_REMOTE_INIT, fingerprint: e });
                            return;
                        }
                        case "pending_login": {
                            let t = o.ticket;
                            if (null == t) return void v();
                            E({ step: _.b.PENDING_LOGIN, ticket: t }),
                                r.Bo.post({
                                    ...
                                    .then(async (t) => {
                                        if (null == x.current) return void v();
                        ...
                        case "pending_ticket": {
                            c._.dispatch(p.jej.WAVE_EMPHASIZE),
                                i("remote auth handshake started, awaiting ticket/cancel.");
                            let e = o.encrypted_user_payload,
                                t = await (0, h.n7)(S(), e);
                            E({ step: _.b.PENDING_TICKET, user: t });
                            return;
                        }
                        case "cancel":
                            i("remote auth handshake cancelled."), N();
                            return;
                        case "hello": {
                ...
                y = async () => {
                    ...
                    i(`connected, handshaking with fingerprint: ${e}`),
                        a.send(JSON.stringify({ op: "init", encoded_public_key: m })),
                        (x.current = o);
                },
                b = (e) => {
            ...
            return (
                a.addEventListener("open", y),
                a.addEventListener("message", C),
                a.addEventListener("close", b),
                a.addEventListener("error", R),
                () => {
                    i("cleaning up"),
                        a.removeEventListener("open", y),
                        a.removeEventListener("message", C),
                        a.removeEventListener("close", b),
