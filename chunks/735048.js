n.d(t, {
    XZ: () => T,
    cO: () => I,
});
var r = n(900090),
    i = n(595707),
    a = n(576418),
    o = n(995827),
    s = n(937397),
    l = n(819851),
    c = n(937226),
    u = n(158821),
    d = n(805204),
    f = n(511746),
    p = n(984940),
    _ = n(69771),
    m = n(485853),
    h = n(21701),
    g = n(698369),
    E = n(880016),
    b = n(413565),
    y = n(401522),
    O = n(473749);
let v = (0, O.createContext)(null),
    S = (0, O.createContext)(null),
    I = (0, O.forwardRef)(function (e, t) {
        var n, r;
        [e, t] = (0, i.pE)(e, t, v);
        let { validationBehavior: d } = (0, i.jn)(o.q) || {},
            f = null != (r = null != (n = e.validationBehavior) ? n : d) ? r : "native",
            p = (0, h.$)({
                ...e,
                validationBehavior: f,
            }),
            [_, m] = (0, i.xc)(!e["aria-label"] && !e["aria-labelledby"]),
            {
                groupProps: g,
                labelProps: b,
                descriptionProps: y,
                errorMessageProps: I,
                ...T
            } = (0, c.u)(
                {
                    ...e,
                    label: m,
                    validationBehavior: f,
                },
                p,
            ),
            A = (0, i.aX)({
                ...e,
                values: {
                    isDisabled: p.isDisabled,
                    isReadOnly: p.isReadOnly,
                    isRequired: e.isRequired || !1,
                    isInvalid: p.isInvalid,
                    state: p,
                },
                defaultClassName: "react-aria-CheckboxGroup",
            }),
            C = (0, E.z)(e, { global: !0 });
        return O.createElement(
            "div",
            {
                ...(0, u.d)(C, A, g),
                ref: t,
                slot: e.slot || void 0,
                "data-readonly": p.isReadOnly || void 0,
                "data-required": e.isRequired || void 0,
                "data-invalid": p.isInvalid || void 0,
                "data-disabled": e.isDisabled || void 0,
            },
            O.createElement(
                i.zt,
                {
                    values: [
                        [S, p],
                        [
                            s.n,
                            {
                                ...b,
                                ref: _,
                                elementType: "span",
                            },
                        ],
                        [
                            l.j,
                            {
                                slots: {
                                    description: y,
                                    errorMessage: I,
                                },
                            },
                        ],
                        [a.E, T],
                    ],
                },
                A.children,
            ),
        );
    }),
    T = (0, O.forwardRef)(function (e, t) {
        var n, a;
        let { inputRef: s = null, ...l } = e;
        [e, t] = (0, i.pE)(l, t, r.cP);
        let { validationBehavior: c } = (0, i.jn)(o.q) || {},
            h = null != (a = null != (n = e.validationBehavior) ? n : c) ? a : "native",
            v = (0, O.useContext)(S),
            I = (0, b.B)((0, O.useMemo)(() => (0, y.l)(s, void 0 !== e.inputRef ? e.inputRef : null), [s, e.inputRef])),
            {
                labelProps: T,
                inputProps: A,
                isSelected: C,
                isDisabled: N,
                isReadOnly: P,
                isPressed: R,
                isInvalid: w,
            } = v
                ? (0, d.a)(
                      {
                          ...(0, i.vl)(e),
                          value: e.value,
                          children: "function" == typeof e.children || e.children,
                      },
                      v,
                      I,
                  )
                : (0, f.O)(
                      {
                          ...(0, i.vl)(e),
                          children: "function" == typeof e.children || e.children,
                          validationBehavior: h,
                      },
                      (0, g.l)(e),
                      I,
                  ),
            { isFocused: D, isFocusVisible: x, focusProps: L } = (0, p.F)(),
            j = N || P,
            { hoverProps: M, isHovered: k } = (0, _.X)({
                ...e,
                isDisabled: j,
            }),
            U = (0, i.aX)({
                ...e,
                defaultClassName: "react-aria-Checkbox",
                values: {
                    isSelected: C,
                    isIndeterminate: e.isIndeterminate || !1,
                    isPressed: R,
                    isHovered: k,
                    isFocused: D,
                    isFocusVisible: x,
                    isDisabled: N,
                    isReadOnly: P,
                    isInvalid: w,
                    isRequired: e.isRequired || !1,
                },
            }),
            G = (0, E.z)(e, { global: !0 });
        return (
            delete G.id,
            delete G.onClick,
            O.createElement(
                "label",
                {
                    ...(0, u.d)(G, T, M, U),
                    ref: t,
                    slot: e.slot || void 0,
                    "data-selected": C || void 0,
                    "data-indeterminate": e.isIndeterminate || void 0,
                    "data-pressed": R || void 0,
                    "data-hovered": k || void 0,
                    "data-focused": D || void 0,
                    "data-focus-visible": x || void 0,
                    "data-disabled": N || void 0,
                    "data-readonly": P || void 0,
                    "data-invalid": w || void 0,
                    "data-required": e.isRequired || void 0,
                },
                O.createElement(
                    m.T,
                    { elementType: "span" },
                    O.createElement("input", {
                        ...(0, u.d)(A, L),
                        ref: I,
                    }),
                ),
                U.children,
            )
        );
    });
