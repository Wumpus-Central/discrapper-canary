n.d(t, { default: () => M });
var r = n(627968),
    a = n(64700),
    l = n(503698),
    s = n.n(l),
    i = n(311907),
    c = n(366010),
    o = n(192308),
    d = n(534514),
    u = n(834730),
    _ = n(821609),
    h = n(736653),
    C = n(775602),
    A = n(793574),
    R = n(979286),
    E = n(607470),
    f = n(318346),
    m = n(761705),
    b = n(106799),
    p = n(829219),
    g = n(341915),
    y = n(890687),
    S = n(92246),
    B = n(109174),
    I = n(524728),
    L = n(654487),
    N = n(652215),
    U = n(758836),
    T = n(985018),
    O = n(823628),
    x = n(627898),
    v = n(341744),
    F = n(484252),
    w = n(460290),
    V = n(181713);
function D(e) {
    let { onVideoLoaded: t } = e;
    return (0, r.jsx)(E.A, {
        className: O.d0,
        playsInline: !0,
        controls: !1,
        autoPlay: !0,
        loop: !0,
        onCanPlayThrough: t,
        children: (0, r.jsx)("source", { src: V.A, type: "video/webm" }),
    });
}
function P(e) {
    let { onVideoLoaded: t, isLoading: n } = e,
        l = (0, i.bG)([C.A], () => C.A.useReducedMotion),
        [o, d] = a.useState(!l),
        u = (0, h.Ay)(),
        _ = (0, c.q)(u),
        A = _ ? v.A : x.A,
        R = _ ? w.A : F.A,
        f = a.useRef(null),
        m = a.useRef(null),
        [b, p] = a.useState(!1),
        [g, y] = a.useState(!1),
        [S, B] = a.useState(!1);
    a.useEffect(() => {
        b && g && S && t();
    }, [b, g, S, t]),
        a.useEffect(() => {
            n ||
                l ||
                setTimeout(() => {
                    f.current?.play();
                }, 200);
        }, [n, l]);
    let I = a.useCallback(() => {
        l || (d(!1), m.current?.play());
    }, [l]);
    return (0, r.jsxs)("div", {
        className: O.GF,
        children: [
            (0, r.jsx)(E.A, {
                ref: m,
                autoPlay: !1,
                loop: !0,
                muted: !0,
                playsInline: !0,
                className: s()(O.nH, O.EZ, { [O.R]: o }),
                controls: !1,
                onCanPlayThrough: a.useCallback(() => y(!0), []),
                children: (0, r.jsx)("source", { src: R, type: "video/webm" }),
            }),
            (0, r.jsx)(E.A, {
                ref: f,
                autoPlay: !1,
                muted: !0,
                playsInline: !0,
                className: O.nH,
                controls: !1,
                onCanPlayThrough: a.useCallback(() => p(!0), []),
                onEnded: I,
                children: (0, r.jsx)("source", { src: A, type: "video/webm" }),
            }),
            (0, r.jsxs)("div", {
                className: O.Se,
                children: [
                    (0, r.jsx)(D, { onVideoLoaded: a.useCallback(() => B(!0), []) }),
                    (0, r.jsx)(D, {}),
                    (0, r.jsx)(D, {}),
                    (0, r.jsx)(D, {}),
                ],
            }),
        ],
    });
}
function j(e) {
    let { isLoading: t, orbsEarned: n, orbsBalance: a, onVideoLoaded: l } = e;
    return (0, r.jsxs)("div", {
        className: O.hQ,
        children: [
            (0, r.jsx)(P, { isLoading: t, onVideoLoaded: l }),
            (0, r.jsxs)("div", {
                className: O.dD,
                children: [
                    (0, r.jsxs)("div", {
                        className: O.R_,
                        children: [
                            (0, r.jsx)(b.B, { shouldUseThemeColor: !0, className: O.Uu }),
                            (0, r.jsx)(d.D, {
                                variant: "heading-xl/semibold",
                                color: "text-strong",
                                children: T.intl.formatToPlainString(T.t.YMor7k, { count: n }),
                            }),
                        ],
                    }),
                    null != a
                        ? (0, r.jsx)(u.E, {
                              variant: "text-sm/medium",
                              color: "text-subtle",
                              children: T.intl.format(T.t.g4t9aw, {
                                  orbsBalance: a,
                                  orbsBalanceHook: (e, t) =>
                                      (0, r.jsx)(
                                          u.E,
                                          { variant: "text-sm/medium", color: "text-strong", tag: "span", children: e },
                                          t,
                                      ),
                              }),
                          })
                        : null,
                ],
            }),
            (0, r.jsx)(_.$, {
                variant: "secondary",
                size: "sm",
                text: T.intl.string(T.t.WYchde),
                onClick: () => {
                    (0, f.Y)({
                        pageType: N.liQ.GLOBAL_DISCOVERY_QUESTS,
                        sectionType: N.JJy.ORBS_REWARD_MODAL,
                        ctaObject: N.ZSU.CTA_TO_ORBS_SHOP,
                    }),
                        (0, o.closeAllModals)(),
                        (0, R.Cz)({ tab: U.G2.ORBS, analyticsLocations: [], analyticsSource: A.A.QUEST_REWARD_MODAL });
                },
                fullWidth: !0,
            }),
        ],
    });
}
function M(e) {
    let { initialQuest: t, onClose: n, transitionState: l, sourceQuestContent: s, location: i } = e,
        c = (0, y.C5)(t.id) ?? t,
        o = c.userStatus?.claimedAt != null,
        d = (0, y.SD)(c),
        u = (0, y.Oq)(),
        { balance: _, error: h } = (0, m.W)(),
        C = a.useMemo(() => (d && u ? (0, S.zB)(c.config) : (0, S._Z)(c.config)), [c.config, d, u]),
        [A, R] = a.useState(o ? "success" : "loading"),
        [E, f] = a.useState(!1),
        b = a.useCallback(() => {
            f(!0);
        }, []);
    a.useEffect(() => {
        o ||
            "loading" !== A ||
            (0, p.Oq)(c.id, g.pY.CROSS_PLATFORM, i)
                .then(() => R("success"))
                .catch(() => R("error"));
    }, [c.id, o, A, i]);
    let N = !E || "loading" === A || (null == _ && null == h),
        U = "error" === A || null == C || (null == _ && null != h);
    return (0, r.jsx)(I.A, {
        onClose: n,
        transitionState: l,
        quest: c,
        sourceQuestContent: s,
        location: L.rE.ORBS_REWARD_MODAL,
        isRewardContentLoading: N,
        rewardContentHasError: U,
        rewardContent: U
            ? null
            : (0, r.jsx)(B.A, {
                  rewardName: T.intl.string(T.t.Lmysvd),
                  children: (0, r.jsx)(j, { orbsEarned: C, orbsBalance: _, isLoading: N, onVideoLoaded: b }),
              }),
    });
}
