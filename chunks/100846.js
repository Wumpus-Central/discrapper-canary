...
async function s(e) {
    ...
    try {
        ...
        for (; 0 !== r._WebPAnimDecoderHasMoreFrames(i); ) {
            ...
            let f = r.HEAPU32[e >> 2],
                c = r.HEAP32[o >> 2],
                d = new Uint8Array(r.HEAPU8.buffer, f, s).slice();
            l.push({ data: d, timestamp: c, width: t, height: a }), r._free(e), r._free(o);
        }
...
async function f(e, r, t) {
    ...
    let f = await l(),
        c = a.quality ?? 85,
        d = a.method ?? 4,
        m = n(f, 128, "WebPAnimEncoderOptions");
    f._WebPAnimEncoderOptionsInitInternal(m, 265), (f.HEAP32[(m >> 2) + 2] = 1), (f.HEAP32[(m >> 2) + 5] = 0);
    let u = f._WebPAnimEncoderNewInternal(r, t, m, 265);
    if ((f._free(m), 0 === u)) throw Error("Failed to create WebP encoder");
    let _ = f._malloc(256);
    if (0 === _)
        throw (f._WebPAnimEncoderDelete(u), Error("Failed to allocate 256 bytes for WebPConfig - out of WASM memory"));
    f._WebPConfigInitInternal(_, 0, c, 527), (f.HEAP32[(_ >> 2) + 2] = d);
    try {
        for (let a = 0; a < e.length; a++) {
            let n = e[a],
                l = n.timestamp ?? 100 * a,
            ...
            f.HEAPU8.set(n.data, d);
            let m = 4 * r,
                h = f._WebPPictureImportRGBA(s, d, m);
            if ((f._free(d), 0 === h)) {
                let e = (function (e, r) {
            ...
            let b = f._WebPAnimEncoderAdd(u, s, l, _);
        ...
        let a = e[e.length - 1].timestamp ?? (e.length - 1) * 100,
            n = f._WebPAnimEncoderAdd(u, 0, a + 100, 0);
        if (0 === n) {
            let e = i(f, u);
        ...
        let s = f._WebPAnimEncoderAssemble(u, l);
        if (0 === s) {
            let e = i(f, u);
        ...
        let c = f.HEAPU32[l >> 2],
            d = f.HEAPU32[(l >> 2) + 1],
            m = new Uint8Array(f.HEAPU8.buffer, c, d).slice();
        return f._WebPFree(c), f._free(l), m;
    } finally {
        f._free(_), f._WebPAnimEncoderDelete(u);
    }
}
async function c(e) {
    let {
    ...
    let { frames: u, width: _, height: h } = await s(r),
        b = u,
        E = _,
        p = h;
    ...
    if (
                ...
                data: (function (e, r, t, a) {
                    ...
                    for (let a = 0; a < t; a++)
                        for (let n = 0; n < r; n++) {
                            ...
                            let d = (f * i + s) * 4;
                            (l[d] = e[c]), (l[d + 1] = e[c + 1]), (l[d + 2] = e[c + 2]), (l[d + 3] = e[c + 3]);
                        }
        ...
        null != c && null == d
            ? (r = Math.round((p / E) * c))
            : null != d && null == c && (e = Math.round((E / p) * d)),
            (e !== E || r !== p) &&
                ((b = b.map((t) => ({
    ...
    return await f(b, E, p, { quality: m });
}
