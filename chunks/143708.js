n.d(t, { Z: () => g });
var r = n(54381);
n(473749);
var i = n(793030),
    a = n(442837),
    o = n(144144),
    s = n(955415),
    l = n(665906),
    c = n(314897),
    u = n(592125),
    d = n(496675),
    f = n(585483),
    p = n(51144),
    _ = n(981631),
    m = n(388032),
    h = n(105341);
function g(e) {
    let t,
        n,
        g,
        { author: E, banned: b, channelId: y } = e,
        O = (0, a.e7)([c.default], () => c.default.getId()),
        v = p.ZP.useName(E),
        S = O === E.id,
        I = (0, a.e7)([u.Z, d.Z], () => {
            let e = u.Z.getChannel(y);
            if (null == e) return !1;
            if (e.isPrivate()) return !0;
            if (e.isThread()) {
                let t = (0, l.xl)(e),
                    n = d.Z.can(_.Plq.SEND_MESSAGES_IN_THREADS, e);
                return !t && n;
            }
            return d.Z.can(_.Plq.SEND_MESSAGES, e);
        });
    function T() {
        let { id: e } = E,
            t = "@".concat(p.ZP.getUserTag(E, { decoration: "never" })),
            n = "<@".concat(e, ">");
        f.S.dispatchToLastSubscribed(_.CkL.INSERT_TEXT, {
            plainText: t,
            rawText: n,
        }),
            null != y && o.Z.startTyping(y);
    }
    return (
        S
            ? ((t = m.intl.string(m.t["F/OLvL"])), (g = m.intl.string(m.t.C89OLE)))
            : ((g = m.intl.string(m.t.YVub5y)),
              b
                  ? (t = m.intl.string(m.t["57nBty"]))
                  : null != v && I
                    ? ((t = m.intl.formatToPlainString(m.t["9Akp1s"], { username: v })),
                      (n = (0, r.jsx)(i.zxk, {
                          onClick: T,
                          text: m.intl.string(m.t.P8tvKG),
                      })))
                    : (t = m.intl.string(m.t["SMJr+a"]))),
        (0, r.jsxs)(s.Z, {
            children: [
                (0, r.jsx)(s.Z.Header, { text: g }),
                (0, r.jsxs)(s.Z.Body, {
                    children: [
                        (0, r.jsxs)("div", {
                            className: h.headerLine,
                            children: [
                                (0, r.jsx)(s.Z.Icon, { expired: !0 }),
                                (0, r.jsx)(s.Z.Info, {
                                    expired: !0,
                                    title: m.intl.string(m.t["Jhx/ud"]),
                                    children: t,
                                }),
                            ],
                        }),
                        n,
                    ],
                }),
            ],
        })
    );
}
