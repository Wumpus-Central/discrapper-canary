let r, i, a, o, s, l, c, u, d, f, _, p, h, m, g, E, b, y, O, v, I, T, S, A, C;
n.d(t, { Z: () => ez }), n(388685);
var N,
    R = n(392711),
    P = n(754700),
    w = n(887003),
    D = n(442837),
    x = n(570140),
    L = n(70956),
    M = n(509212),
    j = n(497505),
    k = n(184299),
    U = n(704161),
    G = n(5881),
    B = n(46140);
function Z(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0,
              })
            : (e[t] = n),
        e
    );
}
function F(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            r = Object.keys(n);
        "function" == typeof Object.getOwnPropertySymbols &&
            (r = r.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                }),
            )),
            r.forEach(function (t) {
                Z(e, t, n[t]);
            });
    }
    return e;
}
function V(e, t) {
    var n = Object.keys(e);
    if (Object.getOwnPropertySymbols) {
        var r = Object.getOwnPropertySymbols(e);
        t &&
            (r = r.filter(function (t) {
                return Object.getOwnPropertyDescriptor(e, t).enumerable;
            })),
            n.push.apply(n, r);
    }
    return n;
}
function H(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : V(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
let Y = 6 * L.Z.Millis.HOUR,
    W = new Map(),
    K = null,
    z = new Map(),
    q = 1000;
function X() {
    (r = !1),
        (i = !1),
        (a = new Map()),
        (o = !1),
        (s = new Map()),
        (l = new Map()),
        (c = new Map()),
        (u = 0),
        (d = 0),
        (f = new Set()),
        (_ = new Set()),
        (p = new Set()),
        (h = new Set()),
        (m = new Set()),
        (W = new Map()),
        (E = new Map()),
        (b = new Map()),
        (y = new Map()),
        (O = null),
        (v = new Map()),
        (g = new Set()),
        (I = new Map()),
        (T = new Map()),
        eH(),
        (S = null),
        (C = new Map()),
        (z = new Map()),
        (A = new Map());
}
function Q(e, t) {
    var n, r, i, a;
    if (null != t.userStatus)
        for (let o of Object.values(null != (r = null == (n = t.userStatus) ? void 0 : n.progress) ? r : {}))
            !(0, R.isNil)(o) &&
                P.T.DESKTOP.has(o.eventName) &&
                ((null == (i = o.heartbeat) ? void 0 : i.lastBeatAt) != null
                    ? g.add(e)
                    : (null == (a = o.heartbeat) ? void 0 : a.lastBeatAt) == null && g.delete(e));
}
function J(e, t) {
    let n = (s = new Map(s)).get(e);
    if (null != n) {
        let r = F({}, n, t);
        if ((Q(e, t), s.set(e, r), C.has(e))) {
            let n = C.get(e);
            null != n && (C = new Map(C)).set(e, F({}, n, t));
        }
    }
}
function $(e, t) {
    let n = new Map(E);
    n.set(e, t), (E = n);
}
function ee(e, t) {
    $(e, t);
    let n = s.get(e),
        r = null == n ? void 0 : n.userStatus;
    null != r && null == r.claimedAt && J(e, { userStatus: H(F({}, r), { claimedAt: t.claimedAt }) });
}
function et(e) {
    var t;
    let { entitlements: n } = e,
        r = null == (t = n.items[0].tenantMetadata) ? void 0 : t.questRewards.reward;
    return (null == r ? void 0 : r.tag) !== w.w.REWARD_CODE ? null : r.rewardCode;
}
function en(e, t) {
    let n = new Map(b);
    n.set(e, t.items), (b = n);
    let r = s.get(e),
        i = null == r ? void 0 : r.userStatus;
    if (null != i && null == i.claimedAt) {
        var a;
        let n = et({ entitlements: t });
        null != n && $(e, n),
            J(e, {
                userStatus: H(F({}, i), {
                    claimedAt: t.claimedAt,
                    claimedTier: null != (a = null == n ? void 0 : n.tier) ? a : null,
                }),
            });
    }
}
function er(e) {
    null != y.get(e) && (y = new Map(y)).delete(e);
}
function ei() {
    eH(), X();
}
function ea() {
    r = !0;
}
function eo(e) {
    let { quests: t, excludedQuests: n, questEnrollmentBlockedUntil: i } = e;
    (u = Date.now()), (r = !1), (s = new Map());
    let a = new Map();
    for (let e of t)
        s.set(e.id, e),
            a.set(e.id, (0, M.zi)(e)),
            e.targetedContent.includes(j.jn.QUEST_BAR) &&
                (0, G.T)({ location: B.dr.QUESTS_STORE }).log(
                    "Delivered ".concat(e.config.messages.questName, " (").concat(e.id, ")"),
                );
    for (let e of ((l = new Map()), n)) l.set(e.id, e);
    for (let e of null == C ? void 0 : C.values()) s.has(e.id) || (s.set(e.id, e), a.set(e.id, (0, M.zi)(e)));
    (T = a), eV(), (S = null != i ? new Date(i) : null);
}
function es() {
    (u = 0), (r = !1);
}
function el(e) {
    let { placement: t } = e;
    (i = !0), (a = new Map(a)).set(t, !0);
}
function ec(e) {
    let { questId: t } = e;
    (m = new Set(m)).add(t), (z = new Map(z)).delete(t);
}
function eu(e) {
    let { questId: t, quest: n } = e;
    (m = new Set(m)).delete(t), (C = new Map(C)).set(t, n), (s = new Map(s)).set(t, n), (z = new Map(z)).delete(t);
}
function ed(e) {
    let { questId: t, error: n } = e;
    (m = new Set(m)).delete(t), (z = new Map(z)).set(t, n);
}
function ef() {
    o = !0;
}
function e_(e) {
    let { quests: t } = e;
    for (let e of ((o = !1), (c = new Map()), t)) c.set(e.id, e);
}
function ep() {
    o = !1;
}
function eh(e) {
    let { questId: t, streamKey: n, userStatus: r } = e;
    g.add(t), J(t, { userStatus: r }), null != n && er(n);
}
function em(e) {
    let { questId: t, streamKey: n } = e;
    null != n &&
        null == y.get(n) &&
        (y = new Map(y)).set(n, {
            questId: t,
            streamKey: n,
            firstFailedAt: Date.now(),
        });
}
function eg(e) {
    let { streamKey: t } = e;
    er(t);
}
function eE(e) {
    let t = new Set(f);
    t.delete(e), (f = t);
}
function eb(e) {
    let { questId: t } = e,
        n = new Set(f);
    n.add(t), (f = n);
}
function ey(e) {
    let { enrolledQuestUserStatus: t } = e;
    J(t.questId, { userStatus: t }), eE(t.questId);
}
function eO(e) {
    let { questId: t } = e;
    eE(t);
}
function ev(e) {
    let { questId: t } = e,
        n = new Set(p);
    n.add(t), (p = n);
}
function eI(e) {
    let { questId: t, rewardCode: n } = e,
        r = new Set(p);
    r.delete(t), (p = r), ee(t, n);
}
function eT(e) {
    let { questId: t } = e,
        n = new Set(p);
    n.delete(t), (p = n);
}
function eS(e) {
    let { questId: t } = e,
        n = new Set(_);
    n.add(t), (_ = n);
}
function eA(e) {
    let { questId: t, entitlements: n } = e,
        r = new Set(_);
    r.delete(t), (_ = r), en(t, n);
}
function eC(e) {
    let { questId: t } = e,
        n = new Set(_);
    n.delete(t), (_ = n);
}
function eN(e) {
    let t = new Set(h);
    t.delete(e), (h = t);
}
function eR(e) {
    let { questId: t } = e,
        n = new Set(h);
    n.add(t), (h = n);
}
function eP(e) {
    let { dismissedQuestUserStatus: t } = e;
    J(t.questId, { userStatus: t }), eN(t.questId);
}
function ew(e) {
    let { questId: t } = e;
    eN(t);
}
function eD(e) {
    let { streamKey: t } = e;
    er(t);
}
function ex(e) {
    let { user_status: t } = e,
        n = (0, G.T)({ location: B.dr.QUESTS_STORE });
    n.log("Received user status update for ".concat(t.quest_id), t);
    let r = (0, M.U3)(t);
    J(t.quest_id, { userStatus: r });
    let i = s.get(t.quest_id);
    if (null != i) {
        let e = (0, M.zi)(i);
        T.get(t.quest_id) !== e && (T = new Map(T).set(t.quest_id, e));
    }
    0 === Object.keys(r.progress).length &&
        W.has(r.questId) &&
        (n.log("Removing optimistic progress for ".concat(r.questId)), W.delete(r.questId));
}
function eL(e) {
    let { previewQuestUserStatus: t } = e;
    J(t.questId, { userStatus: t }),
        null == t.claimedAt && (E = new Map(E)).delete(t.questId),
        null == t.enrolledAt && ((v = new Map(v)).delete(t.questId), k.ZP.getState().resetQuest(t.questId));
    let n = s.get(t.questId);
    if (null != n) {
        let e = (0, M.zi)(n);
        T.get(t.questId) !== e && (T = new Map(T).set(t.questId, e));
    }
}
function eM(e) {
    let { questId: t } = e;
    O = O === t ? null : t;
}
function ej(e) {
    let { questId: t, platform: n } = e;
    (v = new Map(v)), null == n ? v.delete(t) : v.set(t, n);
}
function ek(e) {
    var t;
    let { questId: n, taskEventName: r, progress: i } = e,
        a = null != (t = W.get(n)) ? t : new Map();
    a.set(r, i), W.set(n, a);
}
function eU(e) {
    let { questId: t } = e;
    W.has(t) && W.delete(t), k.ZP.getState().resetQuest(t);
}
function eG(e) {
    let { quest: t, placement: n, adDecisionData: r, adContext: o, responseTtlSeconds: s, metadataRaw: l } = e;
    (d = Date.now()), (i = !1), (a = new Map(a)).set(n, !1);
    let { enableNewRequestBehavior: c } = U.Z.getConfig({ location: "handleFetchQuestToDeliverSuccess" });
    if (c) {
        var u;
        let e = {
            questId: null != (u = null == t ? void 0 : t.id) ? u : null,
            fetchedAt: d,
            ttlMillis: eB(s),
        };
        (A = new Map(A)).set(n, e);
    } else
        null == t
            ? I.delete(n)
            : I.set(n, {
                  quest: t,
                  adDecisionData: r,
                  adContext: o,
                  metadataRaw: l,
              });
}
function eB(e) {
    if (null == e) return Y;
    let t = 1000 * e;
    return t < Y && t > 0 ? t : Y;
}
function eZ(e) {
    let { placement: t } = e;
    I.delete(t), (d = Date.now()), (i = !1), (a = new Map(a)).set(t, !1);
}
function eF() {
    let e = !1,
        t = new Map(T);
    s.forEach((n, r) => {
        !0 !== t.get(r) && ((0, M.zi)(n) ? (t.set(r, !0), (e = !0)) : t.has(r) || t.set(r, !1));
    }),
        e && ((T = t), eK.emitChange());
}
function eV() {
    null === K &&
        (eF(),
        (K = setInterval(() => {
            eF();
        }, q)));
}
function eH() {
    null !== K && (clearInterval(K), (K = null));
}
function eY(e) {
    let { quest_enrollment_blocked_until: t } = e;
    S = null != t ? new Date(t) : null;
}
X();
class eW extends (N = D.ZP.Store) {
    get quests() {
        return s;
    }
    get excludedQuests() {
        return l;
    }
    get claimedQuests() {
        return c;
    }
    get isFetchingCurrentQuests() {
        return r;
    }
    get isFetchingClaimedQuests() {
        return o;
    }
    isFetchingQuestPreview(e) {
        return m.has(e);
    }
    get lastFetchedCurrentQuests() {
        return u;
    }
    get lastFetchedQuestToDeliver() {
        return d;
    }
    get isFetchingQuestToDeliver() {
        return i;
    }
    isFetchingQuestToDeliverByPlacement(e) {
        var t;
        return null != (t = null == a ? void 0 : a.get(e)) && t;
    }
    get questDeliveryOverride() {
        return s.get(null != O ? O : "");
    }
    get questToDeliverForPlacement() {
        return I;
    }
    get questEnrollmentBlockedUntil() {
        return S;
    }
    get questAdDecisionByPlacement() {
        return A;
    }
    getFetchQuestPreviewError(e) {
        return z.get(e);
    }
    isEnrolling(e) {
        return f.has(e);
    }
    isClaimingReward(e) {
        return _.has(e);
    }
    isFetchingRewardCode(e) {
        return p.has(e);
    }
    isDismissingContent(e) {
        return h.has(e);
    }
    getRewardCode(e) {
        return E.get(e);
    }
    getRewards(e) {
        return b.get(e);
    }
    getStreamHeartbeatFailure(e) {
        return y.get(e);
    }
    getQuest(e) {
        return s.get(e);
    }
    isProgressingOnDesktop(e) {
        return g.has(e);
    }
    selectedTaskPlatform(e) {
        var t;
        return null != (t = v.get(e)) ? t : null;
    }
    getOptimisticProgress(e, t) {
        var n;
        return null == (n = W.get(e)) ? void 0 : n.get(t);
    }
    getExpiredQuestsMap() {
        return T;
    }
    isQuestExpired(e) {
        var t;
        return null != (t = T.get(e)) && t;
    }
    getQuestLoadedViaPreview(e) {
        return C.get(e);
    }
}
Z(eW, "displayName", "QuestsStore");
let eK = new eW(x.Z, {
        LOGOUT: ei,
        QUESTS_FETCH_CURRENT_QUESTS_BEGIN: ea,
        QUESTS_FETCH_CURRENT_QUESTS_SUCCESS: eo,
        QUESTS_FETCH_CURRENT_QUESTS_FAILURE: es,
        QUESTS_FETCH_CLAIMED_QUESTS_BEGIN: ef,
        QUESTS_FETCH_CLAIMED_QUESTS_SUCCESS: e_,
        QUESTS_FETCH_CLAIMED_QUESTS_FAILURE: ep,
        QUESTS_FETCH_QUEST_TO_DELIVER_BEGIN: el,
        QUESTS_FETCH_QUEST_TO_DELIVER_SUCCESS: eG,
        QUESTS_FETCH_QUEST_TO_DELIVER_FAILURE: eZ,
        QUESTS_FETCH_PREVIEW_BEGIN: ec,
        QUESTS_FETCH_PREVIEW_SUCCESS: eu,
        QUESTS_FETCH_PREVIEW_FAILURE: ed,
        QUESTS_SEND_HEARTBEAT_SUCCESS: eh,
        QUESTS_SEND_HEARTBEAT_FAILURE: em,
        QUESTS_ENROLL_BEGIN: eb,
        QUESTS_ENROLL_SUCCESS: ey,
        QUESTS_ENROLL_FAILURE: eO,
        QUESTS_FETCH_REWARD_CODE_BEGIN: ev,
        QUESTS_FETCH_REWARD_CODE_SUCCESS: eI,
        QUESTS_FETCH_REWARD_CODE_FAILURE: eT,
        QUESTS_CLAIM_REWARD_BEGIN: eS,
        QUESTS_CLAIM_REWARD_SUCCESS: eA,
        QUESTS_CLAIM_REWARD_FAILURE: eC,
        QUESTS_DISMISS_CONTENT_BEGIN: eR,
        QUESTS_DISMISS_CONTENT_SUCCESS: eP,
        QUESTS_DISMISS_CONTENT_FAILURE: ew,
        QUESTS_USER_STATUS_UPDATE: ex,
        STREAM_CLOSE: eg,
        QUESTS_DISMISS_PROGRESS_TRACKING_FAILURE_NOTICE: eD,
        QUESTS_PREVIEW_UPDATE_SUCCESS: eL,
        QUESTS_DELIVERY_OVERRIDE: eM,
        QUESTS_SELECT_TASK_PLATFORM: ej,
        QUESTS_UPDATE_OPTIMISTIC_PROGRESS: ek,
        QUESTS_RESET_OPTIMISTIC_PROGRESS: eU,
        QUESTS_USER_COMPLETION_UPDATE: eY,
    }),
    ez = eK;
