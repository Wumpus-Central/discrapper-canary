l.d(t, { default: () => F });
var n = l(627968),
    a = l(64700),
    r = l(989349),
    i = l.n(r),
    s = l(772707),
    o = l(189213),
    c = l(311907),
    d = l(783878),
    u = l(451394),
    h = l(292666),
    g = l(834730),
    x = l(260598),
    m = l(939249),
    f = l(658675),
    p = l(964486),
    v = l(47167),
    _ = l(232246),
    j = l(974930),
    A = l(422845),
    b = l(71393),
    y = l(994500),
    w = l(287809),
    N = l(954571),
    C = l(272379),
    E = l(446600),
    I = l(366098),
    T = l(918192),
    M = l(660110),
    k = l(567854),
    D = l(516607),
    S = l(652215),
    O = l(988794),
    R = l(985018),
    V = l(134473),
    Z = l(973324);
function F(e) {
    let { channel: t, onClose: l, transitionState: r } = e,
        i = (0, c.bG)([b.A], () => b.A.getGuild(t.guild_id));
    return (a.useEffect(() => {
        null == i && l();
    }, [i, l]),
    null == i)
        ? null
        : (0, n.jsx)(G, { transitionState: r, guild: i, channel: t, onClose: l });
}
function L(e) {
    let { stageChannelsInGuild: t, channel: l, onSelectChannel: a } = e;
    return null == a
        ? null
        : (0, n.jsx)("div", {
              className: V.BD,
              children: (0, n.jsx)(d.Z, {
                  selectionMode: "single",
                  required: !0,
                  label: R.intl.string(R.t.S7GjDz),
                  value: l.id,
                  options: t.map((e) => ({
                      id: e.id,
                      value: e.id,
                      label: (0, v.m1)(e, w.default, y.A, !0),
                      leading: (0, n.jsx)(u.q, { size: "custom", color: "currentColor", height: 24 }),
                  })),
                  onSelectionChange: (e) => {
                      let l = t.find((t) => t.id === e);
                      null != l && a(l);
                  },
              }),
          });
}
function G(e) {
    let { channel: t, guild: l, onClose: r, onSelectChannel: c, transitionState: d, isEvent: u = !1 } = e,
        { loading: b, error: y, onSave: w } = (0, k.A)(t, r),
        F = a.useMemo(() => E.A.getStageInstanceByChannel(t.id), [t.id]),
        [G, B] = a.useState(F?.topic ?? ""),
        [P, z] = a.useState(""),
        [U] = a.useState(u),
        [H, Q] = a.useState({ startDate: (0, j.jd)() }),
        [q, Y] = a.useState(!1),
        K = (0, C.Z)(t),
        X = (0, C.K)(t),
        J = null == F && K && !U,
        [W, $] = a.useState(J && X),
        ee = O.dD.GUILD_ONLY,
        [et] = a.useState(F?.privacy_level ?? ee),
        [el, en] = a.useState(null),
        ea = (0, I.D3)(t.id),
        er = (0, I.Xk)(t.id),
        [ei, es] = a.useState(!1),
        eo = (0, v.Ay)(t),
        ec = (0, _.A)(l),
        ed = null != c,
        eu = ec.length > 1;
    (0, p.Ay)(() => {
        N.default.track(S.HAw.START_STAGE_OPENED, {
            stage_instance_id: F?.id,
            can_start_public_stage: !1,
            guild_id: t.guild_id,
        });
    });
    let eh = (e) => {
            (e.preventDefault(), et === O.dD.PUBLIC && G.length < 20 && !ei)
                ? es(!0)
                : U || w?.({ topic: G, privacyLevel: et, sendStartNotification: W });
        },
        eg = a.useRef(null);
    a.useEffect(() => {
        eg.current?.focus();
    }, []);
    let ex = q && null != H.startDate && H.startDate >= i()(),
        em = (0, n.jsxs)(n.Fragment, {
            children: [
                (0, n.jsx)("div", {
                    className: V.GU,
                    children: null == F && (ea > 0 || er > 0) && (0, n.jsx)(T.Bw, { channelId: t.id }),
                }),
                (0, n.jsxs)("form", {
                    onSubmit: eh,
                    className: V.Zd,
                    children: [
                        (0, n.jsx)(h.k, {
                            required: !0,
                            error: null != y ? y.getAnyErrorMessage() : void 0,
                            label: u ? R.intl.string(R.t["0HbEQ6"]) : R.intl.string(R.t["5FPBOB"]),
                            onChange: (e) => B(e),
                            helperText: ei ? R.intl.string(R.t.AqTyaR) : void 0,
                            placeholder: R.intl.string(R.t.ZwWruY),
                            maxLength: D.RY,
                            value: G,
                            autoComplete: "off",
                            inputRef: eg,
                        }),
                        ed && eu ? (0, n.jsx)(L, { stageChannelsInGuild: ec, channel: t, onSelectChannel: c }) : null,
                        U &&
                            (0, n.jsxs)(n.Fragment, {
                                children: [
                                    (0, n.jsx)(A.A, {
                                        className: V.kz,
                                        onScheduleChange: Q,
                                        onRecurrenceChange: (e) => {
                                            let t = H.startDate;
                                            null == t || en((0, j.nG)(e, t));
                                        },
                                        schedule: H,
                                        recurrenceRule: el,
                                        timeSelected: q,
                                        onTimeChange: Y,
                                    }),
                                    null != H.startDate && H.startDate < i()()
                                        ? (0, n.jsx)(g.E, {
                                              color: "text-feedback-critical",
                                              variant: "text-xs/normal",
                                              className: V.$e,
                                              children: R.intl.string(R.t.AXR5Ss),
                                          })
                                        : null,
                                ],
                            }),
                        u &&
                            (0, n.jsx)("div", {
                                className: V.kz,
                                children: (0, n.jsx)(x.f, {
                                    label: R.intl.string(R.t["+gRCC7"]),
                                    placeholder: R.intl.string(R.t["kWO/E8"]),
                                    value: P,
                                    onChange: (e) => z(e),
                                    maxLength: O.IJ,
                                }),
                            }),
                        ed && !eu
                            ? (0, n.jsx)(g.E, {
                                  color: "text-default",
                                  variant: "text-xs/normal",
                                  className: V.Qw,
                                  children: R.intl.format(R.t["S+9O7g"], {
                                      stageName: eo,
                                      stageHook: (e, t) => (0, n.jsx)("span", { className: V.HA, children: eo }, t),
                                  }),
                              })
                            : null,
                        (0, n.jsx)(M.A, { className: V.XI, channelId: t.id }),
                    ],
                }),
            ],
        }),
        ef = {
            onClose: r,
            title: null == F ? R.intl.string(R.t.DDF0cJ) : R.intl.string(R.t.YPdQOp),
            subtitle: null == F ? R.intl.string(R.t.bqQIwa) : R.intl.string(R.t["I+9bLx"]),
            transitionState: d,
            actions: [
                { variant: "secondary", text: R.intl.string(R.t["ETE/oC"]), onClick: () => r() },
                {
                    variant: "primary",
                    text: U
                        ? R.intl.string(R.t["60lJ0C"])
                        : null == F
                          ? R.intl.string(R.t.s8mM8A)
                          : R.intl.string(R.t.K344S7),
                    onClick: eh,
                    disabled: "" === G || null == et || (u && !ex),
                    loading: b,
                },
            ],
            actionBarInput: J
                ? (0, n.jsxs)(m.D, {
                      onClick: () => $(!W),
                      className: V.Qy,
                      children: [
                          (0, n.jsx)(f.P, { checked: W }),
                          (0, n.jsx)(g.E, {
                              color: "text-default",
                              variant: "text-sm/normal",
                              children: R.intl.string(R.t["Pe+Pwp"]),
                          }),
                      ],
                  })
                : void 0,
        };
    return null == F
        ? (0, n.jsx)(s.k, { graphic: { type: "image", src: Z.A }, ...ef, children: em })
        : (0, n.jsx)(o.Modal, { ...ef, children: em });
}
