...
(r.prototype.parse = function (e, t, n) {
    ...
    if (!f[I] && (S || (I && !p[I]))) {
        for (var y, C, N = -1, v = 0; v < c.length; v++) {
            var R = g.indexOf(c[v]);
            -1 !== R && (-1 === N || R < N) && (N = R);
        }
        -1 !== (C = -1 === N ? g.lastIndexOf("@") : g.lastIndexOf("@", N)) &&
            ((y = g.slice(0, C)), (g = g.slice(C + 1)), (this.auth = decodeURIComponent(y))),
            (N = -1);
        ...
        -1 === N && (N = g.length),
            (this.host = g.slice(0, N)),
            (g = g.slice(N)),
            this.parseHost(),
            (this.hostname = this.hostname || "");
        var O = "[" === this.hostname[0] && "]" === this.hostname[this.hostname.length - 1];
        if (!O)
            for (var b = this.hostname.split(/\./), v = 0, D = b.length; v < D; v++) {
                var L = b[v];
                if (L && !L.match(d)) {
    ...
    (r.prototype.resolveObject = function (e) {
        ...
        for (
            var N = S.slice(-1)[0],
                v = ((n.host || e.host || S.length > 1) && ("." === N || ".." === N)) || "" === N,
                R = 0,
                O = S.length;
            O >= 0;
            O--
        )
            "." === (N = S[O]) ? S.splice(O, 1) : ".." === N ? (S.splice(O, 1), R++) : R && (S.splice(O, 1), R--);
        if (!I && !T) for (; R--; ) S.unshift("..");
        I && "" !== S[0] && (!S[0] || "/" !== S[0].charAt(0)) && S.unshift(""),
            v && "/" !== S.join("/").substr(-1) && S.push("");
