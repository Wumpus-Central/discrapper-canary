...
let m = (e) => {
    ...
    if (
        null != C &&
        C instanceof i.Ey &&
        (C.code === r.tG.CARD_DECLINED && A && (y += ` ${p.intl.string(p.t.iWvwQS)}`),
        C.code === r.tG.INVALID_GIFT_REDEMPTION_FRAUD_REJECTED && (y = p.intl.string(p.t.ypuSd8)),
        C.code === d.t02.BILLING_NON_REFUNDABLE_PAYMENT_SOURCE && (y = p.intl.string(p.t.mXMmWE)),
        C.code === r.tG.INVALID_CURRENCY_FOR_PAYMENT_SOURCE && (y = p.intl.string(p.t.mC1Fjz)),
        ...
        y = (0, l.jsxs)(l.Fragment, { children: [p.intl.string(p.t["yVIm/G"]), " ", e] });
    }
