...
function _(e) {
    let { channel: t, onJump: _ } = e,
        ...
        C = i.useMemo(() => x.map((e) => e.message), [x]),
        v = (0, l.e7)([h.ZP], () => h.ZP.hasUnreadPins(t.id));
    i.useEffect(() => {
        v && s.Z.ackPins(t.id);
    }, [v, t.id]),
        (0, c.ZP)(() => {
            C.some(u.k5) && s.Z.fetchPins(t.id, { reset: !0 });
        });
    let j = i.useCallback(() => {
            s.Z.fetchPins(t.id);
        }, [t.id]),
        O = i.useCallback(() => {
            var e;
            s.Z.fetchPins(t.id, { before: null == (e = x.at(-1)) ? void 0 : e.pinnedAt });
        }, [t.id, x]),
