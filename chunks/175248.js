n.d(e, { E: () => E, QuestRewardModalUnverified: () => _ });
var s = n(627968),
    i = n(64700),
    r = n(772707),
    l = n(17928),
    u = n(289873),
    a = n(885574),
    o = n(834730),
    c = n(192308),
    C = n(830215),
    d = n(287809),
    A = n(710969),
    f = n(375708),
    T = n(105247),
    g = n(661965);
function _(t) {
    let { transitionState: e, onClose: n } = t,
        c = (0, l.bG)([d.default], () => d.default.getCurrentUser()?.email),
        [_, E] = i.useState({ status: "unknown" });
    i.useEffect(() => {
        E({ status: "loading" }),
            C.A.verifyResend().then(
                () => E({ status: "success" }),
                (t) => {
                    E({ status: "error" }), (0, A.RF)(t, { tags: { location: "QuestsRewardModalUnverified" } });
                },
            );
    }, []);
    let m =
        "error" === _.status
            ? f.intl.string(f.t.vjying)
            : "success" === _.status
              ? f.intl.format(f.t.qP5xYc, { emailAddress: c, emailAddressLink: `mailto:${c}` })
              : void 0;
    return (0, s.jsxs)(r.k, {
        transitionState: e,
        onClose: n,
        graphic: "loading" === _.status ? void 0 : { type: "image", src: g },
        title: "loading" === _.status ? void 0 : f.intl.string(f.t.c8eASM),
        subtitle: m,
        actions:
            "loading" === _.status ? void 0 : [{ variant: "secondary", text: f.intl.string(f.t.cpT0Cq), onClick: n }],
        children: [
            "loading" === _.status && (0, s.jsx)(u.y, {}),
            "success" === _.status &&
                (0, s.jsxs)("div", {
                    className: T.d,
                    children: [
                        (0, s.jsx)(a.m, { size: "xs", color: "currentColor", className: T.q }),
                        (0, s.jsx)(o.E, {
                            variant: "text-sm/normal",
                            color: "text-default",
                            children: f.intl.string(f.t.yb7itQ),
                        }),
                    ],
                }),
        ],
    });
}
function E() {
    (0, c.openModalLazy)(async () => {
        let { QuestRewardModalUnverified: t } = await Promise.resolve().then(n.bind(n, 175248));
        return (e) => (0, s.jsx)(t, { ...e });
    });
}
