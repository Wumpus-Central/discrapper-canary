n.d(t, {
    Vq: () => D,
    Xo: () => Z,
    qE: () => U,
    qe: () => j
}),
    n(415506),
    n(388685);
var r = n(255367),
    i = n(73800),
    a = n(120356),
    o = n.n(a),
    s = n(524979),
    l = n(1561),
    c = n(438784),
    u = n(696826),
    d = n(981729),
    _ = n(481060),
    f = n(686546),
    p = n(51144),
    h = n(182294),
    m = n(981631),
    g = n(419061),
    E = n(388032),
    b = n(661085);
function y(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0
              })
            : (e[t] = n),
        e
    );
}
function O(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            r = Object.keys(n);
        'function' == typeof Object.getOwnPropertySymbols &&
            (r = r.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                })
            )),
            r.forEach(function (t) {
                y(e, t, n[t]);
            });
    }
    return e;
}
function v(e, t) {
    var n = Object.keys(e);
    if (Object.getOwnPropertySymbols) {
        var r = Object.getOwnPropertySymbols(e);
        t &&
            (r = r.filter(function (t) {
                return Object.getOwnPropertyDescriptor(e, t).enumerable;
            })),
            n.push.apply(n, r);
    }
    return n;
}
function I(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : v(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
function T(e, t) {
    if (null == e) return {};
    var n,
        r,
        i = S(e, t);
    if (Object.getOwnPropertySymbols) {
        var a = Object.getOwnPropertySymbols(e);
        for (r = 0; r < a.length; r++) (n = a[r]), !(t.indexOf(n) >= 0) && Object.prototype.propertyIsEnumerable.call(e, n) && (i[n] = e[n]);
    }
    return i;
}
function S(e, t) {
    if (null == e) return {};
    var n,
        r,
        i = {},
        a = Object.keys(e);
    for (r = 0; r < a.length; r++) (n = a[r]), t.indexOf(n) >= 0 || (i[n] = e[n]);
    return i;
}
let A = {
    tension: 1200,
    friction: 70
};
function N() {
    return document.hasFocus();
}
function C(e) {
    var { size: t, isMobile: n, isTyping: i } = e,
        a = T(e, ['size', 'isMobile', 'isTyping']);
    let o = (0, h.UC)(t),
        s = o.status * (n && !i ? h.EW : 1),
        l = o.status * (i ? h.D6 : 1),
        c = i ? (o.status * h.D6 - o.status) / 2 : 0,
        u = o.size - o.status - c - o.offset,
        d = o.size - s - o.offset;
    return (0, r.jsx)(
        'rect',
        O(
            {
                x: u,
                y: d,
                width: l,
                height: s,
                fill: 'transparent',
                'aria-hidden': !0
            },
            a
        )
    );
}
function R(e, t) {
    let n = arguments.length > 2 && void 0 !== arguments[2] && arguments[2];
    return null != t
        ? E.intl.formatToPlainString(E.t['/6mw19'], {
              label: e,
              status: (0, p.u5)(t, n)
          })
        : e;
}
function P(e, t, n, r) {
    if (null == e) return null;
    if (r)
        switch (t) {
            case h.EF.SIZE_16:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_16;
            case h.EF.SIZE_20:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_20;
            case h.EF.SIZE_24:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_24;
            case h.EF.SIZE_32:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_32;
            case h.EF.SIZE_40:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_40;
            case h.EF.SIZE_44:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_44;
            case h.EF.SIZE_48:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_48;
            case h.EF.SIZE_56:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_56;
            case h.EF.SIZE_80:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_80;
            case h.EF.SIZE_120:
                return f.QS.AVATAR_DECORATION_STATUS_TYPING_120;
        }
    if (n)
        switch (t) {
            case h.EF.SIZE_16:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_16;
            case h.EF.SIZE_20:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_20;
            case h.EF.SIZE_24:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_24;
            case h.EF.SIZE_32:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_32;
            case h.EF.SIZE_40:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_40;
            case h.EF.SIZE_44:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_44;
            case h.EF.SIZE_48:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_48;
            case h.EF.SIZE_56:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_56;
            case h.EF.SIZE_80:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_80;
            case h.EF.SIZE_120:
                return f.QS.AVATAR_DECORATION_STATUS_MOBILE_120;
        }
    switch (t) {
        case h.EF.SIZE_16:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_16;
        case h.EF.SIZE_20:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_20;
        case h.EF.SIZE_24:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_24;
        case h.EF.SIZE_32:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_32;
        case h.EF.SIZE_40:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_40;
        case h.EF.SIZE_44:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_44;
        case h.EF.SIZE_48:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_48;
        case h.EF.SIZE_56:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_56;
        case h.EF.SIZE_80:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_80;
        case h.EF.SIZE_120:
            return f.QS.AVATAR_DECORATION_STATUS_ROUND_120;
    }
    return null;
}
function w(e, t, n, r) {
    if (null == e) return f.QS.AVATAR_DEFAULT;
    if (r)
        switch (t) {
            case h.EF.SIZE_16:
                return f.QS.AVATAR_STATUS_TYPING_16;
            case h.EF.SIZE_20:
                return f.QS.AVATAR_STATUS_TYPING_20;
            case h.EF.SIZE_24:
                return f.QS.AVATAR_STATUS_TYPING_24;
            case h.EF.SIZE_32:
                return f.QS.AVATAR_STATUS_TYPING_32;
            case h.EF.SIZE_40:
                return f.QS.AVATAR_STATUS_TYPING_40;
            case h.EF.SIZE_44:
                return f.QS.AVATAR_STATUS_TYPING_44;
            case h.EF.SIZE_48:
                return f.QS.AVATAR_STATUS_TYPING_48;
            case h.EF.SIZE_56:
                return f.QS.AVATAR_STATUS_TYPING_56;
            case h.EF.SIZE_80:
                return f.QS.AVATAR_STATUS_TYPING_80;
            case h.EF.SIZE_120:
                return f.QS.AVATAR_STATUS_TYPING_120;
        }
    if (e === m.Skl.ONLINE && n)
        switch (t) {
            case h.EF.SIZE_16:
                return f.QS.AVATAR_STATUS_MOBILE_16;
            case h.EF.SIZE_20:
                return f.QS.AVATAR_STATUS_MOBILE_20;
            case h.EF.SIZE_24:
                return f.QS.AVATAR_STATUS_MOBILE_24;
            case h.EF.SIZE_32:
                return f.QS.AVATAR_STATUS_MOBILE_32;
            case h.EF.SIZE_40:
                return f.QS.AVATAR_STATUS_MOBILE_40;
            case h.EF.SIZE_44:
                return f.QS.AVATAR_STATUS_MOBILE_44;
            case h.EF.SIZE_48:
                return f.QS.AVATAR_STATUS_MOBILE_48;
            case h.EF.SIZE_56:
                return f.QS.AVATAR_STATUS_MOBILE_56;
            case h.EF.SIZE_80:
                return f.QS.AVATAR_STATUS_MOBILE_80;
            case h.EF.SIZE_120:
                return f.QS.AVATAR_STATUS_MOBILE_120;
        }
    switch (t) {
        case h.EF.SIZE_16:
            return f.QS.AVATAR_STATUS_ROUND_16;
        case h.EF.SIZE_20:
            return f.QS.AVATAR_STATUS_ROUND_20;
        case h.EF.SIZE_24:
            return f.QS.AVATAR_STATUS_ROUND_24;
        case h.EF.SIZE_32:
            return f.QS.AVATAR_STATUS_ROUND_32;
        case h.EF.SIZE_40:
            return f.QS.AVATAR_STATUS_ROUND_40;
        case h.EF.SIZE_44:
            return f.QS.AVATAR_STATUS_ROUND_44;
        case h.EF.SIZE_48:
            return f.QS.AVATAR_STATUS_ROUND_48;
        case h.EF.SIZE_56:
            return f.QS.AVATAR_STATUS_ROUND_56;
        case h.EF.SIZE_80:
            return f.QS.AVATAR_STATUS_ROUND_80;
        case h.EF.SIZE_120:
            return f.QS.AVATAR_STATUS_ROUND_120;
    }
    throw Error(
        'getMaskId(): Unsupported type, size: '
            .concat(t, ', status: ')
            .concat(e, ', isMobile: ')
            .concat(n ? 'true' : 'false')
    );
}
function D(e, t, n) {
    let r = arguments.length > 3 && void 0 !== arguments[3] && arguments[3],
        i = (0, u.W5)(e.status, t, n, r),
        a = (r ? e.size - (i.width / 2 + e.status / 2) : e.size - i.width) - e.offset;
    return I(O({}, i), {
        x: a,
        y: e.size - i.height - e.offset
    });
}
function L(e, t, n) {
    let { size: r, status: i, stroke: a, offset: o } = e,
        s = n ? i * h.D6 : i,
        l = (s - i) / 2,
        c = t ? i * h.EW : i;
    return {
        avatarCutoutX: r - s + l - a - o,
        avatarCutoutY: r - c - a - o,
        avatarCutoutWidth: s + 2 * a,
        avatarCutoutHeight: c + 2 * a,
        avatarCutoutRadius: t ? (c + 2 * a) * h.ZC : (i + 2 * a) / 2
    };
}
function x(e, t, n, i) {
    let a = D(n, i, t, !1),
        o = (0, u.lm)(i, e);
    if (!t) {
        let t = a.height / 2 + n.stroke,
            i = a.x + n.status / 2;
        return (0, r.jsx)('circle', {
            style: { opacity: o },
            fill: e,
            r: t,
            cx: i,
            cy: i
        });
    }
    let s = a.height + 2 * n.stroke,
        l = a.width + 2 * n.stroke,
        c = a.x - n.stroke,
        d = a.y - n.stroke;
    return (0, r.jsx)('rect', {
        fill: e,
        height: s,
        width: l,
        style: { opacity: o },
        x: c,
        y: d,
        rx: n.stroke
    });
}
function M(e) {
    let { status: t, isMobile: n, isTyping: r, size: i } = e,
        a = n && !r && t === m.Skl.ONLINE,
        o = (0, h.UC)(i);
    return O(
        {},
        L(o, a, r),
        (0, u.Dk)({
            status: t,
            size: o.status,
            isMobile: a,
            isTyping: r,
            topOffset: 2,
            leftOffset: 6
        })
    );
}
function k(e) {
    let { children: t, size: n, onClick: i, onMouseDown: a, onKeyDown: s, onContextMenu: c, onMouseEnter: u, onMouseLeave: d, className: _, tabIndex: f, ariaLabel: p, ariaHidden: m, status: E, isMobile: y = !1, isTyping: O = !1, avatarDecoration: v, typingOffset: I, specs: T } = e,
        S = {
            width: (0, h.px)(n),
            height: (0, h.px)(n)
        },
        A = null == p || m ? void 0 : R(p, E, y),
        N = T.size * g.hs,
        C = P(E, n, y, O),
        w =
            null != v &&
            (0, r.jsx)('svg', {
                width: N + I,
                height: N,
                viewBox: '0 0 '.concat(N + I, ' ').concat(N),
                className: b.avatarDecoration,
                'aria-hidden': !0,
                children: (0, r.jsx)('foreignObject', {
                    x: 0,
                    y: 0,
                    width: N,
                    height: N,
                    mask: null != C ? 'url(#'.concat(C, ')') : void 0,
                    children: (0, r.jsx)('div', {
                        className: b.avatarStack,
                        children: (0, r.jsx)('img', {
                            className: b.avatar,
                            src: v,
                            alt: ' ',
                            'aria-hidden': !0
                        })
                    })
                })
            });
    return null != i || null != a
        ? (0, r.jsxs)(l.P, {
              className: o()(b.wrapper, b.pointer, _),
              style: S,
              onClick: i,
              tabIndex: f,
              onContextMenu: c,
              onMouseDown: a,
              onKeyDown: s,
              onMouseEnter: null != u ? u : void 0,
              onMouseLeave: null != d ? d : void 0,
              'aria-label': A,
              'aria-hidden': m,
              children: [t, w]
          })
        : (0, r.jsxs)('div', {
              className: o()(b.wrapper, _),
              style: S,
              onContextMenu: null != c ? c : void 0,
              onMouseEnter: null != u ? u : void 0,
              onMouseLeave: null != d ? d : void 0,
              role: 'img',
              'aria-label': A,
              'aria-hidden': m,
              children: [t, w]
          });
}
let j = i.forwardRef((e, t) => {
    let { src: n, isSpeaking: i, className: a } = e;
    return (0, r.jsxs)(
        'div',
        {
            ref: t,
            className: o()(b.avatarStack, a),
            children: [
                (0, r.jsx)('img', {
                    src: null != n ? n : void 0,
                    alt: ' ',
                    className: b.avatar,
                    'aria-hidden': !0
                }),
                i && (0, r.jsx)('div', { className: b.avatarSpeaking })
            ]
        },
        n
    );
});
function U(e) {
    let { src: t, status: n, size: i, statusColor: a, isMobile: s = !1, isTyping: l = !1, typingIndicatorRef: _, avatarContentRef: f, isSpeaking: g = !1, statusTooltip: E = !1, statusTooltipDelay: y, statusBackdropColor: v, 'aria-hidden': T = !1, 'aria-label': S, imageClassName: A, ref: N } = e,
        C = n !== m.Skl.UNKNOWN ? n : null,
        R = (0, h.UC)(i),
        P = null != C ? Math.ceil((R.status * h.D6 - R.status) / 2) : 0,
        L = R.size + P,
        M = (0, u.vj)(C, a);
    return (0, r.jsx)(
        k,
        I(O({}, e), {
            ariaLabel: S,
            ariaHidden: T,
            status: C,
            specs: R,
            typingOffset: P,
            children: (0, r.jsxs)('svg', {
                ref: N,
                width: L,
                height: L,
                viewBox: '0 0 '.concat(L, ' ').concat(L),
                className: o()(b.mask, b.svg),
                'aria-hidden': !0,
                children: [
                    (0, r.jsx)('foreignObject', {
                        x: 0,
                        y: 0,
                        width: R.size,
                        height: R.size,
                        mask: 'url(#'.concat(w(C, i, s, l), ')'),
                        children: (0, r.jsx)(j, {
                            ref: f,
                            src: t,
                            isSpeaking: g,
                            className: A
                        })
                    }),
                    null != C && null != v ? x(v, s, R, C) : null,
                    null != C
                        ? (0, r.jsx)(d.u, {
                              text: E ? (0, p.u5)(C) : null,
                              'aria-label': !1,
                              position: 'top',
                              spacing: 5 + 1.5 * R.stroke,
                              delay: y,
                              children: (e) =>
                                  (0, r.jsxs)(r.Fragment, {
                                      children: [
                                          (0, r.jsx)(
                                              'rect',
                                              I(O({}, e, D(R, C, s, l)), {
                                                  fill: M,
                                                  mask: 'url(#'.concat((0, u.rs)(C, s, l), ')'),
                                                  className: b.pointerEvents
                                              })
                                          ),
                                          l
                                              ? (0, r.jsx)(c.b, {
                                                    ref: _,
                                                    dotRadius: R.status / 4,
                                                    x: R.size - 1.375 * R.status - R.offset,
                                                    y: R.size - R.status / 1.333 - R.offset
                                                })
                                              : null
                                      ]
                                  })
                          })
                        : null
                ]
            })
        })
    );
}
function G(e, t, n) {
    let r = arguments.length > 3 && void 0 !== arguments[3] && arguments[3];
    return 5 - (n && !r ? 0 : 0.5 * e) + 1.5 * t;
}
function B(e, t, n) {
    let r = e.status * t,
        i = null != n ? (r * h.D6 - r) / 2 : 0,
        a = (e.status - r) / 2,
        o = e.size - r - i - e.offset - a,
        s = e.size - r * h.EW - e.offset - a,
        l = Math.round((o / t) * 100) / 100,
        c = Math.round((s / t) * 100) / 100;
    return 'translate('.concat(l, ', ').concat(c, ')');
}
function V(e) {
    let { fromIsMobile: t = !0, fromStatus: n, fromColor: a, isMobile: l = !1, isTyping: f = !1, typingIndicatorRef: m, isSpeaking: g = !1, size: E, src: y, status: v, statusColor: S, statusTooltip: R = !1, statusTooltipDelay: P, statusBackdropColor: w, 'aria-hidden': D = !1, 'aria-label': L, imageClassName: U, pulseStatusIcon: V } = e,
        F = (0, u.vj)(v, S),
        Z = i.useId(),
        H = i.useId(),
        [Y] = i.useState(() =>
            O(
                { fill: a },
                M({
                    size: E,
                    status: n,
                    isMobile: t,
                    isTyping: !1
                })
            )
        ),
        W = i.useMemo(
            () =>
                O(
                    { fill: F },
                    M({
                        size: E,
                        status: v,
                        isMobile: l,
                        isTyping: f
                    })
                ),
            [F, E, v, l, f]
        ),
        K = (0, _.q_F)(
            {
                config: A,
                from: Y,
                to: W
            },
            N() ? 'animate-always' : 'animate-never'
        ),
        { avatarCutoutX: z, avatarCutoutY: q, avatarCutoutWidth: X, avatarCutoutHeight: Q, avatarCutoutRadius: J, fill: $ } = K,
        ee = T(K, ['avatarCutoutX', 'avatarCutoutY', 'avatarCutoutWidth', 'avatarCutoutHeight', 'avatarCutoutRadius', 'fill']),
        et = (0, h.px)(E),
        en = (0, h.UC)(E),
        er = en.status * h.D6,
        ei = en.status * h.EW,
        ea = null != v ? (en.status * h.D6 - en.status) / 2 : 0,
        eo = en.size + Math.ceil(ea),
        [es, el, ec] = (0, _.q_F)(
            {
                config: {
                    tension: 450,
                    friction: 20,
                    clamp: !0
                },
                from: { scale: 1 },
                to: V ? [{ scale: 0.8 }, { scale: 1.1 }, { scale: 1 }] : []
            },
            N() ? 'animate-always' : 'animate-never',
            [V, v]
        );
    return (0, r.jsx)(
        k,
        I(O({}, e), {
            ariaLabel: L,
            ariaHidden: D,
            typingOffset: ea,
            specs: en,
            children: (0, r.jsxs)('svg', {
                width: eo,
                height: eo,
                viewBox: '0 0 '.concat(eo, ' ').concat(eo),
                className: o()(b.mask, b.svg),
                'aria-hidden': !0,
                children: [
                    (0, r.jsxs)('mask', {
                        id: Z,
                        width: et,
                        height: et,
                        children: [
                            (0, r.jsx)('circle', {
                                cx: et / 2,
                                cy: et / 2,
                                r: et / 2,
                                fill: 'white'
                            }),
                            (0, r.jsx)(s.animated.rect, {
                                color: 'black',
                                x: z,
                                y: q,
                                width: X,
                                height: Q,
                                rx: J,
                                ry: J
                            })
                        ]
                    }),
                    (0, r.jsx)('foreignObject', {
                        className: b.__invalid_foreignObject,
                        x: 0,
                        y: 0,
                        width: et,
                        height: et,
                        mask: 'url(#'.concat(Z, ')'),
                        children: (0, r.jsx)(j, {
                            src: y,
                            isSpeaking: g,
                            className: U
                        })
                    }),
                    null != w && x(w, l, en, v),
                    (0, r.jsx)(d.u, {
                        text: R ? (0, p.u5)(v) : null,
                        'aria-label': !1,
                        position: 'top',
                        spacing: G(en.status, en.stroke, l, f),
                        delay: P,
                        children: (e) =>
                            (0, r.jsxs)(s.animated.g, {
                                transform: es.scale.to((e) => 'scale('.concat(e, ') ').concat(B(en, e, v))),
                                children: [
                                    (0, r.jsxs)('svg', {
                                        width: er,
                                        height: ei,
                                        viewBox: '0 0 '.concat(er, ' ').concat(ei),
                                        className: R ? b.cursorDefault : void 0,
                                        children: [
                                            (0, u.vP)(ee, en.status, H),
                                            (0, r.jsx)(s.animated.rect, {
                                                fill: $,
                                                width: er,
                                                height: ei,
                                                mask: 'url(#'.concat(H, ')')
                                            }),
                                            (0, r.jsx)(c.b, {
                                                ref: m,
                                                dotRadius: en.status / 4,
                                                x: 0.15 * er,
                                                y: 0.5 * ei,
                                                hide: !f
                                            })
                                        ]
                                    }),
                                    (0, r.jsx)(
                                        C,
                                        O(
                                            {
                                                size: E,
                                                isMobile: l,
                                                isTyping: f,
                                                className: b.pointerEvents
                                            },
                                            e
                                        )
                                    )
                                ]
                            })
                    })
                ]
            })
        })
    );
}
function F(e, t, n, r, i) {
    return null != n && null != t && (!!e || t !== n || (t === m.Skl.ONLINE && r !== i));
}
j.displayName = 'AvatarImg';
let Z = i.memo(function (e) {
    var { statusColor: t, status: n } = e,
        a = T(e, ['statusColor', 'status']);
    let { isMobile: o = !1, isTyping: s = !1 } = a,
        l = i.useRef(n),
        c = i.useRef(o),
        d = (0, u.vj)(n, t),
        _ = i.useRef(d),
        f = i.useRef(!1),
        p = f.current || F(s, n, l.current, o, c.current);
    return (
        i.useLayoutEffect(() => {
            (f.current = p), (l.current = n), (c.current = o), (_.current = d);
        }, [n, o, d, p]),
        null != n && null != l.current && p
            ? (0, r.jsx)(
                  V,
                  I(O({}, a), {
                      status: n,
                      statusColor: d,
                      fromStatus: l.current,
                      fromIsMobile: c.current,
                      fromColor: _.current
                  })
              )
            : (0, r.jsx)(
                  U,
                  I(O({}, a), {
                      status: n,
                      statusColor: d
                  })
              )
    );
});
