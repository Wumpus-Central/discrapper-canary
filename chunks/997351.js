"use strict";
n.d(t, { A: () => A });
var r = n(627968),
    i = n(64700),
    a = n(311907),
    s = n(397927),
    o = n(544028),
    l = n(967198),
    u = n(287809),
    c = n(255438),
    d = n(453771),
    _ = n(975571),
    f = n(927578),
    p = n(244975),
    h = n(788868),
    m = n(652215),
    g = n(985018),
    E = n(950503);
function A(e) {
    let { onClose: t, handleLearnMore: n, ...A } = e,
        I = (0, a.bG)([u.default], () => u.default.getCurrentUser()),
        T = (0, a.bG)([o.A], () => o.A.theme),
        y = (0, a.bG)([l.A], () => l.A.getGuildId()),
        S = T === m.NJ8.LIGHT ? "light" : "dark",
        v = (0, r.jsx)("img", {
            className: E.Qw,
            alt: "File Upload Nitro Perk",
            src: `https://cdn.discordapp.com/assets/premium/roadblocks/file_upload_${S}_v2.png`,
        }),
        C = i.useMemo(() => {
            let e = f.Ay.getUserMaxFileSize(I);
            return (0, c.Xq)(e / 1024, { useKibibytes: !0 });
        }, [I]),
        b = (0, d.Iu)({
            guildId: y,
            onClick: () => {
                window.open(_.A.getArticleURL(m.MVz.NITRO_FAQ), "_blank");
            },
        }),
        N = (0, r.jsx)("div", {
            className: E.rf,
            children: (0, r.jsx)(s.Text, { variant: "text-sm/medium", children: b }),
        });
    return (0, r.jsx)(p.A, {
        artElement: v,
        artContainerClassName: E.JS,
        enableArtBoxShadow: !1,
        type: h.e.UPLOAD_ERROR_UPSELL,
        title: g.intl.string(g.t["9C+41g"]),
        body: N,
        context: g.intl.formatToPlainString(g.t.q5fTZp, { maxSize: C }),
        glowUp: b,
        analyticsLocation: { section: m.JJy.FILE_UPLOAD_POPOUT },
        onClose: t,
        subscriptionTier: h.pe.TIER_2,
        secondaryCTA: g.intl.string(g.t.ZnqyZ2),
        onSecondaryClick: n,
        showEnhancedUpsell: !0,
        ...A,
    });
}
