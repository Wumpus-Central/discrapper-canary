...
var r = n(562465),
    ...
    l = n(77729),
    u = n(652215);
let d = new s.A("uploadRtcLogFiles");
async function c(e, t) {
    ...
    try {
        ...
        throw (d.error(`uploadDebugFiles: read error '${e}'`), new i._(i.ct.READ));
    }
    if (0 === s.length) throw new i._(i.ct.NO_FILE);
    let c = { extraInfo: t, mediaEngineState: a.Ay.getState() },
        _ = [
            ...s.map((e) => ({ name: e.name, file: e, filename: e.name })),
            {
                name: "media_engine_state.json",
                filename: "media_engine_state.json",
                file: new Blob([JSON.stringify(c, void 0, 2)]),
            },
        ],
        f = new Set();
    try {
        ...
        throw (d.error(`Debug log upload error: status: ${e.status}, message: ${e.message}`), new i._(i.ct.UPLOAD));
    }
    if ("success_count" in n.body && n.body.success_count !== _.length)
        throw (
            (d.error(`Debug log upload: stored files ${n.body.success_count} !== ${_.length}`), new i._(i.ct.GENERAL))
        );
    if (
