n.d(t, { J: () => w }), n(388685), n(583741);
var r = n(951288),
    i = n(647438),
    a = n(799189),
    o = n(731602),
    s = n(599273),
    l = n(922462),
    c = n(692547),
    u = n(251052),
    d = n(673824),
    f = n(213305),
    _ = n(155127),
    p = n(635041),
    h = n(571755);
function m(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0,
              })
            : (e[t] = n),
        e
    );
}
function g(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            r = Object.keys(n);
        "function" == typeof Object.getOwnPropertySymbols &&
            (r = r.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                }),
            )),
            r.forEach(function (t) {
                m(e, t, n[t]);
            });
    }
    return e;
}
function E(e, t) {
    var n = Object.keys(e);
    if (Object.getOwnPropertySymbols) {
        var r = Object.getOwnPropertySymbols(e);
        t &&
            (r = r.filter(function (t) {
                return Object.getOwnPropertyDescriptor(e, t).enumerable;
            })),
            n.push.apply(n, r);
    }
    return n;
}
function b(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : E(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
let y = "M4.10585 5.3837L5.37864 4.11091L11.884 10.61632L10.6112 11.88912L4.10585 5.3837Z",
    O = "M10.61632 4.11091L11.88912 5.3837L5.3837 11.88912L4.11091 10.61632L10.61632 4.11091Z",
    v = "M5.25333 8.80104L5.25333 7.17346L10.85336 7.17346L10.85336 8.80104L5.25333 8.80104Z",
    I = "M10.84656 7.17346L10.84656 8.80104L5.24954 8.80104L5.24954 7.17346L10.84656 7.17346Z",
    T = "M6.31649 11.88304L5.0437 10.61032L11.44792 4.20604L12.72072 5.47883L6.31649 11.88304Z",
    S = "M3.26914 8.87224L4.54194 7.59943L7.5588 10.61632L6.28601 11.88912L3.26914 8.87224Z",
    A = [0, 0.1, 0.4, 0.6, 0.9, 1],
    C = [0, 0.1, 0.9, 1],
    N = [0, 0.1, 0.2, 0.5, 0.7, 1],
    R = [0, 0.3, 0.5, 0.8, 0.9, 1];
function P(e, t, n, i) {
    let a = e.to({
            range: C,
            output: [t, t, n, n],
            extrapolate: "clamp",
        }),
        o = i ? [y, y, T, T] : [y, v, v, T],
        l = i ? [O, O, S, S] : [O, I, I, S];
    return (0, r.jsx)("svg", {
        viewBox: "0 0 24 24",
        fill: "none",
        children: (0, r.jsxs)("g", {
            transform: "translate(4 4)",
            children: [
                (0, r.jsx)(s.animated.path, {
                    fill: a,
                    d: e.to({
                        range: C,
                        output: o,
                        extrapolate: "clamp",
                    }),
                }),
                (0, r.jsx)(s.animated.path, {
                    fill: a,
                    d: e.to({
                        range: C,
                        output: l,
                        extrapolate: "clamp",
                    }),
                }),
            ],
        }),
    });
}
function w(e) {
    let {
            checked: t,
            disabled: n,
            id: m,
            onChange: E,
            focusProps: y,
            hasIcon: O = !1,
            describedBy: v,
            innerRef: I,
        } = e,
        { reducedMotion: T } = i.useContext(f.S),
        S = i.useRef(null),
        w = i.useRef(null),
        D = null != I ? I : w,
        L = (0, l.l)({
            isSelected: t,
            onChange: E,
            isDisabled: n,
        }),
        {
            inputProps: x,
            labelProps: M,
            isPressed: j,
        } = (0, a.h)(
            {
                id: m,
                isDisabled: n,
                "aria-describedby": v,
            },
            L,
            D,
        ),
        { hoverProps: k, isHovered: U } = (0, o.X)({ isDisabled: n }),
        G = (0, d.d)(c.Z.colors.SWITCH_BACKGROUND_DEFAULT).spring(),
        B = (0, d.d)(c.Z.colors.SWITCH_BACKGROUND_DEFAULT).spring(),
        Z = (0, d.d)(c.Z.colors.SWITCH_BACKGROUND_DEFAULT_SELECTED).spring(),
        F = (0, d.d)(c.Z.colors.SWITCH_BACKGROUND_ACTIVE).spring(),
        V = (0, d.d)(c.Z.colors.SWITCH_BORDER_DEFAULT).spring(),
        H = (0, d.d)(c.Z.colors.SWITCH_BORDER_DEFAULT_SELECTED).spring(),
        Y = (0, d.d)(c.Z.colors.SWITCH_BACKGROUND_HOVER).spring(),
        W = (0, d.d)(c.Z.colors.SWITCH_BACKGROUND_HOVER_SELECTED).spring(),
        K = (0, d.d)(c.Z.colors.SWITCH_BORDER_HOVER).spring(),
        z = (0, d.d)(c.Z.colors.SWITCH_BORDER_HOVER_SELECTED).spring(),
        q = (0, d.d)(c.Z.colors.SWITCH_THUMB_FOREGROUND_DEFAULT).spring(),
        X = (0, d.d)(c.Z.colors.SWITCH_THUMB_FOREGROUND_ACTIVE).spring(),
        Q = (0, d.d)(c.Z.colors.SWITCH_THUMB_BACKGROUND_DEFAULT).spring(),
        J = (0, d.d)(c.Z.colors.SWITCH_THUMB_BACKGROUND_DEFAULT_SELECTED).spring(),
        { state: $, opacity: ee } = (0, u.q)(
            {
                config: { duration: 300 },
                opacity: n ? 0.5 : 1,
                state: j ? (t ? R[R.length - 2] : N[1]) : +!!t,
            },
            "animate-always",
        ),
        et = function (e) {
            let n = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : [...e].reverse();
            return t
                ? $.to({
                      range: R,
                      output: n,
                  })
                : $.to({
                      range: N,
                      output: e,
                  });
        };
    return (0, r.jsx)(
        _.tE,
        b(g({}, y), {
            ringTarget: S,
            focusTarget: D,
            within: !0,
            offset: -2,
            children: (0, r.jsxs)(
                "label",
                b(g({}, M), {
                    className: h.container,
                    children: [
                        (0, r.jsx)(
                            s.animated.div,
                            b(g({}, k), {
                                ref: S,
                                className: h.switchIndicator,
                                "data-size": "md",
                                "data-mana-component": "switch",
                                style: {
                                    opacity: ee,
                                    backgroundColor: $.to({
                                        range: C,
                                        output: [U ? Y : G, B, F, U ? W : Z],
                                        extrapolate: "clamp",
                                    }),
                                    border: "1px solid",
                                    borderColor: $.to({
                                        range: C,
                                        output: [U ? K : V, U ? K : V, U ? z : H, U ? z : H],
                                        extrapolate: "clamp",
                                    }),
                                },
                                children: (0, r.jsxs)(s.animated.svg, {
                                    className: h.thumb,
                                    viewBox: "0 0 24 24",
                                    preserveAspectRatio: "xMidYMin meet",
                                    style: {
                                        left: $.to({
                                            range: A,
                                            output: [1, 1, 1, 24, 24, 24],
                                        }),
                                    },
                                    "aria-hidden": !0,
                                    children: [
                                        (0, r.jsx)(s.animated.rect, {
                                            fill: $.to({
                                                range: C,
                                                output: [Q, Q, J, J],
                                            }),
                                            x: et([4, 4, 2.5, 1, 2.5, 4]),
                                            y: et([4, 7, 2.5, 7, 2.5, 4]),
                                            width: et([16, 16, 19, 22, 19, 16]),
                                            height: et([16, 10, 19, 10, 19, 16]),
                                            rx: et([8, 5, 9.5, 5, 9.5, 8]),
                                        }),
                                        O && P($, q, X, T.enabled),
                                    ],
                                }),
                            }),
                        ),
                        (0, r.jsx)(p.n, { children: (0, r.jsx)("input", b(g({}, x), { ref: D })) }),
                    ],
                }),
            ),
        }),
    );
}
