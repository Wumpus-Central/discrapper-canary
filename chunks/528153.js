...
var i = n(562708),
    ...
    y = n(788868),
    C = n(49999);
let N = new d.A("PremiumGiftingIntentManager");
class v extends u.A {
    actions = {
    ...
    dismissGiftIntentDC() {
        (0, _.uh)(s.M.GIFT_INTENT_MESSAGE, { dismissAction: C.i.AUTO_DISMISS, forceTrack: !0 });
    }
    ...
    maybeSendGiftingPromptSystemMessage(e, t, n, i) {
        if (!p.oz.getSetting()) return;
        let r = g.A.getChannelId(),
            s = m.A.isReady(e);
        if (!S.Ay.isGiftIntentMessageInCooldown(n) && e === r) {
            if (!s)
                return void m.A.whenReady(e, () => {
                    g.A.getChannelId() === e && this.maybeSendGiftingPromptSystemMessage(e, t, n, i);
                });
    ...
    async sendGiftingNotificationIfEligible() {
        ...
        if (null != r)
            try {
                ...
                if (null == t) return;
                if (
                    (m.A.isReady(t.id) || (await h.A.fetchMessages({ channelId: t.id, isPreload: !0 })),
                    m.A.getMessages(t.id).cached)
                )
                    return void N.info("Skipping gift intent notification - fetched messages marked as stale", {
                        channelId: t.id,
                        recipientUserID: r,
                    });
                ...
                N.error("Failed to fetch DM channel data for gifting notification", { recipientUserID: r, error: e });
            }
    }
    onChannelSelect(e) {
        let { channelId: t } = e;
