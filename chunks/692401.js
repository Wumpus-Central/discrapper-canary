...
class u {
    _trackAppBackgrounded(e) {
        this._isBackgrounded !== e && ((this._isBackgrounded = e), this._isBackgrounded && this.hasWorkScheduled && (this.telemetry.track(o.ug.SKIP_IDLE_CALLBACK_DUE_TO_BACKGROUNDED), this._processWorkCallback()));
    }
    _queueIdleCallback() {
    ...
    _processWorkCallback(e) {
        ...
        if (this._hasExceededMaxConsecutiveFlushes) {
            c.log('Unable to fully flush work queue after max retries, skipping future deadline.'), this._workCallbackFn(), this.clearWorkTimeout(), this.telemetry.measure(o.aA.COUNT_FLUSH_BEFORE_QUEUE_EMPTY, this._consecutiveFlushesBeforeQueueEmpty), this.telemetry.track(o.ug.EXCEEDED_MAX_CONSECUTIVE_FLUSHES), (this._consecutiveFlushesBeforeQueueEmpty = 0), (this._nextDispatchTimeout = s.II);
            return;
        }
        let t = performance.now(),
            n = this._workCallbackFn(e),
            r = performance.now();
        this.clearWorkTimeout(), r - t > s.TC ? (this._nextDispatchTimeout = s.II) : (this._nextDispatchTimeout = s.Lu), n ? (this._consecutiveFlushesBeforeQueueEmpty > 0 && this.telemetry.measure(o.aA.COUNT_FLUSH_BEFORE_QUEUE_EMPTY, parseInt(''.concat(this._consecutiveFlushesBeforeQueueEmpty))), (this._consecutiveFlushesBeforeQueueEmpty = 0), (this._criticalWorkScheduled = !1)) : (this._consecutiveFlushesBeforeQueueEmpty += 1);
    }
    ...
    requestWorkTimeout(e) {
        ...
        if (((this._workCallbackFn = e), !this.hasWorkScheduled)) {
            if ((this.telemetry.time(o.JV.TIME_TO_QUEUE_EMPTY), this._nextDispatchTimeout === s.II && this.telemetry.track(o.ug.LONGER_DISPATCH), t)) return void this._queueIdleCallback();
            this._flushTimeoutHandler = setTimeout(() => {
                if ((i()(null != this._workCallbackFn, 'Work callback should be set'), this._isBackgrounded)) return this.telemetry.track(o.ug.SKIP_IDLE_CALLBACK_DUE_TO_BACKGROUNDED), this._processWorkCallback();
                this._queueIdleCallback();
            }, this._nextDispatchTimeout);
        }
    }
    constructor() {
        l(this, '_flushTimeoutHandler', null), l(this, '_flushIdleHandler', null), l(this, '_nextDispatchTimeout', s.Lu), l(this, '_workCallbackFn', null), l(this, '_consecutiveFlushesBeforeQueueEmpty', 0), l(this, '_isBackgrounded', !1), l(this, '_enableRequestIdleCallback', !1), l(this, '_criticalWorkScheduled', !1), l(this, 'telemetry', new o.mA()), l(this, '_logger', c);
    }
}
