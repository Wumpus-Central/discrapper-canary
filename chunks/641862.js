n.d(t, {
    Y: () => g,
    u: () => m
});
var r = n(175730),
    i = n(878369),
    a = n(670732),
    o = n(839826),
    s = n(721056),
    l = n(388388),
    c = new WeakMap(),
    u = /auto|scroll/,
    d = /^tb|vertical/,
    _ = /msie|trident/i.test(l.C.navigator && l.C.navigator.userAgent),
    f = function (e) {
        return parseFloat(e || '0');
    },
    p = function (e, t, n) {
        return (void 0 === e && (e = 0), void 0 === t && (t = 0), void 0 === n && (n = !1), new i.D((n ? t : e) || 0, (n ? e : t) || 0));
    },
    h = (0, s.v)({
        devicePixelContentBoxSize: p(),
        borderBoxSize: p(),
        contentBoxSize: p(),
        contentRect: new a.N(0, 0, 0, 0)
    }),
    m = function (e, t) {
        if ((void 0 === t && (t = !1), c.has(e) && !t)) return c.get(e);
        if ((0, o.xj)(e)) return (c.set(e, h), h);
        var n = getComputedStyle(e),
            r = (0, o.zd)(e) && e.ownerSVGElement && e.getBBox(),
            i = !_ && 'border-box' === n.boxSizing,
            l = d.test(n.writingMode || ''),
            m = !r && u.test(n.overflowY || ''),
            g = !r && u.test(n.overflowX || ''),
            E = r ? 0 : f(n.paddingTop),
            b = r ? 0 : f(n.paddingRight),
            y = r ? 0 : f(n.paddingBottom),
            O = r ? 0 : f(n.paddingLeft),
            v = r ? 0 : f(n.borderTopWidth),
            I = r ? 0 : f(n.borderRightWidth),
            T = r ? 0 : f(n.borderBottomWidth),
            S = r ? 0 : f(n.borderLeftWidth),
            A = O + b,
            N = E + y,
            C = S + I,
            R = v + T,
            P = g ? e.offsetHeight - R - e.clientHeight : 0,
            w = m ? e.offsetWidth - C - e.clientWidth : 0,
            D = i ? A + C : 0,
            L = i ? N + R : 0,
            x = r ? r.width : f(n.width) - D - w,
            k = r ? r.height : f(n.height) - L - P,
            M = x + A + w + C,
            j = k + N + P + R,
            U = (0, s.v)({
                devicePixelContentBoxSize: p(Math.round(x * devicePixelRatio), Math.round(k * devicePixelRatio), l),
                borderBoxSize: p(M, j, l),
                contentBoxSize: p(x, k, l),
                contentRect: new a.N(O, E, x, k)
            });
        return (c.set(e, U), U);
    },
    g = function (e, t, n) {
        var i = m(e, n),
            a = i.borderBoxSize,
            o = i.contentBoxSize,
            s = i.devicePixelContentBoxSize;
        switch (t) {
            case r.z.DEVICE_PIXEL_CONTENT_BOX:
                return s;
            case r.z.BORDER_BOX:
                return a;
            default:
                return o;
        }
    };
