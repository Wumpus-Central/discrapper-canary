(n.d(t, {
    _b: () => h,
    o: () => _,
    zt: () => p,
    zy: () => d
}),
    n(704826),
    n(35282));
var r = n(633302),
    i = n(134432),
    a = n(768581),
    o = n(358085),
    s = n(981631);
let l = ''.concat(location.protocol, '//').concat(window.GLOBAL_ENV.CDN_HOST, '/role-icons'),
    c = ''.concat(location.protocol).concat(window.GLOBAL_ENV.API_ENDPOINT),
    u = (0, o.isAndroid)(),
    d = (e, t) => {
        if (null == e) return null;
        let n = null != e.unicodeEmoji ? r.ZP.getByName(r.ZP.convertSurrogateToName(e.unicodeEmoji, !1)) : void 0;
        return {
            customIconSrc: f(e, t),
            unicodeEmoji: null != n ? n : void 0
        };
    },
    _ = (e, t) => e.replace(/size=[0-9]+/g, 'size='.concat((0, i.oO)(t * (0, i.x_)()))),
    f = (e, t) => {
        let { id: n, icon: r } = e;
        if (null == r) return;
        if (r.startsWith('data:')) return r;
        let o = a.$k ? 'webp' : 'png',
            d = '',
            _ = 'quality=lossless';
        return (null != t && ((d = 'size=' + (0, i.oO)(t * (0, i.x_)())), (_ = u ? '' : '&' + _)), null != window.GLOBAL_ENV.CDN_HOST) ? ''.concat(l, '/').concat(n, '/').concat(r, '.').concat(o, '?').concat(d).concat(_) : ''.concat(c).concat(s.ANM.ROLE_ICON(n, r), '?').concat(d);
    },
    p = (e) => e.startsWith(l) || (e.startsWith(''.concat(c, '/roles')) && e.includes('/icons/')),
    h = (e, t) => {
        var n;
        return (null == t || null == (n = t.tags) ? void 0 : n.subscription_listing_id) != null || e.features.has(s.oNc.ROLE_ICONS);
    };
