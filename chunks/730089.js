n.d(t, { G: () => p }), n(35282);
var r = n(36793),
    i = n(481060),
    l = n(668781),
    a = n(80932),
    s = n(426642),
    o = n(626135),
    c = n(176354),
    d = n(956664),
    u = n(185923),
    g = n(981631),
    f = n(388032),
    m = n(413135).Buffer;
let b = (e, t, n, r) => (
        o.default.track(g.rMx.EMOJI_UPLOAD_FILE_SIZE_LIMIT_EXCEEDED, {
            guild_id: n,
            file_size: t,
            upload_id: r,
        }),
        s.d.TOO_BIG
    ),
    p = async (e) => {
        let {
                data: t,
                file: n,
                guildId: p,
                uploadId: h,
                roles: x,
                image: j,
                hideErrorModal: v,
                analyticsLocation: O,
            } = e,
            C = c.ZP.sanitizeEmojiName(n.name.split(".")[0]);
        if (c.ZP.isFileTooBig(n)) {
            if ("image/gif" === n.type || "image/webp" === n.type || "image/avif" === n.type)
                return b(n.name, n.size, p, h);
            else if (null != j) {
                var y, N;
                let e;
                t = (0, r.Ae)(j, 128, 128);
                try {
                    e = c.ZP.isDataTooBig(t);
                } catch (e) {
                    return b(n.name, n.size, p, h);
                }
                if (
                    ((y = t),
                    (N = e),
                    o.default.track(g.rMx.EMOJI_FILE_RESIZED, {
                        is_animated: !1,
                        file_type: n.type.split("/").pop(),
                        original_file_size_bytes: n.size,
                        resized_file_size_bytes: (0, d.QB)(y),
                        resized_file_too_big: N,
                    }),
                    e)
                )
                    return b(n.name, n.size, p, h);
            }
        }
        try {
            let e = await (0, a.rS)({
                guildId: p,
                image: t,
                name: C,
                roles: x,
                analyticsLocation: O,
            });
            return (
                o.default.track(g.rMx.EMOJI_UPLOAD_COMPLETED, {
                    guild_id: p,
                    upload_id: h,
                }),
                (0, i.showToast)((0, i.createToast)(f.intl.string(f.t["r0w9m/"]), i.ToastType.SUCCESS)),
                e
            );
        } catch (a) {
            let e,
                n,
                { body: r, status: i } = a;
            return (
                null != r &&
                    (r.code === g.evJ.TOO_MANY_EMOJI
                        ? ((e = f.intl.string(f.t["jP/Rqm"])), (n = s.d.TOO_MANY_EMOJI))
                        : r.code === g.evJ.TOO_MANY_ANIMATED_EMOJI
                          ? ((e = f.intl.string(f.t["6v5dP/"])), (n = s.d.TOO_MANY_ANIMATED_EMOJI))
                          : null != r.image || r.code === g.evJ.INVALID_FILE_ASSET_SIZE
                            ? (o.default.track(g.rMx.EMOJI_UPLOAD_FILE_SIZE_LIMIT_EXCEEDED, {
                                  guild_id: p,
                                  file_size: m.byteLength(t),
                                  upload_id: h,
                              }),
                              (e = f.intl.formatToPlainString(f.t.kIO9jy, { maxSize: u.xG })),
                              (n = s.d.TOO_BIG))
                            : (null != r.image || r.code === g.evJ.INVALID_FILE_ASSET_SIZE_RESIZE_GIF) &&
                              (n = s.d.RESIZE_GIF)),
                429 === i && ((e = f.intl.string(f.t.Whhv4w)), (n = s.d.RATE_LIMIT)),
                null == e ||
                    v ||
                    l.Z.show({
                        title: f.intl.string(f.t.iufib1),
                        body: e,
                    }),
                n
            );
        }
    };
