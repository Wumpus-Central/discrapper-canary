n.d(t, {
    C4: () => B,
    Dd: () => N,
    E5: () => g,
    Fm: () => eE,
    GI: () => E,
    JY: () => eF,
    LX: () => P,
    Me: () => eu,
    Ns: () => p,
    O_: () => y,
    Oi: () => m,
    Pi: () => O,
    Q2: () => v,
    Q4: () => f,
    Tv: () => h,
    Us: () => I,
    X: () => u,
    Xr: () => _,
    aV: () => eB,
    az: () => X,
    bE: () => ep,
    c$: () => tn,
    dp: () => te,
    f9: () => x,
    hg: () => T,
    l1: () => S,
    n9: () => A,
    nI: () => d,
    o8: () => D,
    os: () => k,
    p5: () => H,
    sf: () => ef,
    tA: () => em,
    v_: () => en,
    yX: () => ek,
}),
    n(980754),
    n(388685),
    n(415506),
    n(410992),
    n(227481),
    n(730884),
    n(20464),
    n(341884),
    n(364341),
    n(629680),
    n(505025),
    n(918970),
    n(121784),
    n(644351),
    n(146733),
    n(539854);
var r = n(230367),
    i = n(320215),
    o = n(240773),
    a = n(495852),
    s = n(381499),
    l = n(835913),
    c = n(397696),
    u = (function (e) {
        return (
            (e[(e.UNSPECIFIED = 0)] = "UNSPECIFIED"),
            (e[(e.MENTIONS = 1)] = "MENTIONS"),
            (e[(e.UNREADS = 2)] = "UNREADS"),
            (e[(e.TODOS = 3)] = "TODOS"),
            (e[(e.FOR_YOU = 4)] = "FOR_YOU"),
            (e[(e.GAME_INVITES = 5)] = "GAME_INVITES"),
            (e[(e.BOOKMARKS = 6)] = "BOOKMARKS"),
            (e[(e.SCHEDULED = 7)] = "SCHEDULED"),
            e
        );
    })({}),
    d = (function (e) {
        return (
            (e[(e.NO_GUILD_ONBOARDING = 0)] = "NO_GUILD_ONBOARDING"),
            (e[(e.GUILD_NOTICE_SHOWN = 1)] = "GUILD_NOTICE_SHOWN"),
            (e[(e.GUILD_NOTICE_CLEARED = 2)] = "GUILD_NOTICE_CLEARED"),
            e
        );
    })({}),
    f = (function (e) {
        return (
            (e[(e.UNSET_EXPLICIT_CONTENT_REDACTION = 0)] = "UNSET_EXPLICIT_CONTENT_REDACTION"),
            (e[(e.SHOW = 1)] = "SHOW"),
            (e[(e.BLUR = 2)] = "BLUR"),
            (e[(e.BLOCK = 3)] = "BLOCK"),
            e
        );
    })({}),
    _ = (function (e) {
        return (
            (e[(e.DEFAULT_UNSET = 0)] = "DEFAULT_UNSET"),
            (e[(e.DISABLED = 1)] = "DISABLED"),
            (e[(e.NON_FRIENDS = 2)] = "NON_FRIENDS"),
            (e[(e.FRIENDS_AND_NON_FRIENDS = 3)] = "FRIENDS_AND_NON_FRIENDS"),
            e
        );
    })({}),
    p = (function (e) {
        return (
            (e[(e.NOTIFICATIONS_ENABLED = 0)] = "NOTIFICATIONS_ENABLED"),
            (e[(e.ONLY_DMS = 1)] = "ONLY_DMS"),
            (e[(e.NOTIFICATIONS_DISABLED = 2)] = "NOTIFICATIONS_DISABLED"),
            e
        );
    })({}),
    h = (function (e) {
        return (
            (e[(e.ACTIVITY_NOTIFICATIONS_UNSET = 0)] = "ACTIVITY_NOTIFICATIONS_UNSET"),
            (e[(e.ACTIVITY_NOTIFICATIONS_DISABLED = 1)] = "ACTIVITY_NOTIFICATIONS_DISABLED"),
            (e[(e.ACTIVITY_NOTIFICATIONS_ENABLED = 2)] = "ACTIVITY_NOTIFICATIONS_ENABLED"),
            (e[(e.ONLY_GAMES_PLAYED = 3)] = "ONLY_GAMES_PLAYED"),
            e
        );
    })({}),
    m = (function (e) {
        return (
            (e[(e.STATUS_PUSH_UNSET = 0)] = "STATUS_PUSH_UNSET"),
            (e[(e.STATUS_PUSH_ENABLED = 1)] = "STATUS_PUSH_ENABLED"),
            (e[(e.STATUS_PUSH_DISABLED = 2)] = "STATUS_PUSH_DISABLED"),
            e
        );
    })({}),
    g = (function (e) {
        return (
            (e[(e.OFF = 0)] = "OFF"),
            (e[(e.ON_FOR_LARGE_GUILDS = 1)] = "ON_FOR_LARGE_GUILDS"),
            (e[(e.ON = 2)] = "ON"),
            e
        );
    })({}),
    E = (function (e) {
        return (
            (e[(e.ACTIVITY_STATUS_UNSET = 0)] = "ACTIVITY_STATUS_UNSET"),
            (e[(e.ACTIVITY_STATUS_OFF = 1)] = "ACTIVITY_STATUS_OFF"),
            (e[(e.ACTIVITY_STATUS_ON_FOR_LARGE_GUILDS = 2)] = "ACTIVITY_STATUS_ON_FOR_LARGE_GUILDS"),
            (e[(e.ACTIVITY_STATUS_ON = 3)] = "ACTIVITY_STATUS_ON"),
            e
        );
    })({}),
    b = (function (e) {
        return (
            (e[(e.OFF_FOR_NEW_GUILDS = 0)] = "OFF_FOR_NEW_GUILDS"),
            (e[(e.ON_FOR_NEW_GUILDS = 1)] = "ON_FOR_NEW_GUILDS"),
            e
        );
    })({}),
    y = (function (e) {
        return (
            (e[(e.SLAYER_SDK_RECEIVE_IN_GAME_DMS_UNSET = 0)] = "SLAYER_SDK_RECEIVE_IN_GAME_DMS_UNSET"),
            (e[(e.SLAYER_SDK_RECEIVE_IN_GAME_DMS_ALL = 1)] = "SLAYER_SDK_RECEIVE_IN_GAME_DMS_ALL"),
            (e[(e.SLAYER_SDK_RECEIVE_IN_GAME_DMS_USERS_WITH_GAME = 2)] =
                "SLAYER_SDK_RECEIVE_IN_GAME_DMS_USERS_WITH_GAME"),
            (e[(e.SLAYER_SDK_RECEIVE_IN_GAME_DMS_NONE = 3)] = "SLAYER_SDK_RECEIVE_IN_GAME_DMS_NONE"),
            e
        );
    })({}),
    O = (function (e) {
        return (
            (e[(e.UNSET_UI_DENSITY = 0)] = "UNSET_UI_DENSITY"),
            (e[(e.COMPACT = 1)] = "COMPACT"),
            (e[(e.COZY = 2)] = "COZY"),
            (e[(e.RESPONSIVE = 3)] = "RESPONSIVE"),
            (e[(e.DEFAULT = 4)] = "DEFAULT"),
            e
        );
    })({}),
    v = (function (e) {
        return (
            (e[(e.UNSET = 0)] = "UNSET"),
            (e[(e.DARK = 1)] = "DARK"),
            (e[(e.LIGHT = 2)] = "LIGHT"),
            (e[(e.DARKER = 3)] = "DARKER"),
            (e[(e.MIDNIGHT = 4)] = "MIDNIGHT"),
            e
        );
    })({}),
    I = (function (e) {
        return (
            (e[(e.MINT_APPLE = 0)] = "MINT_APPLE"),
            (e[(e.CITRUS_SHERBERT = 1)] = "CITRUS_SHERBERT"),
            (e[(e.RETRO_RAINCLOUD = 2)] = "RETRO_RAINCLOUD"),
            (e[(e.HANAMI = 3)] = "HANAMI"),
            (e[(e.SUNRISE = 4)] = "SUNRISE"),
            (e[(e.COTTON_CANDY = 5)] = "COTTON_CANDY"),
            (e[(e.LOFI_VIBES = 6)] = "LOFI_VIBES"),
            (e[(e.DESERT_KHAKI = 7)] = "DESERT_KHAKI"),
            (e[(e.SUNSET = 8)] = "SUNSET"),
            (e[(e.CHROMA_GLOW = 9)] = "CHROMA_GLOW"),
            (e[(e.FOREST = 10)] = "FOREST"),
            (e[(e.CRIMSON_MOON = 11)] = "CRIMSON_MOON"),
            (e[(e.MIDNIGHT_BLURPLE = 12)] = "MIDNIGHT_BLURPLE"),
            (e[(e.MARS = 13)] = "MARS"),
            (e[(e.DUSK = 14)] = "DUSK"),
            (e[(e.UNDER_THE_SEA = 15)] = "UNDER_THE_SEA"),
            (e[(e.EASTER_EGG = 16)] = "EASTER_EGG"),
            (e[(e.RETRO_STORM = 17)] = "RETRO_STORM"),
            (e[(e.NEON_NIGHTS = 18)] = "NEON_NIGHTS"),
            (e[(e.SEPIA = 19)] = "SEPIA"),
            (e[(e.STRAWBERRY_LEMONADE = 20)] = "STRAWBERRY_LEMONADE"),
            (e[(e.AURORA = 21)] = "AURORA"),
            (e[(e.BLURPLE_TWILIGHT = 22)] = "BLURPLE_TWILIGHT"),
            e
        );
    })({}),
    T = (function (e) {
        return (e[(e.AUTO = 0)] = "AUTO"), (e[(e.H12 = 1)] = "H12"), (e[(e.H23 = 2)] = "H23"), e;
    })({}),
    S = (function (e) {
        return (
            (e[(e.LAUNCH_PAD_DISABLED = 0)] = "LAUNCH_PAD_DISABLED"),
            (e[(e.LAUNCH_PAD_GESTURE_FULL_SCREEN = 1)] = "LAUNCH_PAD_GESTURE_FULL_SCREEN"),
            (e[(e.LAUNCH_PAD_GESTURE_RIGHT_EDGE = 2)] = "LAUNCH_PAD_GESTURE_RIGHT_EDGE"),
            (e[(e.LAUNCH_PAD_PULL_TAB = 3)] = "LAUNCH_PAD_PULL_TAB"),
            e
        );
    })({}),
    A = (function (e) {
        return (
            (e[(e.SWIPE_RIGHT_TO_LEFT_UNSET = 0)] = "SWIPE_RIGHT_TO_LEFT_UNSET"),
            (e[(e.SWIPE_RIGHT_TO_LEFT_CHANNEL_DETAILS = 1)] = "SWIPE_RIGHT_TO_LEFT_CHANNEL_DETAILS"),
            (e[(e.SWIPE_RIGHT_TO_LEFT_REPLY = 2)] = "SWIPE_RIGHT_TO_LEFT_REPLY"),
            e
        );
    })({}),
    N = (function (e) {
        return (
            (e[(e.UNSET_FAVORITE_CHANNEL_TYPE = 0)] = "UNSET_FAVORITE_CHANNEL_TYPE"),
            (e[(e.REFERENCE_ORIGINAL = 1)] = "REFERENCE_ORIGINAL"),
            (e[(e.CATEGORY = 2)] = "CATEGORY"),
            e
        );
    })({}),
    C = (function (e) {
        return (
            (e[(e.UNSET_SAFETY_SETTINGS_PRESET = 0)] = "UNSET_SAFETY_SETTINGS_PRESET"),
            (e[(e.BALANCED = 1)] = "BALANCED"),
            (e[(e.STRICT = 2)] = "STRICT"),
            (e[(e.RELAXED = 3)] = "RELAXED"),
            (e[(e.CUSTOM = 4)] = "CUSTOM"),
            e
        );
    })({}),
    R = (function (e) {
        return (
            (e[(e.UNSPECIFIED = 0)] = "UNSPECIFIED"),
            (e[(e.ALL = 1)] = "ALL"),
            (e[(e.BOOKMARKS = 2)] = "BOOKMARKS"),
            (e[(e.REMINDERS = 3)] = "REMINDERS"),
            e
        );
    })({}),
    P = (function (e) {
        return (
            (e[(e.VOICE = 0)] = "VOICE"),
            (e[(e.STREAM = 1)] = "STREAM"),
            (e[(e.VIDEO_BACKGROUND = 2)] = "VIDEO_BACKGROUND"),
            (e[(e.ACTIVITY = 3)] = "ACTIVITY"),
            (e[(e.IN_APP_REPORTS = 4)] = "IN_APP_REPORTS"),
            (e[(e.USER_DM_MUTE = 5)] = "USER_DM_MUTE"),
            (e[(e.BLOCK_USER = 6)] = "BLOCK_USER"),
            (e[(e.VOICE_FILTER = 7)] = "VOICE_FILTER"),
            (e[(e.SEARCH_RESULTS = 8)] = "SEARCH_RESULTS"),
            (e[(e.AGE_VERIFICATION = 9)] = "AGE_VERIFICATION"),
            e
        );
    })({});
class w extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.versions = c.L.internalBinaryRead(e, e.uint32(), n, o.versions);
                    break;
                case 2:
                    o.inbox = x.internalBinaryRead(e, e.uint32(), n, o.inbox);
                    break;
                case 3:
                    o.guilds = k.internalBinaryRead(e, e.uint32(), n, o.guilds);
                    break;
                case 4:
                    o.userContent = X.internalBinaryRead(e, e.uint32(), n, o.userContent);
                    break;
                case 5:
                    o.voiceAndVideo = en.internalBinaryRead(e, e.uint32(), n, o.voiceAndVideo);
                    break;
                case 6:
                    o.textAndImages = eu.internalBinaryRead(e, e.uint32(), n, o.textAndImages);
                    break;
                case 7:
                    o.notifications = ef.internalBinaryRead(e, e.uint32(), n, o.notifications);
                    break;
                case 8:
                    o.privacy = ep.internalBinaryRead(e, e.uint32(), n, o.privacy);
                    break;
                case 9:
                    o.debug = em.internalBinaryRead(e, e.uint32(), n, o.debug);
                    break;
                case 10:
                    o.gameLibrary = eE.internalBinaryRead(e, e.uint32(), n, o.gameLibrary);
                    break;
                case 11:
                    o.status = ey.internalBinaryRead(e, e.uint32(), n, o.status);
                    break;
                case 12:
                    o.localization = eT.internalBinaryRead(e, e.uint32(), n, o.localization);
                    break;
                case 13:
                    o.appearance = eC.internalBinaryRead(e, e.uint32(), n, o.appearance);
                    break;
                case 14:
                    o.guildFolders = ex.internalBinaryRead(e, e.uint32(), n, o.guildFolders);
                    break;
                case 15:
                    o.favorites = eU.internalBinaryRead(e, e.uint32(), n, o.favorites);
                    break;
                case 16:
                    o.audioContextSettings = eH.internalBinaryRead(e, e.uint32(), n, o.audioContextSettings);
                    break;
                case 17:
                    o.communities = eW.internalBinaryRead(e, e.uint32(), n, o.communities);
                    break;
                case 18:
                    o.broadcast = eJ.internalBinaryRead(e, e.uint32(), n, o.broadcast);
                    break;
                case 19:
                    o.clips = e0.internalBinaryRead(e, e.uint32(), n, o.clips);
                    break;
                case 20:
                    o.forLater = e5.internalBinaryRead(e, e.uint32(), n, o.forLater);
                    break;
                case 21:
                    o.safetySettings = e2.internalBinaryRead(e, e.uint32(), n, o.safetySettings);
                    break;
                case 22:
                    o.icymiSettings = e6.internalBinaryRead(e, e.uint32(), n, o.icymiSettings);
                    break;
                case 23:
                    o.applications = e7.internalBinaryRead(e, e.uint32(), n, o.applications);
                    break;
                case 24:
                    o.ads = ta.internalBinaryRead(e, e.uint32(), n, o.ads);
                    break;
                case 25:
                    o.inAppFeedbackSettings = tu.internalBinaryRead(e, e.uint32(), n, o.inAppFeedbackSettings);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.versions && c.L.internalBinaryWrite(e.versions, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.inbox && x.internalBinaryWrite(e.inbox, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.guilds && k.internalBinaryWrite(e.guilds, t.tag(3, r.TD.LengthDelimited).fork(), n).join(),
            e.userContent && X.internalBinaryWrite(e.userContent, t.tag(4, r.TD.LengthDelimited).fork(), n).join(),
            e.voiceAndVideo && en.internalBinaryWrite(e.voiceAndVideo, t.tag(5, r.TD.LengthDelimited).fork(), n).join(),
            e.textAndImages && eu.internalBinaryWrite(e.textAndImages, t.tag(6, r.TD.LengthDelimited).fork(), n).join(),
            e.notifications && ef.internalBinaryWrite(e.notifications, t.tag(7, r.TD.LengthDelimited).fork(), n).join(),
            e.privacy && ep.internalBinaryWrite(e.privacy, t.tag(8, r.TD.LengthDelimited).fork(), n).join(),
            e.debug && em.internalBinaryWrite(e.debug, t.tag(9, r.TD.LengthDelimited).fork(), n).join(),
            e.gameLibrary && eE.internalBinaryWrite(e.gameLibrary, t.tag(10, r.TD.LengthDelimited).fork(), n).join(),
            e.status && ey.internalBinaryWrite(e.status, t.tag(11, r.TD.LengthDelimited).fork(), n).join(),
            e.localization && eT.internalBinaryWrite(e.localization, t.tag(12, r.TD.LengthDelimited).fork(), n).join(),
            e.appearance && eC.internalBinaryWrite(e.appearance, t.tag(13, r.TD.LengthDelimited).fork(), n).join(),
            e.guildFolders && ex.internalBinaryWrite(e.guildFolders, t.tag(14, r.TD.LengthDelimited).fork(), n).join(),
            e.favorites && eU.internalBinaryWrite(e.favorites, t.tag(15, r.TD.LengthDelimited).fork(), n).join(),
            e.audioContextSettings &&
                eH.internalBinaryWrite(e.audioContextSettings, t.tag(16, r.TD.LengthDelimited).fork(), n).join(),
            e.communities && eW.internalBinaryWrite(e.communities, t.tag(17, r.TD.LengthDelimited).fork(), n).join(),
            e.broadcast && eJ.internalBinaryWrite(e.broadcast, t.tag(18, r.TD.LengthDelimited).fork(), n).join(),
            e.clips && e0.internalBinaryWrite(e.clips, t.tag(19, r.TD.LengthDelimited).fork(), n).join(),
            e.forLater && e5.internalBinaryWrite(e.forLater, t.tag(20, r.TD.LengthDelimited).fork(), n).join(),
            e.safetySettings &&
                e2.internalBinaryWrite(e.safetySettings, t.tag(21, r.TD.LengthDelimited).fork(), n).join(),
            e.icymiSettings &&
                e6.internalBinaryWrite(e.icymiSettings, t.tag(22, r.TD.LengthDelimited).fork(), n).join(),
            e.applications && e7.internalBinaryWrite(e.applications, t.tag(23, r.TD.LengthDelimited).fork(), n).join(),
            e.ads && ta.internalBinaryWrite(e.ads, t.tag(24, r.TD.LengthDelimited).fork(), n).join(),
            e.inAppFeedbackSettings &&
                tu.internalBinaryWrite(e.inAppFeedbackSettings, t.tag(25, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.PreloadedUserSettings", [
            {
                no: 1,
                name: "versions",
                kind: "message",
                T: () => c.L,
            },
            {
                no: 2,
                name: "inbox",
                kind: "message",
                T: () => x,
            },
            {
                no: 3,
                name: "guilds",
                kind: "message",
                T: () => k,
            },
            {
                no: 4,
                name: "user_content",
                kind: "message",
                T: () => X,
            },
            {
                no: 5,
                name: "voice_and_video",
                kind: "message",
                T: () => en,
            },
            {
                no: 6,
                name: "text_and_images",
                kind: "message",
                T: () => eu,
            },
            {
                no: 7,
                name: "notifications",
                kind: "message",
                T: () => ef,
            },
            {
                no: 8,
                name: "privacy",
                kind: "message",
                T: () => ep,
            },
            {
                no: 9,
                name: "debug",
                kind: "message",
                T: () => em,
            },
            {
                no: 10,
                name: "game_library",
                kind: "message",
                T: () => eE,
            },
            {
                no: 11,
                name: "status",
                kind: "message",
                T: () => ey,
            },
            {
                no: 12,
                name: "localization",
                kind: "message",
                T: () => eT,
            },
            {
                no: 13,
                name: "appearance",
                kind: "message",
                T: () => eC,
            },
            {
                no: 14,
                name: "guild_folders",
                kind: "message",
                T: () => ex,
            },
            {
                no: 15,
                name: "favorites",
                kind: "message",
                T: () => eU,
            },
            {
                no: 16,
                name: "audio_context_settings",
                kind: "message",
                T: () => eH,
            },
            {
                no: 17,
                name: "communities",
                kind: "message",
                T: () => eW,
            },
            {
                no: 18,
                name: "broadcast",
                kind: "message",
                T: () => eJ,
            },
            {
                no: 19,
                name: "clips",
                kind: "message",
                T: () => e0,
            },
            {
                no: 20,
                name: "for_later",
                kind: "message",
                T: () => e5,
            },
            {
                no: 21,
                name: "safety_settings",
                kind: "message",
                T: () => e2,
            },
            {
                no: 22,
                name: "icymi_settings",
                kind: "message",
                T: () => e6,
            },
            {
                no: 23,
                name: "applications",
                kind: "message",
                T: () => e7,
            },
            {
                no: 24,
                name: "ads",
                kind: "message",
                T: () => ta,
            },
            {
                no: 25,
                name: "in_app_feedback_settings",
                kind: "message",
                T: () => tu,
            },
        ]);
    }
}
let D = new w();
class L extends a.C {
    create(e) {
        let t = {
            currentTab: 0,
            viewedTutorial: !1,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.currentTab = e.int32();
                    break;
                case 2:
                    o.viewedTutorial = e.bool();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.currentTab && t.tag(1, r.TD.Varint).int32(e.currentTab),
            !1 !== e.viewedTutorial && t.tag(2, r.TD.Varint).bool(e.viewedTutorial);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.InboxSettings", [
            {
                no: 1,
                name: "current_tab",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.InboxTab", u, "INBOX_TAB_"],
            },
            {
                no: 2,
                name: "viewed_tutorial",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let x = new L();
class M extends a.C {
    create(e) {
        let t = { guilds: {} };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) this.binaryReadMap1(o.guilds, e, n);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    binaryReadMap1(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.fixed64().toString();
                    break;
                case 2:
                    a = B.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.AllGuildSettings.guilds",
                    );
            }
        }
        e[null != o ? o : "0"] = null != a ? a : B.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of Object.keys(e.guilds))
            t.tag(1, r.TD.LengthDelimited).fork().tag(1, r.TD.Bit64).fixed64(i),
                t.tag(2, r.TD.LengthDelimited).fork(),
                B.internalBinaryWrite(e.guilds[i], t, n),
                t.join().join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.AllGuildSettings", [
            {
                no: 1,
                name: "guilds",
                kind: "map",
                K: 6,
                V: {
                    kind: "message",
                    T: () => B,
                },
            },
        ]);
    }
}
let k = new M();
class j extends a.C {
    create(e) {
        let t = {
            dismissed: !1,
            lastDismissedVersion: 0,
            lastDismissedAtMs: "0",
            lastDismissedObjectId: "0",
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.dismissed = e.bool();
                    break;
                case 2:
                    o.lastDismissedVersion = e.uint32();
                    break;
                case 3:
                    o.lastDismissedAtMs = e.uint64().toString();
                    break;
                case 4:
                    o.lastDismissedObjectId = e.uint64().toString();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.dismissed && t.tag(1, r.TD.Varint).bool(e.dismissed),
            0 !== e.lastDismissedVersion && t.tag(2, r.TD.Varint).uint32(e.lastDismissedVersion),
            "0" !== e.lastDismissedAtMs && t.tag(3, r.TD.Varint).uint64(e.lastDismissedAtMs),
            "0" !== e.lastDismissedObjectId && t.tag(4, r.TD.Varint).uint64(e.lastDismissedObjectId);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.GuildDismissibleContentState", [
            {
                no: 1,
                name: "dismissed",
                kind: "scalar",
                T: 8,
            },
            {
                no: 2,
                name: "last_dismissed_version",
                kind: "scalar",
                T: 13,
            },
            {
                no: 3,
                name: "last_dismissed_at_ms",
                kind: "scalar",
                T: 4,
            },
            {
                no: 4,
                name: "last_dismissed_object_id",
                kind: "scalar",
                T: 4,
            },
        ]);
    }
}
let U = new j();
class G extends a.C {
    create(e) {
        let t = {
            channels: {},
            hubProgress: 0,
            guildOnboardingProgress: 0,
            dismissedGuildContent: new Uint8Array(0),
            disableRaidAlertPush: !1,
            disableRaidAlertNag: !1,
            leaderboardsDisabled: !1,
            guildDismissibleContentStates: {},
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    this.binaryReadMap1(o.channels, e, n);
                    break;
                case 2:
                    o.hubProgress = e.uint32();
                    break;
                case 3:
                    o.guildOnboardingProgress = e.uint32();
                    break;
                case 4:
                    o.guildRecentsDismissedAt = l.E.internalBinaryRead(e, e.uint32(), n, o.guildRecentsDismissedAt);
                    break;
                case 5:
                    o.dismissedGuildContent = e.bytes();
                    break;
                case 6:
                    o.joinSound = eX.internalBinaryRead(e, e.uint32(), n, o.joinSound);
                    break;
                case 7:
                    o.mobileRedesignChannelListSettings = eA.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.mobileRedesignChannelListSettings,
                    );
                    break;
                case 8:
                    o.disableRaidAlertPush = e.bool();
                    break;
                case 9:
                    o.disableRaidAlertNag = e.bool();
                    break;
                case 10:
                    o.customNotificationSoundConfig = W.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.customNotificationSoundConfig,
                    );
                    break;
                case 11:
                    o.leaderboardsDisabled = e.bool();
                    break;
                case 12:
                    this.binaryReadMap12(o.guildDismissibleContentStates, e, n);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    binaryReadMap1(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.fixed64().toString();
                    break;
                case 2:
                    a = H.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.GuildSettings.channels",
                    );
            }
        }
        e[null != o ? o : "0"] = null != a ? a : H.create();
    }
    binaryReadMap12(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.int32();
                    break;
                case 2:
                    a = U.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.GuildSettings.guild_dismissible_content_states",
                    );
            }
        }
        e[null != o ? o : 0] = null != a ? a : U.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of Object.keys(e.channels))
            t.tag(1, r.TD.LengthDelimited).fork().tag(1, r.TD.Bit64).fixed64(i),
                t.tag(2, r.TD.LengthDelimited).fork(),
                H.internalBinaryWrite(e.channels[i], t, n),
                t.join().join();
        for (let i of (0 !== e.hubProgress && t.tag(2, r.TD.Varint).uint32(e.hubProgress),
        0 !== e.guildOnboardingProgress && t.tag(3, r.TD.Varint).uint32(e.guildOnboardingProgress),
        e.guildRecentsDismissedAt &&
            l.E.internalBinaryWrite(e.guildRecentsDismissedAt, t.tag(4, r.TD.LengthDelimited).fork(), n).join(),
        e.dismissedGuildContent.length && t.tag(5, r.TD.LengthDelimited).bytes(e.dismissedGuildContent),
        e.joinSound && eX.internalBinaryWrite(e.joinSound, t.tag(6, r.TD.LengthDelimited).fork(), n).join(),
        e.mobileRedesignChannelListSettings &&
            eA
                .internalBinaryWrite(e.mobileRedesignChannelListSettings, t.tag(7, r.TD.LengthDelimited).fork(), n)
                .join(),
        !1 !== e.disableRaidAlertPush && t.tag(8, r.TD.Varint).bool(e.disableRaidAlertPush),
        !1 !== e.disableRaidAlertNag && t.tag(9, r.TD.Varint).bool(e.disableRaidAlertNag),
        e.customNotificationSoundConfig &&
            W.internalBinaryWrite(e.customNotificationSoundConfig, t.tag(10, r.TD.LengthDelimited).fork(), n).join(),
        !1 !== e.leaderboardsDisabled && t.tag(11, r.TD.Varint).bool(e.leaderboardsDisabled),
        Object.keys(e.guildDismissibleContentStates)))
            t.tag(12, r.TD.LengthDelimited).fork().tag(1, r.TD.Varint).int32(parseInt(i)),
                t.tag(2, r.TD.LengthDelimited).fork(),
                U.internalBinaryWrite(e.guildDismissibleContentStates[i], t, n),
                t.join().join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.GuildSettings", [
            {
                no: 1,
                name: "channels",
                kind: "map",
                K: 6,
                V: {
                    kind: "message",
                    T: () => H,
                },
            },
            {
                no: 2,
                name: "hub_progress",
                kind: "scalar",
                T: 13,
            },
            {
                no: 3,
                name: "guild_onboarding_progress",
                kind: "scalar",
                T: 13,
            },
            {
                no: 4,
                name: "guild_recents_dismissed_at",
                kind: "message",
                T: () => l.E,
            },
            {
                no: 5,
                name: "dismissed_guild_content",
                kind: "scalar",
                T: 12,
            },
            {
                no: 6,
                name: "join_sound",
                kind: "message",
                T: () => eX,
            },
            {
                no: 7,
                name: "mobile_redesign_channel_list_settings",
                kind: "message",
                T: () => eA,
            },
            {
                no: 8,
                name: "disable_raid_alert_push",
                kind: "scalar",
                T: 8,
            },
            {
                no: 9,
                name: "disable_raid_alert_nag",
                kind: "scalar",
                T: 8,
            },
            {
                no: 10,
                name: "custom_notification_sound_config",
                kind: "message",
                T: () => W,
            },
            {
                no: 11,
                name: "leaderboards_disabled",
                kind: "scalar",
                T: 8,
            },
            {
                no: 12,
                name: "guild_dismissible_content_states",
                kind: "map",
                K: 5,
                V: {
                    kind: "message",
                    T: () => U,
                },
            },
        ]);
    }
}
let B = new G();
class Z extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.id = s.wA.internalBinaryRead(e, e.uint32(), n, o.id);
                    break;
                case 2:
                    o.name = s.Gm.internalBinaryRead(e, e.uint32(), n, o.name);
                    break;
                case 3:
                    o.color = s.wA.internalBinaryRead(e, e.uint32(), n, o.color);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.id && s.wA.internalBinaryWrite(e.id, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.name && s.Gm.internalBinaryWrite(e.name, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.color && s.wA.internalBinaryWrite(e.color, t.tag(3, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ChannelIconEmoji", [
            {
                no: 1,
                name: "id",
                kind: "message",
                T: () => s.wA,
            },
            {
                no: 2,
                name: "name",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 3,
                name: "color",
                kind: "message",
                T: () => s.wA,
            },
        ]);
    }
}
let F = new Z();
class V extends a.C {
    create(e) {
        let t = { collapsedInInbox: !1 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.collapsedInInbox = e.bool();
                    break;
                case 2:
                    o.iconEmoji = F.internalBinaryRead(e, e.uint32(), n, o.iconEmoji);
                    break;
                case 3:
                    o.customNotificationSoundConfig = W.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.customNotificationSoundConfig,
                    );
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.collapsedInInbox && t.tag(1, r.TD.Varint).bool(e.collapsedInInbox),
            e.iconEmoji && F.internalBinaryWrite(e.iconEmoji, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.customNotificationSoundConfig &&
                W.internalBinaryWrite(e.customNotificationSoundConfig, t.tag(3, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ChannelSettings", [
            {
                no: 1,
                name: "collapsed_in_inbox",
                kind: "scalar",
                T: 8,
            },
            {
                no: 2,
                name: "icon_emoji",
                kind: "message",
                T: () => F,
            },
            {
                no: 3,
                name: "custom_notification_sound_config",
                kind: "message",
                T: () => W,
            },
        ]);
    }
}
let H = new V();
class Y extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t)
                o.notificationSoundPackId = s.Gm.internalBinaryRead(e, e.uint32(), n, o.notificationSoundPackId);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.notificationSoundPackId &&
            s.Gm.internalBinaryWrite(e.notificationSoundPackId, t.tag(1, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.CustomNotificationSoundConfig", [
            {
                no: 1,
                name: "notification_sound_pack_id",
                kind: "message",
                T: () => s.Gm,
            },
        ]);
    }
}
let W = new Y();
class K extends a.C {
    create(e) {
        let t = {
            lastDismissedVersion: 0,
            lastDismissedAtMs: "0",
            lastDismissedObjectId: "0",
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.lastDismissedVersion = e.uint32();
                    break;
                case 2:
                    o.lastDismissedAtMs = e.uint64().toString();
                    break;
                case 3:
                    o.lastDismissedObjectId = e.uint64().toString();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.lastDismissedVersion && t.tag(1, r.TD.Varint).uint32(e.lastDismissedVersion),
            "0" !== e.lastDismissedAtMs && t.tag(2, r.TD.Varint).uint64(e.lastDismissedAtMs),
            "0" !== e.lastDismissedObjectId && t.tag(3, r.TD.Varint).uint64(e.lastDismissedObjectId);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.RecurringDismissibleContentState", [
            {
                no: 1,
                name: "last_dismissed_version",
                kind: "scalar",
                T: 13,
            },
            {
                no: 2,
                name: "last_dismissed_at_ms",
                kind: "scalar",
                T: 4,
            },
            {
                no: 3,
                name: "last_dismissed_object_id",
                kind: "scalar",
                T: 4,
            },
        ]);
    }
}
let z = new K();
class q extends a.C {
    create(e) {
        let t = {
            dismissedContents: new Uint8Array(0),
            lastReceivedChangelogId: "0",
            recurringDismissibleContentStates: {},
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.dismissedContents = e.bytes();
                    break;
                case 2:
                    o.lastDismissedOutboundPromotionStartDate = s.Gm.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.lastDismissedOutboundPromotionStartDate,
                    );
                    break;
                case 3:
                    o.premiumTier0ModalDismissedAt = l.E.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.premiumTier0ModalDismissedAt,
                    );
                    break;
                case 4:
                    o.guildOnboardingUpsellDismissedAt = l.E.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.guildOnboardingUpsellDismissedAt,
                    );
                    break;
                case 5:
                    o.safetyUserSentimentNoticeDismissedAt = l.E.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.safetyUserSentimentNoticeDismissedAt,
                    );
                    break;
                case 6:
                    o.lastReceivedChangelogId = e.fixed64().toString();
                    break;
                case 7:
                    this.binaryReadMap7(o.recurringDismissibleContentStates, e, n);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let c = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, c);
            }
        }
        return o;
    }
    binaryReadMap7(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.int32();
                    break;
                case 2:
                    a = z.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.UserContentSettings.recurring_dismissible_content_states",
                    );
            }
        }
        e[null != o ? o : 0] = null != a ? a : z.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of (e.dismissedContents.length && t.tag(1, r.TD.LengthDelimited).bytes(e.dismissedContents),
        e.lastDismissedOutboundPromotionStartDate &&
            s.Gm.internalBinaryWrite(
                e.lastDismissedOutboundPromotionStartDate,
                t.tag(2, r.TD.LengthDelimited).fork(),
                n,
            ).join(),
        e.premiumTier0ModalDismissedAt &&
            l.E.internalBinaryWrite(e.premiumTier0ModalDismissedAt, t.tag(3, r.TD.LengthDelimited).fork(), n).join(),
        e.guildOnboardingUpsellDismissedAt &&
            l.E.internalBinaryWrite(
                e.guildOnboardingUpsellDismissedAt,
                t.tag(4, r.TD.LengthDelimited).fork(),
                n,
            ).join(),
        e.safetyUserSentimentNoticeDismissedAt &&
            l.E.internalBinaryWrite(
                e.safetyUserSentimentNoticeDismissedAt,
                t.tag(5, r.TD.LengthDelimited).fork(),
                n,
            ).join(),
        "0" !== e.lastReceivedChangelogId && t.tag(6, r.TD.Bit64).fixed64(e.lastReceivedChangelogId),
        Object.keys(e.recurringDismissibleContentStates)))
            t.tag(7, r.TD.LengthDelimited).fork().tag(1, r.TD.Varint).int32(parseInt(i)),
                t.tag(2, r.TD.LengthDelimited).fork(),
                z.internalBinaryWrite(e.recurringDismissibleContentStates[i], t, n),
                t.join().join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.UserContentSettings", [
            {
                no: 1,
                name: "dismissed_contents",
                kind: "scalar",
                T: 12,
            },
            {
                no: 2,
                name: "last_dismissed_outbound_promotion_start_date",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 3,
                name: "premium_tier_0_modal_dismissed_at",
                kind: "message",
                T: () => l.E,
            },
            {
                no: 4,
                name: "guild_onboarding_upsell_dismissed_at",
                kind: "message",
                T: () => l.E,
            },
            {
                no: 5,
                name: "safety_user_sentiment_notice_dismissed_at",
                kind: "message",
                T: () => l.E,
            },
            {
                no: 6,
                name: "last_received_changelog_id",
                kind: "scalar",
                T: 6,
            },
            {
                no: 7,
                name: "recurring_dismissible_content_states",
                kind: "map",
                K: 5,
                V: {
                    kind: "message",
                    T: () => z,
                },
            },
        ]);
    }
}
let X = new q();
class Q extends a.C {
    create(e) {
        let t = {
            id: "0",
            assetHash: "",
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.id = e.fixed64().toString();
                    break;
                case 2:
                    o.assetHash = e.string();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        "0" !== e.id && t.tag(1, r.TD.Bit64).fixed64(e.id),
            "" !== e.assetHash && t.tag(2, r.TD.LengthDelimited).string(e.assetHash);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.VideoFilterAsset", [
            {
                no: 1,
                name: "id",
                kind: "scalar",
                T: 6,
            },
            {
                no: 2,
                name: "asset_hash",
                kind: "scalar",
                T: 9,
            },
        ]);
    }
}
let J = new Q();
class $ extends a.C {
    create(e) {
        let t = { useBlur: !1 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.useBlur = e.bool();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.useBlur && t.tag(1, r.TD.Varint).bool(e.useBlur);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.VideoFilterBackgroundBlur", [
            {
                no: 1,
                name: "use_blur",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let ee = new $();
class et extends a.C {
    create(e) {
        let t = { videoBackgroundFilterDesktop: { oneofKind: void 0 } };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.videoBackgroundFilterDesktop = {
                        oneofKind: "blur",
                        blur: ee.internalBinaryRead(e, e.uint32(), n, o.videoBackgroundFilterDesktop.blur),
                    };
                    break;
                case 2:
                    o.videoBackgroundFilterDesktop = {
                        oneofKind: "presetOption",
                        presetOption: e.uint32(),
                    };
                    break;
                case 3:
                    o.videoBackgroundFilterDesktop = {
                        oneofKind: "customAsset",
                        customAsset: J.internalBinaryRead(e, e.uint32(), n, o.videoBackgroundFilterDesktop.customAsset),
                    };
                    break;
                case 5:
                    o.alwaysPreviewVideo = s.D5.internalBinaryRead(e, e.uint32(), n, o.alwaysPreviewVideo);
                    break;
                case 6:
                    o.afkTimeout = s.yC.internalBinaryRead(e, e.uint32(), n, o.afkTimeout);
                    break;
                case 7:
                    o.streamNotificationsEnabled = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.streamNotificationsEnabled,
                    );
                    break;
                case 8:
                    o.nativePhoneIntegrationEnabled = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.nativePhoneIntegrationEnabled,
                    );
                    break;
                case 9:
                    o.soundboardSettings = ez.internalBinaryRead(e, e.uint32(), n, o.soundboardSettings);
                    break;
                case 10:
                    o.disableStreamPreviews = s.D5.internalBinaryRead(e, e.uint32(), n, o.disableStreamPreviews);
                    break;
                case 11:
                    o.soundmojiVolume = s.A8.internalBinaryRead(e, e.uint32(), n, o.soundmojiVolume);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        "blur" === e.videoBackgroundFilterDesktop.oneofKind &&
            ee
                .internalBinaryWrite(e.videoBackgroundFilterDesktop.blur, t.tag(1, r.TD.LengthDelimited).fork(), n)
                .join(),
            "presetOption" === e.videoBackgroundFilterDesktop.oneofKind &&
                t.tag(2, r.TD.Varint).uint32(e.videoBackgroundFilterDesktop.presetOption),
            "customAsset" === e.videoBackgroundFilterDesktop.oneofKind &&
                J.internalBinaryWrite(
                    e.videoBackgroundFilterDesktop.customAsset,
                    t.tag(3, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.alwaysPreviewVideo &&
                s.D5.internalBinaryWrite(e.alwaysPreviewVideo, t.tag(5, r.TD.LengthDelimited).fork(), n).join(),
            e.afkTimeout && s.yC.internalBinaryWrite(e.afkTimeout, t.tag(6, r.TD.LengthDelimited).fork(), n).join(),
            e.streamNotificationsEnabled &&
                s.D5.internalBinaryWrite(e.streamNotificationsEnabled, t.tag(7, r.TD.LengthDelimited).fork(), n).join(),
            e.nativePhoneIntegrationEnabled &&
                s.D5.internalBinaryWrite(
                    e.nativePhoneIntegrationEnabled,
                    t.tag(8, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.soundboardSettings &&
                ez.internalBinaryWrite(e.soundboardSettings, t.tag(9, r.TD.LengthDelimited).fork(), n).join(),
            e.disableStreamPreviews &&
                s.D5.internalBinaryWrite(e.disableStreamPreviews, t.tag(10, r.TD.LengthDelimited).fork(), n).join(),
            e.soundmojiVolume &&
                s.A8.internalBinaryWrite(e.soundmojiVolume, t.tag(11, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.VoiceAndVideoSettings", [
            {
                no: 1,
                name: "blur",
                kind: "message",
                oneof: "videoBackgroundFilterDesktop",
                T: () => ee,
            },
            {
                no: 2,
                name: "preset_option",
                kind: "scalar",
                oneof: "videoBackgroundFilterDesktop",
                T: 13,
            },
            {
                no: 3,
                name: "custom_asset",
                kind: "message",
                oneof: "videoBackgroundFilterDesktop",
                T: () => J,
            },
            {
                no: 5,
                name: "always_preview_video",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 6,
                name: "afk_timeout",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 7,
                name: "stream_notifications_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 8,
                name: "native_phone_integration_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 9,
                name: "soundboard_settings",
                kind: "message",
                T: () => ez,
            },
            {
                no: 10,
                name: "disable_stream_previews",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 11,
                name: "soundmoji_volume",
                kind: "message",
                T: () => s.A8,
            },
        ]);
    }
}
let en = new et();
class er extends a.C {
    create(e) {
        let t = {
            explicitContentGuilds: 0,
            explicitContentFriendDm: 0,
            explicitContentNonFriendDm: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.explicitContentGuilds = e.int32();
                    break;
                case 2:
                    o.explicitContentFriendDm = e.int32();
                    break;
                case 3:
                    o.explicitContentNonFriendDm = e.int32();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.explicitContentGuilds && t.tag(1, r.TD.Varint).int32(e.explicitContentGuilds),
            0 !== e.explicitContentFriendDm && t.tag(2, r.TD.Varint).int32(e.explicitContentFriendDm),
            0 !== e.explicitContentNonFriendDm && t.tag(3, r.TD.Varint).int32(e.explicitContentNonFriendDm);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ExplicitContentSettings", [
            {
                no: 1,
                name: "explicit_content_guilds",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ExplicitContentRedaction", f],
            },
            {
                no: 2,
                name: "explicit_content_friend_dm",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ExplicitContentRedaction", f],
            },
            {
                no: 3,
                name: "explicit_content_non_friend_dm",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ExplicitContentRedaction", f],
            },
        ]);
    }
}
let ei = new er();
class eo extends a.C {
    create(e) {
        let t = {
            goreContentGuilds: 0,
            goreContentFriendDm: 0,
            goreContentNonFriendDm: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.goreContentGuilds = e.int32();
                    break;
                case 2:
                    o.goreContentFriendDm = e.int32();
                    break;
                case 3:
                    o.goreContentNonFriendDm = e.int32();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.goreContentGuilds && t.tag(1, r.TD.Varint).int32(e.goreContentGuilds),
            0 !== e.goreContentFriendDm && t.tag(2, r.TD.Varint).int32(e.goreContentFriendDm),
            0 !== e.goreContentNonFriendDm && t.tag(3, r.TD.Varint).int32(e.goreContentNonFriendDm);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.GoreContentSettings", [
            {
                no: 1,
                name: "gore_content_guilds",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ExplicitContentRedaction", f],
            },
            {
                no: 2,
                name: "gore_content_friend_dm",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ExplicitContentRedaction", f],
            },
            {
                no: 3,
                name: "gore_content_non_friend_dm",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ExplicitContentRedaction", f],
            },
        ]);
    }
}
let ea = new eo();
class es extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.profanity = s.D5.internalBinaryRead(e, e.uint32(), n, o.profanity);
                    break;
                case 2:
                    o.sexualContent = s.D5.internalBinaryRead(e, e.uint32(), n, o.sexualContent);
                    break;
                case 3:
                    o.slurs = s.D5.internalBinaryRead(e, e.uint32(), n, o.slurs);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.profanity && s.D5.internalBinaryWrite(e.profanity, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.sexualContent &&
                s.D5.internalBinaryWrite(e.sexualContent, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.slurs && s.D5.internalBinaryWrite(e.slurs, t.tag(3, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.KeywordFilterSettings", [
            {
                no: 1,
                name: "profanity",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 2,
                name: "sexual_content",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 3,
                name: "slurs",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let el = new es();
class ec extends a.C {
    create(e) {
        let t = {
            emojiPickerCollapsedSections: [],
            stickerPickerCollapsedSections: [],
            soundboardPickerCollapsedSections: [],
            dmSpamFilterV2: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.diversitySurrogate = s.Gm.internalBinaryRead(e, e.uint32(), n, o.diversitySurrogate);
                    break;
                case 2:
                    o.useRichChatInput = s.D5.internalBinaryRead(e, e.uint32(), n, o.useRichChatInput);
                    break;
                case 3:
                    o.useThreadSidebar = s.D5.internalBinaryRead(e, e.uint32(), n, o.useThreadSidebar);
                    break;
                case 4:
                    o.renderSpoilers = s.Gm.internalBinaryRead(e, e.uint32(), n, o.renderSpoilers);
                    break;
                case 5:
                    o.emojiPickerCollapsedSections.push(e.string());
                    break;
                case 6:
                    o.stickerPickerCollapsedSections.push(e.string());
                    break;
                case 7:
                    o.viewImageDescriptions = s.D5.internalBinaryRead(e, e.uint32(), n, o.viewImageDescriptions);
                    break;
                case 8:
                    o.showCommandSuggestions = s.D5.internalBinaryRead(e, e.uint32(), n, o.showCommandSuggestions);
                    break;
                case 9:
                    o.inlineAttachmentMedia = s.D5.internalBinaryRead(e, e.uint32(), n, o.inlineAttachmentMedia);
                    break;
                case 10:
                    o.inlineEmbedMedia = s.D5.internalBinaryRead(e, e.uint32(), n, o.inlineEmbedMedia);
                    break;
                case 11:
                    o.gifAutoPlay = s.D5.internalBinaryRead(e, e.uint32(), n, o.gifAutoPlay);
                    break;
                case 12:
                    o.renderEmbeds = s.D5.internalBinaryRead(e, e.uint32(), n, o.renderEmbeds);
                    break;
                case 13:
                    o.renderReactions = s.D5.internalBinaryRead(e, e.uint32(), n, o.renderReactions);
                    break;
                case 14:
                    o.animateEmoji = s.D5.internalBinaryRead(e, e.uint32(), n, o.animateEmoji);
                    break;
                case 15:
                    o.animateStickers = s.yC.internalBinaryRead(e, e.uint32(), n, o.animateStickers);
                    break;
                case 16:
                    o.enableTtsCommand = s.D5.internalBinaryRead(e, e.uint32(), n, o.enableTtsCommand);
                    break;
                case 17:
                    o.messageDisplayCompact = s.D5.internalBinaryRead(e, e.uint32(), n, o.messageDisplayCompact);
                    break;
                case 19:
                    o.explicitContentFilter = s.yC.internalBinaryRead(e, e.uint32(), n, o.explicitContentFilter);
                    break;
                case 20:
                    o.viewNsfwGuilds = s.D5.internalBinaryRead(e, e.uint32(), n, o.viewNsfwGuilds);
                    break;
                case 21:
                    o.convertEmoticons = s.D5.internalBinaryRead(e, e.uint32(), n, o.convertEmoticons);
                    break;
                case 22:
                    o.expressionSuggestionsEnabled = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.expressionSuggestionsEnabled,
                    );
                    break;
                case 23:
                    o.viewNsfwCommands = s.D5.internalBinaryRead(e, e.uint32(), n, o.viewNsfwCommands);
                    break;
                case 24:
                    o.useLegacyChatInput = s.D5.internalBinaryRead(e, e.uint32(), n, o.useLegacyChatInput);
                    break;
                case 25:
                    o.soundboardPickerCollapsedSections.push(e.string());
                    break;
                case 26:
                    o.dmSpamFilter = s.yC.internalBinaryRead(e, e.uint32(), n, o.dmSpamFilter);
                    break;
                case 27:
                    o.dmSpamFilterV2 = e.int32();
                    break;
                case 28:
                    o.includeStickersInAutocomplete = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.includeStickersInAutocomplete,
                    );
                    break;
                case 29:
                    o.explicitContentSettings = ei.internalBinaryRead(e, e.uint32(), n, o.explicitContentSettings);
                    break;
                case 30:
                    o.keywordFilterSettings = el.internalBinaryRead(e, e.uint32(), n, o.keywordFilterSettings);
                    break;
                case 31:
                    o.includeSoundmojiInAutocomplete = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.includeSoundmojiInAutocomplete,
                    );
                    break;
                case 32:
                    o.goreContentSettings = ea.internalBinaryRead(e, e.uint32(), n, o.goreContentSettings);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.diversitySurrogate &&
            s.Gm.internalBinaryWrite(e.diversitySurrogate, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.useRichChatInput &&
                s.D5.internalBinaryWrite(e.useRichChatInput, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.useThreadSidebar &&
                s.D5.internalBinaryWrite(e.useThreadSidebar, t.tag(3, r.TD.LengthDelimited).fork(), n).join(),
            e.renderSpoilers &&
                s.Gm.internalBinaryWrite(e.renderSpoilers, t.tag(4, r.TD.LengthDelimited).fork(), n).join();
        for (let n = 0; n < e.emojiPickerCollapsedSections.length; n++)
            t.tag(5, r.TD.LengthDelimited).string(e.emojiPickerCollapsedSections[n]);
        for (let n = 0; n < e.stickerPickerCollapsedSections.length; n++)
            t.tag(6, r.TD.LengthDelimited).string(e.stickerPickerCollapsedSections[n]);
        e.viewImageDescriptions &&
            s.D5.internalBinaryWrite(e.viewImageDescriptions, t.tag(7, r.TD.LengthDelimited).fork(), n).join(),
            e.showCommandSuggestions &&
                s.D5.internalBinaryWrite(e.showCommandSuggestions, t.tag(8, r.TD.LengthDelimited).fork(), n).join(),
            e.inlineAttachmentMedia &&
                s.D5.internalBinaryWrite(e.inlineAttachmentMedia, t.tag(9, r.TD.LengthDelimited).fork(), n).join(),
            e.inlineEmbedMedia &&
                s.D5.internalBinaryWrite(e.inlineEmbedMedia, t.tag(10, r.TD.LengthDelimited).fork(), n).join(),
            e.gifAutoPlay && s.D5.internalBinaryWrite(e.gifAutoPlay, t.tag(11, r.TD.LengthDelimited).fork(), n).join(),
            e.renderEmbeds &&
                s.D5.internalBinaryWrite(e.renderEmbeds, t.tag(12, r.TD.LengthDelimited).fork(), n).join(),
            e.renderReactions &&
                s.D5.internalBinaryWrite(e.renderReactions, t.tag(13, r.TD.LengthDelimited).fork(), n).join(),
            e.animateEmoji &&
                s.D5.internalBinaryWrite(e.animateEmoji, t.tag(14, r.TD.LengthDelimited).fork(), n).join(),
            e.animateStickers &&
                s.yC.internalBinaryWrite(e.animateStickers, t.tag(15, r.TD.LengthDelimited).fork(), n).join(),
            e.enableTtsCommand &&
                s.D5.internalBinaryWrite(e.enableTtsCommand, t.tag(16, r.TD.LengthDelimited).fork(), n).join(),
            e.messageDisplayCompact &&
                s.D5.internalBinaryWrite(e.messageDisplayCompact, t.tag(17, r.TD.LengthDelimited).fork(), n).join(),
            e.explicitContentFilter &&
                s.yC.internalBinaryWrite(e.explicitContentFilter, t.tag(19, r.TD.LengthDelimited).fork(), n).join(),
            e.viewNsfwGuilds &&
                s.D5.internalBinaryWrite(e.viewNsfwGuilds, t.tag(20, r.TD.LengthDelimited).fork(), n).join(),
            e.convertEmoticons &&
                s.D5.internalBinaryWrite(e.convertEmoticons, t.tag(21, r.TD.LengthDelimited).fork(), n).join(),
            e.expressionSuggestionsEnabled &&
                s.D5.internalBinaryWrite(
                    e.expressionSuggestionsEnabled,
                    t.tag(22, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.viewNsfwCommands &&
                s.D5.internalBinaryWrite(e.viewNsfwCommands, t.tag(23, r.TD.LengthDelimited).fork(), n).join(),
            e.useLegacyChatInput &&
                s.D5.internalBinaryWrite(e.useLegacyChatInput, t.tag(24, r.TD.LengthDelimited).fork(), n).join();
        for (let n = 0; n < e.soundboardPickerCollapsedSections.length; n++)
            t.tag(25, r.TD.LengthDelimited).string(e.soundboardPickerCollapsedSections[n]);
        e.dmSpamFilter && s.yC.internalBinaryWrite(e.dmSpamFilter, t.tag(26, r.TD.LengthDelimited).fork(), n).join(),
            0 !== e.dmSpamFilterV2 && t.tag(27, r.TD.Varint).int32(e.dmSpamFilterV2),
            e.includeStickersInAutocomplete &&
                s.D5.internalBinaryWrite(
                    e.includeStickersInAutocomplete,
                    t.tag(28, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.explicitContentSettings &&
                ei.internalBinaryWrite(e.explicitContentSettings, t.tag(29, r.TD.LengthDelimited).fork(), n).join(),
            e.keywordFilterSettings &&
                el.internalBinaryWrite(e.keywordFilterSettings, t.tag(30, r.TD.LengthDelimited).fork(), n).join(),
            e.includeSoundmojiInAutocomplete &&
                s.D5.internalBinaryWrite(
                    e.includeSoundmojiInAutocomplete,
                    t.tag(31, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.goreContentSettings &&
                ea.internalBinaryWrite(e.goreContentSettings, t.tag(32, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.TextAndImagesSettings", [
            {
                no: 1,
                name: "diversity_surrogate",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 2,
                name: "use_rich_chat_input",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 3,
                name: "use_thread_sidebar",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 4,
                name: "render_spoilers",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 5,
                name: "emoji_picker_collapsed_sections",
                kind: "scalar",
                repeat: 2,
                T: 9,
            },
            {
                no: 6,
                name: "sticker_picker_collapsed_sections",
                kind: "scalar",
                repeat: 2,
                T: 9,
            },
            {
                no: 7,
                name: "view_image_descriptions",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 8,
                name: "show_command_suggestions",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 9,
                name: "inline_attachment_media",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 10,
                name: "inline_embed_media",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 11,
                name: "gif_auto_play",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 12,
                name: "render_embeds",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 13,
                name: "render_reactions",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 14,
                name: "animate_emoji",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 15,
                name: "animate_stickers",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 16,
                name: "enable_tts_command",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 17,
                name: "message_display_compact",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 19,
                name: "explicit_content_filter",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 20,
                name: "view_nsfw_guilds",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 21,
                name: "convert_emoticons",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 22,
                name: "expression_suggestions_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 23,
                name: "view_nsfw_commands",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 24,
                name: "use_legacy_chat_input",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 25,
                name: "soundboard_picker_collapsed_sections",
                kind: "scalar",
                repeat: 2,
                T: 9,
            },
            {
                no: 26,
                name: "dm_spam_filter",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 27,
                name: "dm_spam_filter_v2",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.DmSpamFilterV2", _],
            },
            {
                no: 28,
                name: "include_stickers_in_autocomplete",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 29,
                name: "explicit_content_settings",
                kind: "message",
                T: () => ei,
            },
            {
                no: 30,
                name: "keyword_filter_settings",
                kind: "message",
                T: () => el,
            },
            {
                no: 31,
                name: "include_soundmoji_in_autocomplete",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 32,
                name: "gore_content_settings",
                kind: "message",
                T: () => ea,
            },
        ]);
    }
}
let eu = new ec();
class ed extends a.C {
    create(e) {
        let t = {
            notificationCenterAckedBeforeId: "0",
            focusModeExpiresAtMs: "0",
            reactionNotifications: 0,
            gameActivityNotifications: 0,
            customStatusPushNotifications: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.showInAppNotifications = s.D5.internalBinaryRead(e, e.uint32(), n, o.showInAppNotifications);
                    break;
                case 2:
                    o.notifyFriendsOnGoLive = s.D5.internalBinaryRead(e, e.uint32(), n, o.notifyFriendsOnGoLive);
                    break;
                case 3:
                    o.notificationCenterAckedBeforeId = e.fixed64().toString();
                    break;
                case 4:
                    o.enableBurstReactionNotifications = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.enableBurstReactionNotifications,
                    );
                    break;
                case 5:
                    o.quietMode = s.D5.internalBinaryRead(e, e.uint32(), n, o.quietMode);
                    break;
                case 6:
                    o.focusModeExpiresAtMs = e.fixed64().toString();
                    break;
                case 7:
                    o.reactionNotifications = e.int32();
                    break;
                case 8:
                    o.gameActivityNotifications = e.int32();
                    break;
                case 9:
                    o.customStatusPushNotifications = e.int32();
                    break;
                case 10:
                    o.gameActivityExcludeSteamNotifications = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.gameActivityExcludeSteamNotifications,
                    );
                    break;
                case 11:
                    o.enableVoiceActivityNotifications = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.enableVoiceActivityNotifications,
                    );
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.showInAppNotifications &&
            s.D5.internalBinaryWrite(e.showInAppNotifications, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.notifyFriendsOnGoLive &&
                s.D5.internalBinaryWrite(e.notifyFriendsOnGoLive, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            "0" !== e.notificationCenterAckedBeforeId &&
                t.tag(3, r.TD.Bit64).fixed64(e.notificationCenterAckedBeforeId),
            e.enableBurstReactionNotifications &&
                s.D5.internalBinaryWrite(
                    e.enableBurstReactionNotifications,
                    t.tag(4, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.quietMode && s.D5.internalBinaryWrite(e.quietMode, t.tag(5, r.TD.LengthDelimited).fork(), n).join(),
            "0" !== e.focusModeExpiresAtMs && t.tag(6, r.TD.Bit64).fixed64(e.focusModeExpiresAtMs),
            0 !== e.reactionNotifications && t.tag(7, r.TD.Varint).int32(e.reactionNotifications),
            0 !== e.gameActivityNotifications && t.tag(8, r.TD.Varint).int32(e.gameActivityNotifications),
            0 !== e.customStatusPushNotifications && t.tag(9, r.TD.Varint).int32(e.customStatusPushNotifications),
            e.gameActivityExcludeSteamNotifications &&
                s.D5.internalBinaryWrite(
                    e.gameActivityExcludeSteamNotifications,
                    t.tag(10, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.enableVoiceActivityNotifications &&
                s.D5.internalBinaryWrite(
                    e.enableVoiceActivityNotifications,
                    t.tag(11, r.TD.LengthDelimited).fork(),
                    n,
                ).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.NotificationSettings", [
            {
                no: 1,
                name: "show_in_app_notifications",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 2,
                name: "notify_friends_on_go_live",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 3,
                name: "notification_center_acked_before_id",
                kind: "scalar",
                T: 6,
            },
            {
                no: 4,
                name: "enable_burst_reaction_notifications",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 5,
                name: "quiet_mode",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 6,
                name: "focus_mode_expires_at_ms",
                kind: "scalar",
                T: 6,
            },
            {
                no: 7,
                name: "reaction_notifications",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ReactionNotificationType", p],
            },
            {
                no: 8,
                name: "game_activity_notifications",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.GameActivityNotificationType", h],
            },
            {
                no: 9,
                name: "custom_status_push_notifications",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.CustomStatusPushNotificationType", m],
            },
            {
                no: 10,
                name: "game_activity_exclude_steam_notifications",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 11,
                name: "enable_voice_activity_notifications",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let ef = new ed();
class e_ extends a.C {
    create(e) {
        let t = {
            restrictedGuildIds: [],
            defaultGuildsRestricted: !1,
            allowAccessibilityDetection: !1,
            activityRestrictedGuildIds: [],
            defaultGuildsActivityRestricted: 0,
            activityJoiningRestrictedGuildIds: [],
            messageRequestRestrictedGuildIds: [],
            guildsLeaderboardOptOutDefault: 0,
            slayerSdkReceiveDmsInGame: 0,
            defaultGuildsActivityRestrictedV2: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.allowActivityPartyPrivacyFriends = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.allowActivityPartyPrivacyFriends,
                    );
                    break;
                case 2:
                    o.allowActivityPartyPrivacyVoiceChannel = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.allowActivityPartyPrivacyVoiceChannel,
                    );
                    break;
                case 3:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; ) o.restrictedGuildIds.push(e.fixed64().toString());
                    else o.restrictedGuildIds.push(e.fixed64().toString());
                    break;
                case 4:
                    o.defaultGuildsRestricted = e.bool();
                    break;
                case 7:
                    o.allowAccessibilityDetection = e.bool();
                    break;
                case 8:
                    o.detectPlatformAccounts = s.D5.internalBinaryRead(e, e.uint32(), n, o.detectPlatformAccounts);
                    break;
                case 9:
                    o.passwordless = s.D5.internalBinaryRead(e, e.uint32(), n, o.passwordless);
                    break;
                case 10:
                    o.contactSyncEnabled = s.D5.internalBinaryRead(e, e.uint32(), n, o.contactSyncEnabled);
                    break;
                case 11:
                    o.friendSourceFlags = s.yC.internalBinaryRead(e, e.uint32(), n, o.friendSourceFlags);
                    break;
                case 12:
                    o.friendDiscoveryFlags = s.yC.internalBinaryRead(e, e.uint32(), n, o.friendDiscoveryFlags);
                    break;
                case 13:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; )
                            o.activityRestrictedGuildIds.push(e.fixed64().toString());
                    else o.activityRestrictedGuildIds.push(e.fixed64().toString());
                    break;
                case 14:
                    o.defaultGuildsActivityRestricted = e.int32();
                    break;
                case 15:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; )
                            o.activityJoiningRestrictedGuildIds.push(e.fixed64().toString());
                    else o.activityJoiningRestrictedGuildIds.push(e.fixed64().toString());
                    break;
                case 16:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; )
                            o.messageRequestRestrictedGuildIds.push(e.fixed64().toString());
                    else o.messageRequestRestrictedGuildIds.push(e.fixed64().toString());
                    break;
                case 17:
                    o.defaultMessageRequestRestricted = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.defaultMessageRequestRestricted,
                    );
                    break;
                case 18:
                    o.dropsOptedOut = s.D5.internalBinaryRead(e, e.uint32(), n, o.dropsOptedOut);
                    break;
                case 19:
                    o.nonSpamRetrainingOptIn = s.D5.internalBinaryRead(e, e.uint32(), n, o.nonSpamRetrainingOptIn);
                    break;
                case 20:
                    o.familyCenterEnabled = s.D5.internalBinaryRead(e, e.uint32(), n, o.familyCenterEnabled);
                    break;
                case 21:
                    o.familyCenterEnabledV2 = s.D5.internalBinaryRead(e, e.uint32(), n, o.familyCenterEnabledV2);
                    break;
                case 22:
                    o.hideLegacyUsername = s.D5.internalBinaryRead(e, e.uint32(), n, o.hideLegacyUsername);
                    break;
                case 23:
                    o.inappropriateConversationWarnings = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.inappropriateConversationWarnings,
                    );
                    break;
                case 24:
                    o.recentGamesEnabled = s.D5.internalBinaryRead(e, e.uint32(), n, o.recentGamesEnabled);
                    break;
                case 25:
                    o.guildsLeaderboardOptOutDefault = e.int32();
                    break;
                case 26:
                    o.allowGameFriendDmsInDiscord = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.allowGameFriendDmsInDiscord,
                    );
                    break;
                case 27:
                    o.defaultGuildsRestrictedV2 = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.defaultGuildsRestrictedV2,
                    );
                    break;
                case 28:
                    o.slayerSdkReceiveDmsInGame = e.int32();
                    break;
                case 29:
                    o.defaultGuildsActivityRestrictedV2 = e.int32();
                    break;
                case 30:
                    o.quests3PDataOptedOut = s.D5.internalBinaryRead(e, e.uint32(), n, o.quests3PDataOptedOut);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        if (
            (e.allowActivityPartyPrivacyFriends &&
                s.D5.internalBinaryWrite(
                    e.allowActivityPartyPrivacyFriends,
                    t.tag(1, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.allowActivityPartyPrivacyVoiceChannel &&
                s.D5.internalBinaryWrite(
                    e.allowActivityPartyPrivacyVoiceChannel,
                    t.tag(2, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.restrictedGuildIds.length)
        ) {
            t.tag(3, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.restrictedGuildIds.length; n++) t.fixed64(e.restrictedGuildIds[n]);
            t.join();
        }
        if (
            (!1 !== e.defaultGuildsRestricted && t.tag(4, r.TD.Varint).bool(e.defaultGuildsRestricted),
            !1 !== e.allowAccessibilityDetection && t.tag(7, r.TD.Varint).bool(e.allowAccessibilityDetection),
            e.detectPlatformAccounts &&
                s.D5.internalBinaryWrite(e.detectPlatformAccounts, t.tag(8, r.TD.LengthDelimited).fork(), n).join(),
            e.passwordless && s.D5.internalBinaryWrite(e.passwordless, t.tag(9, r.TD.LengthDelimited).fork(), n).join(),
            e.contactSyncEnabled &&
                s.D5.internalBinaryWrite(e.contactSyncEnabled, t.tag(10, r.TD.LengthDelimited).fork(), n).join(),
            e.friendSourceFlags &&
                s.yC.internalBinaryWrite(e.friendSourceFlags, t.tag(11, r.TD.LengthDelimited).fork(), n).join(),
            e.friendDiscoveryFlags &&
                s.yC.internalBinaryWrite(e.friendDiscoveryFlags, t.tag(12, r.TD.LengthDelimited).fork(), n).join(),
            e.activityRestrictedGuildIds.length)
        ) {
            t.tag(13, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.activityRestrictedGuildIds.length; n++) t.fixed64(e.activityRestrictedGuildIds[n]);
            t.join();
        }
        if (
            (0 !== e.defaultGuildsActivityRestricted && t.tag(14, r.TD.Varint).int32(e.defaultGuildsActivityRestricted),
            e.activityJoiningRestrictedGuildIds.length)
        ) {
            t.tag(15, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.activityJoiningRestrictedGuildIds.length; n++)
                t.fixed64(e.activityJoiningRestrictedGuildIds[n]);
            t.join();
        }
        if (e.messageRequestRestrictedGuildIds.length) {
            t.tag(16, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.messageRequestRestrictedGuildIds.length; n++)
                t.fixed64(e.messageRequestRestrictedGuildIds[n]);
            t.join();
        }
        e.defaultMessageRequestRestricted &&
            s.D5.internalBinaryWrite(
                e.defaultMessageRequestRestricted,
                t.tag(17, r.TD.LengthDelimited).fork(),
                n,
            ).join(),
            e.dropsOptedOut &&
                s.D5.internalBinaryWrite(e.dropsOptedOut, t.tag(18, r.TD.LengthDelimited).fork(), n).join(),
            e.nonSpamRetrainingOptIn &&
                s.D5.internalBinaryWrite(e.nonSpamRetrainingOptIn, t.tag(19, r.TD.LengthDelimited).fork(), n).join(),
            e.familyCenterEnabled &&
                s.D5.internalBinaryWrite(e.familyCenterEnabled, t.tag(20, r.TD.LengthDelimited).fork(), n).join(),
            e.familyCenterEnabledV2 &&
                s.D5.internalBinaryWrite(e.familyCenterEnabledV2, t.tag(21, r.TD.LengthDelimited).fork(), n).join(),
            e.hideLegacyUsername &&
                s.D5.internalBinaryWrite(e.hideLegacyUsername, t.tag(22, r.TD.LengthDelimited).fork(), n).join(),
            e.inappropriateConversationWarnings &&
                s.D5.internalBinaryWrite(
                    e.inappropriateConversationWarnings,
                    t.tag(23, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.recentGamesEnabled &&
                s.D5.internalBinaryWrite(e.recentGamesEnabled, t.tag(24, r.TD.LengthDelimited).fork(), n).join(),
            0 !== e.guildsLeaderboardOptOutDefault && t.tag(25, r.TD.Varint).int32(e.guildsLeaderboardOptOutDefault),
            e.allowGameFriendDmsInDiscord &&
                s.D5.internalBinaryWrite(
                    e.allowGameFriendDmsInDiscord,
                    t.tag(26, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            e.defaultGuildsRestrictedV2 &&
                s.D5.internalBinaryWrite(e.defaultGuildsRestrictedV2, t.tag(27, r.TD.LengthDelimited).fork(), n).join(),
            0 !== e.slayerSdkReceiveDmsInGame && t.tag(28, r.TD.Varint).int32(e.slayerSdkReceiveDmsInGame),
            0 !== e.defaultGuildsActivityRestrictedV2 &&
                t.tag(29, r.TD.Varint).int32(e.defaultGuildsActivityRestrictedV2),
            e.quests3PDataOptedOut &&
                s.D5.internalBinaryWrite(e.quests3PDataOptedOut, t.tag(30, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.PrivacySettings", [
            {
                no: 1,
                name: "allow_activity_party_privacy_friends",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 2,
                name: "allow_activity_party_privacy_voice_channel",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 3,
                name: "restricted_guild_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 4,
                name: "default_guilds_restricted",
                kind: "scalar",
                T: 8,
            },
            {
                no: 7,
                name: "allow_accessibility_detection",
                kind: "scalar",
                T: 8,
            },
            {
                no: 8,
                name: "detect_platform_accounts",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 9,
                name: "passwordless",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 10,
                name: "contact_sync_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 11,
                name: "friend_source_flags",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 12,
                name: "friend_discovery_flags",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 13,
                name: "activity_restricted_guild_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 14,
                name: "default_guilds_activity_restricted",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.GuildActivityStatusRestrictionDefault", g],
            },
            {
                no: 15,
                name: "activity_joining_restricted_guild_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 16,
                name: "message_request_restricted_guild_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 17,
                name: "default_message_request_restricted",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 18,
                name: "drops_opted_out",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 19,
                name: "non_spam_retraining_opt_in",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 20,
                name: "family_center_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 21,
                name: "family_center_enabled_v2",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 22,
                name: "hide_legacy_username",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 23,
                name: "inappropriate_conversation_warnings",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 24,
                name: "recent_games_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 25,
                name: "guilds_leaderboard_opt_out_default",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.GuildsLeaderboardOptOutDefault", b],
            },
            {
                no: 26,
                name: "allow_game_friend_dms_in_discord",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 27,
                name: "default_guilds_restricted_v2",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 28,
                name: "slayer_sdk_receive_dms_in_game",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.SlayerSDKReceiveInGameDMs", y],
            },
            {
                no: 29,
                name: "default_guilds_activity_restricted_v2",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.GuildActivityStatusRestrictionDefaultV2", E],
            },
            {
                no: 30,
                name: "quests_3p_data_opted_out",
                kind: "message",
                jsonName: "quests3pDataOptedOut",
                T: () => s.D5,
            },
        ]);
    }
}
let ep = new e_();
class eh extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t)
                o.rtcPanelShowVoiceStates = s.D5.internalBinaryRead(e, e.uint32(), n, o.rtcPanelShowVoiceStates);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.rtcPanelShowVoiceStates &&
            s.D5.internalBinaryWrite(e.rtcPanelShowVoiceStates, t.tag(1, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.DebugSettings", [
            {
                no: 1,
                name: "rtc_panel_show_voice_states",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let em = new eh();
class eg extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.installShortcutDesktop = s.D5.internalBinaryRead(e, e.uint32(), n, o.installShortcutDesktop);
                    break;
                case 2:
                    o.installShortcutStartMenu = s.D5.internalBinaryRead(e, e.uint32(), n, o.installShortcutStartMenu);
                    break;
                case 3:
                    o.disableGamesTab = s.D5.internalBinaryRead(e, e.uint32(), n, o.disableGamesTab);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.installShortcutDesktop &&
            s.D5.internalBinaryWrite(e.installShortcutDesktop, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.installShortcutStartMenu &&
                s.D5.internalBinaryWrite(e.installShortcutStartMenu, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.disableGamesTab &&
                s.D5.internalBinaryWrite(e.disableGamesTab, t.tag(3, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.GameLibrarySettings", [
            {
                no: 1,
                name: "install_shortcut_desktop",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 2,
                name: "install_shortcut_start_menu",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 3,
                name: "disable_games_tab",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let eE = new eg();
class eb extends a.C {
    create(e) {
        let t = { statusExpiresAtMs: "0" };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.status = s.Gm.internalBinaryRead(e, e.uint32(), n, o.status);
                    break;
                case 2:
                    o.customStatus = ev.internalBinaryRead(e, e.uint32(), n, o.customStatus);
                    break;
                case 3:
                    o.showCurrentGame = s.D5.internalBinaryRead(e, e.uint32(), n, o.showCurrentGame);
                    break;
                case 4:
                    o.statusExpiresAtMs = e.fixed64().toString();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.status && s.Gm.internalBinaryWrite(e.status, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.customStatus && ev.internalBinaryWrite(e.customStatus, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.showCurrentGame &&
                s.D5.internalBinaryWrite(e.showCurrentGame, t.tag(3, r.TD.LengthDelimited).fork(), n).join(),
            "0" !== e.statusExpiresAtMs && t.tag(4, r.TD.Bit64).fixed64(e.statusExpiresAtMs);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.StatusSettings", [
            {
                no: 1,
                name: "status",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 2,
                name: "custom_status",
                kind: "message",
                T: () => ev,
            },
            {
                no: 3,
                name: "show_current_game",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 4,
                name: "status_expires_at_ms",
                kind: "scalar",
                T: 6,
            },
        ]);
    }
}
let ey = new eb();
class eO extends a.C {
    create(e) {
        let t = {
            text: "",
            emojiId: "0",
            emojiName: "",
            expiresAtMs: "0",
            createdAtMs: "0",
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.text = e.string();
                    break;
                case 2:
                    o.emojiId = e.fixed64().toString();
                    break;
                case 3:
                    o.emojiName = e.string();
                    break;
                case 4:
                    o.expiresAtMs = e.fixed64().toString();
                    break;
                case 5:
                    o.createdAtMs = e.fixed64().toString();
                    break;
                case 6:
                    o.label = s.Gm.internalBinaryRead(e, e.uint32(), n, o.label);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        "" !== e.text && t.tag(1, r.TD.LengthDelimited).string(e.text),
            "0" !== e.emojiId && t.tag(2, r.TD.Bit64).fixed64(e.emojiId),
            "" !== e.emojiName && t.tag(3, r.TD.LengthDelimited).string(e.emojiName),
            "0" !== e.expiresAtMs && t.tag(4, r.TD.Bit64).fixed64(e.expiresAtMs),
            "0" !== e.createdAtMs && t.tag(5, r.TD.Bit64).fixed64(e.createdAtMs),
            e.label && s.Gm.internalBinaryWrite(e.label, t.tag(6, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.CustomStatus", [
            {
                no: 1,
                name: "text",
                kind: "scalar",
                T: 9,
            },
            {
                no: 2,
                name: "emoji_id",
                kind: "scalar",
                T: 6,
            },
            {
                no: 3,
                name: "emoji_name",
                kind: "scalar",
                T: 9,
            },
            {
                no: 4,
                name: "expires_at_ms",
                kind: "scalar",
                T: 6,
            },
            {
                no: 5,
                name: "created_at_ms",
                kind: "scalar",
                T: 6,
            },
            {
                no: 6,
                name: "label",
                kind: "message",
                T: () => s.Gm,
            },
        ]);
    }
}
let ev = new eO();
class eI extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.locale = s.Gm.internalBinaryRead(e, e.uint32(), n, o.locale);
                    break;
                case 2:
                    o.timezoneOffset = s.T4.internalBinaryRead(e, e.uint32(), n, o.timezoneOffset);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.locale && s.Gm.internalBinaryWrite(e.locale, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.timezoneOffset &&
                s.T4.internalBinaryWrite(e.timezoneOffset, t.tag(2, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.LocalizationSettings", [
            {
                no: 1,
                name: "locale",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 2,
                name: "timezone_offset",
                kind: "message",
                T: () => s.T4,
            },
        ]);
    }
}
let eT = new eI();
class eS extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.layout = s.Gm.internalBinaryRead(e, e.uint32(), n, o.layout);
                    break;
                case 2:
                    o.messagePreviews = s.Gm.internalBinaryRead(e, e.uint32(), n, o.messagePreviews);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.layout && s.Gm.internalBinaryWrite(e.layout, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.messagePreviews &&
                s.Gm.internalBinaryWrite(e.messagePreviews, t.tag(2, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ChannelListSettings", [
            {
                no: 1,
                name: "layout",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 2,
                name: "message_previews",
                kind: "message",
                T: () => s.Gm,
            },
        ]);
    }
}
let eA = new eS();
class eN extends a.C {
    create(e) {
        let t = {
            theme: 0,
            developerMode: !1,
            mobileRedesignDisabled: !1,
            timestampHourCycle: 0,
            launchPadMode: 0,
            uiDensity: 0,
            swipeRightToLeftMode: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.theme = e.int32();
                    break;
                case 2:
                    o.developerMode = e.bool();
                    break;
                case 3:
                    o.clientThemeSettings = eD.internalBinaryRead(e, e.uint32(), n, o.clientThemeSettings);
                    break;
                case 4:
                    o.mobileRedesignDisabled = e.bool();
                    break;
                case 6:
                    o.channelListLayout = s.Gm.internalBinaryRead(e, e.uint32(), n, o.channelListLayout);
                    break;
                case 7:
                    o.messagePreviews = s.Gm.internalBinaryRead(e, e.uint32(), n, o.messagePreviews);
                    break;
                case 8:
                    o.searchResultExactCountEnabled = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.searchResultExactCountEnabled,
                    );
                    break;
                case 9:
                    o.timestampHourCycle = e.int32();
                    break;
                case 10:
                    o.happeningNowCardsDisabled = s.D5.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.happeningNowCardsDisabled,
                    );
                    break;
                case 11:
                    o.launchPadMode = e.int32();
                    break;
                case 12:
                    o.uiDensity = e.int32();
                    break;
                case 13:
                    o.swipeRightToLeftMode = e.int32();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.theme && t.tag(1, r.TD.Varint).int32(e.theme),
            !1 !== e.developerMode && t.tag(2, r.TD.Varint).bool(e.developerMode),
            e.clientThemeSettings &&
                eD.internalBinaryWrite(e.clientThemeSettings, t.tag(3, r.TD.LengthDelimited).fork(), n).join(),
            !1 !== e.mobileRedesignDisabled && t.tag(4, r.TD.Varint).bool(e.mobileRedesignDisabled),
            e.channelListLayout &&
                s.Gm.internalBinaryWrite(e.channelListLayout, t.tag(6, r.TD.LengthDelimited).fork(), n).join(),
            e.messagePreviews &&
                s.Gm.internalBinaryWrite(e.messagePreviews, t.tag(7, r.TD.LengthDelimited).fork(), n).join(),
            e.searchResultExactCountEnabled &&
                s.D5.internalBinaryWrite(
                    e.searchResultExactCountEnabled,
                    t.tag(8, r.TD.LengthDelimited).fork(),
                    n,
                ).join(),
            0 !== e.timestampHourCycle && t.tag(9, r.TD.Varint).int32(e.timestampHourCycle),
            e.happeningNowCardsDisabled &&
                s.D5.internalBinaryWrite(e.happeningNowCardsDisabled, t.tag(10, r.TD.LengthDelimited).fork(), n).join(),
            0 !== e.launchPadMode && t.tag(11, r.TD.Varint).int32(e.launchPadMode),
            0 !== e.uiDensity && t.tag(12, r.TD.Varint).int32(e.uiDensity),
            0 !== e.swipeRightToLeftMode && t.tag(13, r.TD.Varint).int32(e.swipeRightToLeftMode);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.AppearanceSettings", [
            {
                no: 1,
                name: "theme",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.Theme", v],
            },
            {
                no: 2,
                name: "developer_mode",
                kind: "scalar",
                T: 8,
            },
            {
                no: 3,
                name: "client_theme_settings",
                kind: "message",
                T: () => eD,
            },
            {
                no: 4,
                name: "mobile_redesign_disabled",
                kind: "scalar",
                T: 8,
            },
            {
                no: 6,
                name: "channel_list_layout",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 7,
                name: "message_previews",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 8,
                name: "search_result_exact_count_enabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 9,
                name: "timestamp_hour_cycle",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.TimestampHourCycle", T],
            },
            {
                no: 10,
                name: "happening_now_cards_disabled",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 11,
                name: "launch_pad_mode",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.LaunchPadMode", S],
            },
            {
                no: 12,
                name: "ui_density",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.UIDensity", O],
            },
            {
                no: 13,
                name: "swipe_right_to_left_mode",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.SwipeRightToLeftMode", A],
            },
        ]);
    }
}
let eC = new eN();
class eR extends a.C {
    create(e) {
        let t = {
            colors: [],
            gradientColorStops: [],
            gradientAngle: 0,
            baseMix: 0,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.colors.push(e.string());
                    break;
                case 2:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; ) o.gradientColorStops.push(e.float());
                    else o.gradientColorStops.push(e.float());
                    break;
                case 3:
                    o.gradientAngle = e.int32();
                    break;
                case 4:
                    o.baseMix = e.int32();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        for (let n = 0; n < e.colors.length; n++) t.tag(1, r.TD.LengthDelimited).string(e.colors[n]);
        if (e.gradientColorStops.length) {
            t.tag(2, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.gradientColorStops.length; n++) t.float(e.gradientColorStops[n]);
            t.join();
        }
        0 !== e.gradientAngle && t.tag(3, r.TD.Varint).int32(e.gradientAngle),
            0 !== e.baseMix && t.tag(4, r.TD.Varint).int32(e.baseMix);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.CustomUserThemeSettings", [
            {
                no: 1,
                name: "colors",
                kind: "scalar",
                repeat: 2,
                T: 9,
            },
            {
                no: 2,
                name: "gradient_color_stops",
                kind: "scalar",
                repeat: 1,
                T: 2,
            },
            {
                no: 3,
                name: "gradient_angle",
                kind: "scalar",
                T: 5,
            },
            {
                no: 4,
                name: "base_mix",
                kind: "scalar",
                T: 5,
            },
        ]);
    }
}
let eP = new eR();
class ew extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 2:
                    o.backgroundGradientPresetId = s.yC.internalBinaryRead(
                        e,
                        e.uint32(),
                        n,
                        o.backgroundGradientPresetId,
                    );
                    break;
                case 4:
                    o.customUserThemeSettings = eP.internalBinaryRead(e, e.uint32(), n, o.customUserThemeSettings);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.backgroundGradientPresetId &&
            s.yC.internalBinaryWrite(e.backgroundGradientPresetId, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.customUserThemeSettings &&
                eP.internalBinaryWrite(e.customUserThemeSettings, t.tag(4, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ClientThemeSettings", [
            {
                no: 2,
                name: "background_gradient_preset_id",
                kind: "message",
                T: () => s.yC,
            },
            {
                no: 4,
                name: "custom_user_theme_settings",
                kind: "message",
                T: () => eP,
            },
        ]);
    }
}
let eD = new ew();
class eL extends a.C {
    create(e) {
        let t = {
            folders: [],
            guildPositions: [],
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.folders.push(ek.internalBinaryRead(e, e.uint32(), n));
                    break;
                case 2:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; ) o.guildPositions.push(e.fixed64().toString());
                    else o.guildPositions.push(e.fixed64().toString());
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        for (let i = 0; i < e.folders.length; i++)
            ek.internalBinaryWrite(e.folders[i], t.tag(1, r.TD.LengthDelimited).fork(), n).join();
        if (e.guildPositions.length) {
            t.tag(2, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.guildPositions.length; n++) t.fixed64(e.guildPositions[n]);
            t.join();
        }
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.GuildFolders", [
            {
                no: 1,
                name: "folders",
                kind: "message",
                repeat: 1,
                T: () => ek,
            },
            {
                no: 2,
                name: "guild_positions",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
        ]);
    }
}
let ex = new eL();
class eM extends a.C {
    create(e) {
        let t = { guildIds: [] };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; ) o.guildIds.push(e.fixed64().toString());
                    else o.guildIds.push(e.fixed64().toString());
                    break;
                case 2:
                    o.id = s.r1.internalBinaryRead(e, e.uint32(), n, o.id);
                    break;
                case 3:
                    o.name = s.Gm.internalBinaryRead(e, e.uint32(), n, o.name);
                    break;
                case 4:
                    o.color = s.wA.internalBinaryRead(e, e.uint32(), n, o.color);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        if (e.guildIds.length) {
            t.tag(1, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.guildIds.length; n++) t.fixed64(e.guildIds[n]);
            t.join();
        }
        e.id && s.r1.internalBinaryWrite(e.id, t.tag(2, r.TD.LengthDelimited).fork(), n).join(),
            e.name && s.Gm.internalBinaryWrite(e.name, t.tag(3, r.TD.LengthDelimited).fork(), n).join(),
            e.color && s.wA.internalBinaryWrite(e.color, t.tag(4, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.GuildFolder", [
            {
                no: 1,
                name: "guild_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 2,
                name: "id",
                kind: "message",
                T: () => s.r1,
            },
            {
                no: 3,
                name: "name",
                kind: "message",
                T: () => s.Gm,
            },
            {
                no: 4,
                name: "color",
                kind: "message",
                T: () => s.wA,
            },
        ]);
    }
}
let ek = new eM();
class ej extends a.C {
    create(e) {
        let t = {
            favoriteChannels: {},
            muted: !1,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    this.binaryReadMap1(o.favoriteChannels, e, n);
                    break;
                case 2:
                    o.muted = e.bool();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    binaryReadMap1(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.fixed64().toString();
                    break;
                case 2:
                    a = eB.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.Favorites.favorite_channels",
                    );
            }
        }
        e[null != o ? o : "0"] = null != a ? a : eB.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of Object.keys(e.favoriteChannels))
            t.tag(1, r.TD.LengthDelimited).fork().tag(1, r.TD.Bit64).fixed64(i),
                t.tag(2, r.TD.LengthDelimited).fork(),
                eB.internalBinaryWrite(e.favoriteChannels[i], t, n),
                t.join().join();
        !1 !== e.muted && t.tag(2, r.TD.Varint).bool(e.muted);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.Favorites", [
            {
                no: 1,
                name: "favorite_channels",
                kind: "map",
                K: 6,
                V: {
                    kind: "message",
                    T: () => eB,
                },
            },
            {
                no: 2,
                name: "muted",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let eU = new ej();
class eG extends a.C {
    create(e) {
        let t = {
            nickname: "",
            type: 0,
            position: 0,
            parentId: "0",
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.nickname = e.string();
                    break;
                case 2:
                    o.type = e.int32();
                    break;
                case 3:
                    o.position = e.uint32();
                    break;
                case 4:
                    o.parentId = e.fixed64().toString();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        "" !== e.nickname && t.tag(1, r.TD.LengthDelimited).string(e.nickname),
            0 !== e.type && t.tag(2, r.TD.Varint).int32(e.type),
            0 !== e.position && t.tag(3, r.TD.Varint).uint32(e.position),
            "0" !== e.parentId && t.tag(4, r.TD.Bit64).fixed64(e.parentId);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.FavoriteChannel", [
            {
                no: 1,
                name: "nickname",
                kind: "scalar",
                T: 9,
            },
            {
                no: 2,
                name: "type",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.FavoriteChannelType", N],
            },
            {
                no: 3,
                name: "position",
                kind: "scalar",
                T: 13,
            },
            {
                no: 4,
                name: "parent_id",
                kind: "scalar",
                T: 6,
            },
        ]);
    }
}
let eB = new eG();
class eZ extends a.C {
    create(e) {
        let t = {
            muted: !1,
            volume: 0,
            modifiedAt: "0",
            soundboardMuted: !1,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.muted = e.bool();
                    break;
                case 2:
                    o.volume = e.float();
                    break;
                case 3:
                    o.modifiedAt = e.fixed64().toString();
                    break;
                case 4:
                    o.soundboardMuted = e.bool();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.muted && t.tag(1, r.TD.Varint).bool(e.muted),
            0 !== e.volume && t.tag(2, r.TD.Bit32).float(e.volume),
            "0" !== e.modifiedAt && t.tag(3, r.TD.Bit64).fixed64(e.modifiedAt),
            !1 !== e.soundboardMuted && t.tag(4, r.TD.Varint).bool(e.soundboardMuted);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.AudioContextSetting", [
            {
                no: 1,
                name: "muted",
                kind: "scalar",
                T: 8,
            },
            {
                no: 2,
                name: "volume",
                kind: "scalar",
                T: 2,
            },
            {
                no: 3,
                name: "modified_at",
                kind: "scalar",
                T: 6,
            },
            {
                no: 4,
                name: "soundboard_muted",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let eF = new eZ();
class eV extends a.C {
    create(e) {
        let t = {
            user: {},
            stream: {},
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    this.binaryReadMap1(o.user, e, n);
                    break;
                case 2:
                    this.binaryReadMap2(o.stream, e, n);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    binaryReadMap1(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.fixed64().toString();
                    break;
                case 2:
                    a = eF.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.AudioSettings.user",
                    );
            }
        }
        e[null != o ? o : "0"] = null != a ? a : eF.create();
    }
    binaryReadMap2(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.fixed64().toString();
                    break;
                case 2:
                    a = eF.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.AudioSettings.stream",
                    );
            }
        }
        e[null != o ? o : "0"] = null != a ? a : eF.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of Object.keys(e.user))
            t.tag(1, r.TD.LengthDelimited).fork().tag(1, r.TD.Bit64).fixed64(i),
                t.tag(2, r.TD.LengthDelimited).fork(),
                eF.internalBinaryWrite(e.user[i], t, n),
                t.join().join();
        for (let i of Object.keys(e.stream))
            t.tag(2, r.TD.LengthDelimited).fork().tag(1, r.TD.Bit64).fixed64(i),
                t.tag(2, r.TD.LengthDelimited).fork(),
                eF.internalBinaryWrite(e.stream[i], t, n),
                t.join().join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.AudioSettings", [
            {
                no: 1,
                name: "user",
                kind: "map",
                K: 6,
                V: {
                    kind: "message",
                    T: () => eF,
                },
            },
            {
                no: 2,
                name: "stream",
                kind: "map",
                K: 6,
                V: {
                    kind: "message",
                    T: () => eF,
                },
            },
        ]);
    }
}
let eH = new eV();
class eY extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.disableHomeAutoNav = s.D5.internalBinaryRead(e, e.uint32(), n, o.disableHomeAutoNav);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.disableHomeAutoNav &&
            s.D5.internalBinaryWrite(e.disableHomeAutoNav, t.tag(1, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.CommunitiesSettings", [
            {
                no: 1,
                name: "disable_home_auto_nav",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let eW = new eY();
class eK extends a.C {
    create(e) {
        let t = { volume: 0 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.volume = e.float();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.volume && t.tag(1, r.TD.Bit32).float(e.volume);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.SoundboardSettings", [
            {
                no: 1,
                name: "volume",
                kind: "scalar",
                T: 2,
            },
        ]);
    }
}
let ez = new eK();
class eq extends a.C {
    create(e) {
        let t = {
            soundId: "0",
            guildId: "0",
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.soundId = e.fixed64().toString();
                    break;
                case 2:
                    o.guildId = e.fixed64().toString();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        "0" !== e.soundId && t.tag(1, r.TD.Bit64).fixed64(e.soundId),
            "0" !== e.guildId && t.tag(2, r.TD.Bit64).fixed64(e.guildId);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.CustomCallSound", [
            {
                no: 1,
                name: "sound_id",
                kind: "scalar",
                T: 6,
            },
            {
                no: 2,
                name: "guild_id",
                kind: "scalar",
                T: 6,
            },
        ]);
    }
}
let eX = new eq();
class eQ extends a.C {
    create(e) {
        let t = {
            allowedGuildIds: [],
            allowedUserIds: [],
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.allowFriends = s.D5.internalBinaryRead(e, e.uint32(), n, o.allowFriends);
                    break;
                case 2:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; ) o.allowedGuildIds.push(e.fixed64().toString());
                    else o.allowedGuildIds.push(e.fixed64().toString());
                    break;
                case 3:
                    if (i === r.TD.LengthDelimited)
                        for (let t = e.int32() + e.pos; e.pos < t; ) o.allowedUserIds.push(e.fixed64().toString());
                    else o.allowedUserIds.push(e.fixed64().toString());
                    break;
                case 4:
                    o.autoBroadcast = s.D5.internalBinaryRead(e, e.uint32(), n, o.autoBroadcast);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        if (
            (e.allowFriends &&
                s.D5.internalBinaryWrite(e.allowFriends, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.allowedGuildIds.length)
        ) {
            t.tag(2, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.allowedGuildIds.length; n++) t.fixed64(e.allowedGuildIds[n]);
            t.join();
        }
        if (e.allowedUserIds.length) {
            t.tag(3, r.TD.LengthDelimited).fork();
            for (let n = 0; n < e.allowedUserIds.length; n++) t.fixed64(e.allowedUserIds[n]);
            t.join();
        }
        e.autoBroadcast && s.D5.internalBinaryWrite(e.autoBroadcast, t.tag(4, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.BroadcastSettings", [
            {
                no: 1,
                name: "allow_friends",
                kind: "message",
                T: () => s.D5,
            },
            {
                no: 2,
                name: "allowed_guild_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 3,
                name: "allowed_user_ids",
                kind: "scalar",
                repeat: 1,
                T: 6,
            },
            {
                no: 4,
                name: "auto_broadcast",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let eJ = new eQ();
class e$ extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.allowVoiceRecording = s.D5.internalBinaryRead(e, e.uint32(), n, o.allowVoiceRecording);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.allowVoiceRecording &&
            s.D5.internalBinaryWrite(e.allowVoiceRecording, t.tag(1, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ClipsSettings", [
            {
                no: 1,
                name: "allow_voice_recording",
                kind: "message",
                T: () => s.D5,
            },
        ]);
    }
}
let e0 = new e$();
class e1 extends a.C {
    create(e) {
        let t = {
            safetySettingsPreset: 0,
            ignoreProfileSpeedbumpDisabled: !1,
        };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.safetySettingsPreset = e.int32();
                    break;
                case 2:
                    o.ignoreProfileSpeedbumpDisabled = e.bool();
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.safetySettingsPreset && t.tag(1, r.TD.Varint).int32(e.safetySettingsPreset),
            !1 !== e.ignoreProfileSpeedbumpDisabled && t.tag(2, r.TD.Varint).bool(e.ignoreProfileSpeedbumpDisabled);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.SafetySettings", [
            {
                no: 1,
                name: "safety_settings_preset",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.SafetySettingsPresetType", C],
            },
            {
                no: 2,
                name: "ignore_profile_speedbump_disabled",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let e2 = new e1();
class e3 extends a.C {
    create(e) {
        let t = { currentTab: 0 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.currentTab = e.int32();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        0 !== e.currentTab && t.tag(1, r.TD.Varint).int32(e.currentTab);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ForLaterSettings", [
            {
                no: 1,
                name: "current_tab",
                kind: "enum",
                T: () => ["discord_protos.discord_users.v1.ForLaterTab", R, "FOR_LATER_TAB_"],
            },
        ]);
    }
}
let e5 = new e3();
class e4 extends a.C {
    create(e) {
        let t = { feedGeneratedAt: "0" };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.feedGeneratedAt = e.fixed64().toString();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        "0" !== e.feedGeneratedAt && t.tag(1, r.TD.Bit64).fixed64(e.feedGeneratedAt);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ICYMISettings", [
            {
                no: 1,
                name: "feed_generated_at",
                kind: "scalar",
                T: 6,
            },
        ]);
    }
}
let e6 = new e4();
class e8 extends a.C {
    create(e) {
        let t = { appSettings: {} };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) this.binaryReadMap1(o.appSettings, e, n);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    binaryReadMap1(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.fixed64().toString();
                    break;
                case 2:
                    a = te.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.AllApplicationSettings.app_settings",
                    );
            }
        }
        e[null != o ? o : "0"] = null != a ? a : te.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of Object.keys(e.appSettings))
            t.tag(1, r.TD.LengthDelimited).fork().tag(1, r.TD.Bit64).fixed64(i),
                t.tag(2, r.TD.LengthDelimited).fork(),
                te.internalBinaryWrite(e.appSettings[i], t, n),
                t.join().join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.AllApplicationSettings", [
            {
                no: 1,
                name: "app_settings",
                kind: "map",
                K: 6,
                V: {
                    kind: "message",
                    T: () => te,
                },
            },
        ]);
    }
}
let e7 = new e8();
class e9 extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.appDmSettings = tn.internalBinaryRead(e, e.uint32(), n, o.appDmSettings);
                    break;
                case 2:
                    o.appSharingSettings = ti.internalBinaryRead(e, e.uint32(), n, o.appSharingSettings);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let s = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.appDmSettings && tn.internalBinaryWrite(e.appDmSettings, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.appSharingSettings &&
                ti.internalBinaryWrite(e.appSharingSettings, t.tag(2, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ApplicationSettings", [
            {
                no: 1,
                name: "app_dm_settings",
                kind: "message",
                T: () => tn,
            },
            {
                no: 2,
                name: "app_sharing_settings",
                kind: "message",
                T: () => ti,
            },
        ]);
    }
}
let te = new e9();
class tt extends a.C {
    create(e) {
        let t = { allowMobilePush: !1 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (2 === t) o.allowMobilePush = e.bool();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.allowMobilePush && t.tag(2, r.TD.Varint).bool(e.allowMobilePush);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ApplicationDMSettings", [
            {
                no: 2,
                name: "allow_mobile_push",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let tn = new tt();
class tr extends a.C {
    create(e) {
        let t = { disableApplicationActivitySharing: !1 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.disableApplicationActivitySharing = e.bool();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.disableApplicationActivitySharing && t.tag(1, r.TD.Varint).bool(e.disableApplicationActivitySharing);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.ApplicationSharingSettings", [
            {
                no: 1,
                name: "disable_application_activity_sharing",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let ti = new tr();
class to extends a.C {
    create(e) {
        let t = { alwaysDeliver: !1 };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) o.alwaysDeliver = e.bool();
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        !1 !== e.alwaysDeliver && t.tag(1, r.TD.Varint).bool(e.alwaysDeliver);
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.AdsSettings", [
            {
                no: 1,
                name: "always_deliver",
                kind: "scalar",
                T: 8,
            },
        ]);
    }
}
let ta = new to();
class ts extends a.C {
    create(e) {
        let t = {};
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            switch (t) {
                case 1:
                    o.lastImpressionTime = s.wA.internalBinaryRead(e, e.uint32(), n, o.lastImpressionTime);
                    break;
                case 2:
                    o.optOutExpiryTime = s.wA.internalBinaryRead(e, e.uint32(), n, o.optOutExpiryTime);
                    break;
                default:
                    let a = n.readUnknownField;
                    if ("throw" === a)
                        throw new globalThis.Error(
                            "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                        );
                    let l = e.skip(i);
                    !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, l);
            }
        }
        return o;
    }
    internalBinaryWrite(e, t, n) {
        e.lastImpressionTime &&
            s.wA.internalBinaryWrite(e.lastImpressionTime, t.tag(1, r.TD.LengthDelimited).fork(), n).join(),
            e.optOutExpiryTime &&
                s.wA.internalBinaryWrite(e.optOutExpiryTime, t.tag(2, r.TD.LengthDelimited).fork(), n).join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.InAppFeedbackState", [
            {
                no: 1,
                name: "last_impression_time",
                kind: "message",
                T: () => s.wA,
            },
            {
                no: 2,
                name: "opt_out_expiry_time",
                kind: "message",
                T: () => s.wA,
            },
        ]);
    }
}
let tl = new ts();
class tc extends a.C {
    create(e) {
        let t = { inAppFeedbackStates: {} };
        return (
            globalThis.Object.defineProperty(t, o.C, {
                enumerable: !1,
                value: this,
            }),
            void 0 !== e && (0, i.l)(this, t, e),
            t
        );
    }
    internalBinaryRead(e, t, n, i) {
        let o = null != i ? i : this.create(),
            a = e.pos + t;
        for (; e.pos < a; ) {
            let [t, i] = e.tag();
            if (1 === t) this.binaryReadMap1(o.inAppFeedbackStates, e, n);
            else {
                let a = n.readUnknownField;
                if ("throw" === a)
                    throw new globalThis.Error(
                        "Unknown field ".concat(t, " (wire type ").concat(i, ") for ").concat(this.typeName),
                    );
                let s = e.skip(i);
                !1 !== a && (!0 === a ? r.z.onRead : a)(this.typeName, o, t, i, s);
            }
        }
        return o;
    }
    binaryReadMap1(e, t, n) {
        let r = t.uint32(),
            i = t.pos + r,
            o,
            a;
        for (; t.pos < i; ) {
            let [e, r] = t.tag();
            switch (e) {
                case 1:
                    o = t.int32();
                    break;
                case 2:
                    a = tl.internalBinaryRead(t, t.uint32(), n);
                    break;
                default:
                    throw new globalThis.Error(
                        "unknown map entry field for field discord_protos.discord_users.v1.InAppFeedbackSettings.in_app_feedback_states",
                    );
            }
        }
        e[null != o ? o : 0] = null != a ? a : tl.create();
    }
    internalBinaryWrite(e, t, n) {
        for (let i of Object.keys(e.inAppFeedbackStates))
            t.tag(1, r.TD.LengthDelimited).fork().tag(1, r.TD.Varint).int32(parseInt(i)),
                t.tag(2, r.TD.LengthDelimited).fork(),
                tl.internalBinaryWrite(e.inAppFeedbackStates[i], t, n),
                t.join().join();
        let i = n.writeUnknownFields;
        return !1 !== i && (!0 == i ? r.z.onWrite : i)(this.typeName, e, t), t;
    }
    constructor() {
        super("discord_protos.discord_users.v1.InAppFeedbackSettings", [
            {
                no: 1,
                name: "in_app_feedback_states",
                kind: "map",
                K: 5,
                V: {
                    kind: "message",
                    T: () => tl,
                },
            },
        ]);
    }
}
let tu = new tc();
