n.d(t, {
    $H: () => eL,
    $J: () => e5,
    AV: () => e_,
    B2: () => ty,
    B3: () => tf,
    BM: () => e7,
    Bg: () => eb,
    Bz: () => e8,
    C1: () => ts,
    C9: () => to,
    Dr: () => ew,
    F9: () => tE,
    FE: () => ej,
    FI: () => th,
    FX: () => eg,
    Fs: () => tl,
    GN: () => ep,
    Gd: () => tp,
    Jg: () => eN,
    K: () => eI,
    KM: () => ex,
    Kr: () => eE,
    LM: () => eS,
    MM: () => tS,
    Mo: () => tT,
    Nj: () => eR,
    OG: () => tr,
    PM: () => ed,
    Qe: () => V,
    Rs: () => eu,
    U3: () => ee,
    V$: () => tn,
    VB: () => tg,
    Vl: () => eX,
    WP: () => et,
    Xh: () => el,
    Xv: () => eO,
    ZZ: () => W,
    Zp: () => tm,
    _D: () => Y,
    _j: () => ta,
    _p: () => td,
    b7: () => eq,
    bA: () => tb,
    f$: () => tu,
    f2: () => eU,
    fY: () => te,
    gI: () => tt,
    hF: () => tv,
    hQ: () => ei,
    iQ: () => Q,
    il: () => ez,
    lQ: () => z,
    o9: () => eM,
    oo: () => eC,
    pO: () => eD,
    q6: () => X,
    q8: () => e3,
    qe: () => tO,
    t2: () => ef,
    u7: () => t_,
    vQ: () => ev,
    vR: () => eT,
    xN: () => ey,
    xn: () => em,
    yH: () => tc,
    yI: () => ea,
    ys: () => eA,
    zE: () => eh,
    zK: () => ek,
    zi: () => q
}),
    n(388685),
    n(314940),
    n(953529),
    n(415506),
    n(642613),
    n(784620),
    n(973216),
    n(539854);
var r = n(991998),
    i = n(392711),
    a = n(278074),
    o = n(754700),
    s = n(551910),
    l = n(34738),
    c = n(887003),
    u = n(742635),
    d = n(570140),
    f = n(230711),
    _ = n(782568);
n(597688);
var p = n(231757),
    h = n(706454),
    m = n(866960),
    g = n(572004),
    E = n(585483),
    b = n(70956),
    y = n(630388),
    O = n(823379),
    v = n(49012),
    I = n(960048),
    S = n(617136),
    T = n(272008),
    A = n(569984),
    N = n(497505),
    C = n(184299);
n(306560);
var P = n(75137),
    R = n(566078),
    w = n(312046),
    D = n(46140),
    L = n(981631),
    x = n(701488),
    k = n(388032);
function M(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0
              })
            : (e[t] = n),
        e
    );
}
function j(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            r = Object.keys(n);
        'function' == typeof Object.getOwnPropertySymbols &&
            (r = r.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                })
            )),
            r.forEach(function (t) {
                M(e, t, n[t]);
            });
    }
    return e;
}
function U(e, t) {
    var n = Object.keys(e);
    if (Object.getOwnPropertySymbols) {
        var r = Object.getOwnPropertySymbols(e);
        t &&
            (r = r.filter(function (t) {
                return Object.getOwnPropertyDescriptor(e, t).enumerable;
            })),
            n.push.apply(n, r);
    }
    return n;
}
function G(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : U(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
let B = 2592000000,
    F = new Set([l.n.PREMIUM_EXTENSION, l.n.PREMIUM_PERMANENT]);
function V(e) {
    try {
        return (0, a.EQ)(e.config)
            .with({ config_version: 2 }, () => !0)
            .exhaustive();
    } catch (n) {
        var t;
        return console.error("Unknown config version '".concat(null == e || null == (t = e.config) ? void 0 : t.config_version, "'"), n), !1;
    }
}
let Z = (e) => e.application_id === x.Ev || e.platform === L.M7m.XBOX,
    H = (e) => e.platform === L.M7m.PS4 || e.platform === L.M7m.PS5;
function Y(e, t) {
    if (null == e) return !1;
    let n = e.name.toLowerCase(),
        r = R.r.build(t.config).application.name.toLowerCase();
    return Z(e) || H(e) ? n === r : null != e.application_id && K(e.application_id, t);
}
function W(e, t) {
    for (let [n, r] of e) if (Y(t, r) && !q(r)) return r;
}
function K(e, t) {
    return null != R.r.build(t.config).application.ids.find((t) => t === e);
}
function z(e, t) {
    let n;
    for (let [r, i] of e)
        if (K(t, i) && !q(i)) {
            n = i;
            break;
        }
    return n;
}
function q(e) {
    return new Date(e.config.expiresAt).valueOf() <= Date.now();
}
function Q(e) {
    if (!q(e)) return !1;
    let t = Date.now() - B,
        n = new Date(e.config.expiresAt).valueOf();
    return null != e.config.expiresAt && !(n <= t);
}
function X(e) {
    return (0, a.EQ)(e)
        .with({ config_version: 2 }, (e) => (0, w.Q)(e))
        .exhaustive();
}
function J(e) {
    return null == e
        ? null
        : {
              lastBeatAt: e.last_beat_at,
              expiresAt: e.expires_at
          };
}
function $(e) {
    let t = {};
    for (let [n, r] of Object.entries(e))
        t[n] = {
            eventName: r.event_name,
            value: r.value,
            updatedAt: r.updated_at,
            completedAt: r.completed_at,
            heartbeat: J(r.heartbeat)
        };
    return t;
}
function ee(e) {
    var t;
    return {
        userId: e.user_id,
        questId: e.quest_id,
        enrolledAt: e.enrolled_at,
        completedAt: e.completed_at,
        claimedAt: e.claimed_at,
        claimedTier: null != (t = e.claimed_tier) ? t : null,
        lastStreamHeartbeatAt: e.last_stream_heartbeat_at,
        streamProgressSeconds: e.stream_progress_seconds,
        dismissedQuestContent: e.dismissed_quest_content,
        progress: $(e.progress)
    };
}
function et(e) {
    return {
        id: e.id,
        preview: e.preview,
        config: X(e.config),
        userStatus: null == e.user_status ? null : ee(e.user_status),
        targetedContent: e.targeted_content
    };
}
function en(e) {
    return e.type === c.w.VIRTUAL_CURRENCY
        ? {
              skuId: e.sku_id,
              type: e.type,
              name: e.name,
              nameWithArticle: e.name_with_article,
              collectibleProduct: e.collectible_product,
              orbQuantity: e.orb_quantity
          }
        : {
              skuId: e.sku_id,
              type: e.type,
              name: e.name,
              nameWithArticle: e.name_with_article,
              asset: e.asset,
              assetVideo: e.asset_video,
              collectibleProduct: e.collectible_product
          };
}
function er(e) {
    return {
        id: e.id,
        startsAt: e.starts_at,
        expiresAt: e.expires_at,
        features: e.features,
        messages: {
            questName: e.messages.quest_name,
            gamePublisher: e.messages.game_publisher,
            gameTitle: e.messages.game_title
        },
        assets: {
            hero: e.assets.hero,
            heroVideo: e.assets.hero_video,
            questBarHero: e.assets.quest_bar_hero,
            questBarHeroVideo: e.assets.quest_bar_hero_video,
            gameTile: e.assets.game_tile,
            logotype: e.assets.logotype
        },
        colors: {
            primary: e.colors.primary,
            secondary: e.colors.secondary
        },
        rewards: e.rewards.map(en),
        cosponsorMetadata: (0, w.s)(e.cosponsor_metadata)
    };
}
function ei(e) {
    return {
        id: e.id,
        config: er(e.config),
        userStatus: null == e.user_status ? null : ee(e.user_status)
    };
}
function ea(e) {
    var t;
    return {
        userId: e.user_id,
        questId: e.quest_id,
        code: e.code,
        platform: e.platform,
        claimedAt: e.claimed_at,
        tier: null != (t = e.tier) ? t : null
    };
}
function eo(e) {
    if ((null == e ? void 0 : e.quest_rewards) == null) return null;
    let t = e.quest_rewards;
    switch (t.reward.tag) {
        case c.w.IN_GAME:
            return { questRewards: { reward: { tag: t.reward.tag } } };
        case c.w.REWARD_CODE:
            return {
                questRewards: {
                    reward: {
                        tag: t.reward.tag,
                        rewardCode: ea(t.reward.reward_code)
                    }
                }
            };
    }
}
function es(e) {
    return {
        skuId: e.sku_id,
        tenantMetadata: eo(e.tenant_metadata),
        consumed: e.consumed
    };
}
function el(e) {
    return {
        claimedAt: e.claimed_at,
        items: e.entitlements.map(es),
        errors: e.errors
    };
}
function ec(e) {
    return {
        eventName: e.event_name,
        title: e.title,
        description: e.description,
        target: e.target
    };
}
let eu = (e) => ''.concat(location.protocol, '//').concat(location.host, '/quests/').concat(e);
function ed(e, t, n) {
    let r = t.get(n);
    if (null == r) return;
    let i = e.get(r.quest.id);
    if (null != i && !q(i)) return i;
}
let ef = (e) => {
    switch (e) {
        case N.y$.XBOX:
            return k.intl.string(k.t.G84UWV);
        case N.y$.PLAYSTATION:
            return k.intl.string(k.t['6IeKx8']);
        case N.y$.SWITCH:
            return k.intl.string(k.t['1pp0sr']);
        case N.y$.PC:
            return k.intl.string(k.t['YK+wUl']);
        case N.y$.CROSS_PLATFORM:
            return k.intl.string(k.t.UWVbzc);
    }
};
function e_(e) {
    var t, n;
    let { quest: r, taskDetails: i, thirdPartyTaskDetails: a } = e;
    if ((null == (t = r.userStatus) ? void 0 : t.completedAt) != null) return k.intl.string(k.t.BzFeTE);
    if ((null == (n = r.userStatus) ? void 0 : n.enrolledAt) != null) {
        let { percentComplete: e } = i,
            t = null != a ? a.percentComplete : e;
        return t >= 0.75 ? k.intl.string(k.t.gvCR4O) : t >= 0.45 && t <= 0.55 ? k.intl.string(k.t.JNx8sL) : t > 0 ? k.intl.string(k.t.JMbfnZ) : k.intl.string(k.t['7e5k7O']);
    }
    return k.intl.formatToPlainString(k.t.EQa7oq, { questName: r.config.messages.questName });
}
function ep(e) {
    return Object.keys(D.a_).includes(N.jn[e]);
}
function eh(e, t) {
    if (!ep(t)) return !1;
    let n = N.jn[t];
    return (0, y.yE)(e.dismissedQuestContent, D.a_[n]);
}
function em(e) {
    let t = e.rewardsConfig.rewards.find((e) => e.type === c.w.COLLECTIBLE);
    return (null == t ? void 0 : t.type) === c.w.COLLECTIBLE ? t : null;
}
function eg(e) {
    let t = em(e);
    return null != t && 'expiresAtPremium' in t && null != t.expiresAtPremium ? ti(t.expiresAtPremium) : null;
}
function eE(e) {
    let t = em(e);
    if (null == t || !('expiresAt' in t) || null == t.expiresAt) return null;
    let n = new Date(e.expiresAt),
        i = new Date(t.expiresAt),
        a = (0, r.Z)(i, n),
        o = Math.floor(a / 30) + +(a % 30 >= 25);
    if (o >= 12) {
        let e = Math.floor(o / 12);
        return k.intl.formatToPlainString(k.t.PClsr6, { years: e });
    }
    {
        if (o > 0) return k.intl.formatToPlainString(k.t.kridzM, { months: o });
        let e = (0, r.Z)(i, n);
        if (!(e >= 7)) return k.intl.formatToPlainString(k.t.k2UNz8, { days: e });
        {
            let t = Math.ceil(e / 7);
            return k.intl.formatToPlainString(k.t.EmoBDw, { weeks: t });
        }
    }
}
function eb(e) {
    return e.rewardsConfig.rewards.some((e) => e.type === c.w.FRACTIONAL_PREMIUM);
}
function ey(e) {
    return e.rewardsConfig.rewards.some((e) => e.type === c.w.VIRTUAL_CURRENCY);
}
function eO(e) {
    return null != em(e);
}
function ev(e) {
    return e.rewardsConfig.rewards.some((e) => e.type === c.w.IN_GAME);
}
function eI(e) {
    var t;
    return null != (t = e.rewardsConfig.rewards.find((e) => e.type === c.w.IN_GAME)) ? t : null;
}
function eS(e) {
    let t = e.rewardsConfig.rewards.find((e) => e.type === c.w.VIRTUAL_CURRENCY);
    return null == t ? void 0 : t.orbQuantity;
}
function eT(e, t) {
    return e.targetedContent.includes(t);
}
function eA(e, t) {
    I.Z.captureException(e, G(j({}, t), { tags: G(j({}, null == t ? void 0 : t.tags), { app_context: 'quests' }) }));
}
function eN(e, t) {
    if (null == t || null == e) return null;
    for (let n of t) {
        let t = W(e, n);
        if (null != t) return t;
    }
    return null;
}
function eC(e) {
    let {
            quest: { config: t }
        } = e,
        n = t.rewardsConfig;
    return n.assignmentMethod === s.j.TIERED && n.rewards.length > 0 && n.rewards.every((e) => e.type === c.w.REWARD_CODE);
}
function eP(e) {
    return e.taskConfig.type === u.L.FIRST_PARTY && null != e.taskConfig.tasks[o.X.PLAY_ON_DESKTOP];
}
function eR(e) {
    let { quest: t } = e;
    return eP(t.config);
}
function ew(e) {
    let { quest: t } = e;
    return t.config.taskConfig.type === u.L.FIRST_PARTY && null != t.config.taskConfig.tasks[o.X.STREAM_ON_DESKTOP];
}
function eD(e) {
    return e.config.taskConfig.type === u.L.FIRST_PARTY && null != e.config.taskConfig.tasks[o.X.PLAY_ACTIVITY];
}
function eL(e) {
    return null != e && eR({ quest: e });
}
function ex(e) {
    let t = R.r.build(e.config).application.id;
    return eD(e) && t === D.Ts;
}
function ek(e, t) {
    return R.r.build(e.config).features.has(t);
}
function eM(e) {
    let { quest: t, idx: n } = e;
    if (null == n) return null;
    let r = t.config.rewardsConfig.rewards[n];
    return r.type === c.w.REWARD_CODE ? r : null;
}
function ej(e, t) {
    let n = R.r.build(e.config).application.link;
    (0, v.q)({
        href: n,
        onConfirm: () => {
            (0, S._3)({
                questId: e.id,
                questContent: t.content,
                questContentCTA: t.ctaContent,
                questContentPosition: t.position,
                impressionId: t.impressionId
            }),
                E.S.dispatch(L.CkL.QUEST_GAME_LINK_OPENED),
                (0, _.Z)(n);
        }
    }),
        (0, S.dA)({
            questId: e.id,
            event: L.rMx.QUEST_THIRD_PARTY_LINK_CLICKED
        });
}
let eU = (e, t) => {
        (0, S._3)({
            questId: e,
            questContent: t.content,
            questContentCTA: t.ctaContent,
            questContentPosition: t.position,
            impressionId: t.impressionId
        }),
            (0, g.JG)(eu(e));
    },
    eG = (e, t) => (e > 0 ? (0, i.floor)(Math.min(t / e, 1), 4) : 0),
    eB = (e) => e8(e) || A.Z.isProgressingOnDesktop(e.id),
    eF = (e, t) => {
        var n, r, a, o;
        let s = null == (o = e.userStatus) || null == (a = o.progress) || null == (r = a[t.eventName]) || null == (n = r.heartbeat) ? void 0 : n.lastBeatAt;
        if (null == s || !eB(e)) return 0;
        let l = Date.now() - new Date(s).valueOf();
        return (0, i.floor)(l / b.Z.Millis.SECOND, 2);
    },
    eV = (e, t) => {
        var n, r, i, a, o;
        let s = null == (r = e.userStatus) || null == (n = r.progress) ? void 0 : n[t.eventName],
            l = null != (o = null != (a = null == s ? void 0 : s.value) ? a : null == (i = e.userStatus) ? void 0 : i.streamProgressSeconds) ? o : 0;
        if (e3(e)) {
            let n = A.Z.getOptimisticProgress(e.id, t.eventName);
            return null == n || n < l ? l : n;
        }
        return l + eF(e, t);
    },
    eZ = 0.99,
    eH = (e, t) => {
        var n;
        let r = t.target;
        if ((null == (n = e.userStatus) ? void 0 : n.completedAt) != null) return r;
        let a = Math.min(r * eZ, eV(e, t));
        return Math.max((0, i.floor)(a, 2), 0);
    },
    eY = (e) => {
        var t, n;
        let { quest: r, taskType: i, includeTaskTypes: a = o.T.ALL } = e,
            s = r.config.taskConfig;
        if (s.type !== u.L.FIRST_PARTY) throw Error('Cannot retrieve task details for task config with type '.concat(s.type, '!'));
        let l = null != i ? i : null == (t = Object.values(s.tasks).filter((e) => a.has(e.eventName))[0]) ? void 0 : t.eventName,
            c = null != (n = s.tasks[l]) ? n : s.tasks[o.X.STREAM_ON_DESKTOP];
        if (null == c) throw Error('No task with type '.concat(i, ' found for quest ').concat(r.id, '!'));
        let d = c.target,
            f = eH(r, c);
        return {
            progressSeconds: f,
            targetSeconds: d,
            targetMinutes: Math.ceil(d / b.Z.Seconds.MINUTE),
            percentComplete: eG(d, f),
            taskType: l
        };
    },
    eW = (e) => (o.T.ALL.has(e) ? e : null),
    eK = (e) => {
        var t, n;
        let { quest: r, includeTaskTypes: i = o.T.ALL } = e;
        for (let e of Object.values(null != (n = null == (t = r.userStatus) ? void 0 : t.progress) ? n : {})
            .sort((e, t) => {
                var n, r;
                let i = null == e || null == (n = e.heartbeat) ? void 0 : n.lastBeatAt,
                    a = null == t || null == (r = t.heartbeat) ? void 0 : r.lastBeatAt;
                return null != i && null != a ? (new Date(i).valueOf() > new Date(a).valueOf() ? -1 : 1) : null == i && null == a && (null == e ? void 0 : e.updatedAt) != null && (null == t ? void 0 : t.updatedAt) != null ? (new Date(e.updatedAt).valueOf() > new Date(t.updatedAt).valueOf() ? -1 : 1) : null != i && null == a ? -1 : 1;
            })
            .filter(O.lm)) {
            let t = eW(e.eventName);
            if (null != t && (null == i ? void 0 : i.has(t)))
                return eY({
                    quest: r,
                    taskType: t,
                    includeTaskTypes: i
                });
        }
        return eY({
            quest: r,
            includeTaskTypes: i
        });
    },
    ez = (e, t) => {
        if (e.config.taskConfig.type !== u.L.FIRST_PARTY)
            return {
                progressSeconds: 0,
                targetSeconds: 1,
                targetMinutes: 1,
                percentComplete: 0,
                taskType: o.X.STREAM_ON_DESKTOP
            };
        if (e2(e))
            return eK({
                quest: e,
                includeTaskTypes: null != t ? t : e8(e) ? o.T.CONSOLE : o.T.ALL
            });
        if (e3(e))
            if (e4(e))
                return eY({
                    quest: e,
                    taskType: o.X.WATCH_VIDEO_ON_MOBILE
                });
            else
                return eY({
                    quest: e,
                    taskType: o.X.WATCH_VIDEO
                });
        return eL(e)
            ? eY({
                  quest: e,
                  taskType: o.X.PLAY_ON_DESKTOP
              })
            : eD(e)
              ? eY({
                    quest: e,
                    taskType: o.X.PLAY_ACTIVITY
                })
              : eY({
                    quest: e,
                    taskType: o.X.STREAM_ON_DESKTOP
                });
    };
function eq(e) {
    var t, n, r, i;
    let a = R.r.build(e.config).defaultInGameTask;
    if (null == a) return null;
    let o = null != (i = null == (r = e.userStatus) || null == (n = r.progress) || null == (t = n[a.eventName]) ? void 0 : t.value) ? i : 0,
        s = eG(a.target, o);
    return {
        title: a.title,
        description: a.description,
        target: a.target,
        progress: o,
        percentComplete: s
    };
}
let eQ = (e) => (t) => e.some((e) => null != t.config.taskConfig.tasks[e]);
function eX(e) {
    return e.config.taskConfig.type === u.L.THIRD_PARTY;
}
function eJ(e) {
    return e$(e.targetSeconds - e.progressSeconds);
}
function e$(e) {
    return {
        minutes: Math.max(0, Math.floor(e / 60)),
        seconds: Math.max(0, Math.floor(e % 60))
    };
}
function e0(e) {
    let t = eJ(e);
    return e1(t.minutes, t.seconds);
}
function e1(e, t) {
    return ''.concat(String(e).padStart(2, '0'), ':').concat(String(t).padStart(2, '0'));
}
let e2 = eQ([o.X.PLAY_ON_XBOX, o.X.PLAY_ON_PLAYSTATION]),
    e3 = eQ([o.X.WATCH_VIDEO, o.X.WATCH_VIDEO_ON_MOBILE]);
eQ([o.X.WATCH_VIDEO]);
let e4 = eQ([o.X.WATCH_VIDEO_ON_MOBILE]);
function e5(e) {
    return e2(e);
}
let e6 = (e, t) => {
    var n, r;
    let i = null == e || null == (r = e.progress[t]) || null == (n = r.heartbeat) ? void 0 : n.expiresAt;
    if (null == i) return !1;
    let a = new Date(i).valueOf();
    return !isNaN(a) && a > Date.now();
};
function e8(e) {
    return null != e.userStatus && (e6(e.userStatus, o.X.PLAY_ON_XBOX) || e6(e.userStatus, o.X.PLAY_ON_PLAYSTATION));
}
function e7(e) {
    return !!e3(e) && (0, n(952265).nf)(t_(e.id));
}
function e9() {
    f.Z.open(L.oAB.CONNECTIONS);
}
function te(e, t) {
    let { platformType: n, quest: r } = e;
    (0, S._3)({
        questId: r.id,
        questContent: t.content,
        questContentCTA: t.ctaContent,
        impressionId: t.impressionId
    }),
        (0, p.Z)({
            platformType: n,
            location: t.ctaContent
        });
}
function tt(e, t) {
    let { quest: n } = e;
    (0, S._3)({
        questId: n.id,
        questContent: t.content,
        questContentPosition: t.position,
        questContentRowIndex: t.rowIndex,
        questContentCTA: t.ctaContent,
        impressionId: t.impressionId
    });
    let r = tf(n);
    if (1 === r.length) return (0, p.Z)({ platformType: r.at(0) });
    d.Z.dispatch({
        type: 'CONNECTIONS_GRID_MODAL_SHOW',
        onComplete: (e) => (0, p.Z)({ platformType: e }),
        includedPlatformTypes: new Set(r)
    });
}
function tn(e, t) {
    let { quest: n } = e;
    (0, S._3)({
        questId: n.id,
        questContent: t.content,
        questContentPosition: t.position,
        questContentCTA: t.ctaContent,
        impressionId: t.impressionId
    }),
        e9();
}
function tr() {
    return window.location.pathname.startsWith(L.Z5c.QUEST_HOME);
}
function ti(e) {
    let t = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : { dateStyle: 'short' };
    return null == e ? '' : new Date(e).toLocaleDateString(h.default.locale, t);
}
function ta(e) {
    return 'xbox' === e.connected_account_type ? L.ABu.XBOX : L.ABu.PLAYSTATION;
}
function to(e) {
    return ta(e) === L.ABu.XBOX ? k.t.mytEv7 : k.t.iDiwb2;
}
function ts(e) {
    var t, n, r, i;
    let { quest: a, rewardCode: o, selectedPlatformType: s, sharedQuestFields: l } = e,
        c = eC({ quest: a }),
        u = null != s ? s : null == o ? void 0 : o.platform,
        d = c
            ? eM({
                  quest: a,
                  idx: null != (r = null == o ? void 0 : o.tier) ? r : null == (t = a.userStatus) ? void 0 : t.claimedTier
              })
            : null,
        f = null != (i = null == d || null == (n = d.messages) ? void 0 : n.redemptionInstructionsByPlatform) ? i : l.defaultRewardRedemptionInstructionsByPlatform;
    return null != u ? f[u] : void 0;
}
function tl(e) {
    let { isTargetedDisclosure: t, gamePublisher: n, gameTitle: r, cosponsorName: i } = e;
    return t
        ? null == i
            ? k.intl.formatToPlainString(k.t.Piihy8, { gamePublisher: n })
            : k.intl.formatToPlainString(k.t.DV47Gx, {
                  gamePublisher: n,
                  cosponsorName: i
              })
        : k.intl.formatToPlainString(k.t.tOWwxM, {
              gamePublisher: n,
              gameTitle: r
          });
}
function tc(e) {
    let t = eR({ quest: e }) || ew({ quest: e }),
        n = e2(e),
        r = [];
    return t && r.push(D.cd.DESKTOP), n && r.push(D.cd.CONSOLE), r;
}
function tu(e) {
    var t;
    let n = null == (t = em(e)) ? void 0 : t.expirationMode;
    return !!(0, P.U)() && !!n && F.has(n);
}
function td(e) {
    var t;
    return tu(e) && (null == (t = em(e)) ? void 0 : t.expirationMode) === l.n.PREMIUM_PERMANENT;
}
function tf(e) {
    let t = Object.keys(e.config.taskConfig.tasks),
        n = [];
    for (let e of t)
        switch (e) {
            case o.X.PLAY_ON_XBOX:
                n.push(L.ABu.XBOX);
                break;
            case o.X.PLAY_ON_PLAYSTATION:
                n.push(L.ABu.PLAYSTATION);
        }
    return n;
}
function t_(e) {
    return 'VIDEO-QUEST-'.concat(e);
}
function tp(e) {
    let t = e3(e),
        n = eD(e);
    return t || n;
}
function th(e, t) {
    var n, r;
    q(e) || (null == (n = e.userStatus) ? void 0 : n.enrolledAt) == null || (null == (r = e.userStatus) ? void 0 : r.completedAt) != null || (0, T.cT)(e.id, t);
}
function tm(e) {
    return {
        [N.jn.QUEST_BAR]: N.Ok.DESKTOP_ACCOUNT_PANEL_AREA,
        [N.jn.QUEST_BAR_V2]: N.Ok.DESKTOP_ACCOUNT_PANEL_AREA,
        [N.jn.QUEST_BAR_MOBILE]: N.Ok.MOBILE_HOME_DOCK_AREA
    }[e];
}
function tg(e) {
    let t = tm(e);
    return null != t && D.v6.has(t);
}
let tE = (e) => (e.percentComplete > 0 ? k.intl.formatToPlainString(k.t['c59/Tk'], { remainTime: e0(e) }) : k.intl.formatToPlainString(k.t.GNsKiY, { remainTime: e0(e) }));
function tb(e, t) {
    return e <= 0 || t <= 0 ? 0 : e >= t ? 1 : Math.min(1, Math.round((e / t) * 100) / 100);
}
function ty(e) {
    var t, n, r, i;
    return null != (i = null != (r = null == (t = e.videoMetadata) ? void 0 : t.messages.videoEndCtaButtonLabel) ? r : null == (n = e.videoMetadata) ? void 0 : n.messages.videoEndCtaTitle) ? i : k.intl.string(k.t.iiTtpK);
}
function tO(e) {
    var t;
    let n = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : A.Z,
        r = tm(e),
        i = null != r ? n.questToDeliverForPlacement.get(r) : void 0;
    return null != (t = null == i ? void 0 : i.adDecisionData) ? t : {};
}
function tv(e) {
    var t, n;
    let r = null != (n = null == (t = e.userStatus) ? void 0 : t.claimedTier) ? n : 0;
    return 'rewardsConfig' in e.config ? e.config.rewardsConfig.rewards[r] : e.config.rewards[r];
}
function tI(e) {
    return new Set(e.config.taskConfig.type === u.L.FIRST_PARTY ? Object.keys(e.config.taskConfig.tasks) : [o.X.ACHIEVEMENT_IN_ACTIVITY]);
}
function tS(e, t) {
    let n = new Map();
    for (let [r, i] of e)
        for (let e of tI(i))
            if (t.has(e)) {
                n.set(r, i);
                break;
            }
    return n;
}
function tT(e) {
    var t, n;
    let { questId: r, videoSessionId: i } = e,
        a = C.ZP.getState().getVideoProgress(r);
    if (null == a) return;
    let o = A.Z.getQuest(r);
    null != o && (null == (t = o.userStatus) ? void 0 : t.enrolledAt) != null && (null == (n = o.userStatus) ? void 0 : n.completedAt) == null && th(o, a.maxTimestampSec);
    let s = tb(a.maxTimestampSec, a.duration);
    (0, S.dA)({
        questId: r,
        event: L.rMx.QUEST_VIDEO_PROGRESSED,
        properties: {
            progress: s,
            video_timestamp_seconds: a.maxTimestampSec,
            video_session_id: i
        }
    }),
        (0, S.dA)({
            questId: r,
            event: L.rMx.QUEST_VIDEO_MODAL_CLOSED,
            properties: {
                video_progress: s,
                video_session_id: i,
                network_connection_speed: m.Z.getEffectiveConnectionSpeed()
            }
        });
}
