...
async function S(t, e) {
    let {
        ...
        if (!(0, c.yS)({ location: e })) return { shouldFetch: !1, filteredSkuIds: [], applicationId: null };
        if ("application" === t.type) {
            let i = o.A.getFetchStateForApplicationId(t.applicationId),
                l = h(i, e);
            return null != i && ("loading" === i.type || i.fetchedAt > Date.now() - l)
    ...
    if (!i) return;
    let p = null != u ? { type: "application", applicationId: u } : { type: "skus", skuIds: l };
    try {
        ...
        a.h.dispatch({ type: "SKUS_PRICING_FETCH_SUCCESS", priceId: p, data: (0, s.Oj)(t) });
    } catch {
