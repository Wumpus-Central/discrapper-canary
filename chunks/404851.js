...
function $(e) {
    return null == e
        ? null
        : e.version !== v.Ir
          ? (w.warn(`Throwing away client session with invalid version: ${e.version}, expected ${v.Ir}`), null)
          : e;
}
async function z() {
    let e = await en(!1);
...
function q() {
    ...
    B !== e && ((B = e), a.w.remove(D), (U = { state: "loaded", session: null }), H(), (k = 0)), K();
}
function X() {
    let e = h.A.getState();
    G !== e && ((G = e), K());
...
function et() {
    E.A.addBreadcrumb({ message: "Initializing SessionHeartbeatScheduler" }),
        ...
        null == P &&
            (P = {
                id: setInterval(() => {
                    !(null != B && C.getConfig({ location: "shouldLogClientHeartbeatSkipped" }).logHeartbeatSkipped) ||
                        performance.now() - k <= O ||
...
async function en() {
    ...
    if (W()) {
        (null == t || (0, v.aE)(t)) &&
            ((t = { uuid: (0, i.A)(), createdAtTimestamp: s, lastUsedTimestamp: s, version: v.Ir }), (x = 0)),
