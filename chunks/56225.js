s.d(t, { Cf: () => k, wx: () => H });
var n = s(627968),
    a = s(64700),
    r = s(834730),
    l = s(366010),
    i = s(947641),
    d = s(408278),
    o = s(624479),
    c = s(821609),
    u = s(695366),
    m = s(194261),
    h = s(534514),
    x = s(17928),
    C = s(661531),
    p = s(144165),
    _ = s(31502),
    g = s(914410),
    f = s(363195),
    b = s(954571),
    R = s(58703),
    v = s(975571),
    j = s(38405),
    E = s(398523),
    A = s(881373),
    N = s(264779),
    P = s(852218);
let I = () =>
        (0, n.jsxs)("svg", {
            width: "20",
            height: "20",
            viewBox: "0 0 20 20",
            fill: "none",
            xmlns: "http://www.w3.org/2000/svg",
            children: [
                (0, n.jsxs)("g", {
                    clipPath: "url(#clip0_5398_22750)",
                    children: [
                        (0, n.jsx)("mask", {
                            id: "mask0_5398_22750",
                            style: { maskType: "luminance" },
                            maskUnits: "userSpaceOnUse",
                            x: "0",
                            y: "0",
                            width: "20",
                            height: "20",
                            children: (0, n.jsx)("path", { d: "M0 0H20V20H0V0Z", fill: "white" }),
                        }),
                        (0, n.jsxs)("g", {
                            mask: "url(#mask0_5398_22750)",
                            children: [
                                (0, n.jsx)("path", {
                                    d: "M11.7346 16.986C8.84262 16.986 6.49024 14.6399 6.49024 11.756C6.49024 8.87208 8.84262 6.52593 11.7346 6.52593C14.6257 6.52593 16.9781 8.87208 16.9781 11.756C16.9781 14.6399 14.6257 16.986 11.7346 16.986ZM13.2458 3.65049V1.50705C13.2458 0.674633 12.5688 -0.00012207 11.7346 -0.00012207C10.9 -0.00012207 10.2235 0.674633 10.2235 1.50705V3.65002C6.87415 4.26987 4.22771 6.90882 3.60668 10.2497H1.51117C0.676547 10.2497 0 10.924 0 11.756C0 12.5884 0.676547 13.2631 1.51117 13.2631H3.60668C4.31898 17.0925 7.69231 19.9999 11.7346 19.9999C16.2926 19.9999 20 16.3019 20 11.756C20 7.72482 17.084 4.36044 13.2458 3.65049Z",
                                    fill: "#FBFBFB",
                                }),
                                (0, n.jsx)("path", {
                                    d: "M10.7447 11.765C10.7447 11.2291 11.1817 10.7932 11.7181 10.7932C12.2558 10.7932 12.6929 11.2291 12.6929 11.765C12.6929 12.3003 12.2558 12.7363 11.7181 12.7363C11.1817 12.7363 10.7447 12.3003 10.7447 11.765ZM8.47461 11.765C8.47461 13.549 9.9298 15.0008 11.7181 15.0008C13.5073 15.0008 14.9625 13.549 14.9625 11.765C14.9625 9.98094 13.5073 8.52914 11.7181 8.52914C9.9298 8.52914 8.47461 9.98094 8.47461 11.765Z",
                                    fill: "#FBFBFB",
                                }),
                                (0, n.jsx)("path", {
                                    d: "M5.42451 4.15601C5.42451 4.9889 4.74703 5.66553 3.91146 5.66553C3.07542 5.66553 2.39746 4.9889 2.39746 4.15601C2.39746 3.32266 3.07542 2.6465 3.91146 2.6465C4.74703 2.6465 5.42451 3.32266 5.42451 4.15601Z",
                                    fill: "#FBFBFB",
                                }),
                            ],
                        }),
                    ],
                }),
                (0, n.jsx)("defs", {
                    children: (0, n.jsx)("clipPath", {
                        id: "clip0_5398_22750",
                        children: (0, n.jsx)("rect", { width: "20", height: "20", fill: "white" }),
                    }),
                }),
            ],
        }),
    y = () =>
        (0, n.jsxs)("svg", {
            width: "20",
            height: "20",
            viewBox: "0 0 20 20",
            fill: "none",
            xmlns: "http://www.w3.org/2000/svg",
            children: [
                (0, n.jsxs)("g", {
                    clipPath: "url(#clip0_5398_22756)",
                    children: [
                        (0, n.jsx)("mask", {
                            id: "mask0_5398_22756",
                            style: { maskType: "luminance" },
                            maskUnits: "userSpaceOnUse",
                            x: "0",
                            y: "0",
                            width: "20",
                            height: "20",
                            children: (0, n.jsx)("path", { d: "M0 0H20V20H0V0Z", fill: "white" }),
                        }),
                        (0, n.jsxs)("g", {
                            mask: "url(#mask0_5398_22756)",
                            children: [
                                (0, n.jsx)("path", {
                                    d: "M11.7346 16.986C8.84262 16.986 6.49024 14.6399 6.49024 11.756C6.49024 8.87208 8.84262 6.52593 11.7346 6.52593C14.6257 6.52593 16.9781 8.87208 16.9781 11.756C16.9781 14.6399 14.6257 16.986 11.7346 16.986ZM13.2458 3.65049V1.50705C13.2458 0.674633 12.5688 -0.00012207 11.7346 -0.00012207C10.9 -0.00012207 10.2235 0.674633 10.2235 1.50705V3.65002C6.87415 4.26987 4.22771 6.90882 3.60668 10.2497H1.51117C0.676547 10.2497 0 10.924 0 11.756C0 12.5884 0.676547 13.2631 1.51117 13.2631H3.60668C4.31898 17.0925 7.69231 19.9999 11.7346 19.9999C16.2926 19.9999 20 16.3019 20 11.756C20 7.72482 17.084 4.36044 13.2458 3.65049Z",
                                    fill: "#2F3035",
                                }),
                                (0, n.jsx)("path", {
                                    d: "M10.7447 11.765C10.7447 11.2291 11.1817 10.7932 11.7181 10.7932C12.2558 10.7932 12.6929 11.2291 12.6929 11.765C12.6929 12.3003 12.2558 12.7363 11.7181 12.7363C11.1817 12.7363 10.7447 12.3003 10.7447 11.765ZM8.47461 11.765C8.47461 13.549 9.9298 15.0008 11.7181 15.0008C13.5073 15.0008 14.9625 13.549 14.9625 11.765C14.9625 9.98094 13.5073 8.52914 11.7181 8.52914C9.9298 8.52914 8.47461 9.98094 8.47461 11.765Z",
                                    fill: "#2F3035",
                                }),
                                (0, n.jsx)("path", {
                                    d: "M5.42451 4.15601C5.42451 4.9889 4.74703 5.66553 3.91146 5.66553C3.07542 5.66553 2.39746 4.9889 2.39746 4.15601C2.39746 3.32266 3.07542 2.6465 3.91146 2.6465C4.74703 2.6465 5.42451 3.32266 5.42451 4.15601Z",
                                    fill: "#2F3035",
                                }),
                            ],
                        }),
                    ],
                }),
                (0, n.jsx)("defs", {
                    children: (0, n.jsx)("clipPath", {
                        id: "clip0_5398_22756",
                        children: (0, n.jsx)("rect", { width: "20", height: "20", fill: "white" }),
                    }),
                }),
            ],
        }),
    D = () =>
        (0, n.jsxs)("svg", {
            width: "20",
            height: "20",
            viewBox: "0 0 20 20",
            fill: "none",
            xmlns: "http://www.w3.org/2000/svg",
            children: [
                (0, n.jsxs)("g", {
                    clipPath: "url(#clip0_5402_26507)",
                    children: [
                        (0, n.jsx)("path", {
                            d: "M10 19.1252C5.31783 19.1252 1.52212 15.3294 1.52212 10.6474C1.52212 5.96514 5.31783 2.16943 10 2.16943C14.6821 2.16943 18.4778 5.96514 18.4778 10.6474C18.4778 15.3294 14.6821 19.1252 10 19.1252ZM10 -2.28882e-05C4.47718 -2.28882e-05 0 4.47716 0 10C0 15.5228 4.47718 20 10 20C15.5228 20 20 15.5228 20 10C20 4.47716 15.5228 -2.28882e-05 10 -2.28882e-05Z",
                            fill: "#FBFBFB",
                        }),
                        (0, n.jsx)("path", {
                            d: "M16.518 10.0407C16.146 8.811 15.3522 7.95319 13.9908 8.03448C13.3579 8.07227 12.8226 8.29801 12.2416 8.52348C11.5189 8.80392 10.7774 8.95798 10 8.95798C9.22266 8.95798 8.48125 8.80392 7.75856 8.52348C7.17751 8.29801 6.64229 8.07227 6.00932 8.03448C4.64799 7.95319 3.85404 8.811 3.4821 10.0407C3.15876 11.1098 3.00341 12.4285 3.51689 13.4654C4.05593 14.5539 5.43159 14.9536 6.45678 14.2962C7.64683 13.5333 8.40792 11.8853 10 11.9133C11.5922 11.8853 12.3533 13.5333 13.5434 14.2962C14.5685 14.9536 15.9441 14.5539 16.4832 13.4654C16.9967 12.4285 16.8414 11.1098 16.518 10.0407Z",
                            fill: "#FBFBFB",
                        }),
                        (0, n.jsx)("path", {
                            d: "M12.0012 17.0763H7.90866C7.51404 17.0763 7.19336 16.7555 7.19336 16.3609C7.19336 15.9663 7.51404 15.6456 7.90866 15.6456H12.0012C12.3958 15.6456 12.7165 15.9663 12.7165 16.3609C12.7165 16.7555 12.3958 17.0763 12.0012 17.0763Z",
                            fill: "#FBFBFB",
                        }),
                    ],
                }),
                (0, n.jsx)("defs", {
                    children: (0, n.jsx)("clipPath", {
                        id: "clip0_5402_26507",
                        children: (0, n.jsx)("rect", { width: "20", height: "20", fill: "white" }),
                    }),
                }),
            ],
        }),
    S = () =>
        (0, n.jsxs)("svg", {
            width: "20",
            height: "20",
            viewBox: "0 0 20 20",
            fill: "none",
            xmlns: "http://www.w3.org/2000/svg",
            children: [
                (0, n.jsxs)("g", {
                    clipPath: "url(#clip0_5402_26509)",
                    children: [
                        (0, n.jsx)("path", {
                            d: "M10 19.1252C5.31783 19.1252 1.52212 15.3294 1.52212 10.6474C1.52212 5.96514 5.31783 2.16943 10 2.16943C14.6821 2.16943 18.4778 5.96514 18.4778 10.6474C18.4778 15.3294 14.6821 19.1252 10 19.1252ZM10 -2.28882e-05C4.47718 -2.28882e-05 0 4.47716 0 10C0 15.5228 4.47718 20 10 20C15.5228 20 20 15.5228 20 10C20 4.47716 15.5228 -2.28882e-05 10 -2.28882e-05Z",
                            fill: "#2F3035",
                        }),
                        (0, n.jsx)("path", {
                            d: "M16.518 10.0407C16.146 8.811 15.3522 7.95319 13.9908 8.03448C13.3579 8.07227 12.8226 8.29801 12.2416 8.52348C11.5189 8.80392 10.7774 8.95798 10 8.95798C9.22266 8.95798 8.48125 8.80392 7.75856 8.52348C7.17751 8.29801 6.64229 8.07227 6.00932 8.03448C4.64799 7.95319 3.85404 8.811 3.4821 10.0407C3.15876 11.1098 3.00341 12.4285 3.51689 13.4654C4.05593 14.5539 5.43159 14.9536 6.45678 14.2962C7.64683 13.5333 8.40792 11.8853 10 11.9133C11.5922 11.8853 12.3533 13.5333 13.5434 14.2962C14.5685 14.9536 15.9441 14.5539 16.4832 13.4654C16.9967 12.4285 16.8414 11.1098 16.518 10.0407Z",
                            fill: "#2F3035",
                        }),
                        (0, n.jsx)("path", {
                            d: "M12.0012 17.0763H7.90866C7.51404 17.0763 7.19336 16.7555 7.19336 16.3609C7.19336 15.9663 7.51404 15.6456 7.90866 15.6456H12.0012C12.3958 15.6456 12.7165 15.9663 12.7165 16.3609C12.7165 16.7555 12.3958 17.0763 12.0012 17.0763Z",
                            fill: "#2F3035",
                        }),
                    ],
                }),
                (0, n.jsx)("defs", {
                    children: (0, n.jsx)("clipPath", {
                        id: "clip0_5402_26509",
                        children: (0, n.jsx)("rect", { width: "20", height: "20", fill: "white" }),
                    }),
                }),
            ],
        });
var w = s(652215),
    O = s(962995),
    T = s(985018),
    M = s(75662),
    L = s(208556);
function k() {
    let e = (0, _.Q)("getRecurringPromotionPartnerAttributes");
    return {
        [P.XY]: {
            title: e ? O.default.PF1aT5 : O.default.LsJ9hj,
            titleParams: e ? { discountPercentage: A.aW } : { discountPercent: A.aW },
            body: e ? O.default.KoKwMn : O.default.Yl5ww1,
            bodyWithExpiration: O.default["6KjkUa"],
            redemptionURL: "https://www.logitech.com/",
            logos: {
                dark: (0, n.jsx)(a.Fragment, {}, "logitech-logo-dark"),
                light: (0, n.jsx)(a.Fragment, {}, "logitech-logo-light"),
            },
            asset: L.A,
        },
        [P.KS]: {
            title: e ? T.t.p7BkHh : T.t.sfGgOG,
            titleParams: e ? { discountPercentage: E.H7 } : void 0,
            body: e ? T.t.nmvvaN : T.t.igiSKe,
            bodyWithExpiration: T.t.w8CXUl,
            redemptionURL: "https://steelseries.com/",
            logos: { dark: (0, n.jsx)(I, {}), light: (0, n.jsx)(y, {}) },
            asset: "https://cdn.discordapp.com/assets/content/c23a186569a2e8606d206cd4b5cc7f4ee8f880c844bc0debd5fd3400400ff7bc.png",
        },
        [P.Cs]: {
            title: e ? T.t.QEJnti : T.t["9N8obd"],
            titleParams: e ? { discountPercentage: E.F0 } : void 0,
            body: e ? T.t.eT5vm0 : T.t["34g+Z3"],
            bodyWithExpiration: T.t.rqmpXc,
            redemptionURL: "https://www.kontrolfreek.com/",
            logos: { dark: (0, n.jsx)(D, {}), light: (0, n.jsx)(S, {}) },
            asset: "https://cdn.discordapp.com/assets/content/6706fc54c35e86999d3537dcc3ef8bfe91f25db6a141b4b6e5b451ebc5cae86b.png",
        },
    };
}
let B = () => {
        let e = (0, R.N5)(),
            t = (0, R.P6)();
        return (0, n.jsxs)("div", {
            className: `${M.G9} ${M.sQ}`,
            children: [
                (0, n.jsx)(r.E, {
                    variant: "text-sm/normal",
                    color: "text-muted",
                    className: M.kT,
                    children: T.intl.format(O.default["65EEvD"], { days: t }),
                }),
                (0, n.jsx)(g.Ay, {
                    progress: e,
                    variant: g.qP.UNSET,
                    override: { default: { gradientStart: "var(--illo-blue-70)", gradientEnd: "var(--illo-blue-10)" } },
                }),
            ],
        });
    },
    U = (e) => {
        let { children: t } = e;
        return (0, n.jsx)("div", { className: M.oP, children: (0, n.jsx)("div", { className: M.t0, children: t }) });
    },
    F = (e) => {
        let { recurrence: t } = e,
            s = (0, n.jsx)(i.r, { color: C.A.colors.CONTROL_CONNECTED_BACKGROUND_DEFAULT }),
            l = (0, n.jsx)("span", {
                className: M.nP,
                children: (0, n.jsx)(d.K, {
                    icon: o.T,
                    size: "sm",
                    variant: "icon-only",
                    "aria-label": "",
                    onClick: () => {
                        navigator.clipboard.writeText(t.code), m(s);
                    },
                }),
            }),
            [u, m] = a.useState(() => l);
        if (null != t.code)
            return (0, n.jsx)("div", {
                className: M.oP,
                children: (0, n.jsxs)("div", {
                    className: M.t0,
                    children: [
                        (0, n.jsx)("div", {
                            className: M.cD,
                            children: (0, n.jsx)(r.E, {
                                variant: "text-md/medium",
                                color: "text-strong",
                                children: t.code,
                            }),
                        }),
                        u,
                        (0, n.jsx)(c.$, {
                            variant: "secondary",
                            size: "sm",
                            text: T.intl.formatToPlainString(T.t.DF68t7, { redemptionURL: t.redemptionURL }),
                            onClick: () => {
                                window.open(t.redemptionURL, "_blank"),
                                    b.default.track(w.HAw.RECURRING_PROMOTION_CLAIMED);
                            },
                        }),
                    ],
                }),
            });
    },
    G = (e) => {
        let {
            recurrence: t,
            canBeClaimed: s,
            hasClaimError: a,
            setCode: r,
            setHasClaimError: l,
            className: i,
            analyticsLocations: d = [],
        } = e;
        return s
            ? a
                ? (0, n.jsx)("div", {
                      className: i,
                      children: (0, n.jsx)(c.$, {
                          icon: u.E,
                          variant: "critical-secondary",
                          size: "sm",
                          disabled: !0,
                          text: T.intl.string(T.t["8LKchl"]),
                      }),
                  })
                : (0, n.jsx)("div", {
                      className: i,
                      children: (0, n.jsx)(c.$, {
                          icon: m.X,
                          variant: "secondary",
                          size: "sm",
                          text: T.intl.string(T.t.vwASIl),
                          onClick: () => {
                              (0, N.kd)({ promotionId: t.id, analyticsLocations: d })
                                  .then((e) => {
                                      r(e.code);
                                  })
                                  .catch((e) => {
                                      l(!0), j.A.captureException(e);
                                  });
                          },
                      }),
                  })
            : null;
    },
    H = (e) => {
        var t;
        let s,
            r,
            {
                recurrence: i,
                titleClassName: d,
                showPartnerLogo: o = !1,
                showPartnerImage: c = !1,
                titleVariant: u = "primary",
                displayCTAInDedicatedFooter: m = !1,
                analyticsLocations: C = [],
            } = e,
            [_, g] = a.useState(i.code),
            [b, R] = a.useState(!1),
            j =
                ((t = i.partnerId),
                (s = (0, x.bG)([f.A], () => (0, l.M)(f.A.theme))),
                null == (r = k()[t]) ? null : s ? r.logos.dark : r.logos.light),
            E = null == _;
        return (0, n.jsxs)("div", {
            className: M.lA,
            children: [
                (0, n.jsxs)("div", {
                    className: M.LV,
                    children: [
                        (0, n.jsxs)("div", {
                            className: M.JN,
                            children: [
                                o && j,
                                (0, n.jsxs)("div", {
                                    className: null != d ? `${M.yO} ${d}` : M.yO,
                                    children: [
                                        (0, n.jsx)(h.D, {
                                            variant: "secondary" === u ? "heading-md/semibold" : "heading-lg/semibold",
                                            color: "text-strong",
                                            children: i.title,
                                        }),
                                        (0, n.jsx)(h.D, {
                                            variant: "heading-sm/medium",
                                            color: "text-subtle",
                                            children: b
                                                ? T.intl.format(T.t.i2EuFO, {
                                                      helpdeskArticle: v.A.getArticleURL(w.MVz.RECURRING_PROMOTION),
                                                  })
                                                : E
                                                  ? i.body
                                                  : null != i.bodyWithExpiration
                                                    ? T.intl.format(i.bodyWithExpiration, { date: i.endDate })
                                                    : i.body,
                                        }),
                                    ],
                                }),
                                !m &&
                                    (0, n.jsx)(G, {
                                        recurrence: i,
                                        canBeClaimed: E,
                                        hasClaimError: b,
                                        setCode: g,
                                        setHasClaimError: R,
                                        analyticsLocations: C,
                                    }),
                            ],
                        }),
                        c &&
                            (0, n.jsx)("div", {
                                className: M.R4,
                                children: (0, n.jsx)(p._, {
                                    src: i.asset ?? "",
                                    className: M.Ys,
                                    imageClassName: i.partnerId === P.XY ? M.Cy : void 0,
                                    width: 100,
                                    height: 100,
                                    zoomable: !1,
                                }),
                            }),
                    ],
                }),
                m && (0, n.jsx)(B, {}),
                null != _ && (0, n.jsx)(F, { recurrence: { ...i, code: _ } }),
                m &&
                    null == _ &&
                    (0, n.jsx)(U, {
                        children: (0, n.jsx)(G, {
                            recurrence: i,
                            canBeClaimed: E,
                            hasClaimError: b,
                            setCode: g,
                            setHasClaimError: R,
                            className: M.qx,
                            analyticsLocations: C,
                        }),
                    }),
            ],
        });
    };
