n.d(t, { A: () => eT });
var a = n(627968),
    l = n(64700),
    r = n(435558),
    s = n(702841),
    i = n(885386),
    d = n(351906),
    o = n(287809),
    c = n(375708);
let u = "legacy_username";
function f(e, t) {
    let n = i.m$.useSetting(),
        a = (0, s.bG)([o.default], () => o.default.getCurrentUser()),
        l = (0, s.bG)([d.A], () => d.A.hidePersonalInformation);
    if (null == e) return [];
    let r = e?.getBadges() ?? [];
    return (
        null != a && a.id === e.userId && (void 0 !== t ? t : n) && (r = r.filter((e) => e.id !== u)),
        l && (r = r.map((e) => ({ ...e, description: e.id === u ? c.intl.string(c.t.Br1ls3) : e.description }))),
        r
    );
}
var E = n(503698),
    m = n.n(E),
    g = n(17928),
    p = n(192308),
    _ = n(349288),
    b = n(866665),
    h = n(983555),
    A = n(274670),
    R = n(144779),
    x = n(793574),
    N = n(688810),
    I = n(682618),
    v = n(992526),
    T = n(982240),
    j = n(609782);
let S = (0, n(945810).mj)({
    name: "2026-06-use-new-badge-image-source",
    kind: "user",
    defaultConfig: { enabled: !1 },
    variations: { 0: { enabled: !1 }, 1: { enabled: !0 } },
});
var C = n(70283),
    D = n(775602),
    y = n(461536),
    M = n(321191);
function O(e) {
    let t,
        n,
        r,
        { children: s, legacyBadgeId: i, userId: d, fallbackTitle: o, fallbackIconSrc: u } = e,
        f = (0, j.w0)(i),
        {
            badgeData: E,
            currentTier: m,
            obtainedAt: p,
        } = (0, g.cf)(
            [T.Ay],
            () =>
                null == f || null == d
                    ? { badgeData: void 0, currentTier: void 0, obtainedAt: void 0 }
                    : {
                          badgeData: T.Ay.getBadgeById(f, d),
                          currentTier: T.Ay.getCurrentTier(f, d),
                          obtainedAt: T.Ay.getObtainedAt(f, d),
                      },
            [f, d],
        ),
        _ = (0, g.bG)([D.Ay], () => D.Ay.useReducedMotion),
        b = l.useCallback(() => {
            null != d && T.Ay.isCatalogStaleFor(d) && (0, I.RS)(d);
        }, [d]),
        h = E?.badge_id === C.$.PREMIUM_TENURE,
        A = f === C.$.LEGACY_USERNAME,
        R = m?.name,
        x = h ? c.intl.string(c.t.Ipxkog) : (E?.name ?? o);
    h ? (r = n = null != R ? `${x} ${R}` : x) : null != R ? ((t = x), (n = R), (r = `${x} ${R}`)) : ((n = x), (r = x));
    let N = _ ? void 0 : E?.complex_icon_animated_url,
        v = N ?? E?.complex_icon_static_url ?? E?.simple_icon_url ?? u,
        S = null != N && v === N;
    if (A) return (0, a.jsx)(P, { userId: d, title: n, badgeImage: v, children: s });
    let M = E?.info_label ?? (null != p ? c.intl.formatToPlainString(c.t["0aEh2a"], { date: new Date(p) }) : void 0);
    return (0, a.jsx)(y.A, {
        title: n,
        eyebrow: t,
        body: M,
        badgeImage: v,
        badgeName: r,
        variant: h ? "nitro" : "default",
        isAnimated: S,
        onShow: b,
        children: s,
    });
}
function P(e) {
    let { children: t, userId: n, title: l, badgeImage: r } = e,
        s = (0, g.bG)(
            [M.A, d.A],
            () =>
                d.A.hidePersonalInformation
                    ? c.intl.string(c.t.Br1ls3)
                    : null != n
                      ? (M.A.getUserProfile(n)?.legacyUsername ?? void 0)
                      : void 0,
            [n],
        );
    return (0, a.jsx)(y.A, { title: l, body: s, badgeImage: r, badgeName: l, children: t });
}
var U = n(470739),
    k = n(34332),
    B = n(459192);
function G(e) {
    let { targetElementRef: t, delay: n, forceOpen: l, badgeDescription: r, children: s } = e;
    return (0, a.jsx)(B.u, {
        targetElementRef: t,
        delay: n,
        forceOpen: l,
        asset: {
            type: "image",
            src: "https://cdn.discordapp.com/assets/content/c41152a51207aef6b145fd309053f9fa5d5bd5fab7208289710c70d61a47d2f4.svg",
        },
        title: "Last Meadow Online",
        body: r,
        children: s,
    });
}
var w = n(116833),
    L = n(704640),
    H = n(425713),
    V = n(642481),
    $ = n(720879),
    X = n(202541),
    F = n(49999),
    z = n(518477);
let W = function (e) {
    let {
            badgeId: t = X.Ac.PREMIUM_TENURE_1_MONTH,
            markAsDismissed: n,
            children: r,
            targetElementRef: s,
            progressCircleText: i,
            progressCirclePercent: d,
            progressCircleUrgency: o,
            onShow: u,
        } = e,
        f = X.VD[t].nameUnformattedNitro,
        E = c.intl.string(f).toLocaleUpperCase(),
        m = X.VD[t].rarity,
        g = (0, L.A)(t),
        p = (0, H.I)(t).standard;
    (0, l.useEffect)(() => {
        w.m[w.DynamicGraphicComponent.BADGE_IMAGE_WITH_PROGRESS_CIRCLE](), (new Image().src = p);
    }, [p]);
    let _ = (0, l.useCallback)(() => {
        n?.(F.i.USER_DISMISS);
    }, [n]);
    if ("mini-tooltip" === e.mode) {
        let t = (function (e, t, n) {
            if (null != e && null != t && null != n) return { text: e, percent: t, urgency: n };
        })(i, d, o);
        return (0, a.jsx)(y.A, {
            targetElementRef: s,
            onShow: u,
            title: E,
            body: e.body,
            variant: "nitro",
            rarity: m,
            badgeImage: p,
            badgeName: E,
            progressCircle: t,
            children: r,
        });
    }
    let b = {
        gradientColor: g,
        size: "lg",
        graphic: {
            type: "dynamic",
            component: w.DynamicGraphicComponent.BADGE_IMAGE_WITH_PROGRESS_CIRCLE,
            aspectRatio: "6/4",
            props: { src: p, alt: E, progressCircleText: i, progressCirclePercent: d, progressCircleUrgency: o },
        },
        title: E,
        body: e.body,
        actions: e.actions,
    };
    return "tooltip" === e.mode
        ? (0, a.jsx)($.A, {
              targetElementRef: s,
              estimatedTooltipHeight: e.estimatedTooltipHeight ?? 300,
              delay: z.In,
              onShow: u,
              ...b,
              children: r,
          })
        : (0, a.jsxs)(a.Fragment, {
              children: [
                  r,
                  (0, a.jsx)(V.A, {
                      targetElementRef: s,
                      shouldShow: !0,
                      onRequestClose: _,
                      align: "right",
                      position: "top",
                      caretConfig: { align: "center" },
                      ...b,
                  }),
              ],
          });
};
var Y = n(297264),
    K = n(834730),
    q = n(214981);
let Q = function (e) {
    let { assetComponent: t, tooltipWordmarkComponent: n, children: l } = e;
    return (0, a.jsxs)("div", { className: q.R, children: [t, (0, a.jsx)("div", { className: q.A, children: n }), l] });
};
var Z = n(224016),
    J = n(632707);
let ee = function (e) {
    let { profileBadge: t, tenureBadge: n } = e,
        l = (0, H.I)(n.id).standard;
    return (0, a.jsxs)(Q, {
        assetComponent: (0, a.jsx)("img", { src: l, alt: "" }),
        tooltipWordmarkComponent: (0, a.jsx)(Z.A, { width: 56, className: J.R }),
        children: [
            (0, a.jsx)(Y.D, { variant: "heading-xl/extrabold", children: c.intl.string(n.nameUnformatted) }),
            (0, a.jsx)(K.E, {
                variant: "text-xs/normal",
                color: "text-muted",
                className: J.K,
                children: t.description,
            }),
        ],
    });
};
var et = n(30084),
    en = n(24001),
    ea = n(104886),
    el = n(590202),
    er = n(976860),
    es = n(305003),
    ei = n(312640),
    ed = n(798618),
    eo = n(752079),
    ec = n(641066);
function eu(e) {
    let { children: t, targetElementRef: n, delay: r, showSubtext: s, forceOpen: i } = e,
        [d, o] = l.useState(!1),
        c = l.useRef(null),
        u = l.useCallback(() => {
            null != c.current && (clearTimeout(c.current), (c.current = null));
        }, []);
    l.useEffect(() => u, [u]);
    let f = l.useCallback(() => {
        u(),
            (c.current = window.setTimeout(() => {
                (c.current = null), o(!0);
            }, r));
    }, [r, u]);
    l.useEffect(() => {
        i && !d && f();
    }, [i, d, f]);
    let E = l.useCallback(() => {
            f();
        }, [f]),
        m = l.useCallback(() => {
            u(), o(!1);
        }, [u]);
    return (0, a.jsxs)("div", {
        onMouseEnter: E,
        onMouseLeave: m,
        children: [
            t,
            (0, a.jsxs)(ei.x, {
                targetElementRef: n,
                shouldShow: !!(d || i),
                position: "top",
                caretConfig: { align: "center" },
                children: [(0, a.jsx)(ef, { showSubtext: s }), (0, a.jsx)(ed.F, {})],
            }),
        ],
    });
}
function ef(e) {
    let { showSubtext: t = !1 } = e;
    return (0, a.jsxs)(Q, {
        assetComponent: (0, a.jsx)(eo.A, { className: ec.jc }),
        tooltipWordmarkComponent: (0, a.jsx)(Y.D, {
            variant: "heading-md/extrabold",
            className: ec.RK,
            children: c.intl.string(c.t.ElYQFS),
        }),
        children: [
            (0, a.jsx)(Y.D, {
                variant: "heading-xl/extrabold",
                className: t ? void 0 : ec.Qn,
                children: c.intl.string(c.t.lTHkqd),
            }),
            t
                ? (0, a.jsx)(K.E, { variant: "text-xs/normal", className: ec.lV, children: c.intl.string(c.t.MEV0GI) })
                : void 0,
        ],
    });
}
var eE = n(174459),
    em = n(474090),
    eg = n(439174),
    ep = n(183555),
    e_ = n(47675),
    eb = n(999291),
    eh = n(652215),
    eA = n(758836);
let eR = "premium";
var ex = n(892724);
function eN(e) {
    let {
            badges: t,
            className: n,
            badgeClassName: r,
            displayProfile: s,
            onClose: i,
            shouldOpenBadgeTooltip: d,
            shouldGlowTenureBadge: u,
        } = e,
        E = l.useRef(null),
        C = l.useRef(null),
        D = l.useRef(null),
        { analyticsLocations: y } = (0, N.Ay)(x.A.BADGE),
        { context: M, trackUserProfileAction: P } = (0, ep.NJ)(),
        B = o.default.getCurrentUser(),
        w = (0, em.CC)(B?.premiumType, X.PremiumTypes.TIER_2),
        L = (0, v.J9)({ location: "UserProfileBadgeList" }),
        H = (function (e) {
            let { location: t } = e;
            return S.useConfig({ location: t }).enabled;
        })({ location: "UserProfileBadgeList" });
    l.useEffect(() => {
        (L || H) && s?.userId != null && !T.Ay.hasCatalogFor(s.userId) && (0, I.RS)(s.userId);
    }, [L, H, s?.userId]);
    let V = s?.userId,
        $ = (0, g.bG)([T.Ay], () => {
            if (!H || null == V) return null;
            let e = {};
            for (let n of t) {
                let t = (0, j.w0)(n.id);
                if (null == t) continue;
                let a = T.Ay.getBadgeById(t, V)?.simple_icon_url;
                null != a && (e[n.id] = a);
            }
            return e;
        }, [H, V, t]),
        F = f((0, eb.AP)(B?.id ?? null)).some((e) => e.id === es.A.ORB_PROFILE_BADGE);
    return (0, a.jsx)("div", {
        className: m()(ex.kL, n),
        "aria-label": c.intl.string(c.t.VWV0y5),
        role: "group",
        children: t.map((e, t) => {
            let n = e.id === es.A.ORB_PROFILE_BADGE,
                l = (0, eg.e0)(e.id),
                c = null != l || e.id === eR,
                f = s?.userId === B?.id,
                g = "april_fools_2026" === e.id,
                N = void 0 !== l && e.id !== eR,
                I = $?.[e.id],
                v = (0, a.jsx)("img", {
                    alt: " ",
                    "aria-hidden": !0,
                    src: e.iconSrc ?? I ?? (0, z.L7)(e.icon),
                    className: m()(ex.qS, null != I && ex.Do, r),
                }),
                T = null != d && d(e.id),
                S = {
                    onClick: (a) => {
                        P({ action: "PRESS_BADGE" }),
                            (0, e_.vP)({
                                badgeId: (0, j.w0)(e.id),
                                badgeAction: "PRESS_BADGE",
                                position: t,
                                analyticsLocations: y,
                                ...M,
                            });
                        let l = s?.userId != null ? o.default.getUser(s.userId) : null;
                        if (L && l?.bot !== !0) {
                            a.preventDefault(),
                                (0, U._)({
                                    initialBadgeId: (0, j.w0)(e.id),
                                    targetUserId: s?.userId,
                                    targetUsername: l?.globalName ?? l?.username,
                                });
                            return;
                        }
                        if (n) {
                            (0, k.Cz)({
                                tab: eA.G2.ORBS,
                                analyticsLocations: y,
                                analyticsSource: x.A.VIRTUAL_CURRENCY_ORB_PROFILE_BADGE,
                            }),
                                i?.(),
                                (0, p.closeAllModals)();
                            return;
                        }
                        if (c) {
                            if (
                                (a.preventDefault(),
                                eE.default.track(eh.HAw.TIERED_TENURE_BADGE_CLICKED, {
                                    badge: e.id,
                                    premium_type: w,
                                    viewed_user_id: s?.userId,
                                }),
                                w)
                            ) {
                                f
                                    ? ((0, er.pX)(eh.BVt.NITRO_HOME), (0, p.closeAllModals)())
                                    : (0, et.D)({ analyticsLocations: y, displayProfile: s }),
                                    i?.();
                                return;
                            }
                            if (f) {
                                let t = null != e.link ? (0, h.default)(e.link, { analyticsLocations: y }) : null;
                                if (null == t) return;
                                return i?.(), (0, p.closeAllModals)(), t(a);
                            }
                            return (0, et.D)({ analyticsLocations: y, displayProfile: s }), void i?.();
                        }
                        let r = null != e.link ? (0, h.default)(e.link, { analyticsLocations: y }) : null;
                        if (null != r) return i?.(), (0, p.closeAllModals)(), r(a);
                    },
                    onMouseEnter: () => {
                        "quest_completed" === e.id &&
                            ((0, ea.E5)(ea.kI.STEP_4_VIEWED_NON_IMPRESSION, "quest_completed_badge_hover")
                                ? (0, A.r)({
                                      type: R.F.VIEW_INTERNAL_SURFACE_IMPRESSION,
                                      surfaceId: en.uF.QUEST_BADGE,
                                      isTargeted: !1,
                                  })
                                : eE.default.track(eh.HAw.QUEST_CONTENT_VIEWED, {
                                      ...(0, el.fF)(en.uF.QUEST_BADGE),
                                      is_targeted: !1,
                                  })),
                            P({ action: "HOVER_BADGE" }),
                            (0, e_.vP)({
                                badgeId: (0, j.w0)(e.id),
                                badgeAction: "HOVER_BADGE",
                                position: t,
                                analyticsLocations: y,
                                ...M,
                            });
                    },
                    href: e.link,
                    "aria-label": e.description,
                    style: { filter: u && null != l ? `drop-shadow(0 0 5px ${l.glowColor})` : void 0 },
                };
            if (L)
                return (0, a.jsx)(
                    O,
                    {
                        legacyBadgeId: e.id,
                        userId: s?.userId,
                        fallbackTitle: e.description,
                        fallbackIconSrc: e.iconSrc ?? e.simple_icon_url ?? (0, z.L7)(e.icon),
                        children: (0, a.jsx)(_.Anchor, { ...S, children: v }),
                    },
                    `${e.id}-${t}`,
                );
            if (N) {
                let n = (0, a.jsx)(_.Anchor, { ...S, ref: E, children: v });
                return (0, a.jsx)(
                    "div",
                    {
                        children: (0, a.jsx)(W, {
                            badgeId: e.id,
                            targetElementRef: E,
                            mode: "tooltip",
                            body: e.description,
                            onShow: () => {
                                eE.default.track(eh.HAw.TOOLTIP_VIEWED, {
                                    type: "tiered_tenure_badge_profile_" + (f ? "self" : "other"),
                                });
                            },
                            estimatedTooltipHeight: 220,
                            children: n,
                        }),
                    },
                    `${e.id}-${t}`,
                );
            }
            if (n) {
                let n = (0, a.jsx)(_.Anchor, { ...S, ref: C, children: v });
                return (0, a.jsx)(
                    eu,
                    {
                        targetElementRef: C,
                        delay: z.In,
                        showSubtext: !F && !e.isPreviewMode,
                        forceOpen: T,
                        children: n,
                    },
                    `${e.id}-${t}`,
                );
            }
            if (g) {
                let n = (0, a.jsx)(_.Anchor, { ...S, ref: D, children: v });
                return (0, a.jsx)(
                    G,
                    { targetElementRef: D, delay: z.In, forceOpen: T, badgeDescription: e.description, children: n },
                    `${e.id}-${t}`,
                );
            }
            let H = (0, a.jsx)(_.Anchor, { ...S, children: v }),
                V = (function (e) {
                    let { badge: t, tieredTenureBadge: n } = e;
                    return void 0 !== n && t.id !== eR
                        ? (0, a.jsx)(ee, { profileBadge: t, tenureBadge: n })
                        : t.description;
                })({ badge: e, tieredTenureBadge: c && e.id !== eR ? l : void 0 });
            return (0, a.jsx)(
                b.m,
                { __unsupportedReactNodeAsText: V, forceOpen: T, delay: z.In, ariaHidden: !0, children: H },
                `${e.id}-${t}`,
            );
        }),
    });
}
var eI = n(996988);
let ev = { [eI.d.POPOUT]: 12, [eI.d.MODAL]: 26, [eI.d.SIDEBAR]: 13, [eI.d.VIDEO_TILE_BACKGROUND]: 12 };
function eT(e) {
    let {
            displayProfile: t,
            themeType: n,
            isRedesignEnabled: s,
            pendingLegacyUsernameDisabled: i,
            pendingBadges: d,
            onClose: o,
            shouldOpenBadgeTooltip: c,
            shouldGlowTenureBadge: u,
        } = e,
        E = f(t, i),
        m = l.useMemo(() => {
            if (null == d) return E;
            let e = new Set(E.map((e) => e.id));
            return [...E, ...d.filter((t) => !e.has(t.id))];
        }, [E, d]);
    if (0 === m.length) return null;
    let g = n === eI.d.MODAL_V2 ? [m] : (0, r.chunk)(m, !0 === s && n === eI.d.SIDEBAR ? ev[eI.d.POPOUT] : ev[n]);
    return (0, a.jsx)(a.Fragment, {
        children: g.map((e) =>
            (0, a.jsx)(
                eN,
                { badges: e, displayProfile: t, onClose: o, shouldOpenBadgeTooltip: c, shouldGlowTenureBadge: u },
                e[0].id,
            ),
        ),
    });
}
