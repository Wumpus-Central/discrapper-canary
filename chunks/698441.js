"use strict";
n.d(t, { AZ: () => D, Ay: () => w, Fd: () => v, W$: () => b, d6: () => C, ej: () => _ }), n(321073);
var r = n(735438),
    i = n.n(r),
    s = n(311907),
    a = n(713402),
    o = n(73153),
    l = n(961350),
    u = n(696451),
    d = n(661191),
    c = n(988794);
let _ = {
        EVENT: "event",
        EVENT_ACTIVE: "active",
        EVENT_UPCOMING: "event-upcoming",
        GUILD_EVENT: (e) => `${e}-${_.EVENT}`,
        GUILD_EVENT_ACTIVE: (e) => `${e}-${_.EVENT_ACTIVE}`,
        GUILD_EVENT_UPCOMING: (e) => `${e}-${_.EVENT_UPCOMING}`,
        CHANNEL_EVENT: (e) => `${e}-${_.EVENT}`,
        CHANNEL_EVENT_ACTIVE: (e) => `${e}-${_.EVENT_ACTIVE}`,
        CHANNEL_EVENT_UPCOMING: (e) => `${e}-${_.EVENT_UPCOMING}`,
    },
    f = new a.J(
        (e) => {
            let { guild_id: t, entity_id: n, channel_id: r } = e,
                i = [t];
            return (
                null != n && i.push(n),
                i.push(_.GUILD_EVENT(t)),
                null != r && i.push(_.CHANNEL_EVENT(r)),
                v(e) &&
                    (i.push(_.EVENT_ACTIVE),
                    i.push(_.GUILD_EVENT_ACTIVE(t)),
                    null != r && i.push(_.CHANNEL_EVENT_ACTIVE(r))),
                C(e) &&
                    (i.push(_.EVENT_UPCOMING),
                    i.push(_.GUILD_EVENT_UPCOMING(t)),
                    null != r && i.push(_.CHANNEL_EVENT_UPCOMING(r))),
                i
            );
        },
        function (e) {
            let { id: t, scheduled_start_time: n } = e,
                r = v(e) ? "\0" : "\x01";
            return `${r}-${new Date(n).getTime()}-${t}`;
        },
    ),
    E = 0,
    h = [],
    p = "SERIES",
    m = {},
    g = {};
function A(e) {
    f.set(e.id, e), (E += 1);
}
function I(e) {
    let t = !(arguments.length > 1) || void 0 === arguments[1] || arguments[1];
    f.delete(e), delete m[e], t && delete g[e], (E += 1);
}
function T(e) {
    var t;
    let n,
        r,
        i,
        s = !(arguments.length > 1) || void 0 === arguments[1] || arguments[1],
        a = !(arguments.length > 2) || void 0 === arguments[2] || arguments[2],
        o = e.guild_scheduled_event_id;
    null == m[o] && (m[o] = {});
    let l = e.guild_scheduled_event_exception_id ?? p;
    null == m[o][l] && (m[o][l] = {}),
        (m[o][l][e.user_id] = e),
        s &&
            ((n = (t = e).guild_scheduled_event_exception_id ?? p),
            (r = g[t.guild_scheduled_event_id]?.[n] ?? 0),
            (i =
                (null != t.guild_scheduled_event_exception_id && t.response === c.Qi.UNINTERESTED) ||
                (null == t.guild_scheduled_event_exception_id && t.response === c.Qi.INTERESTED)
                    ? 1
                    : -1),
            y(t.guild_scheduled_event_id, t.guild_scheduled_event_exception_id, r + i)),
        a && (E += 1);
}
function S(e) {
    var t;
    let n,
        r,
        i,
        s = !(arguments.length > 1) || void 0 === arguments[1] || arguments[1],
        a = e.guild_scheduled_event_exception_id ?? p,
        o = m[e.guild_scheduled_event_id]?.[a]?.[e.user_id] != null,
        u = e.user_id === l.default.getId();
    (o || !u) &&
        (delete m[e.guild_scheduled_event_id]?.[a]?.[e.user_id],
        (n = (t = e).guild_scheduled_event_exception_id ?? p),
        (r = g[t.guild_scheduled_event_id]?.[n] ?? 0),
        (i =
            (null != t.guild_scheduled_event_exception_id && t.response === c.Qi.UNINTERESTED) ||
            (null == t.guild_scheduled_event_exception_id && t.response === c.Qi.INTERESTED)
                ? -1
                : 1),
        y(t.guild_scheduled_event_id, t.guild_scheduled_event_exception_id, r + i),
        s && (E += 1));
}
function y(e, t, n) {
    null == g[e] && (g[e] = {}), (g[e][t ?? p] = n);
}
function N(e, t) {
    f.values(_.GUILD_EVENT(e)).forEach((e) => I(e.id, t));
}
function O(e) {
    let { guildScheduledEvent: t } = e;
    return A(t), !0;
}
function R(e) {
    let { eventException: t } = e,
        n = f.get(t.event_id);
    if (null == n) return !1;
    let r = n.guild_scheduled_event_exceptions.findIndex((e) => e.event_exception_id === t.event_exception_id),
        i = [...n.guild_scheduled_event_exceptions];
    return r < 0 ? i.push(t) : (i[r] = t), A({ ...n, guild_scheduled_event_exceptions: i }), !0;
}
function v(e) {
    return null != e && e?.status === c.XG.ACTIVE;
}
function C(e) {
    return !c.lV.has(e.status);
}
function b(e, t) {
    return new Date(e.scheduled_start_time).getTime() < Date.now() + 1e3 * t;
}
function D(e) {
    return null != e && c.lV.has(e.status);
}
class L extends s.Ay.Store {
    initialize() {
        this.waitFor(l.default, u.Ay);
    }
    static displayName = "GuildScheduledEventStore";
    getGuildScheduledEvent(e) {
        return null == e ? null : (f.get(e) ?? null);
    }
    getGuildEventCountByIndex(e) {
        return f.size(e);
    }
    getGuildScheduledEventsForGuild(e) {
        return null == e ? [] : f.values(e);
    }
    getGuildScheduledEventsByIndex(e) {
        return f.values(e);
    }
    getRsvpVersion() {
        return E;
    }
    getRsvp(e, t, n) {
        return null == e ? null : m[e]?.[t ?? p]?.[n];
    }
    isInterestedInEventRecurrence(e, t) {
        let n = l.default.getId(),
            r = this.getRsvp(e, null, n),
            i = this.getRsvp(e, t, n),
            s = r?.response === c.Qi.INTERESTED,
            a = i?.response === c.Qi.INTERESTED,
            o = i?.response === c.Qi.UNINTERESTED;
        return (s && !o) || a;
    }
    getUserCount(e, t) {
        if (null == e) return 0;
        let n = g[e]?.[p] ?? 0;
        return null == t ? n : n - (g[e]?.[t] ?? 0);
    }
    hasUserCount(e, t) {
        return g[e]?.[t ?? p] != null;
    }
    isActive(e) {
        return null != e && v(f.get(e));
    }
    getActiveEventByChannel(e) {
        if (null != e) return this.getGuildScheduledEventsByIndex(_.CHANNEL_EVENT_ACTIVE(e))[0];
    }
    getUsersForGuildEvent(e, t) {
        return null == e ? {} : (m[e]?.[t ?? p] ?? {});
    }
}
let w = new L(o.h, {
    CONNECTION_OPEN: function (e) {
        let { guilds: t } = e;
        return (
            f.clear(),
            (E = 0),
            (m = {}),
            (g = {}),
            h.forEach(A),
            t.forEach((e) => e.guild_scheduled_events.forEach((e) => A(e))),
            !0
        );
    },
    GUILD_CREATE: function (e) {
        let { guild: t } = e;
        return N(t.id, !1), t.guild_scheduled_events.forEach((e) => A(e)), !0;
    },
    GUILD_DELETE: function (e) {
        let { guild: t } = e;
        return N(t.id, !0), !0;
    },
    FETCH_GUILD_EVENT: function (e) {
        let { guildScheduledEvent: t } = e;
        A(t);
    },
    FETCH_GUILD_EVENTS_FOR_GUILD: function (e) {
        let { guildId: t, guildScheduledEvents: n } = e,
            r = f.values(_.GUILD_EVENT(t), !0).map((e) => e.id),
            s = n.map((e) => e.id);
        for (let e of (i()
            .difference(r, s)
            .forEach((e) => {
                I(e);
            }),
        n))
            A(e);
        return !0;
    },
    GUILD_SCHEDULED_EVENT_CREATE: O,
    GUILD_SCHEDULED_EVENT_UPDATE: O,
    GUILD_SCHEDULED_EVENT_DELETE: function (e) {
        let { guildScheduledEvent: t } = e;
        return I(t.id), !0;
    },
    GUILD_SCHEDULED_EVENT_USER_ADD: function (e) {
        let { userId: t, guildEventId: n, guildId: r, guildEventExceptionId: i, response: s } = e,
            a = i ?? p,
            o = m[n]?.[a]?.[t];
        null != o && S(o, !1);
        let l = u.Ay.getMember(r, t);
        return (
            T({
                user_id: t,
                guild_scheduled_event_id: n,
                member: l ?? void 0,
                guild_scheduled_event_exception_id: i,
                response: s,
            }),
            !0
        );
    },
    GUILD_SCHEDULED_EVENT_USER_REMOVE: function (e) {
        let { userId: t, guildEventId: n, guildEventExceptionId: r, response: i } = e;
        S({ user_id: t, guild_scheduled_event_id: n, guild_scheduled_event_exception_id: r, response: i });
    },
    GUILD_SCHEDULED_EVENT_RSVPS_FETCH_SUCESS: function (e) {
        let { guildScheduledEventUsers: t } = e;
        return t.forEach((e) => T(e, !1, !1)), (E += 1), !0;
    },
    GUILD_SCHEDULED_EVENT_USERS_FETCH_SUCCESS: function (e) {
        let { guildScheduledEventUsers: t } = e;
        return t.forEach((e) => T(e, !1, !1)), (E += 1), !0;
    },
    GUILD_SCHEDULED_EVENT_USER_COUNTS_FETCH_SUCCESS: function (e) {
        let { eventId: t, counts: n } = e;
        y(t, null, n.eventCount),
            d.default.forEachKey(n.recurrenceCounts, (e) => {
                y(t, e, n.eventCount - n.recurrenceCounts[e]);
            });
    },
    INVITE_RESOLVE_SUCCESS: function (e) {
        let { invite: t } = e,
            n = t.guild_scheduled_event;
        return null != n && (A(n), !0);
    },
    GUILD_SCHEDULED_EVENT_EXCEPTION_CREATE: R,
    GUILD_SCHEDULED_EVENT_EXCEPTION_UPDATE: R,
    GUILD_SCHEDULED_EVENT_EXCEPTION_DELETE: function (e) {
        let { eventException: t } = e,
            n = f.get(t.event_id);
        if (null == n) return !1;
        let r = n.guild_scheduled_event_exceptions.filter((e) => e.event_exception_id !== t.event_exception_id);
        return A({ ...n, guild_scheduled_event_exceptions: r }), !0;
    },
    GUILD_SCHEDULED_EVENT_EXCEPTIONS_DELETE: function (e) {
        let { eventId: t } = e,
            n = f.get(t);
        return null != n && (A({ ...n, guild_scheduled_event_exceptions: [] }), !0);
    },
    LOGOUT: function () {
        return f.clear(), !0;
    },
});
