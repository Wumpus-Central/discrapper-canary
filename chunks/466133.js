...
let E = (e) => {
    let { location: t } = e,
        ...
        x = (0, h.Jp)("RSL - Landing Page"),
        [I, v] = s.useState(!1),
        [N, j] = s.useState(A.intl.string(A.t["9exy+V"])),
        [C, T] = s.useState(!0),
        y = (e) => {
            switch (e) {
                case p.t02.INVALID_FORM_BODY:
                case p.t02.DSA_RSL_REPORT_NOT_FOUND:
                    j(A.intl.string(A.t.bzXDfc));
                    break;
                case p.t02.DSA_RSL_ALREADY_REQUESTED:
                    j(A.intl.string(A.t.rV00wq));
                    break;
                case p.t02.DSA_RSL_LIMITED_TIME:
                    j(A.intl.string(A.t["0dI29h"]));
                    break;
                case p.t02.DSA_RSL_REPORT_INELIGIBLE:
        ...
        s.useEffect(() => {
            let e = async (e) => {
                try {
                    ...
                    null != t ? j(A.intl.string(A.t.e6mZMt)) : y(t.body?.code);
                } catch (e) {
                    y(e.body?.code);
                } finally {
                    v(!1);
