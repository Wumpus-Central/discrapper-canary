...
class O {
    ...
    reset() {
        ...
        let a = r().map(
            d.A.getSuggestions(),
            (e) =>
                new y({ key: e.key, userId: e.key, type: 99, nickname: e.name, ...S(e.key), ...N(e.key), ...C(e.key) }),
        );
        return new O(r().concat(t, n, a));
    }
    clone() {
...
function P() {
    ...
    R && (e || (b !== I.m3P.ONLINE && b !== I.m3P.ADD_FRIEND)) && !v && ((R = !1), (v = !0), o.A.fetchRelationships());
}
function M() {
    if (((R = !0), D ? (v = !1) : P(), (L = L.reset()), w)) return;
...
class k extends s.Ay.Store {
    ...
    initialize() {
        this.waitFor(c.A, f.A, u.A, d.A, _.A, E.Ay, p.A, h.Ay, m.A, g.A, A.default),
            this.syncWith([g.A], x),
            this.syncWith([_.A], x),
            this.syncWith([d.A], x),
            this.syncWith([h.Ay], x),
            this.syncWith([A.default], U(S)),
            this.syncWith([m.A, f.A], U(N)),
            M();
    }
    getState() {
        return { fetching: v, section: b, rows: L };
    }
}
let G = new k(a.h, {
