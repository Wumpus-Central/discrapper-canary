    ...
    R = (e, r) => {
        ...
        (t.onload = (t) => {
            null == t.target || "string" != typeof t.target.result
                ? b("Error uploading file. Try again!")
                : r(e, t.target.result);
        }),
            t.readAsDataURL(e);
    },
    ...
    N = async (e, r, t) => {
        for (let a of await T(e))
            if (a.isFile) {
                ...
                let e = await D(a);
                t.ignoredFilenames.push(...e.map((e) => w(a.name, e.name)));
            }
    },
    P = async (e, r) => {
        let t = e.name,
    ...
    U = async (e, r) => {
        for (let t of await T(e))
            t.isDirectory
                ...
                : t.isFile && t.name !== g && r.ignoredFilenames.push(w(e.name, t.name));
    },
    S = async (e, r) => {
        for (let t of await T(e))
            if (t.isDirectory) {
                ...
                if ("collection" === e.name) await N(e, "collection", r);
                    ...
                    let t = await D(e);
                    r.ignoredFilenames.push(...t.map((r) => w(e.name, r.name)));
                }
            }
    },
    I = async (e) => {
