...
class u {
    ...
    _trackAppBackgrounded(e) {
        this._isBackgrounded === e ||
            ...
            this._isBackgrounded &&
                ...
                (this.telemetry.track(s.KQ.SKIP_IDLE_CALLBACK_DUE_TO_BACKGROUNDED), this._processWorkCallback()));
    }
    _queueIdleCallback() {
    ...
    _processWorkCallback(e) {
        ...
        if (this._hasExceededMaxConsecutiveFlushes) {
            l.log("Unable to fully flush work queue after max retries, skipping future deadline."),
                ...
                this.telemetry.track(s.KQ.EXCEEDED_MAX_CONSECUTIVE_FLUSHES),
    ...
    requestWorkTimeout(e) {
        ...
        if (((this._workCallbackFn = e), !this.hasWorkScheduled)) {
            if (
                (this.telemetry.time(s.YE.TIME_TO_QUEUE_EMPTY),
                this._nextDispatchTimeout === o.ct && this.telemetry.track(s.KQ.LONGER_DISPATCH),
                t)
            )
