"use strict";
n.d(t, {
    C$: () => A,
    S3: () => g,
    cl: () => d.clD,
    i0: () => b,
    nW: () => N,
    p8: () => C,
    pF: () => v,
    v: () => y,
    yi: () => E,
});
var r = n(627968);
n(64700);
var i = n(503698),
    a = n.n(i),
    s = n(563495),
    o = n(827734),
    l = n(582754);
n(397927);
var u = n(749401),
    c = n(573435),
    d = n(652215),
    _ = n(778712),
    f = n(519626);
let h = 8;
function p(e) {
    switch (e) {
        case d.clD.ONLINE:
            return o.A.unsafe_rawColors.GREEN_NEW_38;
        case d.clD.IDLE:
            return o.A.unsafe_rawColors.YELLOW_NEW_30;
        case d.clD.DND:
            return o.A.unsafe_rawColors.RED_NEW_46;
        case d.clD.STREAMING:
            return o.A.unsafe_rawColors.TWITCH;
        case d.clD.INVISIBLE:
        case d.clD.UNKNOWN:
        case d.clD.OFFLINE:
        default:
            return o.A.unsafe_rawColors.NEUTRAL_34;
    }
}
function g(e, t) {
    let n = p(e).resolve({ saturation: 1 }).hex(),
        r = t ?? n;
    return (0, u.a)(r) ?? void 0;
}
function E(e, t) {
    return t === _.F_.WHITE && e === d.clD.IDLE ? _.Iq.HIGH : _.Iq.LOW;
}
function A(e) {
    return (0, l.Mw)(e) ? _.F_.BLACK : _.F_.WHITE;
}
function I(e, t, n, r) {
    return `scale(${t}) translate(${0.5625 * e + r} ${0.75 * e + n})`;
}
function T(e, t, n) {
    return `${0.5625 * e + n}px ${0.75 * e + t}px`;
}
function y(e) {
    let { size: t, status: n, isMobile: r, isTyping: i, topOffset: a = 0, leftOffset: s = 0 } = e,
        o = (a / h) * t,
        l = (s / h) * t;
    if (i)
        return {
            bgRadius: 0.5 * t,
            bgY: 0.25 * t + o,
            bgX: 0,
            bgHeight: t,
            bgWidth: 2.5 * t,
            cutoutX: 0.5 * t + l,
            cutoutY: 0.75 * t + o,
            cutoutWidth: 0,
            cutoutHeight: 0,
            cutoutRadius: 0,
            polygonScale: I(t, 0, o, l),
            polygonOrigin: T(t, o, l),
            dotY: 0.75 * t + o,
            dotX: 0.5 * t + l,
            dotRadius: 0,
        };
    switch (n) {
        case d.clD.ONLINE:
            if (r)
                return {
                    bgRadius: 1.5 * t * 0.125,
                    bgY: 0,
                    bgX: l,
                    bgHeight: 1.5 * t,
                    bgWidth: t,
                    cutoutX: 0.125 * t + l,
                    cutoutY: 0.25 * t,
                    cutoutWidth: 0.75 * t,
                    cutoutHeight: 0.75 * t,
                    cutoutRadius: 0,
                    polygonScale: I(t, 0, o, l),
                    polygonOrigin: T(t, o, l),
                    dotY: 1.25 * t,
                    dotX: 0.5 * t + l,
                    dotRadius: 0.125 * t,
                };
            return {
                bgRadius: 0.5 * t,
                bgY: 0.25 * t + o,
                bgX: l,
                bgHeight: t,
                bgWidth: t,
                cutoutX: 0.5 * t + l,
                cutoutY: 0.75 * t + o,
                cutoutWidth: 0,
                cutoutHeight: 0,
                cutoutRadius: 0,
                polygonScale: I(t, 0, o, l),
                polygonOrigin: T(t, o, l),
                dotY: 0.75 * t + o,
                dotX: 0.5 * t + l,
                dotRadius: 0,
            };
        case d.clD.IDLE:
            return {
                bgRadius: 0.5 * t,
                bgY: 0.25 * t + o,
                bgX: l,
                bgHeight: t,
                bgWidth: t,
                cutoutX: -(0.125 * t) + l,
                cutoutY: 0.125 * t + o,
                cutoutWidth: 0.75 * t,
                cutoutHeight: 0.75 * t,
                cutoutRadius: 0.375 * t,
                polygonScale: I(t, 0, o, l),
                polygonOrigin: T(t, o, l),
                dotY: 0.75 * t + o,
                dotX: 0.5 * t + l,
                dotRadius: 0,
            };
        case d.clD.DND:
            return {
                bgRadius: 0.5 * t,
                bgY: 0.25 * t + o,
                bgX: l,
                bgHeight: t,
                bgWidth: t,
                cutoutX: 0.125 * t + l,
                cutoutY: 0.625 * t + o,
                cutoutWidth: 0.75 * t,
                cutoutHeight: 0.25 * t,
                cutoutRadius: 0.125 * t,
                polygonScale: I(t, 0, o, l),
                polygonOrigin: T(t, o, l),
                dotY: 0.75 * t + o,
                dotX: 0.5 * t + l,
                dotRadius: 0,
            };
        case d.clD.STREAMING:
            return {
                bgRadius: 0.5 * t,
                bgY: 0.25 * t + o,
                bgX: l,
                bgHeight: t,
                bgWidth: t,
                cutoutX: 0.5 * t + l,
                cutoutY: 0.75 * t + o,
                cutoutWidth: 0,
                cutoutHeight: 0,
                cutoutRadius: 0.25 * t,
                polygonScale: I(t, 1, o, l),
                polygonOrigin: T(t, o, l),
                dotY: 0.75 * t + o,
                dotX: 0.5 * t + l,
                dotRadius: 0,
            };
        case d.clD.INVISIBLE:
        case d.clD.UNKNOWN:
        case d.clD.OFFLINE:
        default:
            return {
                bgRadius: 0.5 * t,
                bgY: 0.25 * t + o,
                bgX: l,
                bgHeight: t,
                bgWidth: t,
                cutoutX: 0.25 * t + l,
                cutoutY: 0.5 * t + o,
                cutoutWidth: 0.5 * t,
                cutoutHeight: 0.5 * t,
                cutoutRadius: 0.25 * t,
                polygonScale: I(t, 0, o, l),
                polygonOrigin: T(t, o, l),
                dotY: 0.75 * t + o,
                dotX: 0.5 * t + l,
                dotRadius: 0,
            };
    }
}
function S(e) {
    return `-${0.216506 * e},-${0.25 * e} ${0.216506 * e},0 -${0.216506 * e},${0.25 * e}`;
}
function v(e, t, n) {
    let {
        bgRadius: i,
        bgY: a,
        bgX: o,
        bgHeight: l,
        bgWidth: u,
        cutoutX: c,
        cutoutY: d,
        cutoutWidth: _,
        cutoutHeight: f,
        cutoutRadius: h,
        polygonScale: p,
        polygonOrigin: g,
        dotY: E,
        dotX: A,
        dotRadius: I,
    } = e;
    return (0, r.jsxs)("mask", {
        id: n,
        children: [
            (0, r.jsx)(s.animated.rect, { x: o, y: a, width: u, height: l, rx: i, ry: i, fill: "white" }),
            (0, r.jsx)(s.animated.rect, { x: c, y: d, width: _, height: f, rx: h, ry: h, fill: "black" }),
            (0, r.jsx)(s.animated.polygon, {
                points: S(t),
                fill: "black",
                transform: p,
                style: { transformOrigin: g },
            }),
            (0, r.jsx)(s.animated.circle, { fill: "black", cx: A, cy: E, r: I }),
        ],
    });
}
function C(e, t) {
    let { isMobile: n, isTyping: r, isVR: i } = t;
    if (r) return c.Ay.Masks.STATUS_TYPING;
    switch (e) {
        case d.clD.IDLE:
            return c.Ay.Masks.STATUS_IDLE;
        case d.clD.DND:
            return c.Ay.Masks.STATUS_DND;
        case d.clD.STREAMING:
            return c.Ay.Masks.STATUS_STREAMING;
        case d.clD.INVISIBLE:
        case d.clD.UNKNOWN:
        case d.clD.OFFLINE:
            return c.Ay.Masks.STATUS_OFFLINE;
        case d.clD.ONLINE:
        default:
            if (i) return c.Ay.Masks.STATUS_ONLINE_VR;
            if (n) return c.Ay.Masks.STATUS_ONLINE_MOBILE;
            return c.Ay.Masks.STATUS_ONLINE;
    }
}
function b(e, t, n) {
    let { isMobile: r, isTyping: i, isVR: a } = n;
    return i
        ? { width: 2.5 * e, height: e }
        : t === d.clD.ONLINE && r && !a
          ? { width: e, height: 1.5 * e }
          : { width: e, height: e };
}
function N(e) {
    let { status: t, isMobile: n = !1, isVR: i = !1, size: s = h, className: o, style: l, color: u } = e,
        _ = t === d.clD.ONLINE && n,
        p = t === d.clD.ONLINE && i,
        E = g(t, u);
    return (0, r.jsx)(c.Ay, {
        mask: C(t, { isMobile: _, isTyping: !1, isVR: p }),
        className: a()(f.d, o),
        style: l,
        ...b(s, t, { isMobile: _, isTyping: !1, isVR: p }),
        children: (0, r.jsx)("div", { style: { backgroundColor: E }, className: f.h }),
    });
}
