n.d(t, { A: () => T, n: () => m });
var l = n(627968);
n(64700);
var r = n(192308),
    i = n(228366),
    a = n(272355),
    s = n(267102),
    o = n(287809),
    u = n(207803),
    c = n(958805),
    d = n(591179),
    f = n(485745),
    h = n(652215);
let A = new Set();
function m(e, t) {
    return `USER_PROFILE_MODAL_KEY:${e}:${null == t ? "" : t}`;
}
async function S(e) {
    let {
            userId: t,
            tabSection: i,
            scrollTarget: a,
            guildId: S,
            originGuildId: g,
            channelId: x,
            appContext: T,
            customStatusPrompt: E,
            onModalOpen: p,
            ...R
        } = e,
        v = o.default.getUser(t);
    if (null == v) return;
    let y = o.default.getCurrentUser();
    if (null == y) return;
    let C = m(t, S);
    A.add(
        await (0, r.openModalLazy)(
            async () => {
                let e = (
                    await Promise.all([
                        n.e("4803"),
                        n.e("70843"),
                        n.e("69944"),
                        n.e("59012"),
                        n.e("91555"),
                        n.e("47173"),
                        n.e("60361"),
                        n.e("49451"),
                        n.e("48626"),
                        n.e("92417"),
                        n.e("58127"),
                        n.e("14603"),
                        n.e("36252"),
                        n.e("28041"),
                        n.e("79249"),
                        n.e("62867"),
                        n.e("30967"),
                        n.e("27019"),
                        n.e("65264"),
                        n.e("15750"),
                        n.e("29205"),
                        n.e("10931"),
                        n.e("73435"),
                        n.e("996"),
                        n.e("14211"),
                        n.e("30170"),
                        n.e("59252"),
                        n.e("72197"),
                        n.e("90266"),
                        n.e("76744"),
                        n.e("10949"),
                        n.e("68501"),
                        n.e("71519"),
                        n.e("42335"),
                        n.e("57507"),
                        n.e("16334"),
                        n.e("1744"),
                        n.e("71406"),
                        n.e("56985"),
                        n.e("81619"),
                        n.e("67039"),
                        n.e("72651"),
                        n.e("29363"),
                        n.e("16881"),
                        n.e("17268"),
                        n.e("69979"),
                        n.e("30593"),
                        n.e("86122"),
                        n.e("20056"),
                        n.e("14831"),
                        n.e("25821"),
                        n.e("90645"),
                        n.e("44400"),
                        n.e("25781"),
                        n.e("86832"),
                        n.e("76342"),
                        n.e("60988"),
                        n.e("69273"),
                        n.e("72210"),
                        n.e("3155"),
                        n.e("82104"),
                        n.e("41711"),
                        n.e("49623"),
                        n.e("35316"),
                        n.e("38838"),
                        n.e("45214"),
                        n.e("99253"),
                        n.e("58846"),
                        n.e("17202"),
                        n.e("90862"),
                        n.e("15476"),
                        n.e("97189"),
                        n.e("42791"),
                        n.e("16918"),
                        n.e("33730"),
                        n.e("24059"),
                        n.e("31837"),
                        n.e("42306"),
                        n.e("11291"),
                        n.e("96846"),
                        n.e("19709"),
                        n.e("4251"),
                        n.e("47412"),
                        n.e("65327"),
                        n.e("41625"),
                        n.e("96490"),
                        n.e("85484"),
                        n.e("51772"),
                        n.e("6605"),
                        n.e("11262"),
                        n.e("54483"),
                        n.e("62461"),
                        n.e("25677"),
                        n.e("73679"),
                        n.e("73178"),
                        n.e("333"),
                        n.e("97638"),
                        n.e("22995"),
                        n.e("59096"),
                        n.e("55953"),
                        n.e("56386"),
                        n.e("38759"),
                        n.e("51943"),
                        n.e("90072"),
                        n.e("88016"),
                        n.e("74907"),
                        n.e("84133"),
                        n.e("13627"),
                        n.e("57940"),
                        n.e("58171"),
                        n.e("44511"),
                        n.e("39375"),
                        n.e("47944"),
                        n.e("84473"),
                        n.e("70990"),
                        n.e("15130"),
                        n.e("80816"),
                        n.e("74051"),
                        n.e("50541"),
                        n.e("39156"),
                        n.e("62448"),
                        n.e("38123"),
                        n.e("56795"),
                        n.e("85723"),
                        n.e("62646"),
                        n.e("21377"),
                        n.e("53702"),
                        n.e("12375"),
                        n.e("25711"),
                        n.e("44671"),
                        n.e("53417"),
                        n.e("23549"),
                        n.e("23924"),
                        n.e("42205"),
                        n.e("55675"),
                        n.e("48226"),
                        n.e("36167"),
                        n.e("68201"),
                        n.e("87183"),
                        n.e("34786"),
                        n.e("97452"),
                        n.e("10471"),
                        n.e("64827"),
                        n.e("63232"),
                        n.e("68261"),
                        n.e("7167"),
                        n.e("12721"),
                        n.e("84569"),
                        n.e("60235"),
                        n.e("61737"),
                        n.e("92789"),
                        n.e("21335"),
                        n.e("77333"),
                        n.e("47834"),
                        n.e("11735"),
                        n.e("85927"),
                        n.e("834"),
                        n.e("2368"),
                        n.e("33115"),
                        n.e("6338"),
                        n.e("18573"),
                        n.e("78157"),
                        n.e("55936"),
                        n.e("85404"),
                        n.e("44057"),
                        n.e("1453"),
                        n.e("88774"),
                        n.e("10120"),
                        n.e("62175"),
                        n.e("31988"),
                        n.e("44571"),
                        n.e("34303"),
                        n.e("97270"),
                        n.e("45959"),
                        n.e("73122"),
                        n.e("8304"),
                        n.e("93159"),
                        n.e("71202"),
                        n.e("8739"),
                        n.e("42834"),
                        n.e("88131"),
                        n.e("43430"),
                        n.e("94723"),
                        n.e("62931"),
                        n.e("58529"),
                        n.e("58038"),
                        n.e("62531"),
                        n.e("22103"),
                        n.e("6174"),
                        n.e("93103"),
                        n.e("34552"),
                        n.e("9233"),
                        n.e("72963"),
                        n.e("21402"),
                        n.e("8757"),
                        n.e("7575"),
                        n.e("24303"),
                        n.e("85968"),
                        n.e("88599"),
                        n.e("75842"),
                        n.e("68403"),
                        n.e("32191"),
                        n.e("71273"),
                        n.e("29787"),
                        n.e("9640"),
                        n.e("8891"),
                        n.e("52548"),
                        n.e("53984"),
                        n.e("43780"),
                        n.e("54625"),
                        n.e("28662"),
                        n.e("77084"),
                        n.e("76273"),
                        n.e("23532"),
                        n.e("52557"),
                        n.e("55654"),
                        n.e("76428"),
                        n.e("48900"),
                        n.e("35485"),
                        n.e("83161"),
                        n.e("38513"),
                        n.e("63645"),
                        n.e("63123"),
                        n.e("24038"),
                        n.e("37687"),
                        n.e("52229"),
                        n.e("34691"),
                        n.e("95340"),
                        n.e("27048"),
                        n.e("60177"),
                        n.e("77473"),
                        n.e("63618"),
                        n.e("1462"),
                        n.e("28752"),
                        n.e("2329"),
                        n.e("35996"),
                        n.e("61268"),
                        n.e("40362"),
                        n.e("25279"),
                        n.e("88003"),
                        n.e("78412"),
                        n.e("88990"),
                        n.e("9793"),
                        n.e("30938"),
                        n.e("43549"),
                        n.e("54067"),
                        n.e("97288"),
                        n.e("10575"),
                        n.e("19666"),
                        n.e("91254"),
                        n.e("54630"),
                        n.e("42489"),
                        n.e("21574"),
                        n.e("31445"),
                        n.e("59880"),
                        n.e("6723"),
                        n.e("9729"),
                        n.e("60658"),
                        n.e("81987"),
                        n.e("21856"),
                        n.e("5563"),
                        n.e("27323"),
                        n.e("69747"),
                        n.e("65826"),
                        n.e("49520"),
                        n.e("14461"),
                        n.e("72883"),
                        n.e("54241"),
                        n.e("20683"),
                        n.e("25486"),
                        n.e("21930"),
                        n.e("49644"),
                        n.e("3663"),
                        n.e("20009"),
                        n.e("72877"),
                        n.e("11133"),
                        n.e("89523"),
                        n.e("29260"),
                        n.e("82486"),
                        n.e("21060"),
                        n.e("4098"),
                        n.e("755"),
                        n.e("42191"),
                        n.e("13708"),
                        n.e("97069"),
                        n.e("607"),
                        n.e("63333"),
                        n.e("32207"),
                        n.e("29413"),
                        n.e("38042"),
                        n.e("41786"),
                        n.e("22067"),
                        n.e("55266"),
                        n.e("94138"),
                        n.e("72103"),
                        n.e("46430"),
                        n.e("38090"),
                        n.e("49333"),
                        n.e("17460"),
                        n.e("86262"),
                        n.e("80077"),
                        n.e("3168"),
                        n.e("30878"),
                        n.e("69073"),
                        n.e("58262"),
                        n.e("49490"),
                        n.e("90779"),
                        n.e("4374"),
                        n.e("86546"),
                        n.e("82969"),
                        n.e("9528"),
                        n.e("39808"),
                        n.e("95841"),
                        n.e("48407"),
                        n.e("54791"),
                        n.e("37065"),
                        n.e("36926"),
                        n.e("43256"),
                        n.e("66147"),
                        n.e("41384"),
                        n.e("86507"),
                        n.e("156"),
                        n.e("73566"),
                        n.e("45419"),
                        n.e("20491"),
                        n.e("80262"),
                        n.e("36973"),
                        n.e("87791"),
                        n.e("78377"),
                        n.e("70272"),
                        n.e("53683"),
                        n.e("84601"),
                        n.e("47339"),
                        n.e("19075"),
                        n.e("92638"),
                        n.e("92992"),
                        n.e("86470"),
                        n.e("53308"),
                        n.e("55552"),
                        n.e("78421"),
                        n.e("54876"),
                        n.e("79211"),
                        n.e("84050"),
                        n.e("90590"),
                        n.e("14501"),
                        n.e("36467"),
                        n.e("27880"),
                        n.e("67685"),
                        n.e("17298"),
                        n.e("8032"),
                        n.e("77970"),
                        n.e("14248"),
                        n.e("78"),
                        n.e("75514"),
                        n.e("55482"),
                        n.e("72163"),
                        n.e("57931"),
                        n.e("71367"),
                        n.e("75072"),
                        n.e("32498"),
                        n.e("17583"),
                        n.e("36674"),
                        n.e("9856"),
                        n.e("31694"),
                        n.e("63363"),
                        n.e("49282"),
                        n.e("35683"),
                        n.e("2091"),
                        n.e("54134"),
                    ]).then(n.bind(n, 577593))
                ).default;
                return (t) =>
                    (0, l.jsx)(e, {
                        user: v,
                        currentUser: y,
                        guildId: S,
                        originGuildId: g ?? S,
                        initialTabSection: i,
                        initialScrollTarget: a,
                        channelId: x,
                        customStatusPrompt: E,
                        ...t,
                        ...R,
                    });
            },
            {
                modalKey: C,
                contextKey: (0, r.modalContextFromAppContext)(T ?? (0, s.zd)() ?? h.BRT.APP),
                onCloseRequest: () => {
                    let e = !(0, d.E)("UserProfileModalManager");
                    t === y.id && (0, f.b)(e)
                        ? (0, u.VQ)()
                        : ((0, r.closeModal)(C), A.delete(C), c.A.clearPendingWidgets(), e || (0, u.XQ)());
                },
            },
        ),
    ),
        p?.();
}
function g() {
    if (0 !== A.size) {
        for (let e of A) (0, r.closeModal)(e);
        A.clear(), c.A.clearPendingWidgets(), (0, d.E)("UserProfileModalManager") && (0, u.XQ)();
    }
}
class x extends a.A {
    _initialize() {
        i.h.subscribe("USER_PROFILE_MODAL_OPEN", S), i.h.subscribe("USER_PROFILE_MODAL_CLOSE", g);
    }
    _terminate() {
        i.h.unsubscribe("USER_PROFILE_MODAL_OPEN", S), i.h.unsubscribe("USER_PROFILE_MODAL_CLOSE", g);
    }
}
let T = new x();
