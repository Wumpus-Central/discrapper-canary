n.d(t, { A: () => T, n: () => m });
var l = n(627968);
n(64700);
var r = n(192308),
    a = n(228366),
    i = n(272355),
    s = n(267102),
    o = n(287809),
    u = n(207803),
    c = n(958805),
    d = n(591179),
    f = n(485745),
    h = n(652215);
let A = new Set();
function m(e, t) {
    return `USER_PROFILE_MODAL_KEY:${e}:${null == t ? "" : t}`;
}
async function x(e) {
    let {
            userId: t,
            tabSection: a,
            scrollTarget: i,
            guildId: x,
            originGuildId: g,
            channelId: S,
            appContext: T,
            customStatusPrompt: E,
            ...p
        } = e,
        v = o.default.getUser(t);
    if (null == v) return;
    let R = o.default.getCurrentUser();
    if (null == R) return;
    let y = m(t, x);
    A.add(
        await (0, r.openModalLazy)(
            async () => {
                let e = (
                    await Promise.all([
                        n.e("23495"),
                        n.e("76631"),
                        n.e("41475"),
                        n.e("87028"),
                        n.e("28130"),
                        n.e("33063"),
                        n.e("89412"),
                        n.e("45043"),
                        n.e("21277"),
                        n.e("18240"),
                        n.e("17095"),
                        n.e("73435"),
                        n.e("26943"),
                        n.e("70294"),
                        n.e("4460"),
                        n.e("47048"),
                        n.e("22274"),
                        n.e("2677"),
                        n.e("24410"),
                        n.e("81804"),
                        n.e("19393"),
                        n.e("4355"),
                        n.e("84764"),
                        n.e("72451"),
                        n.e("11819"),
                        n.e("30260"),
                        n.e("46334"),
                        n.e("26963"),
                        n.e("38805"),
                        n.e("9662"),
                        n.e("80653"),
                        n.e("51692"),
                        n.e("8981"),
                        n.e("91888"),
                        n.e("6324"),
                        n.e("96575"),
                        n.e("6162"),
                        n.e("13627"),
                        n.e("50748"),
                        n.e("38123"),
                        n.e("48898"),
                        n.e("38804"),
                        n.e("23924"),
                        n.e("42205"),
                        n.e("71091"),
                        n.e("68201"),
                        n.e("34786"),
                        n.e("10471"),
                        n.e("63232"),
                        n.e("11301"),
                        n.e("61737"),
                        n.e("60235"),
                        n.e("68261"),
                        n.e("64827"),
                        n.e("7167"),
                        n.e("96643"),
                        n.e("92789"),
                        n.e("2368"),
                        n.e("21335"),
                        n.e("47834"),
                        n.e("11735"),
                        n.e("3932"),
                        n.e("84569"),
                        n.e("62156"),
                        n.e("6338"),
                        n.e("85404"),
                        n.e("44057"),
                        n.e("19455"),
                        n.e("28504"),
                        n.e("38251"),
                        n.e("62175"),
                        n.e("7743"),
                        n.e("44571"),
                        n.e("45959"),
                        n.e("60658"),
                        n.e("8304"),
                        n.e("18573"),
                        n.e("55936"),
                        n.e("8739"),
                        n.e("8434"),
                        n.e("31988"),
                        n.e("39667"),
                        n.e("34303"),
                        n.e("28866"),
                        n.e("2419"),
                        n.e("73122"),
                        n.e("58038"),
                        n.e("93159"),
                        n.e("25246"),
                        n.e("13444"),
                        n.e("94723"),
                        n.e("62931"),
                        n.e("50417"),
                        n.e("9907"),
                        n.e("6174"),
                        n.e("34552"),
                        n.e("93103"),
                        n.e("34530"),
                        n.e("8757"),
                        n.e("21921"),
                        n.e("9233"),
                        n.e("72963"),
                        n.e("85968"),
                        n.e("68403"),
                        n.e("29787"),
                        n.e("88599"),
                        n.e("75842"),
                        n.e("8891"),
                        n.e("71273"),
                        n.e("9640"),
                        n.e("98765"),
                        n.e("52548"),
                        n.e("28083"),
                        n.e("53984"),
                        n.e("43780"),
                        n.e("28662"),
                        n.e("77084"),
                        n.e("76273"),
                        n.e("75361"),
                        n.e("76428"),
                        n.e("23532"),
                        n.e("52557"),
                        n.e("37687"),
                        n.e("48900"),
                        n.e("4450"),
                        n.e("60177"),
                        n.e("5636"),
                        n.e("33861"),
                        n.e("68230"),
                        n.e("23276"),
                        n.e("77473"),
                        n.e("63618"),
                        n.e("58984"),
                        n.e("78412"),
                        n.e("63213"),
                        n.e("30938"),
                        n.e("17859"),
                        n.e("48720"),
                        n.e("25279"),
                        n.e("72539"),
                        n.e("18997"),
                        n.e("88003"),
                        n.e("56689"),
                        n.e("75016"),
                        n.e("2329"),
                        n.e("59880"),
                        n.e("74016"),
                        n.e("52229"),
                        n.e("95340"),
                        n.e("12168"),
                        n.e("77333"),
                        n.e("28752"),
                        n.e("33134"),
                        n.e("80112"),
                        n.e("74907"),
                        n.e("6723"),
                        n.e("54791"),
                        n.e("21856"),
                        n.e("19666"),
                        n.e("21574"),
                        n.e("31445"),
                        n.e("14461"),
                        n.e("8694"),
                        n.e("81987"),
                        n.e("8948"),
                        n.e("36877"),
                        n.e("65826"),
                        n.e("49520"),
                        n.e("69747"),
                        n.e("72883"),
                        n.e("25486"),
                        n.e("61268"),
                        n.e("49644"),
                        n.e("38501"),
                        n.e("27323"),
                        n.e("72877"),
                        n.e("21930"),
                        n.e("3930"),
                        n.e("3663"),
                        n.e("20683"),
                        n.e("92077"),
                        n.e("42191"),
                        n.e("63333"),
                        n.e("89523"),
                        n.e("29260"),
                        n.e("97069"),
                        n.e("82486"),
                        n.e("21060"),
                        n.e("32207"),
                        n.e("88889"),
                        n.e("755"),
                        n.e("29413"),
                        n.e("38042"),
                        n.e("11133"),
                        n.e("607"),
                        n.e("13708"),
                        n.e("17460"),
                        n.e("54241"),
                        n.e("22067"),
                        n.e("41786"),
                        n.e("55266"),
                        n.e("94138"),
                        n.e("93461"),
                        n.e("96574"),
                        n.e("46430"),
                        n.e("49333"),
                        n.e("86262"),
                        n.e("27752"),
                        n.e("58262"),
                        n.e("49490"),
                        n.e("90779"),
                        n.e("38090"),
                        n.e("36674"),
                        n.e("4374"),
                        n.e("86546"),
                        n.e("3168"),
                        n.e("82969"),
                        n.e("55895"),
                        n.e("39808"),
                        n.e("31616"),
                        n.e("95841"),
                        n.e("20491"),
                        n.e("37065"),
                        n.e("36926"),
                        n.e("43256"),
                        n.e("41384"),
                        n.e("43372"),
                        n.e("19119"),
                        n.e("73566"),
                        n.e("45419"),
                        n.e("87791"),
                        n.e("36973"),
                        n.e("78377"),
                        n.e("70272"),
                        n.e("53683"),
                        n.e("84601"),
                        n.e("47339"),
                        n.e("9598"),
                        n.e("1719"),
                        n.e("45879"),
                        n.e("97150"),
                        n.e("86470"),
                        n.e("55552"),
                        n.e("53308"),
                        n.e("80262"),
                        n.e("21351"),
                        n.e("54876"),
                        n.e("79211"),
                        n.e("78"),
                        n.e("40042"),
                        n.e("27870"),
                        n.e("27880"),
                        n.e("67685"),
                        n.e("8032"),
                        n.e("58272"),
                        n.e("77970"),
                        n.e("92405"),
                        n.e("787"),
                        n.e("72163"),
                        n.e("57931"),
                        n.e("75072"),
                        n.e("17583"),
                        n.e("67876"),
                        n.e("49282"),
                        n.e("35683"),
                        n.e("94775"),
                        n.e("54134"),
                    ]).then(n.bind(n, 438857))
                ).default;
                return (t) =>
                    (0, l.jsx)(e, {
                        user: v,
                        currentUser: R,
                        guildId: x,
                        originGuildId: g ?? x,
                        initialTabSection: a,
                        initialScrollTarget: i,
                        channelId: S,
                        customStatusPrompt: E,
                        ...t,
                        ...p,
                    });
            },
            {
                modalKey: y,
                contextKey: (0, r.modalContextFromAppContext)(T ?? (0, s.zd)() ?? h.BRT.APP),
                onCloseRequest: () => {
                    let e = !(0, d.E)("UserProfileModalManager");
                    t === R.id && (0, f.b)(e)
                        ? (0, u.VQ)()
                        : ((0, r.closeModal)(y), A.delete(y), c.A.clearPendingWidgets(), e || (0, u.XQ)());
                },
            },
        ),
    );
}
function g() {
    if (0 !== A.size) {
        for (let e of A) (0, r.closeModal)(e);
        A.clear(), c.A.clearPendingWidgets(), (0, d.E)("UserProfileModalManager") && (0, u.XQ)();
    }
}
class S extends i.A {
    _initialize() {
        a.h.subscribe("USER_PROFILE_MODAL_OPEN", x), a.h.subscribe("USER_PROFILE_MODAL_CLOSE", g);
    }
    _terminate() {
        a.h.unsubscribe("USER_PROFILE_MODAL_OPEN", x), a.h.unsubscribe("USER_PROFILE_MODAL_CLOSE", g);
    }
}
let T = new S();
