r.d(t, { x: () => k, e: () => b });
var n = r(627968),
    a = r(64700),
    l = r(503698),
    s = r.n(l),
    i = r(785651),
    o = r(113325),
    c = r(315629),
    u = r(17928),
    d = r(453903),
    h = r(765671),
    p = r(186111),
    f = r(795127),
    m = r(844222),
    g = r(866323);
let x = { mass: 1, tension: 800, friction: 32 },
    v = { mass: 1, tension: 1200, friction: 48 };
var j = r(818348),
    C = r(824078);
let b = a.createContext(null),
    y = j.FX;
function k(e) {
    let {
            children: t,
            targetElementRef: r,
            shouldShow: l = !0,
            onRequestClose: j,
            position: k = "top",
            align: w,
            alignmentStrategy: N = "trigger-center",
            caretConfig: E,
            hasVideo: L = !1,
            gradientColor: R,
            gradientOffsetBottom: A = 0.4,
            onPositionChange: S,
            onNudgeChange: M,
            scrollBehavior: B = "sticky",
            modal: _ = !1,
            returnRef: T,
            experimental_ignoreModalClicks: $ = !0,
            closeOnClickOutside: Z = !1,
        } = e,
        [D, F] = a.useState(l),
        [I, P] = a.useState(k),
        O = a.useRef(k),
        Q = a.useRef(0),
        { ref: U, width: q, height: V } = (0, h.Ay)(),
        z = (0, u.bG)([p.A], () => p.A.getLayers()),
        G = z[z.length - 1] ?? "base",
        W = a.useRef(S);
    a.useEffect(() => {
        W.current = S;
    }, [S]);
    let J = a.useCallback((e) => {
            null != e && e !== O.current && ((O.current = e), P(e), W.current?.(e));
        }, []),
        H = a.useMemo(
            () => null == r.current || (r.current.closest("[data-layer]")?.getAttribute("data-layer") ?? "base") === G,
            [r, G],
        );
    a.useEffect(() => {
        H && l ? F(!0) : H || F(!1);
    }, [H, l]),
        a.useEffect(() => {
            J(k);
        }, [k, J]);
    let K = (function (e) {
            let { shouldShow: t, caretPosition: r, onExitComplete: n } = e,
                { reducedMotion: l } = a.useContext(m.C),
                { config: s, ...i } = ((e, t) => {
                    let r = { opacity: 0, transform: "" },
                        n = { opacity: 0, transform: "" };
                    if (t)
                        return {
                            from: { opacity: 0 },
                            enter: { opacity: 1 },
                            leave: { opacity: 0 },
                            config: { duration: 150 },
                        };
                    switch (e) {
                        case "top":
                            (r.transform = "translate3d(0, -10px, 0)"), (n.transform = "translate3d(0, -10px, 0)");
                            break;
                        case "bottom":
                            (r.transform = "translate3d(0, 10px, 0)"), (n.transform = "translate3d(0, 10px, 0)");
                            break;
                        case "left":
                            (r.transform = "translate3d(-10px, 0, 0)"), (n.transform = "translate3d(-10px, 0, 0)");
                            break;
                        case "right":
                            (r.transform = "translate3d(10px, 0, 0)"), (n.transform = "translate3d(10px, 0, 0)");
                    }
                    return {
                        from: r,
                        enter: { opacity: 1, transform: "translate3d(0px, 0px, 0)" },
                        leave: n,
                        config: void 0,
                    };
                })(r, l.enabled);
            return (0, g.p)(
                t,
                {
                    ...i,
                    config: s ?? ((e, r) => (t ? x : v)),
                    onRest: () => {
                        t || null == n || n();
                    },
                },
                "animate-always",
            );
        })({
            shouldShow: l,
            caretPosition: (0, f.g)(I),
            onExitComplete: () => {
                F(!1);
            },
        }),
        Y = a.useMemo(() => {
            if ("edge" === N && null != w) {
                let e = "top" === I || "bottom" === I,
                    t = "left" === I || "right" === I;
                if (e) {
                    if ("left" === w || "center" === w || "right" === w) return w;
                } else if (t && ("top" === w || "center" === w || "bottom" === w)) return w;
            }
            return "center";
        }, [N, w, I]),
        X = a.useMemo(() => {
            if ("edge" !== N)
                return (function () {
                    let e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : "center",
                        t = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : "top",
                        r = arguments.length > 2 ? arguments[2] : void 0,
                        n = arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : 0;
                    if ("center" === e || "custom" === e) return 0;
                    let a = "left" === t || "right" === t,
                        l = a ? n : (r ?? 240);
                    if (0 === l) return 0;
                    let s = l / 2 - (a ? 22 : 20);
                    switch (e) {
                        case "start":
                            return s;
                        case "end":
                            return -s;
                        default:
                            return 0;
                    }
                })(E?.align ?? "center", I, q, V);
        }, [N, E, I, q, V]),
        ee = a.useMemo(() => ({ position: I, caretConfig: E ?? { align: "center" } }), [I, E]);
    return (0, n.jsx)(d.$, {
        targetElementRef: r,
        shouldShow: D,
        onRequestClose: j,
        position: I,
        align: Y,
        spacing: 14,
        offset: X,
        layerContext: void 0,
        positionKey: null != X ? `${I}-${X}` : void 0,
        popoutKey: void 0,
        fixed: !1,
        autoInvert: !0,
        nudgeAlignIntoViewport: "top" === I || "bottom" === I,
        closeOnClickOutside: Z,
        ignoreModalClicks: $,
        scrollBehavior: B,
        renderPopout: (e) => {
            let { setPopoutRef: r, position: a, nudge: l, ...u } = e;
            return (
                J(a),
                l !== Q.current && ((Q.current = l), M?.(l)),
                K((e, a) => {
                    if (!a) return null;
                    let d = (0, n.jsx)(o.l, {
                        ...u,
                        setDialogRef: r,
                        modal: _,
                        className: s()(null != R ? C.popoverContentWithGradient : C.popover, {
                            [C["popover--video"]]: L,
                        }),
                        returnRef: T,
                        children: (0, n.jsx)(b.Provider, { value: ee, children: t }),
                    });
                    return (0, n.jsx)(i.animated.div, {
                        ref: U,
                        "data-mana-component": "popover",
                        style: {
                            ...e,
                            "--custom-caret-edge-offset-horizontal": "20px",
                            "--custom-caret-edge-offset-vertical": "22px",
                            "--custom-caret-edge-offset-horizontal-nudge": `${l}px`,
                            "--custom-popover-width": "240px",
                        },
                        children:
                            null != R
                                ? (0, n.jsx)(c.h, {
                                      offsetBottom: A,
                                      color: R,
                                      className: C.popoverGradientWrapper,
                                      children: d,
                                  })
                                : d,
                    });
                })
            );
        },
        children: y,
    });
}
