...
function v(e) {
    for (let t of (f.set(e.id, e), S.set(e.guild_id, e.application_id), e.subscription_listings ?? [])) C(t);
}
function C(e) {
    p.set(e.id, e);
}
...
let D = new b(l.h, {
    ...
    GUILD_ROLE_SUBSCRIPTIONS_FETCH_LISTINGS: function (e) {
        let { guildId: t } = e;
        for (let e of ((E[t] = 1), N(t))) for (let t of (f.delete(e.id), e.subscription_listings_ids)) p.delete(t);
    },
    ...
    GUILD_ROLE_SUBSCRIPTIONS_UPDATE_GROUP_LISTING: function (e) {
        let { listing: t } = e;
        v(t), null != t.benefit_channels && R(t.benefit_channels);
    },
