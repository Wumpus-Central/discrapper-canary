n.d(t, { A: () => E });
var i = n(627968),
    l = n(64700),
    a = n(17928),
    s = n(367513),
    r = n(401843),
    o = n(90804),
    c = n(313961),
    d = n(643501),
    u = n(279250),
    p = n(175203),
    h = n(734057),
    m = n(71393),
    A = n(576705),
    x = n(309010),
    g = n(977997),
    f = n(920639),
    C = n(806931),
    j = n(125730);
let y = [];
function v(e) {
    let { channelId: t, participant: n, popoutType: l } = e,
        y = (0, a.bG)([x.A], () => x.A.getVoiceChannelId() === t),
        v = (0, a.bG)([h.A], () => h.A.getChannel(t)),
        E = (0, a.bG)([c.A], () => c.A.getSelectedParticipant(t)),
        [N, I] = (0, a.yK)([g.A, m.A, A.A, d.default], () => (0, u.eo)(v, g.A, m.A, A.A, d.default));
    if (null == v) return null;
    let T = async () => {
        let e = y;
        e || (e = await (0, o.A)({ channelId: t })),
            e &&
                ((0, C.Ay)(n) && N && (0, r.A9)(n.stream, { forceMultiple: !0 }),
                E?.id === n.id
                    ? s.A.selectParticipant(v.id, null)
                    : (s.A.selectParticipant(v.id, n.id),
                      (0, f.n0)({ interactionType: "call_tile_expanded", channelId: t })));
    };
    return (0, i.jsx)(p.Ay, {
        participant: n,
        inCall: !0,
        channel: v,
        width: 100,
        popoutType: l,
        className: j.Vs,
        onClick: T,
    });
}
function E(e) {
    let { channelId: t, popoutType: n, aspectRatio: s } = e,
        r = (0, l.useRef)(null),
        [o, d] = (0, l.useState)(0),
        u = (0, a.bG)([x.A], () => x.A.getVoiceChannelId()),
        { streamParticipants: p, videoParticipants: h } = (0, a.cf)([c.A], () => ({
            streamParticipants: c.A.getStreamParticipants(t),
            videoParticipants: u === t ? c.A.getVideoParticipants(t) : y,
        }));
    return (
        (0, l.useEffect)(() => {
            function e() {
                r.current?.getBoundingClientRect().top != null && d(r.current?.getBoundingClientRect().top);
            }
            return e(), window.addEventListener("resize", e), () => window.removeEventListener("resize", e);
        }, []),
        (0, i.jsxs)("div", {
            className: j.kL,
            style: { aspectRatio: s, paddingTop: 82 > o + 16 ? 82 - o : 16 },
            ref: r,
            children: [
                p.map((e) => (0, i.jsx)(v, { participant: e, channelId: t, popoutType: n }, e.id)),
                h.map((e) => (0, i.jsx)(v, { participant: e, channelId: t, popoutType: n }, e.id)),
            ],
        })
    );
}
