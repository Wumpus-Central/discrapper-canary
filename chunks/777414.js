...
var r = n(835245),
    ...
    N = n(652215),
    v = n(672396);
let C = new u.A("OverlayUsageStatsManager");
C.verbose = () => {};
class O {
    actions = { [v.uj.Viewed]: 0, [v.uj.Clicked]: 0 };
    increment(e) {
        ++this.actions[e];
    }
    getAnalytics(e, t) {
        let n = this.actions[v.uj.Viewed],
            r = this.actions[v.uj.Clicked];
        return 0 === n && 0 === r ? null : { event_uuid: t, notification_type: e, viewed_count: n, clicked_count: r };
    }
}
class R {
    actionCounters = { [v.uj.Viewed]: 0, [v.uj.Clicked]: 0 };
    groupCounters = { [v.uj.Viewed]: R.makeEmptyGroupAnalytics(), [v.uj.Clicked]: R.makeEmptyGroupAnalytics() };
    counters = R.makeCounters();
    ...
    increment(e, t) {
        ...
        if (null == n) return void C.error(`NotificationCounter: Unknown notification action: ${t}`);
        let r = (0, v.Vo)(e);
        if (!(r in n)) return void C.error(`NotificationCounter: Unknown notification action: ${e}`);
        ++n[r], ++this.actionCounters[t];
        let i = this.counters[e];
        null == i ? C.error(`NotificationCounter: Unknown notification type: ${e}`) : i.increment(t);
    }
    getAnalytics() {
        let e = this.groupCounters[v.uj.Viewed],
            t = this.groupCounters[v.uj.Clicked];
        return {
            notices_viewed: this.actionCounters[v.uj.Viewed],
            notices_clicked: this.actionCounters[v.uj.Clicked],
            notice_nudge_viewed: e[v.BR.Nudge],
            notice_text_chat_viewed: e[v.BR.TextChat],
            notice_voice_call_viewed: e[v.BR.VoiceCall],
            notice_activity_viewed: e[v.BR.Activity],
            notice_clips_viewed: e[v.BR.Clips],
            notice_other_viewed: e[v.BR.Other],
            notice_nudge_clicked: t[v.BR.Nudge],
            notice_text_chat_clicked: t[v.BR.TextChat],
            notice_voice_call_clicked: t[v.BR.VoiceCall],
            notice_activity_clicked: t[v.BR.Activity],
            notice_clips_clicked: t[v.BR.Clips],
            notice_other_clicked: t[v.BR.Other],
        };
    }
...
class D {
    ...
    update() {
        ...
        if (t !== this.lastscreenType) {
            if (!(t in this.counters)) {
                ...
                C.error(`ScreenTypeAnalytics: Unknown screen type for ${this.game.name}: ${t}`, {
                    rawGame: this.game,
    ...
    getAnalytics() {
        ...
        isNaN(r) && C.error(`ScreenTypeAnalytics: Unknown most used screen type: ${n}`, t);
...
class L {
    ...
    uuid = (function () {
        try {
            ...
            return C.error("OverlayUsageStatsManager: Failed to generate UUID", e), (0, r.A)();
        }
    })();
    overlayStatus;
    overlayMethod = null;
    ...
    static async destroy(e) {
        ...
        if (null != n) {
            ...
            C.verbose(`OVERLAY_USAGE_STATS: ${e.name}`, t), delete L.gamesByPid[e.pid];
        }
    ...
    async getAnalytics() {
        let e = this.overlayMethod ?? _.Ue.Disabled,
            t = {
                ...
                setting_chat_notification: p.A.isNotificationDisabled(v.KS.TextChat) ? "DISABLED" : "ENABLED",
            },
            n = (0, d.hw)(this.game),
            r = (0, c.wH)(this.game),
...
function M(e) {
    for (let t of (C.verbose("handleRunningGamesChange", e), e.added)) {
        if (null != L.getByPid(t.pid)) {
            C.verbose("handleRunningGamesChange added", t, "already tracked");
            continue;
        }
...
function P(e) {
    ...
    (C.verbose("OVERLAY_SET_INPUT_LOCKED", t), null == t)
...
function k(e) {
    ...
    let t = L.getByName(e.gameName ?? e.gameId);
    if (null == t) return void C.error("OVERLAY_WIDGET_CHANGED: Game not found", e, L.debug);
...
function U(e) {
    if (
        ...
        return;
    let t = L.getByPid(e.pid);
    null == t ? C.error("OVERLAY_FOCUSED: Game not found", e, L.debug) : t.gameSetFocused(!0);
}
function G(e) {
    if ((C.verbose("SOUNDBOARD_SET_OVERLAY_ENABLED", e), e.pid === I.DEV_PID)) return;
    let t = L.getByPid(e.pid);
    null == t
...
function F(e) {
    ...
    let t = L.getByName(e.gameName ?? e.gameId);
    if (null == t) return void C.error("OVERLAY_MESSAGE_EVENT_ACTION: Game not found", e, L.debug);
    switch (e.eventType) {
        case "ack":
...
function H(e) {
    ...
    null != t &&
        t !== I.DEV_PID &&
        t !== I.UNSET_PID &&
        (C.verbose("AUDIO_TOGGLE_SELF_MUTE", e), L.handleMuteToggled());
}
function j(e) {
    C.verbose("WINDOW_FOCUS", e);
    let t = (0, T.Xg)();
    e.windowId !== t
        ? C.verbose("WINDOW_FOCUS: Not main window", { action: e, mainWindowId: t })
        : L.desktopSetFocused(e.focused);
}
function Y(e) {
    if (e.pid === I.DEV_PID || e.pid === I.UNSET_PID) return;
    let t = L.getByPid(e.pid);
    null == t ? C.error("OVERLAY_SUCCESSFULLY_SHOWN: Game not found", e, L.debug) : (t.successfullyShown = !0);
}
function W(e) {
    let t = L.getByPid(e.pid);
    null == t
        ...
        : e.overlayMethod !== _.Ue.Disabled &&
          (C.verbose("OVERLAY_UPDATE_OVERLAY_METHOD", e), t.setOverlayMethod(e.overlayMethod));
}
function K(e) {
    let t = L.getByPid(e.pid);
    if (null == t) return void C.error("OVERLAY_TRACK_STATE_CHANGED: Game not found", e, L.debug);
...
class q extends o.A {
    actions = __OVERLAY__
        ...
        : {
              ...
              OVERLAY_UPDATE_OVERLAY_METHOD: W,
