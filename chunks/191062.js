n.d(t, { Z: () => y }), n(704826), n(35282), n(388685);
var r = n(951288),
    i = n(647438),
    l = n(120356),
    a = n.n(l),
    o = n(442837),
    s = n(481060),
    c = n(771340),
    u = n(600164),
    d = n(903223),
    p = n(388032),
    f = n(167207);
function h(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0,
              })
            : (e[t] = n),
        e
    );
}
let m = {
        position: "bottom",
        autoInvert: !1,
    },
    g = (e) => {
        let { region: t, className: n = f.regionSelectName } = e,
            i = t.name.replace(/ \(VIP\)$/, "");
        return (0, r.jsx)("div", {
            className: n,
            children: i,
        });
    };
class b extends i.PureComponent {
    render() {
        let { region: e, selected: t } = this.props;
        return (0, r.jsxs)(u.Z, {
            align: u.Z.Align.CENTER,
            className: a()({ selected: t }),
            children: [
                (0, r.jsx)(g, {
                    region: e,
                    className: f.regionSelectName,
                }),
                t ? (0, r.jsx)("i", { className: f.check }) : null,
            ],
        });
    }
}
class _ extends i.PureComponent {
    componentDidMount() {
        null == this.props.regions && c.Z.fetchRegions(null);
    }
    render() {
        let { regions: e } = this.props;
        if (null == e) return null;
        let t = this.getSelectedRegion(),
            n = {
                label: t.name,
                value: t,
            },
            i = e
                .filter((e) => !e.deprecated && !e.hidden)
                .map((e) => ({
                    label: e.name,
                    value: e,
                    key: e.id,
                }));
        return (0, r.jsx)(s.nsm, {
            className: f.root,
            label: p.intl.string(p.t.w8gMqh),
            options: i,
            value: n,
            onChange: this.handleChangeVoiceRegion,
            renderOption: this.renderOption,
            popoutProps: m,
            popoutClassName: f.regionSelectPopout,
        });
    }
    constructor(...e) {
        super(...e),
            h(this, "renderOption", (e, t) =>
                (0, r.jsx)(
                    b,
                    {
                        region: e.value,
                        selected: t,
                    },
                    e.value.id,
                ),
            ),
            h(this, "getSelectedRegion", () => {
                var e;
                let { regions: t, call: n } = this.props;
                return n.regionUpdated && null != t && null != n.region
                    ? null !=
                      (e = t.find((e) => {
                          let { id: t } = e;
                          return t === n.region;
                      }))
                        ? e
                        : {
                              name: n.region,
                              id: n.region,
                          }
                    : {
                          id: "automatic",
                          name: p.intl.string(p.t.JEmsap),
                          vip: !1,
                          deprecated: !1,
                          hidden: !1,
                      };
            }),
            h(this, "handleChangeVoiceRegion", (e) => {
                let { value: t } = e,
                    { channelId: n } = this.props.call;
                c.Z.changeCallRegion(n, t.id);
            });
    }
}
let y = o.ZP.connectStores([d.Z], () => ({ regions: d.Z.getRegions(null) }))(_);
