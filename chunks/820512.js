...
function u(e) {
    s.h.dispatch({ type: "VOICE_CHANNEL_HISTORY_START_TRACKING", channelId: e });
}
function h(e) {
    let t = o.A.getLastFetchTime(e);
    (null != t && Date.now() - t < d) ||
        (s.h.dispatch({ type: "VOICE_CHANNEL_HISTORY_UPDATE_LAST_FETCH_TIME", channelId: e, timestamp: Date.now() }),
        A(e));
}
async function A(e) {
    try {
        ...
        s.h.dispatch({ type: "VOICE_CHANNEL_HISTORY_FETCH_COMPLETE", channelId: e, voiceLeaves: d, activities: u });
    } catch {}
}
