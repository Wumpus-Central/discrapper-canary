...
e.exports = function (e, t, r, m, v) {
    ...
    if (!o({ IsAccessorDescriptor: l, IsDataDescriptor: u }, v))
        throw new n("`current`, when present, must be a fully populated and valid Property Descriptor");
    if (!v["[[Configurable]]"]) {
        if (
            ("[[Configurable]]" in m && m["[[Configurable]]"]) ||
            ("[[Enumerable]]" in m && !d(m["[[Enumerable]]"], v["[[Enumerable]]"])) ||
            (!c(m) && !d(l(m), l(v)))
        )
            return !1;
        if (l(v)) {
            if (
                ("[[Get]]" in m && !d(m["[[Get]]"], v["[[Get]]"])) ||
                ("[[Set]]" in m && !d(m["[[Set]]"], v["[[Set]]"]))
            )
                return !1;
        } else if (
            !v["[[Writable]]"] &&
            (("[[Writable]]" in m && m["[[Writable]]"]) || ("[[Value]]" in m && !d(m["[[Value]]"], v["[[Value]]"])))
        )
            return !1;
    }
    return (
