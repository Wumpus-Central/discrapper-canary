...
async function eo() {
    ...
    U.nx.info(`${C.A.getSettings().clipsLength / 1e3}s clip save took ${Math.round(performance.now() - N)}ms`);
}
async function el(e) {
    let t, n;
    ...
    if (
        ...
        null == n && null != (n = h.Ay.getVisibleGame()) && U.nx.log("Using visible game for screenshot:", n.name),
        n?.windowHandle != null)
    )
