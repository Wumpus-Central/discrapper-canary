...
function m() {
    ...
    if (l.Ay.isObservedAppRunning(n.A.get(u.fg2.SPOTIFY).name) && t.length > 0) {
        let { socket: e, device: a } = t[0];
        return (0, s.VR)(e.accountId, a.id), Promise.resolve({ socket: e, device: a });
    }
    return new Promise((e, a) => {
        let r = setTimeout(() => {
                c.A.removeChangeListener(n), a(Error("timeout launching spotify"));
            }, h),
            n = () => {
        ...
        c.A.addChangeListener(n), window.open(`${d.gY}:`);
    });
}
function f() {
    let e = c.A.getActiveSocketAndDevice();
...
async function g(e, t) {
    let a = await (0, r.yb)(e, t),
        n = (0, d.NJ)(_(a.type ?? d.M0.TRACK));
    if (null === n) throw Error(`invalid type ${a.type}`);
    return {
