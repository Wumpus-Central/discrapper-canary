...
class g extends s.Z {
    ...
    constructor(e, t, n, r) {
        ...
        s.on('addtrack', (e, t) => this.createOutput(m(e), t)),
            ...
            s.on('connecting', () => this.setConnectionState(f.$j.DTLS_CONNECTING)),
            s.on('checking', () => this.setConnectionState(f.$j.ICE_CHECKING)),
            s.on('failed', () => this.setConnectionState(f.$j.NO_ROUTE)),
            s.on('disconnected', () => this.setConnectionState(f.$j.DISCONNECTED)),
            s.on('closed', () => this.setConnectionState(f.$j.DISCONNECTED)),
            s.on('offer', (e) => {
                let { sdp: t } = e,
