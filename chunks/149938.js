...
i.registerLanguage("ansi", function () {
    let e = [
...
let _ = /(<script\/?\\?>)|(html\\?`)/g,
    h = new Set(["html", "xml", "javascript", "typescript", "handlebars"]),
    f = /^[a-z0-9_+\-.#]+$/,
    p = i.listLanguages(),
    E = new Map();
for (let e of p) {
    e = e.toLowerCase();
    let t = i.getLanguage(e);
    if (null == t) continue;
    let n = t.aliases;
    if ((E.set(e, e), null != n)) for (let t of n) E.set(t.toLowerCase(), e);
}
for (let [e, t] of E) if (null == e.match(f)) throw Error(`Language name does not match regex: ${e}`);
function m(e) {
...
let g = {
        highlight(e, t, n) {
            if (
                ...
                return null;
            Date.now();
            let r = i.highlight(t, { ignoreIllegals: n, language: e });
            Date.now();
            let s = r.value.match(/</g)?.length;
            return null != s && s > 2e3 ? null : r;
        },
        hasLanguage: (e) => null != i.getLanguage(e),
