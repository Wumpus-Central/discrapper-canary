n.d(t, { Z: () => o });
var r = n(376606),
    i = n(463424);
let o = { read: p },
    a = 84,
    s = 128,
    l = "acsp",
    c = "desc",
    u = "mluc",
    d = "text",
    f = "sig ",
    _ = 12;
function p(e, t, n) {
    return n && t[0].compressionMethod !== i.p4 ? h(e, t) : g(e, t);
}
function h(e, t) {
    if (!m(t[0].compressionMethod)) return {};
    let n = new DataView(e.buffer.slice(t[0].offset, t[0].offset + t[0].length));
    return (0, i.Lj)(n, t[0].compressionMethod, "utf-8", "dataview")
        .then(O)
        .catch(() => ({}));
}
function m(e) {
    return e === i.Nf;
}
function g(e, t) {
    try {
        let n = t.reduce((e, t) => e + t.length, 0),
            r = new Uint8Array(n),
            i = 0,
            o = E(e);
        for (let e = 1; e <= t.length; e++) {
            let n = t.find((t) => t.chunkNumber === e);
            if (!n) throw Error(`ICC chunk ${e} not found`);
            let a = o.slice(n.offset, n.offset + n.length),
                s = new Uint8Array(a);
            r.set(s, i), (i += s.length);
        }
        return O(new DataView(r.buffer));
    } catch (e) {
        return {};
    }
}
function E(e) {
    return Array.isArray(e) ? new DataView(Uint8Array.from(e).buffer).buffer : e.buffer;
}
function b(e) {
    return e.length < s + 4;
}
function y(e, t) {
    return e.length < t + _;
}
function O(e) {
    let t = e.buffer,
        n = e.getUint32();
    if (e.byteLength !== n) throw Error("ICC profile length not matching");
    if (e.byteLength < a) throw Error("ICC profile too short");
    let o = {},
        s = Object.keys(r.m);
    for (let t = 0; t < s.length; t++) {
        let n = s[t],
            i = r.m[n],
            a = i.value(e, parseInt(n, 10)),
            l = a;
        i.description && (l = i.description(a)),
            (o[i.name] = {
                value: a,
                description: l,
            });
    }
    if (v(t.slice(36, 40)) !== l) throw Error("ICC profile: missing signature");
    if (b(t)) return o;
    let _ = e.getUint32(128),
        p = 132;
    for (let n = 0; n < _ && !y(t, p); n++) {
        let n = (0, i.oH)(e, p, 4),
            r = e.getUint32(p + 4),
            a = e.getUint32(p + 8);
        if (r > t.length) break;
        let s = (0, i.oH)(e, r, 4);
        if (s === c) {
            let i = e.getUint32(r + 8);
            if (i > a) return o;
            I(o, n, v(t.slice(r + 12, r + i + 11)));
        } else if (s === u) {
            let t = e.getUint32(r + 8),
                a = e.getUint32(r + 12),
                s = r + 16,
                l = [];
            for (let n = 0; n < t; n++) {
                let t = (0, i.oH)(e, s + 0, 2),
                    n = (0, i.oH)(e, s + 2, 2),
                    o = e.getUint32(s + 4),
                    c = e.getUint32(s + 8),
                    u = (0, i.cs)(e, r + c, o);
                l.push({
                    languageCode: t,
                    countryCode: n,
                    text: u,
                }),
                    (s += a);
            }
            if (1 === t) I(o, n, l[0].text);
            else {
                let e = {};
                for (let t = 0; t < l.length; t++) e[`${l[t].languageCode}-${l[t].countryCode}`] = l[t].text;
                I(o, n, e);
            }
        } else s === d ? I(o, n, v(t.slice(r + 8, r + a - 7))) : s === f && I(o, n, v(t.slice(r + 8, r + 12)));
        p += 12;
    }
    return o;
}
function v(e) {
    return String.fromCharCode.apply(null, new Uint8Array(e));
}
function I(e, t, n) {
    r.w[t]
        ? (e[r.w[t].name] = {
              value: n,
              description: n,
          })
        : (e[t] = {
              value: n,
              description: n,
          });
}
