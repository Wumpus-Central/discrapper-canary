n.d(t, { Z: () => ea }), n(388685), n(539854);
var i,
    r = n(512722),
    a = n.n(r),
    s = n(772848),
    o = n(442837),
    l = n(579092),
    c = n(570140),
    d = n(13245),
    u = n(836932),
    f = n(175553),
    h = n(158979),
    p = n(181912),
    g = n(194295),
    b = n(998033),
    m = n(181364),
    y = n(435064),
    v = n(668761),
    O = n(864060),
    E = n(358446),
    x = n(371651),
    S = n(624864),
    _ = n(695346),
    Z = n(163612),
    j = n(314897),
    C = n(592125),
    I = n(292959),
    P = n(158776),
    w = n(944486),
    T = n(885110),
    N = n(246946),
    D = n(594174),
    k = n(974180),
    R = n(808506),
    A = n(237997),
    L = n(70956),
    M = n(145597),
    z = n(449224),
    V = n(981631),
    W = n(987650),
    U = n(388032);
function G(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0,
              })
            : (e[t] = n),
        e
    );
}
function B(e) {
    for (var t = 1; t < arguments.length; t++) {
        var n = null != arguments[t] ? arguments[t] : {},
            i = Object.keys(n);
        "function" == typeof Object.getOwnPropertySymbols &&
            (i = i.concat(
                Object.getOwnPropertySymbols(n).filter(function (e) {
                    return Object.getOwnPropertyDescriptor(n, e).enumerable;
                }),
            )),
            i.forEach(function (t) {
                G(e, t, n[t]);
            });
    }
    return e;
}
function F(e, t) {
    return (
        (t = null != t ? t : {}),
        Object.getOwnPropertyDescriptors
            ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t))
            : (function (e, t) {
                  var n = Object.keys(e);
                  if (Object.getOwnPropertySymbols) {
                      var i = Object.getOwnPropertySymbols(e);
                      n.push.apply(n, i);
                  }
                  return n;
              })(Object(t)).forEach(function (n) {
                  Object.defineProperty(e, n, Object.getOwnPropertyDescriptor(t, n));
              }),
        e
    );
}
let H = new l.Yd("LegacyOverlayNotificationsStore"),
    Y = 5 * L.Z.Millis.SECOND,
    K = 8 * L.Z.Millis.SECOND,
    X = 30 * L.Z.Millis.SECOND,
    J = Object.freeze({
        priority: 0,
        duration: Y,
        expirationExternallyManaged: !1,
        type: 0,
    }),
    q = [],
    Q = (e, t, n) => {
        let i = t ? V._1z.TIMED_OUT : V._1z.DISMISSED;
        return setTimeout(() => d.Z.updateNotificationStatus(e, i), null != n ? n : Y);
    };
function $(e) {
    let t = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : V._1z.DISMISSED;
    if (null == e) return !1;
    let n = q.findIndex((t) => t.id === e);
    if (-1 === n) return !1;
    let i = q[n];
    clearTimeout(i.timerId), (q = [...q]), t === V._1z.DISMISSED ? q.splice(n, 1) : (q[n] = F(B({}, i), { status: t }));
}
function ee(e) {
    let t = q.length;
    return (q = q.filter((t) => 1 !== t.type || t.channelId !== e)).length !== t;
}
function et(e) {
    let t = q.find((t) => 2 === t.type && t.channelId === e);
    return null != t ? t.id : null;
}
function en(e, t) {
    let n = B({}, J, t);
    if (2 !== n.priority && !A.default.isInstanceFocused()) return null;
    let i = (0, s.Z)(),
        r = B(
            {
                id: i,
                status: V._1z.ACTIVE,
                timerId: Q(i, n.expirationExternallyManaged, n.duration),
                props: e,
            },
            n,
        ),
        a = (q = [...q]).findIndex((e) => e.priority <= n.priority);
    return -1 === a ? q.push(r) : q.splice(a, 0, r), q.length > 10 && clearTimeout(q.pop().timerId), i;
}
function ei(e) {
    let { channelId: t, ringing: n } = e,
        i = et(t);
    if (!n.includes(j.default.getId())) return $(i);
    if (null != i) return !1;
    let r = C.Z.getChannel(t);
    if (
        null == r ||
        !r.isRingable() ||
        ("GUILD_RING_START" === e.type &&
            !Z.Z.getCurrentConfig({
                guildId: e.guildId,
                location: "OverlayStartRinging",
            }).enabled) ||
        T.Z.getStatus() === V.Skl.DND ||
        _.QZ.getSetting()
    )
        return !1;
    let a = q.find((e) => 1 === e.type && e.channelId === t && e.messageType === V.uaV.CALL);
    null != a && $(a.id),
        en((0, p.Z)(r), {
            priority: 1,
            expirationExternallyManaged: !0,
            type: 2,
            channelId: r.id,
        });
}
class er extends (i = o.ZP.Store) {
    initialize() {
        this.waitFor(
            j.default,
            C.Z,
            y.Z,
            I.Z,
            R.default,
            x.default,
            z.Z,
            S.Z,
            A.default,
            P.Z,
            w.Z,
            T.Z,
            N.Z,
            D.default,
        );
    }
    getNotifications() {
        return q;
    }
}
G(er, "displayName", "OverlayNotificationsStore");
let ea = new er(
    c.Z,
    !__OVERLAY__
        ? {}
        : {
              OVERLAY_UPDATE_NOTIFICATION_STATUS: function (e) {
                  let { notificationId: t, status: n } = e;
                  $(t, n);
              },
              OVERLAY_MOUNTED: function (e) {
                  var t;
                  let { nudges: n } = e;
                  if (x.default.hasChangedRenderMode(null != (t = R.default.getFocusedPID()) ? t : (0, M.getPID)()))
                      return void H.info("Overlay mounted, but render modes have changed", { nudges: n });
                  let i = n[0];
                  if (
                      (H.info("Overlay mounted", {
                          nudges: n,
                          selectedNudge: i,
                      }),
                      null == i)
                  )
                      return;
                  let r = (function (e) {
                      switch (e.type) {
                          case W.nc.GO_LIVE_VOICE:
                          case W.nc.GO_LIVE_NON_VOICE:
                              return (0, h.Z)(e);
                          case W.nc.KEYBIND_INDICATORS:
                              return (0, E.Z)(e);
                          case W.nc.NEWS:
                          default:
                              return (0, m.Z)(e);
                      }
                  })(i);
                  null != r &&
                      en(r, {
                          priority: 2,
                          duration: K,
                      });
              },
              OVERLAY_SET_INPUT_LOCKED: function (e) {
                  let { locked: t } = e;
                  if (t) return !1;
                  q = q.map((e) =>
                      e.status === V._1z.ACTIVE
                          ? (clearTimeout(e.timerId), F(B({}, e), { timerId: Q(e.id, e.expirationExternallyManaged) }))
                          : e,
                  );
              },
              MESSAGE_CREATE: function (e) {
                  var t, n, i;
                  let { channelId: r, message: s } = e,
                      o = C.Z.getChannel(r),
                      l = D.default.getUser(null == (t = s.author) ? void 0 : t.id);
                  if (null == o || null == l) return !1;
                  if (
                      (null == (n = s.activity) ? void 0 : n.type) === V.mFx.JOIN ||
                      (null == (i = s.activity) ? void 0 : i.type) === V.mFx.JOIN_REQUEST
                  ) {
                      if (!(0, O.eF)(s, r, !0, !0)) return !1;
                      let e = (function (e, t, n) {
                          let i, r;
                          if ((a()(null != t.activity, "received null message activity"), n.id === j.default.getId()))
                              return !1;
                          let s = z.Z.getGame();
                          if (null == s) return !1;
                          switch (t.activity.type) {
                              case V.mFx.JOIN:
                                  if (
                                      null == (i = P.Z.getApplicationActivity(n.id, s.id)) ||
                                      null == i.party ||
                                      i.party.id !== t.activity.party_id
                                  )
                                      return !1;
                                  r = (0, u.Z)(e, t, n, s, i);
                                  break;
                              case V.mFx.JOIN_REQUEST:
                                  if (
                                      null == (i = T.Z.getApplicationActivity(s.id)) ||
                                      null == i.party ||
                                      i.party.id !== t.activity.party_id
                                  )
                                      return !1;
                                  r = (0, g.Z)(e, n, s, i);
                          }
                          if (null == r) return !1;
                          let o = en(r, {
                              priority: 2,
                              expirationExternallyManaged: !0,
                              channelId: e.id,
                          });
                          return null != o && Q(o, !1, X), !0;
                      })(o, s, l);
                      if (!1 !== e) return e;
                  }
                  if (
                      ((!A.default.isInstanceLocked() || A.default.isPinned(V.Odu.TEXT)) && r === w.Z.getChannelId()) ||
                      S.Z.isNotificationDisabled(W.n0.TextChat) ||
                      N.Z.disableNotifications ||
                      !(0, O.eF)(s, r)
                  )
                      return !1;
                  let c = !I.Z.isSoundDisabled(k.Ay);
                  en((0, b.Z)(o, s, l, c), {
                      type: 1,
                      channelId: o.id,
                      messageType: s.type,
                  });
              },
              CHANNEL_SELECT: function (e) {
                  let { channelId: t } = e;
                  return null != t && ee(t);
              },
              MESSAGE_ACK: function (e) {
                  let { channelId: t } = e;
                  return ee(t);
              },
              CALL_CREATE: ei,
              CALL_UPDATE: ei,
              CALL_DELETE: function (e) {
                  let { channelId: t } = e;
                  $(et(t));
              },
              GUILD_RING_START: ei,
              GUILD_RING_STOP: function (e) {
                  let { channelId: t, guildId: n, ringing: i } = e;
                  if (
                      !Z.Z.getCurrentConfig({
                          guildId: n,
                          location: "OverlayStopRinging",
                      }).enabled
                  )
                      return !1;
                  i.includes(j.default.getId()) && $(et(t));
              },
              ACTIVITY_USER_ACTION: function (e) {
                  let t,
                      { actionType: n, user: i, applicationId: r } = e,
                      a = z.Z.getGame();
                  return (
                      null != a &&
                      a.id === r &&
                      (n === V.mFx.JOIN && (t = (0, f.Z)(i, a)),
                      null != t &&
                          void en(t, {
                              priority: 2,
                              type: 0,
                          }))
                  );
              },
              CLIPS_SAVE_CLIP_START: function (e) {
                  "manual" === e.clipMethod && en((0, v.f)(U.intl.string(U.t.NBMK9m)));
              },
              CLIPS_SAVE_CLIP_ERROR: function () {
                  en((0, v.f)(U.intl.string(U.t["1ZbZuh"])));
              },
              STREAM_START: function (e) {
                  let t = (0, v.y)();
                  null != t && en(t);
              },
          },
);
