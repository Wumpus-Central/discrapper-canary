"use strict";
n.d(t, { A: () => y });
var i = n(562708),
    r = n(765178),
    s = n(636537),
    a = n(228366),
    o = n(398590),
    l = n(626584),
    u = n(568185),
    c = n(529942),
    d = n(164956),
    _ = n(857071),
    f = n(976860),
    h = n(138298),
    p = n(495544),
    E = n(696451),
    m = n(71393),
    g = n(499785),
    A = n(555337),
    I = n(652215),
    T = n(375708);
let S = new l.A("GuildSettingsActionCreators"),
    N = {
        init(e, t, n, i) {
            a.h.dispatch({ type: "GUILD_SETTINGS_INIT", guildId: e, section: t, subsection: i, location: n });
        },
        async open(e, t, i, r) {
            await Promise.all([
                n.e("74389"),
                n.e("32824"),
                n.e("79924"),
                n.e("29690"),
                n.e("28951"),
                n.e("60361"),
                n.e("80203"),
                n.e("9023"),
                n.e("685"),
                n.e("74103"),
                n.e("39655"),
                n.e("77691"),
                n.e("79149"),
                n.e("54266"),
                n.e("93110"),
                n.e("95664"),
                n.e("35769"),
                n.e("40671"),
                n.e("73953"),
                n.e("43778"),
                n.e("22513"),
                n.e("13498"),
                n.e("32993"),
                n.e("48157"),
                n.e("42971"),
                n.e("71934"),
                n.e("7053"),
                n.e("13233"),
                n.e("39995"),
                n.e("25568"),
                n.e("16254"),
                n.e("19360"),
                n.e("18125"),
                n.e("5959"),
                n.e("86028"),
                n.e("9432"),
                n.e("45650"),
                n.e("55030"),
                n.e("31837"),
                n.e("27660"),
                n.e("85484"),
                n.e("31135"),
                n.e("25677"),
                n.e("55990"),
                n.e("8506"),
                n.e("47459"),
                n.e("333"),
                n.e("15615"),
                n.e("22995"),
                n.e("79705"),
                n.e("33957"),
                n.e("39111"),
                n.e("28643"),
                n.e("25610"),
                n.e("97444"),
                n.e("49579"),
                n.e("41069"),
                n.e("2661"),
                n.e("43908"),
                n.e("87845"),
                n.e("85970"),
                n.e("49089"),
                n.e("45970"),
                n.e("5007"),
                n.e("31227"),
                n.e("45550"),
                n.e("54531"),
                n.e("14399"),
                n.e("72228"),
                n.e("1472"),
                n.e("83852"),
                n.e("80186"),
                n.e("23924"),
                n.e("42205"),
                n.e("71091"),
                n.e("24199"),
                n.e("207"),
                n.e("89281"),
                n.e("57036"),
                n.e("98839"),
                n.e("19402"),
                n.e("49681"),
                n.e("96382"),
                n.e("22687"),
                n.e("62052"),
                n.e("63202"),
                n.e("82596"),
                n.e("34786"),
                n.e("28367"),
                n.e("10471"),
                n.e("35429"),
                n.e("63232"),
                n.e("11301"),
                n.e("60235"),
                n.e("64827"),
                n.e("30938"),
                n.e("96643"),
                n.e("94493"),
                n.e("92789"),
                n.e("47834"),
                n.e("74016"),
                n.e("19551"),
                n.e("84569"),
                n.e("6338"),
                n.e("50689"),
                n.e("7743"),
                n.e("45959"),
                n.e("60658"),
                n.e("8304"),
                n.e("18573"),
                n.e("55936"),
                n.e("31988"),
                n.e("39667"),
                n.e("5812"),
                n.e("26073"),
                n.e("34303"),
                n.e("28866"),
                n.e("1368"),
                n.e("58038"),
                n.e("93159"),
                n.e("31716"),
                n.e("8751"),
                n.e("62931"),
                n.e("81987"),
                n.e("50417"),
                n.e("74548"),
                n.e("88941"),
                n.e("34530"),
                n.e("76602"),
                n.e("9233"),
                n.e("28229"),
                n.e("21921"),
                n.e("62680"),
                n.e("76418"),
                n.e("71210"),
                n.e("61379"),
                n.e("21825"),
                n.e("98125"),
                n.e("42724"),
                n.e("43437"),
                n.e("82263"),
                n.e("40258"),
                n.e("18441"),
                n.e("66495"),
                n.e("76640"),
                n.e("88342"),
                n.e("25693"),
                n.e("86127"),
                n.e("39970"),
                n.e("6174"),
                n.e("28154"),
                n.e("88077"),
                n.e("91146"),
                n.e("8555"),
                n.e("32606"),
                n.e("46070"),
                n.e("14879"),
                n.e("66900"),
                n.e("42451"),
                n.e("13681"),
                n.e("10004"),
                n.e("55314"),
                n.e("89094"),
                n.e("29177"),
                n.e("72789"),
                n.e("33902"),
                n.e("32551"),
                n.e("31658"),
                n.e("4369"),
                n.e("23353"),
                n.e("53930"),
                n.e("46270"),
                n.e("86949"),
                n.e("6949"),
                n.e("50015"),
                n.e("86814"),
                n.e("17249"),
                n.e("11523"),
                n.e("88599"),
                n.e("1555"),
                n.e("44695"),
                n.e("21690"),
                n.e("35027"),
                n.e("44376"),
                n.e("53917"),
                n.e("99999"),
                n.e("31644"),
                n.e("70653"),
                n.e("62422"),
                n.e("45723"),
                n.e("25949"),
                n.e("90365"),
                n.e("70697"),
                n.e("18546"),
                n.e("89088"),
                n.e("38835"),
                n.e("37622"),
                n.e("71273"),
                n.e("90244"),
                n.e("75842"),
                n.e("36863"),
                n.e("18943"),
                n.e("67861"),
                n.e("84103"),
                n.e("1518"),
                n.e("15186"),
                n.e("98793"),
                n.e("9915"),
                n.e("9640"),
                n.e("53614"),
                n.e("31825"),
                n.e("98765"),
                n.e("47626"),
                n.e("23354"),
                n.e("20320"),
                n.e("8891"),
                n.e("95387"),
                n.e("99657"),
                n.e("82644"),
                n.e("47017"),
                n.e("52548"),
                n.e("3589"),
                n.e("51243"),
                n.e("44265"),
                n.e("99141"),
                n.e("92731"),
                n.e("25990"),
                n.e("20590"),
                n.e("11527"),
                n.e("63095"),
                n.e("68647"),
                n.e("66541"),
                n.e("63070"),
                n.e("65437"),
                n.e("64615"),
                n.e("74810"),
                n.e("87225"),
                n.e("28083"),
                n.e("75134"),
                n.e("80830"),
                n.e("93158"),
                n.e("84967"),
                n.e("55184"),
                n.e("87079"),
                n.e("79745"),
                n.e("73500"),
                n.e("31145"),
                n.e("78707"),
                n.e("73547"),
                n.e("84456"),
                n.e("53984"),
                n.e("99593"),
                n.e("80854"),
                n.e("35395"),
                n.e("43780"),
                n.e("54975"),
                n.e("62168"),
                n.e("55602"),
                n.e("28128"),
                n.e("46844"),
                n.e("63235"),
                n.e("91381"),
                n.e("45413"),
                n.e("86672"),
                n.e("10034"),
                n.e("90664"),
                n.e("25252"),
                n.e("32756"),
                n.e("64850"),
                n.e("70104"),
                n.e("52617"),
                n.e("2564"),
                n.e("20287"),
                n.e("47417"),
                n.e("19558"),
                n.e("58946"),
                n.e("48330"),
                n.e("14285"),
                n.e("95752"),
                n.e("8362"),
                n.e("41678"),
                n.e("72401"),
                n.e("71133"),
                n.e("62875"),
                n.e("4744"),
                n.e("2698"),
                n.e("77084"),
                n.e("5500"),
                n.e("501"),
                n.e("73384"),
                n.e("82783"),
                n.e("86243"),
                n.e("32209"),
                n.e("54625"),
                n.e("39171"),
                n.e("37021"),
                n.e("234"),
                n.e("49697"),
                n.e("87478"),
                n.e("80230"),
                n.e("37687"),
                n.e("48900"),
                n.e("95093"),
                n.e("63645"),
                n.e("71234"),
                n.e("27773"),
                n.e("83952"),
                n.e("44780"),
                n.e("5636"),
                n.e("86230"),
                n.e("834"),
                n.e("60177"),
                n.e("21570"),
                n.e("5557"),
                n.e("59957"),
                n.e("62718"),
                n.e("69933"),
                n.e("56753"),
                n.e("26728"),
                n.e("89916"),
                n.e("1354"),
                n.e("20282"),
                n.e("49145"),
                n.e("60773"),
                n.e("23276"),
                n.e("78412"),
                n.e("63213"),
                n.e("9662"),
                n.e("8018"),
                n.e("80239"),
                n.e("92295"),
                n.e("75016"),
                n.e("68763"),
                n.e("4193"),
                n.e("95444"),
                n.e("29666"),
                n.e("76087"),
                n.e("58273"),
                n.e("48720"),
                n.e("20930"),
                n.e("33134"),
                n.e("80112"),
                n.e("59880"),
                n.e("14461"),
                n.e("54791"),
                n.e("18024"),
                n.e("26001"),
                n.e("30997"),
                n.e("14591"),
                n.e("52111"),
                n.e("28136"),
                n.e("73234"),
                n.e("31445"),
                n.e("57458"),
                n.e("58337"),
                n.e("77473"),
                n.e("28152"),
                n.e("16084"),
                n.e("69747"),
                n.e("97073"),
                n.e("1243"),
                n.e("42204"),
                n.e("65826"),
                n.e("49520"),
                n.e("78195"),
                n.e("15920"),
                n.e("20683"),
                n.e("35996"),
                n.e("37977"),
                n.e("58164"),
                n.e("21930"),
                n.e("3930"),
                n.e("44057"),
                n.e("3663"),
                n.e("18997"),
                n.e("87317"),
                n.e("71470"),
                n.e("26437"),
                n.e("10943"),
                n.e("37490"),
                n.e("72963"),
                n.e("66580"),
                n.e("22261"),
                n.e("16916"),
                n.e("78046"),
                n.e("80093"),
                n.e("44326"),
                n.e("83420"),
                n.e("36149"),
                n.e("72877"),
                n.e("38601"),
                n.e("73669"),
                n.e("63618"),
                n.e("32326"),
                n.e("7775"),
                n.e("22547"),
                n.e("60989"),
                n.e("82486"),
                n.e("12682"),
                n.e("65617"),
                n.e("74021"),
                n.e("78050"),
                n.e("13708"),
                n.e("51778"),
                n.e("84971"),
                n.e("45830"),
                n.e("20382"),
                n.e("61058"),
                n.e("50342"),
                n.e("33097"),
                n.e("3710"),
                n.e("80007"),
                n.e("967"),
                n.e("71217"),
                n.e("94607"),
                n.e("97069"),
                n.e("47018"),
                n.e("77602"),
                n.e("10262"),
                n.e("99835"),
                n.e("28752"),
                n.e("95602"),
                n.e("46313"),
                n.e("74907"),
                n.e("32207"),
                n.e("5896"),
                n.e("27779"),
                n.e("80436"),
                n.e("69178"),
                n.e("30221"),
                n.e("22266"),
                n.e("58216"),
                n.e("36150"),
                n.e("73165"),
                n.e("34268"),
                n.e("29569"),
                n.e("36002"),
                n.e("4199"),
                n.e("82178"),
                n.e("43233"),
                n.e("29413"),
                n.e("6809"),
                n.e("38042"),
                n.e("41786"),
                n.e("63333"),
                n.e("29961"),
                n.e("15172"),
                n.e("2329"),
                n.e("91643"),
                n.e("63726"),
                n.e("71329"),
                n.e("49345"),
                n.e("11471"),
                n.e("34409"),
                n.e("49333"),
                n.e("59414"),
                n.e("43114"),
                n.e("18489"),
                n.e("70008"),
                n.e("44385"),
                n.e("83827"),
                n.e("42749"),
                n.e("56026"),
                n.e("16301"),
                n.e("22191"),
                n.e("25961"),
                n.e("30501"),
                n.e("8364"),
                n.e("84586"),
                n.e("96268"),
                n.e("60178"),
                n.e("66147"),
                n.e("83477"),
                n.e("89541"),
                n.e("77333"),
                n.e("74337"),
                n.e("62213"),
                n.e("21574"),
                n.e("24689"),
                n.e("52823"),
                n.e("56967"),
                n.e("47011"),
                n.e("31203"),
                n.e("68802"),
                n.e("60249"),
                n.e("49490"),
                n.e("66913"),
                n.e("38090"),
                n.e("17460"),
                n.e("81202"),
                n.e("8563"),
                n.e("46149"),
                n.e("71719"),
                n.e("72847"),
                n.e("93767"),
                n.e("3168"),
                n.e("98254"),
                n.e("13848"),
                n.e("94393"),
                n.e("39214"),
                n.e("99108"),
                n.e("14878"),
                n.e("88307"),
                n.e("26508"),
                n.e("61347"),
                n.e("84820"),
                n.e("73091"),
                n.e("93907"),
                n.e("86692"),
                n.e("10014"),
                n.e("10142"),
                n.e("44912"),
                n.e("84783"),
                n.e("16237"),
                n.e("11802"),
                n.e("16373"),
                n.e("36674"),
                n.e("81398"),
                n.e("24484"),
                n.e("44943"),
                n.e("7548"),
                n.e("32347"),
                n.e("8458"),
                n.e("11810"),
                n.e("48111"),
                n.e("39735"),
                n.e("90152"),
                n.e("87973"),
                n.e("59617"),
                n.e("57197"),
                n.e("14863"),
                n.e("95841"),
                n.e("10823"),
                n.e("84951"),
                n.e("96680"),
                n.e("97116"),
                n.e("63874"),
                n.e("71167"),
                n.e("12390"),
                n.e("53203"),
                n.e("95067"),
                n.e("33416"),
                n.e("39353"),
                n.e("18792"),
                n.e("9793"),
                n.e("61309"),
                n.e("70284"),
                n.e("94529"),
                n.e("15799"),
                n.e("93305"),
                n.e("69167"),
                n.e("21300"),
                n.e("66016"),
                n.e("83518"),
                n.e("79211"),
                n.e("90213"),
                n.e("78"),
                n.e("52694"),
                n.e("53683"),
                n.e("91218"),
                n.e("72238"),
                n.e("37038"),
                n.e("52212"),
                n.e("71790"),
                n.e("19148"),
                n.e("52191"),
                n.e("14656"),
                n.e("39855"),
                n.e("98356"),
                n.e("15046"),
                n.e("98700"),
            ]).then(n.bind(n, 422559)),
                m.A.getGuild(e)?.features.has(I.GuildFeatures.COMMUNITY) &&
                    (t === I.BEX.GUILD_AUTOMOD && ((t = I.BEX.SAFETY), (r = I.nd0.SAFETY_AUTOMOD)),
                    t === I.BEX.MEMBER_VERIFICATION && ((t = I.BEX.SAFETY), (r = I.nd0.SAFETY_DM_AND_SPAM_PROTECTION))),
                N.init(e, t, i, r),
                h.A.closeGuildSidebar(e),
                (0, o.id)(I.zgK.GUILD_SETTINGS);
        },
        close() {
            a.h.dispatch({ type: "GUILD_SETTINGS_CLOSE" });
        },
        saveRouteStack(e) {
            a.h.dispatch({ type: "GUILD_SETTINGS_SAVE_ROUTE_STACK", state: e });
        },
        setSection(e, t) {
            let n = A.A.getGuildId();
            null != n &&
                (m.A.getGuild(n)?.features.has(I.GuildFeatures.COMMUNITY) &&
                    (e === I.BEX.GUILD_AUTOMOD && ((e = I.BEX.SAFETY), (t = I.nd0.SAFETY_AUTOMOD)),
                    e === I.BEX.MEMBER_VERIFICATION && ((e = I.BEX.SAFETY), (t = I.nd0.SAFETY_DM_AND_SPAM_PROTECTION))),
                a.h.dispatch({ type: "GUILD_SETTINGS_SET_SECTION", section: e, subsection: t }));
        },
        setSearchQuery(e) {
            a.h.dispatch({ type: "GUILD_SETTINGS_SET_SEARCH_QUERY", searchQuery: e });
        },
        selectRole: (e, t) => a.h.dispatch({ type: "GUILD_SETTINGS_ROLE_SELECT", roleId: e, searchQuery: t }),
        updateEmbed: (e, t, n) =>
            s.Bo.patch({
                url: I.Rsh.GUILD_WIDGET(e),
                body: { enabled: t, channel_id: n },
                oldFormErrors: !0,
                rejectWithError: !0,
            }).then((t) => {
                a.h.dispatch({
                    type: "GUILD_SETTINGS_SET_WIDGET",
                    guildId: e,
                    enabled: t.body.enabled,
                    channelId: t.body.channel_id,
                });
            }),
        updateMFALevel(e) {
            let { guildId: t, level: n } = e;
            return s.Bo.post({
                url: I.Rsh.GUILD_MFA(t),
                body: { level: n },
                oldFormErrors: !0,
                rejectWithError: !1,
            }).then((e) => a.h.dispatch({ type: "GUILD_SETTINGS_SET_MFA_SUCCESS", level: e.body.level }));
        },
        updateIcon(e, t) {
            s.Bo.patch({ url: I.Rsh.GUILD(e), body: { icon: t }, oldFormErrors: !0, rejectWithError: !1 }).then(
                () => {
                    a.h.dispatch({ type: "GUILD_SETTINGS_UPDATE", icon: t }), u.A.checkGuildTemplateDirty(e);
                },
                (e) => a.h.dispatch({ type: "GUILD_SETTINGS_SUBMIT_FAILURE", errors: e.body }),
            );
        },
        cancelChanges(e) {
            a.h.dispatch({ type: "GUILD_SETTINGS_CANCEL_CHANGES", guildId: e });
        },
        updateGuild(e) {
            let { safetyAlertsChannelId: t, profile: n, ...i } = e,
                r = { ...i };
            null != n && (r.profile = { ...(r.profile ?? {}), ...n }),
                null != A.A.getGuildId() && null != t && (r.safetyAlertsChannelId = t),
                a.h.dispatch({ type: "GUILD_SETTINGS_UPDATE", ...r });
        },
        updateGuildProfile(e, t) {
            a.h.dispatch({ type: "GUILD_SETTINGS_PROFILE_UPDATE", guildId: e, ...t });
        },
        saveGuild(e, t) {
            let {
                    name: n,
                    description: i,
                    icon: r,
                    splash: o,
                    banner: l,
                    homeHeader: c,
                    afkChannelId: d,
                    afkTimeout: _,
                    systemChannelId: f,
                    verificationLevel: h,
                    defaultMessageNotifications: p,
                    explicitContentFilter: E,
                    features: m,
                    systemChannelFlags: g,
                    preferredLocale: A,
                    rulesChannelId: T,
                    safetyAlertsChannelId: N,
                    ownerConfiguredContentLevel: y,
                    discoverySplash: C,
                    publicUpdatesChannelId: v,
                    premiumProgressBarEnabled: O,
                    profile: R,
                    moderatorReportingEnabled: b,
                    officialMessageColor: D,
                    verificationRoleId: L,
                } = t,
                w = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : {},
                M = {
                    name: n,
                    description: i,
                    icon: r,
                    splash: o,
                    banner: l,
                    home_header: c,
                    features: m,
                    preferred_locale: A,
                    afk_channel_id: d,
                    afk_timeout: _,
                    system_channel_id: f,
                    verification_level: h,
                    default_message_notifications: p,
                    explicit_content_filter: E,
                    system_channel_flags: g,
                    rules_channel_id: T,
                    owner_configured_content_level: y,
                    discovery_splash: C,
                    public_updates_channel_id: v,
                    safety_alerts_channel_id: N,
                    ...(null != O ? { premium_progress_bar_enabled: O } : null),
                    profile: null != R ? { tag: R.tag } : R,
                    moderator_reporting_enabled: b,
                    official_message_color: D,
                    verification_role_id: L,
                };
            return (
                a.h.dispatch({ type: "GUILD_SETTINGS_SUBMIT" }),
                s.Bo.patch({
                    url: I.Rsh.GUILD(e),
                    query: { for_discovery: w.isForDiscovery },
                    body: M,
                    oldFormErrors: !0,
                    rejectWithError: !1,
                }).then(
                    (t) => {
                        a.h.dispatch({ type: "GUILD_SETTINGS_SUBMIT_SUCCESS", guild: t.body }),
                            u.A.checkGuildTemplateDirty(e);
                    },
                    (e) => {
                        if (
                            (a.h.dispatch({ type: "GUILD_SETTINGS_SUBMIT_FAILURE", errors: e.body }),
                            S.error("Failed to save guild settings", { errors: e.body }),
                            w.throwErr)
                        )
                            throw e.body;
                    },
                )
            );
        },
        updateGuildModeration: (e, t) =>
            s.Bo.patch({
                url: I.Rsh.GUILD(e),
                body: { verification_level: t.verificationLevel, explicit_content_filter: t.explicitContentFilter },
                oldFormErrors: !0,
                rejectWithError: !1,
            }).then((t) => (u.A.checkGuildTemplateDirty(e), t)),
        transferOwnership(e, t) {
            let n = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : null,
                r = arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : null;
            return g.A.patch({
                url: I.Rsh.GUILD(e),
                body: { owner_id: t, code: r },
                oldFormErrors: !0,
                trackedActionData: {
                    event: i.NetworkActionNames.GUILD_TRANSFER_OWNERSHIP,
                    properties: { guild_id: e, verification_type: n },
                },
                rejectWithError: !1,
            });
        },
        sendTransferOwnershipPincode(e) {
            let t = arguments.length > 1 && void 0 !== arguments[1] && arguments[1];
            return g.A.put({
                url: I.Rsh.GUILD_PINCODE(e),
                oldFormErrors: !0,
                trackedActionData: {
                    event: i.NetworkActionNames.GUILD_TRANSFER_OWNERSHIP_SEND_CODE,
                    properties: { guild_id: e, is_resend: t },
                },
                rejectWithError: !1,
            });
        },
        deleteGuild: (e) =>
            s.Bo.post({ url: I.Rsh.GUILD_DELETE(e), oldFormErrors: !0, rejectWithError: !1 }).then(() => {
                N.close();
            }),
        async leaveGuild(e) {
            let t = arguments.length > 1 && void 0 !== arguments[1] && arguments[1],
                n = _.A.isLurking(e);
            await s.Bo.del({
                url: I.Rsh.GUILD_LEAVE(e),
                body: { lurking: n || E.Ay.isCurrentUserGuest(e) },
                oldFormErrors: !0,
                rejectWithError: !1,
            }).then(() => {
                r.O.announce(T.intl.string(T.t["7iPyVW"]));
            }),
                N.close(),
                t && n && (0, f.pX)(I.BVt.GUILD_DISCOVERY);
        },
        async updateMemberRoles(e, t, n, i, r) {
            d.A.isFullServerPreview(e) && t === p.default.getId()
                ? (0, c.ID)(e, n)
                : (await s.Bo.patch({
                      url: I.Rsh.GUILD_MEMBER(e, t),
                      body: { roles: n },
                      oldFormErrors: !0,
                      rejectWithError: !1,
                  }),
                  i.forEach((n) => a.h.dispatch({ type: "GUILD_ROLE_MEMBER_ADD", guildId: e, roleId: n, userId: t })),
                  r.forEach((n) =>
                      a.h.dispatch({ type: "GUILD_ROLE_MEMBER_REMOVE", guildId: e, roleId: n, userId: t }),
                  ));
        },
        bulkAddMemberRoles: (e, t, n) =>
            s.Bo.patch({ url: I.Rsh.GUILD_ROLE_MEMBERS(e, t), body: { member_ids: n }, rejectWithError: !1 }).then(
                (n) => {
                    a.h.dispatch({ type: "GUILD_ROLE_MEMBER_BULK_ADD", guildId: e, roleId: t, added: n.body });
                },
            ),
        enableIntegration: (e, t, n) =>
            s.Bo.post({
                url: I.Rsh.GUILD_INTEGRATIONS(e),
                body: { type: t, id: n },
                oldFormErrors: !0,
                rejectWithError: !1,
            }),
        disableIntegration: (e, t) =>
            s.Bo.del({ url: I.Rsh.GUILD_INTEGRATION(e, t), oldFormErrors: !0, rejectWithError: !1 }),
        updateIntegration: (e, t, n, i, r) =>
            s.Bo.patch({
                url: I.Rsh.GUILD_INTEGRATION(e, t),
                body: { expire_behavior: n, expire_grace_period: i, enable_emoticons: r },
                oldFormErrors: !0,
                rejectWithError: !1,
            }),
        syncIntegration(e, t) {
            s.Bo.post({ url: I.Rsh.GUILD_INTEGRATION_SYNC(e, t), oldFormErrors: !0, rejectWithError: !1 });
        },
        async migratePinPermission(e) {
            await s.Bo.post({ url: I.Rsh.GUILD_MIGRATE_PIN_PERMISSION(e), rejectWithError: !0 }).then(() =>
                a.h.dispatch({ type: "GUILD_SETTINGS_PIN_PERMISSION_MIGRATED", guildId: e }),
            );
        },
        async migrateSlowmodePermission(e) {
            await s.Bo.post({ url: I.Rsh.GUILD_MIGRATE_SLOWMODE_PERMISSION(e), rejectWithError: !0 }).then(() =>
                a.h.dispatch({ type: "GUILD_SETTINGS_SLOWMODE_PERMISSION_MIGRATED", guildId: e }),
            );
        },
        async migratePermissions(e, t) {
            let { migratePin: n, migrateSlowmode: i } = t;
            n && (await N.migratePinPermission(e)), i && (await N.migrateSlowmodePermission(e));
        },
    },
    y = N;
