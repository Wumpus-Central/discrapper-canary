...
function f(e) {
    e = E[e] ?? e;
    let t = (0, i.parse)(e.replace(/[_-]/g, "-"));
    if (null == t || null == t.langtag.language || null == t.langtag.region)
        return void h.error(`${e} is not a valid locale.`);
    let { language: n, region: r } = t.langtag;
    return `${n.language.toLowerCase()}-${r.toUpperCase()}`;
}
class g {
    languageDetector;
    regionPreference;
    ...
    corrections = [];
    constructor(e) {
        const [t, n] = d.default.locale.split("-");
        this.regionPreference = n;
        const i = this.getAvailableLanguages(e);
        (this.languageDetector = new c(t, (n) => {
...
async function A() {
    let e = new g(((await m.getAvailableDictionaries()) ?? []).map(f).filter(_.Vq));
    return null != document.body && document.body.addEventListener("beforeinput", (t) => p(e, t.target), !0), e;
}
