n.d(t, {
    Z: () => O,
    r: () => A
}),
    n(653041),
    n(47120);
var i = n(200651),
    s = n(192379),
    l = n(642128),
    r = n(399606),
    a = n(481060),
    o = n(607070),
    c = n(975298),
    d = n(158153),
    u = n(280942),
    h = n(314684),
    m = n(209747),
    g = n(430824),
    x = n(267642),
    _ = n(709054),
    p = n(357956),
    E = n(275909),
    C = n(300037),
    f = n(981631),
    N = n(474936),
    I = n(388032),
    T = n(305322);
function S(e) {
    let { guildTier: t, guildBoostSlot: n, showAltText: r, isCanceled: o, premiumSubscription: c } = e,
        d = s.useMemo(() => {
            if (o) return I.intl.format(I.t.Z4ULRE, { date: c.currentPeriodEnd });
            let e = null != n.premiumGuildSubscription ? _.default.extractTimestamp(n.premiumGuildSubscription.id) : 0;
            return I.intl.formatToPlainString(I.t.lY2Bur, { date: new Date(e) });
        }, [n, o, c]),
        u = s.useMemo(
            () =>
                (function (e) {
                    if (null == e || e === f.Eu4.NONE) return '';
                    let t = [
                        I.intl.formatToPlainString(I.t.dLlKX1, { numEmojiSlots: N.HO[e].limits.emoji }),
                        I.intl.formatToPlainString(I.t['+ANIfn'], { numStickerSlots: N.HO[e].limits.stickers }),
                        I.intl.formatToPlainString(I.t['4gt60d'], { numSoundboardSlots: N.HO[e].limits.soundboardSounds }),
                        I.intl.formatToPlainString(I.t.XahSjY, {
                            resolution: N.HO[e].limits.screenShareQualityResolution,
                            framerate: N.HO[e].limits.screenShareQualityFramerate
                        }),
                        I.intl.formatToPlainString(I.t.NbNs7e, { bitrate: N.HO[e].limits.bitrate / 1000 }),
                        I.intl.formatToPlainString(I.t.VVKcpq, { filesize: N.HO[e].limits.fileSize / 1024 / 1024 }),
                        I.intl.formatToPlainString(I.t.TbpCvr, { numVideoStageSeats: N.HO[e].limits.stageVideoUsers }),
                        I.intl.string(I.t.LDyX3t),
                        I.intl.string(I.t.YtGlPT)
                    ];
                    e >= f.Eu4.TIER_2 && (t.push(I.intl.string(I.t.SztbtL)), t.push(I.intl.string(I.t['3GK91t']))), e >= f.Eu4.TIER_3 && t.push(I.intl.string(I.t.XUUJd3));
                    let n = t[Math.floor(Math.random() * t.length)];
                    return I.intl.format(I.t['/dOAmZ'], { perk: n });
                })(t),
            [t]
        ),
        h = (0, a.Yzy)(
            r,
            {
                from: { opacity: 0 },
                enter: { opacity: 1 },
                leave: { opacity: 0 }
            },
            'animate-always'
        );
    return o || t === f.Eu4.NONE
        ? (0, i.jsx)('div', {
              className: T.boostDescriptionInnerContainer,
              children: (0, i.jsx)(a.Text, {
                  variant: 'text-sm/medium',
                  color: 'text-muted',
                  children: d
              })
          })
        : h((e, t) =>
              (0, i.jsx)(l.animated.div, {
                  style: e,
                  className: T.boostDescriptionInnerContainer,
                  children: (0, i.jsx)(a.Text, {
                      variant: 'text-sm/medium',
                      color: 'text-muted',
                      children: t ? u : d
                  })
              })
          );
}
function j(e) {
    let { guildTier: t, guildBoostSlot: n, premiumSubscription: l, hasCancelableGuildBoostSlot: d, showAltText: h, isLastGuildBoostSlot: m } = e,
        g = (0, r.e7)([o.Z], () => o.Z.useReducedMotion),
        _ = s.useMemo(() => (null != n.cooldownEndsAt ? new Date(n.cooldownEndsAt) : null), [n]),
        C = s.useMemo(() => null != _ && _ > new Date(), [_]),
        f = (0, x.tl)(n),
        { fractionalState: N } = (0, c.Z)();
    return (0, i.jsxs)('div', {
        className: T.boostContainer,
        children: [
            (0, i.jsxs)('div', {
                className: T.boostInnerContainer,
                children: [
                    (0, i.jsx)(E.Z, {
                        isCanceled: f,
                        hasCooldown: C,
                        useReducedMotion: g
                    }),
                    (0, i.jsx)('div', {
                        className: T.boostDescriptionContainer,
                        children:
                            null != _ && C && !f
                                ? (0, i.jsx)(p.Z, {
                                      className: T.boostDescriptionInnerContainer,
                                      cooldown: _.getTime()
                                  })
                                : (0, i.jsx)(S, {
                                      guildTier: t,
                                      guildBoostSlot: n,
                                      showAltText: h,
                                      isCanceled: f,
                                      premiumSubscription: l
                                  })
                    }),
                    (0, i.jsx)(a.yRy, {
                        renderPopout: function (e) {
                            let { closePopout: t } = e;
                            return (0, i.jsx)(u.Z, {
                                onClose: t,
                                guildBoostSlot: n,
                                premiumSubscription: l,
                                hasCancelableGuildBoostSlot: d,
                                fractionalState: N
                            });
                        },
                        position: 'right',
                        align: 'center',
                        children: (e) =>
                            (0, i.jsx)(a.P3F, {
                                ...e,
                                'aria-label': I.intl.string(I.t.PdRCRk),
                                className: T.boostSlotMenuIcon,
                                children: (0, i.jsx)(a.Huf, {
                                    size: 'md',
                                    color: 'currentColor'
                                })
                            })
                    })
                ]
            }),
            !m && (0, i.jsx)('div', { className: T.boostContainerSeparator })
        ]
    });
}
function v(e) {
    let { guildId: t, guildBoostSlotRecords: n, premiumSubscription: s, hasCancelableGuildBoostSlot: l, showAltText: a } = e,
        o = (0, r.e7)([g.Z], () => g.Z.getGuild(t), [t]);
    return (0, i.jsxs)('div', {
        className: T.appliedBoostContainer,
        children: [
            (0, i.jsx)(C.Z, {
                guildId: t,
                boostingVariant: !1
            }),
            n.map((e, t) =>
                (0, i.jsx)(
                    j,
                    {
                        guildTier: null == o ? void 0 : o.premiumTier,
                        guildBoostSlot: e,
                        premiumSubscription: s,
                        hasCancelableGuildBoostSlot: l,
                        showAltText: a,
                        isLastGuildBoostSlot: t === n.length - 1
                    },
                    e.id
                )
            )
        ]
    });
}
function b(e) {
    let { guildId: t, appliedGuildBoosts: n, premiumSubscription: s } = e,
        l = (0, r.e7)([g.Z], () => g.Z.getGuild(t), [t]),
        a = _.default.fromTimestamp(Date.now());
    if (
        (n.forEach((e) => {
            (null == a || 0 > _.default.compare(e.id, a)) && (a = e.id);
        }),
        null == a)
    )
        return null;
    let o = m.Z.createFromServer(
        {
            id: _.default.fromTimestamp(Date.now()),
            subscription_id: s.id,
            canceled: !1,
            premium_guild_subscription: {
                id: a,
                guild_id: t
            },
            cooldown_ends_at: null
        },
        s
    );
    return (0, i.jsxs)('div', {
        className: T.appliedBoostContainer,
        children: [
            (0, i.jsx)(C.Z, {
                guildId: t,
                boostingVariant: !1
            }),
            n.map((e, t) =>
                (0, i.jsx)(
                    j,
                    {
                        guildTier: null == l ? void 0 : l.premiumTier,
                        guildBoostSlot: o,
                        premiumSubscription: s,
                        hasCancelableGuildBoostSlot: !1,
                        showAltText: !1,
                        isLastGuildBoostSlot: t === n.length - 1
                    },
                    t
                )
            )
        ]
    });
}
function A(e) {
    let { appliedGuildBoosts: t, premiumSubscription: n } = e,
        l = s.useMemo(() => {
            let e = {};
            return (
                t.forEach((t) => {
                    let n = t.guildId;
                    n in e || (e[n] = []), e[n].push(t);
                }),
                e
            );
        }, [t]);
    return null == n || 0 === t.length
        ? null
        : (0, i.jsxs)('div', {
              className: T.wrapper,
              children: [
                  (0, i.jsx)('div', {
                      className: T.container,
                      children: _.default.keys(l).map((e) =>
                          (0, i.jsx)(
                              b,
                              {
                                  guildId: e,
                                  premiumSubscription: n,
                                  appliedGuildBoosts: l[e]
                              },
                              e
                          )
                      )
                  }),
                  (0, i.jsx)('div', { className: T.mainSeparator })
              ]
          });
}
function O(e) {
    let { guildBoostSlots: t, premiumSubscription: n } = e,
        [l, r] = s.useState(!1),
        a = (0, h.Vp)();
    s.useEffect(() => {
        let e = -1;
        return (
            null != n &&
                (e = setInterval(() => {
                    r((e) => !e);
                }, 3000)),
            () => {
                clearInterval(e);
            }
        );
    }, [n]);
    let { boostsByGuildId: o, numActiveGuildBoostSlots: c } = s.useMemo(() => {
        let e = 0,
            n = {};
        return (
            Object.keys(t).forEach((i) => {
                let s = t[i];
                if ((!(0, x.tl)(s) && e++, null != s.premiumGuildSubscription)) {
                    let e = s.premiumGuildSubscription.guildId;
                    e in n || (n[e] = []), n[e].push(s);
                }
            }),
            {
                boostsByGuildId: n,
                numActiveGuildBoostSlots: e
            }
        );
    }, [t]);
    if (null == n || 0 === Object.keys(o).length) return null;
    let u = c > (0, d.G)(n, a);
    return (0, i.jsxs)('div', {
        className: T.wrapper,
        children: [
            (0, i.jsx)('div', {
                className: T.container,
                children: _.default.keys(o).map((e) =>
                    (0, i.jsx)(
                        v,
                        {
                            guildId: e,
                            guildBoostSlotRecords: o[e],
                            premiumSubscription: n,
                            hasCancelableGuildBoostSlot: u,
                            showAltText: l
                        },
                        e
                    )
                )
            }),
            (0, i.jsx)('div', { className: T.mainSeparator })
        ]
    });
}
