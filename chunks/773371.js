...
function T() {
    0 !== I.length &&
        (o.h.dispatch({ type: "OVERLAY_ADD_LOGS_BATCH", logs: I }),
        (I = []),
    ...
    try {
        if (G.Ay.supportsFeature(F.BYE.CREATE_HOST_ON_ATTACH))
            if (W.size > 0) {
                ...
                let t = await ef();
                (ea = "reconcile.createHostProcess"), e.createHostProcess(t, eI, eA);
            } else (ea = "reconcile.destroyHostProcess"), e.destroyHostProcess(), em((0, x.getPID)());
        else if (j) {
            let t = await ef();
            e.createHostProcess(t, eI, eA);
...
class ey extends s.Ay.Store {
    ...
    initialize() {
        !(0, x.supportsLegacy)() ||
            __OVERLAY__ ||
            (this.waitFor(P.default, g.A, b.A, k.A, C.default, L.A, v.A, D.A, A.Ay),
        ...
        __OVERLAY__
            ...
            : {
                  ...
                  RPC_SERVER_READY: function (e) {
                      ...
                      n.append("build_id", "0a0718a062fd94a3495a186cf5f494e82e2aa84b"),
