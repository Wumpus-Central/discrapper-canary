...
class ee extends J {
    shouldShowEmptyCategory() {
        return !!(super.shouldShowEmptyCategory() || (A.Z.can(G.Pl.MANAGE_CHANNELS, this.record) && A.Z.can(G.Pl.VIEW_CHANNEL, this.record) && s().isEmpty(this.channels) && (!this.guild.optInEnabled || this.guild.optedInChannels.has(this.id))));
    }
    constructor(e, t, n, i) {
...
class ea {
    ...
    get lastMessageTimestamp() {
        return Math.max(O.ZP.lastMessageTimestamp(this.id), ...this.threadIds.map(O.ZP.lastMessageTimestamp));
    }
    updateChannel(e, t) {
        let n = !1;
...
class eo extends ea {
    computeState(e) {
        ...
        if (!A.Z.can(G.Pl.VIEW_CHANNEL, this.record)) {
            if (this.id === i)
                return {
                    renderLevel: 4,
                    threadIds: []
        ...
        return o.optInEnabled && o.hideResourceChannels && this.record.hasFlag(F.zZ.IS_GUILD_RESOURCE_CHANNEL)
            ...
            : !o.optInEnabled || o.optedInChannels.has(this.id) || (null != a && o.optedInChannels.has(a))
              ? d || c || !s().isEmpty(g) || O.ZP.getMentionCount(this.id) > 0
                  ? {
                        renderLevel: 4,
                        threadIds: g
                    }
                  : o.hideMutedChannels && o.mutedChannelIds.has(this.id)
                    ...
                    : this.category.isCollapsed && (o.mutedChannelIds.has(this.id) || (null != a && o.mutedChannelIds.has(a)) || this.record.isGuildVocal() || this.record.type === U.d4z.GUILD_STORE || ((0, I.vc)(this.record.type) && !O.ZP.hasUnread(this.record.id)))
                      ? {
                            renderLevel: 3,
                            threadIds: g
                        }
...
class ed extends ea {
    computeState(e) {
        ...
        return A.Z.can(G.Pl.VIEW_CHANNEL, this.record)
            ? {
                  renderLevel: 4,
...
class ec extends eo {
    computeState(e) {
        ...
        if (t > 1) {
            ...
            l.mutedChannelIds.has(this.id) || (null != i && l.mutedChannelIds.has(i)) ? (t = 2) : 4 === t ? (t = 3) : 2 === t && ev(this.category.guild, this.record, e) && (t = 3), 3 === t && eC(this, e) && (t = 4), (n = s().sortBy(n, (e) => -O.ZP.lastMessageTimestamp(e)));
        }
        return {
            renderLevel: t,
            threadIds: n
...
class eh extends eo {
    getRenderLevel(e) {
        ...
        return !A.Z.can(G.Pl.VIEW_CHANNEL, this.record) || 4 === e || 3 === e || ep(t, this.record) ? 1 : this.category.isCollapsed ? (s().some(L.Z.getVoiceStatesForChannel(this.record.id)) ? 4 : 3) : 4;
    }
    computeState(e) {
...
function ef(e, t, n, i, l) {
    ...
    if (I.uC.has(e.type)) {
        let e = s()
            ...
            .map((e) => e.channel.id);
        return r ? e : a ? (n.id in t || e.unshift(n.id), e) : l ? e.filter((e) => !m.Z.isMuted(e) || O.ZP.getMentionCount(e) > 0) : e;
    }
    return [];
}
function ep(e, t) {
...
function eC(e, t) {
    ...
    if (O.ZP.getMentionCount(e.id) > 0) return !0;
...
function ev(e, t, n) {
    ...
    if ((null != i && (i.id === t.id || (i.isThread() && i.parent_id === t.id))) || O.ZP.getMentionCount(t.id) > 0) return !0;
    let r = E.Z.getNewChannelIds(e.id),
        s = Array.from(r).sort((e, t) => D.default.compare(t, e));
    if (r.has(t.id) && s.indexOf(t.id) < B) return !0;
    for (let e in l[t.id]) if (O.ZP.getMentionCount(e) > 0 || O.ZP.hasUnread(e) || O.ZP.hasRecentlyVisitedAndRead(e)) return !0;
    return !(e.mutedChannelIds.has(t.id) || (null != t.parent_id && e.mutedChannelIds.has(t.parent_id))) && !!O.ZP.hasRecentlyVisitedAndRead(t.id);
}
