...
function lw(e) {
    ...
    a.useEffect(() => {
        ey === z.oH.HOME && eN && to.P.getState().setActiveMainLink(z.oH.ALL_CLIPS);
    }, [ey, eN]);
    let ek = a.useMemo(() => (null != ej ? [ej] : []), [ej]),
        ...
        ew = a.useMemo(() => {
            if (null != ej) {
                let e = X.find((e) => e.key === ej)?.name ?? eb.intl.string(eb.t.dPVrEv);
        ...
        eP = a.useCallback(() => {
            eo("editing"), y.A.useReducedMotion && eg(x.ip.ENTERED);
        }, []),
        eI = a.useCallback((e) => {
            B((t) => {
                let l = new Set(t);
