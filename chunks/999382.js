let i, r, a, s, o, l, u, c, d, f;
n.d(t, { Z: () => e0 }), n(47120), n(789020), n(724458);
var _,
    p = n(392711),
    h = n.n(p),
    m = n(913527),
    g = n.n(m),
    E = n(442837),
    v = n(544891),
    y = n(433517),
    I = n(570140),
    T = n(749210),
    b = n(314852),
    S = n(131704),
    A = n(601964),
    N = n(758449),
    C = n(598077),
    R = n(430824),
    O = n(594174),
    D = n(709054),
    L = n(372454),
    x = n(330010),
    P = n(978946),
    w = n(981631),
    M = n(128449),
    k = n(135899);
function U(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0
              })
            : (e[t] = n),
        e
    );
}
let G = !0,
    B = ['name', 'description', 'icon', 'splash', 'banner', 'homeHeader', 'afkChannelId', 'afkTimeout', 'systemChannelId', 'verificationLevel', 'defaultMessageNotifications', 'explicitContentFilter', 'features', 'systemChannelFlags', 'preferredLocale', 'rulesChannelId', 'safetyAlertsChannelId', 'discoverySplash', 'publicUpdatesChannelId', 'premiumProgressBarEnabled', 'profile'],
    Z = ['brandColorPrimary', 'description', 'icon', 'name', 'traits', 'visibility', 'gameApplicationIds'],
    F = new Set(['icon', 'splash', 'banner', 'discoverySplash', 'homeHeader']),
    V = !1,
    j = w.QZA.CLOSED,
    H = {},
    Y = null,
    W = !1,
    K = null,
    z = null,
    q = 0,
    Q = w.BpS.NONE,
    X = null,
    J = {
        primaryCategoryId: M.o3,
        secondaryCategoryIds: [],
        keywords: [],
        emojiDiscoverabilityEnabled: G,
        partnerActionedTimestamp: null,
        partnerApplicationTimestamp: null,
        isPublished: !1,
        reasonsToJoin: [],
        socialLinks: [],
        about: ''
    },
    $ = !1,
    ee = J,
    et = J,
    en = null,
    ei = 0,
    er = null,
    ea = null,
    es = null;
function eo(e) {
    if (null == o || null == s || s.id !== e) return !1;
    let t = R.Z.getGuild(e);
    return null != t && (s === o ? (o = s = t) : (s = t), !0);
}
function el(e) {
    (V = !0), eu(e);
}
function eu(e) {
    var t;
    let { guildId: n, section: r, subsection: a, location: d } = e,
        _ = R.Z.getGuild(n);
    if (null == _) return ec();
    let p = b.Z.getProfile(n);
    (s = o = _),
        (l = u = p),
        (j = w.QZA.OPEN),
        (H = {}),
        (Y = null),
        (c = D.default.castGuildIdAsEveryoneGuildRoleId(n)),
        (Q = o.mfaLevel),
        (et = ee),
        (f = null),
        (X = d),
        ef({
            section: null !== (t = null != r ? r : i) && void 0 !== t ? t : (0, P.r)(n),
            subsection: null != a ? a : null
        });
}
function ec() {
    (V = !1), (j = w.QZA.CLOSED), (s = o = null), (W = !1), (K = null), (z = null), (q = 0), (en = null), (ea = null), (es = null), (i = null), (r = null), (a = null), (Q = w.BpS.NONE), (d = void 0);
}
function ed(e) {
    let { state: t } = e;
    return (d = t), !1;
}
function ef(e) {
    if (null == o) return !1;
    let t = i;
    if (((i = e.section), (r = e.subsection), i === w.pNK.BANS)) {
        let { enabled: e } = L.T.getCurrentConfig({
            guildId: o.id,
            location: '7f0c91_1'
        });
        e || T.Z.fetchGuildBans(o.id);
    } else if (i === w.pNK.INSTANT_INVITES || i === w.pNK.INVITES)
        v.tn
            .get({
                url: w.ANM.GUILD_INSTANT_INVITES(o.id),
                oldFormErrors: !0,
                rejectWithError: !0
            })
            .then((e) => {
                I.Z.dispatch({
                    type: 'GUILD_SETTINGS_LOADED_INVITES',
                    invites: e.body
                });
            });
    else if (i === w.pNK.WIDGET)
        v.tn
            .get({
                url: w.ANM.GUILD_WIDGET(o.id),
                oldFormErrors: !0,
                rejectWithError: !0
            })
            .then((e) => {
                I.Z.dispatch({
                    type: 'GUILD_SETTINGS_SET_WIDGET',
                    enabled: e.body.enabled,
                    channelId: e.body.channel_id
                });
            });
    else if (i === w.pNK.INTEGRATIONS || i === w.pNK.ROLES) {
        if (((c = null), t !== e.section)) return eB(e);
    } else
        i === w.pNK.MEMBERS
            ? (c = o.getEveryoneRoleId())
            : i === w.pNK.VANITY_URL
              ? v.tn
                    .get({
                        url: w.ANM.GUILD_VANITY_URL(o.id),
                        oldFormErrors: !0,
                        rejectWithError: !0
                    })
                    .then((e) => {
                        let {
                            body: { code: t, uses: n, error: i }
                        } = e;
                        I.Z.dispatch({
                            type: 'GUILD_SETTINGS_SET_VANITY_URL',
                            code: t,
                            uses: n,
                            error: i
                        });
                    })
              : i === w.pNK.SAFETY &&
                I.Z.dispatch({
                    type: 'GUILD_SETTINGS_SAFETY_SET_SUBSECTION',
                    subsection: null == r ? w.KsC.SAFETY_OVERVIEW : r
                });
}
function e_(e) {
    a = e.searchQuery;
}
function ep(e) {
    let { guildId: t } = e;
    H = {};
    let n = R.Z.getGuild(t);
    null != n && (s = o = n);
}
function eh() {
    (j = w.QZA.SUBMITTING), (H = {});
}
function em() {
    j = w.QZA.OPEN;
}
function eg(e) {
    var t;
    (j = w.QZA.OPEN), (i = null != i ? i : (0, P.r)(null == o ? void 0 : o.id)), (r = null), (H = null !== (t = e.errors) && void 0 !== t ? t : {});
}
function eE() {
    if (null == o) return !1;
    let e = o.toJS(),
        t = s.toJS();
    B.some((n) => e[n] !== t[n]) || (o = s);
}
function ev(e) {
    if (null == o) return !1;
    B.forEach((t) => {
        null != o && e.hasOwnProperty(t) && (o = o.set(t, e[t]));
    }),
        eE();
}
function ey(e) {
    let { guildId: t } = e;
    if (null == u || null == o || o.id !== t) return !1;
    Z.forEach((t) => {
        if (null != u && e.hasOwnProperty(t)) {
            let n = u[t],
                i = e[t];
            null != i &&
                (u = {
                    ...u,
                    [t]: null != i ? i : n
                });
        }
    });
}
function eI(e) {
    var t;
    return new N.Z({
        code: e.code,
        temporary: e.temporary,
        revoked: e.revoked,
        inviter: null != e.inviter ? new C.Z(e.inviter) : null,
        channel: (0, S.jD)(e.channel),
        guild: null != e.guild ? new A.ZP(e.guild) : null,
        uses: e.uses,
        maxUses: e.max_uses,
        maxAge: e.max_age,
        createdAt: g()(null !== (t = e.created_at) && void 0 !== t ? t : void 0),
        flags: e.flags
    });
}
function eT(e) {
    ea = e.invites.reduce((e, t) => ((e[t.code] = eI(t)), e), {});
}
function eb(e) {
    (ea = { ...ea }), delete ea[e.code];
}
function eS(e) {
    ea = {
        ...ea,
        [e.invite.code]: eI(e.invite)
    };
}
function eA(e) {
    (en = e.bans.reduce((e, t) => (null != t.user && null != t.user.id && e.set(t.user.id, t), e), new Map())), ei++;
}
function eN(e) {
    let { bans: t, guildId: n } = e;
    (er !== n || null == en) && ((er = n), (en = new Map())), (en = t.reduce((e, t) => (null != t.user && null != t.user.id && e.set(t.user.id, t), e), en)), ei++;
}
function eC(e) {
    let { user: t, guildId: n } = e;
    if (null == en || null == o || o.id !== n) return !1;
    en.set(t.id, {
        user: t,
        reason: null
    }),
        ei++;
}
function eR(e) {
    let { user: t, guildId: n } = e;
    if (null == en || null == o || o.id !== n) return !1;
    en.delete(t.id), ei++;
}
function eO(e) {
    if (null == o || o.id !== e.guild.id) return !1;
    {
        let e = R.Z.getGuild(o.id);
        if (null == e) return !1;
        let t = (s = e),
            n = o.toJS();
        B.forEach((e) => {
            if (!F.has(e) && (('rulesChannelId' !== e && 'publicUpdatesChannelId' !== e) || n[e] !== k.b4)) {
                if ('features' === e) {
                    t.set(e, new Set(n[e]));
                    return;
                }
                t = t.set(e, n[e]);
            }
        }),
            (o = t);
    }
}
function eD(e) {
    if (null == o || o.id !== e.guild.id) return !1;
    ec();
}
function eL(e) {
    let { roleId: t } = e;
    c = null != t ? t : null;
}
function ex(e) {
    let { guildId: t } = e;
    if (!eo(t)) return !1;
}
function eP(e) {
    let { guildId: t } = e;
    if (!eo(t)) return !1;
}
function ew(e) {
    let { guildId: t, roleId: n } = e;
    if (!eo(t)) return !1;
    c === n && (c = null);
}
function eM(e) {
    (W = e.enabled), (K = e.channelId);
}
function ek(e) {
    var t;
    (z = null !== (t = e.code) && void 0 !== t ? t : null), (q = e.uses);
}
function eU(e) {
    let { level: t } = e;
    Q = t;
}
function eG(e) {
    es = e.integrations;
}
function eB(e) {
    if (null == o || j !== w.QZA.OPEN || ('GUILD_INTEGRATIONS_UPDATE' === e.type && e.guildId !== o.id)) return !1;
    (0, x.i)(o.id);
}
function eZ(e) {
    var t, n, i, r, a, s, l, u, c, d;
    let { guildId: f, metadata: _ } = e;
    null != o &&
        f === o.id &&
        (!1 === $ && ($ = !0),
        (et = ee =
            {
                primaryCategoryId: null !== (t = _.primaryCategoryId) && void 0 !== t ? t : M.o3,
                secondaryCategoryIds: null !== (n = _.secondaryCategoryIds) && void 0 !== n ? n : [],
                keywords: null !== (i = _.keywords) && void 0 !== i ? i : [],
                emojiDiscoverabilityEnabled: null !== (r = _.emojiDiscoverabilityEnabled) && void 0 !== r ? r : G,
                partnerActionedTimestamp: null !== (a = _.partnerActionedTimestamp) && void 0 !== a ? a : null,
                partnerApplicationTimestamp: null !== (s = _.partnerApplicationTimestamp) && void 0 !== s ? s : null,
                isPublished: null !== (l = _.isPublished) && void 0 !== l && l,
                reasonsToJoin: null !== (u = _.reasonsToJoin) && void 0 !== u ? u : [],
                socialLinks: null !== (c = _.socialLinks) && void 0 !== c ? c : [],
                about: null !== (d = _.about) && void 0 !== d ? d : ''
            }),
        (H = {}));
}
function eF() {
    ee = et = J;
}
function eV(e) {
    let { slug: t } = e;
    f = t;
}
function ej(e) {
    let {} = e;
    f = null;
}
function eH(e) {
    let { guildId: t, categoryId: n } = e;
    null != o &&
        t === o.id &&
        ((et = {
            ...et,
            secondaryCategoryIds: [...et.secondaryCategoryIds, n]
        }),
        (ee = {
            ...ee,
            secondaryCategoryIds: [...ee.secondaryCategoryIds, n]
        }));
}
function eY(e) {
    let t,
        { guildId: n, categoryId: i } = e;
    if (null == o || n !== o.id) return;
    let r = et.secondaryCategoryIds.indexOf(i);
    -1 !== r &&
        ((t = [...et.secondaryCategoryIds]).splice(r, 1),
        (et = {
            ...et,
            secondaryCategoryIds: t
        })),
        -1 !== (r = ee.secondaryCategoryIds.indexOf(i)) &&
            ((t = [...ee.secondaryCategoryIds]).splice(r, 1),
            (ee = {
                ...ee,
                secondaryCategoryIds: t
            }));
}
function eW(e) {
    let { guildId: t, errors: n } = e;
    null != o && t === o.id && (H = null != n ? n : {});
}
function eK(e) {
    let { guildId: t, primaryCategoryId: n, keywords: i, emojiDiscoverabilityEnabled: r, isPublished: a, reasonsToJoin: s, socialLinks: l, about: u } = e;
    null != o &&
        t === o.id &&
        (et = {
            ...et,
            primaryCategoryId: null != n ? n : et.primaryCategoryId,
            keywords: null != i ? i : et.keywords,
            emojiDiscoverabilityEnabled: null != r ? r : et.emojiDiscoverabilityEnabled,
            isPublished: null != a ? a : et.isPublished,
            reasonsToJoin: null != s ? s : et.reasonsToJoin,
            socialLinks: null != l ? l : et.socialLinks,
            about: null != u ? u : et.about
        });
}
function ez(e) {
    let { guildId: t, errors: n } = e;
    null != o && t === o.id && (H = null != n ? n : {});
}
function eq(e) {
    let { profile: t } = e;
    t.id === (null == o ? void 0 : o.id) && (l = u = t);
}
function eQ(e) {
    let { guildId: t } = e;
    if (null == o || o.id !== t) return !1;
    Y = null;
}
function eX(e) {
    let { profile: t } = e;
    if ((null == u ? void 0 : u.id) == null || !eo(u.id)) return !1;
    t.id === (null == o ? void 0 : o.id) && ((l = u = t), (Y = null));
}
function eJ(e) {
    let { guildId: t, error: n } = e;
    if (null == o || o.id !== t) return !1;
    Y = n;
}
class e$ extends (_ = E.ZP.Store) {
    initialize() {
        this.waitFor(R.Z, b.Z, O.default);
    }
    getMetadata() {
        return et;
    }
    hasChanges() {
        return !h().isEqual(o, s) || !h().isEqual(et, ee) || !h().isEqual(u, l);
    }
    isOpen() {
        return V;
    }
    getSavedRouteState() {
        return d;
    }
    getSection() {
        return i;
    }
    showNotice() {
        return this.hasChanges();
    }
    getGuildId() {
        return null != o ? o.id : null;
    }
    showPublicSuccessModal() {
        return !y.K.get(k.zs);
    }
    getGuild() {
        return o;
    }
    getGuildProfile() {
        return u;
    }
    isSubmitting() {
        return j === w.QZA.SUBMITTING;
    }
    isGuildMetadataLoaded() {
        return $;
    }
    getErrors() {
        return H;
    }
    getError(e) {
        var t;
        return null !== (t = H[e]) && void 0 !== t ? t : null;
    }
    getProfileError() {
        return Y;
    }
    getSelectedRoleId() {
        return c;
    }
    getSlug() {
        return f;
    }
    getBans() {
        return [en, ei];
    }
    getProps() {
        return {
            submitting: this.isSubmitting(),
            integrations: es,
            section: i,
            subsection: r,
            errors: H,
            guild: o,
            bans: en,
            bansVersion: ei,
            invites: ea,
            selectedRoleId: c,
            embedEnabled: W,
            embedChannelId: K,
            mfaLevel: Q,
            searchQuery: a,
            vanityURLCode: z,
            vanityURLUses: q,
            originalGuild: s,
            hasChanges: this.hasChanges(),
            guildMetadata: et,
            analyticsLocation: X,
            isGuildMetadataLoaded: $
        };
    }
}
U(e$, 'displayName', 'GuildSettingsStore');
let e0 = new e$(
    I.Z,
    __OVERLAY__
        ? {}
        : {
              GUILD_SETTINGS_INIT: eu,
              GUILD_SETTINGS_OPEN: el,
              GUILD_SETTINGS_CLOSE: ec,
              GUILD_SETTINGS_UPDATE: ev,
              GUILD_SETTINGS_PROFILE_UPDATE: ey,
              GUILD_SETTINGS_CANCEL_CHANGES: ep,
              GUILD_SETTINGS_SAVE_ROUTE_STACK: ed,
              GUILD_SETTINGS_SUBMIT: eh,
              GUILD_SETTINGS_SUBMIT_SUCCESS: em,
              GUILD_SETTINGS_SUBMIT_FAILURE: eg,
              GUILD_SETTINGS_SET_SECTION: ef,
              GUILD_SETTINGS_SET_SEARCH_QUERY: e_,
              GUILD_SETTINGS_LOADED_BANS: eA,
              GUILD_SETTINGS_LOADED_BANS_BATCH: eN,
              GUILD_SETTINGS_LOADED_INVITES: eT,
              GUILD_SETTINGS_SET_WIDGET: eM,
              GUILD_SETTINGS_SET_VANITY_URL: ek,
              GUILD_SETTINGS_SET_MFA_SUCCESS: eU,
              GUILD_SETTINGS_ROLE_SELECT: eL,
              GUILD_SETTINGS_LOADED_INTEGRATIONS: eG,
              GUILD_BAN_ADD: eC,
              GUILD_BAN_REMOVE: eR,
              GUILD_ROLE_CREATE: ex,
              GUILD_ROLE_UPDATE: eP,
              GUILD_ROLE_DELETE: ew,
              GUILD_UPDATE: eO,
              GUILD_DELETE: eD,
              GUILD_PROFILE_FETCH_SUCCESS: eq,
              GUILD_PROFILE_UPDATE: eQ,
              GUILD_PROFILE_UPDATE_SUCCESS: eX,
              GUILD_PROFILE_UPDATE_FAILURE: eJ,
              USER_CONNECTIONS_UPDATE: eB,
              GUILD_INTEGRATIONS_UPDATE: eB,
              INSTANT_INVITE_REVOKE_SUCCESS: eb,
              INSTANT_INVITE_CREATE_SUCCESS: eS,
              GUILD_UPDATE_DISCOVERY_METADATA_FROM_SERVER: eZ,
              GUILD_DISCOVERY_METADATA_FETCH_FAIL: eF,
              GUILD_DISCOVERY_CATEGORY_ADD: eH,
              GUILD_DISCOVERY_CATEGORY_DELETE: eY,
              GUILD_DISCOVERY_CATEGORY_UPDATE_FAIL: eW,
              GUILD_UPDATE_DISCOVERY_METADATA: eK,
              GUILD_UPDATE_DISCOVERY_METADATA_FAIL: ez,
              GUILD_DISCOVERY_SLUG_FETCH_SUCCESS: eV,
              GUILD_DISCOVERY_SLUG_FETCH_FAIL: ej
          }
);
