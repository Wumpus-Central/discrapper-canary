"use strict";
n.r(t), n.d(t, { default: () => Y }), n(323874), n(14289), n(35956), n(134528), n(947204);
var r = n(607399),
    i = n(696292),
    a = n(73153),
    s = n(308528),
    o = n(846293),
    l = n(795816),
    u = n(956549),
    c = n(257269),
    d = n(574152),
    _ = n(58149),
    f = n(121401),
    p = n(587895),
    h = n(725606),
    m = n(167189),
    g = n(833291),
    E = n(707592),
    A = n(698441),
    I = n(268313),
    T = n(376943),
    y = n(22007),
    S = n(677402),
    v = n(807876),
    C = n(545986),
    b = n(976860),
    N = n(788995),
    R = n(556022);
n(345953);
var O = n(961350),
    D = n(696451),
    L = n(299091),
    w = n(309010),
    x = n(967198),
    P = n(711014),
    M = n(954571),
    k = n(67486),
    U = n(998218),
    G = n(975807),
    V = n(652215);
n(758836);
var F = n(654487);
async function B(e, t) {
    await a.h.dispatch({ type: "INVITE_MODAL_OPEN", invite: e, code: t, context: V.BRT.APP });
}
async function j(e) {
    let t = L.A.getInvite(e.code);
    if (null == t) {
        let { invite: n } = await o.Ay.resolveInvite(e.code, "Markdown Link");
        t = n;
    }
    if (null == t) return;
    if (t.state === V.elq.EXPIRED || t.state === V.elq.BANNED || t.state === V.elq.ERROR)
        return void (await B(t, e.code));
    let n = P.Ay.getFlattenedGuildIds(),
        r = t?.guild?.id,
        i = null != r && n.includes(r),
        a = !1;
    if (i && null != t.roles && t.roles.length > 0) {
        let e = O.default.getId(),
            n = D.Ay.getMember(r, e),
            i = new Set(n?.roles ?? []);
        a = t.roles.some((e) => !i.has(e.id));
    }
    i && !a ? o.Ay.transitionToInviteSync(t) : await B(t, e.code);
}
let H = { skipExtensionCheck: void 0, analyticsLocations: [] };
function Y(e) {
    let { skipExtensionCheck: t, analyticsLocations: a } =
            arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : H,
        o = (0, g.br)(e);
    if (null != o && (o.type === m.I.INVITE || o.type === m.I.EMBEDDED_ACTIVITY_INVITE))
        return (e) => (e?.preventDefault(), j(o), !0);
    if (
        null != o &&
        (o.type === m.I.APP_DIRECTORY_PROFILE ||
            o.type === m.I.APP_DIRECTORY_STOREFRONT ||
            o.type === m.I.APP_DIRECTORY_STOREFRONT_SKU)
    )
        return (t) => {
            t?.preventDefault();
            let { code: i } = o,
                { applicationId: a, skuId: s } =
                    o.type === m.I.APP_DIRECTORY_PROFILE || o.type === m.I.APP_DIRECTORY_STOREFRONT
                        ? { applicationId: i, skuId: void 0 }
                        : ((0, f.u)(i) ?? { applicationId: void 0, skuId: void 0 }),
                l = x.A.getGuildId() ?? void 0;
            return (
                null == a
                    ? (0, G.A)(e)
                    : (M.default.track(V.HAw.APP_DIRECTORY_PROFILE_EMBED_URL_CLICKED, {
                          application_id: a,
                          device_platform: r.Fr ? "mobile_web" : "desktop_web",
                          guild_id: l,
                          channel_id: w.A.getChannelId(),
                      }),
                      n
                          .e("38042")
                          .then(n.bind(n, 837057))
                          .then((e) => {
                              let { transitionToGlobalDiscovery: t } = e,
                                  { GlobalDiscoveryTab: r } = n(488995),
                                  { GlobalDiscoveryAppsSections: i } = n(435220);
                              t({
                                  tab: r.APPS,
                                  applicationId: a,
                                  section: (0, I.A)(i, o.type === m.I.APP_DIRECTORY_PROFILE ? i.ABOUT : i.STORE),
                                  skuId: s,
                              });
                          })),
                !0
            );
        };
    if (null != o && o.type === m.I.ACTIVITY_BOOKMARK)
        return (e) => {
            e?.preventDefault();
            let { code: t, url: n } = o,
                r = p.A.getApplication(t),
                i = new URL(n),
                _ = i.searchParams.get("referrer_id") ?? void 0,
                f = (0, d.A)(),
                { currentChannelId: m, instanceId: g, isCurrentlyInInstance: E, canLaunchInChannel: A } = (0, h.t)(t);
            if (A)
                return (
                    !E &&
                    null != m &&
                    ((0, c.d9)(t, i.searchParams.get("link_id"), i.searchParams.get("custom_id"))
                        .then(async (e) => {
                            let { customId: n } = e;
                            await (0, l.su)({
                                channelId: m,
                                applicationId: t,
                                isStart: null == g,
                                embeddedActivitiesManager: f,
                                customId: n,
                                referrerId: _,
                                analyticsLocations: a,
                            });
                        })
                        .catch(() => {}),
                    !0)
                );
            {
                let e = r?.bot?.id;
                return (
                    null != e &&
                    (s.A.openPrivateChannel({ recipientIds: e })
                        .then(async (e) => {
                            let { customId: n } = await (0, c.d9)(
                                t,
                                i.searchParams.get("link_id"),
                                i.searchParams.get("custom_id"),
                            );
                            (0, u.A)({
                                targetApplicationId: t,
                                channelId: e,
                                analyticsLocations: a,
                                customId: n,
                                referrerId: _,
                            });
                        })
                        .catch(() => {}),
                    !0)
                );
            }
        };
    if (null != o && o.type === m.I.GUILD_PRODUCT)
        return (e) => {
            e?.preventDefault();
            let [t, r] = o.code.split("-");
            return (
                n
                    .e("49607")
                    .then(n.bind(n, 811546))
                    .then((e) => {
                        let { openGuildProductLink: n } = e;
                        n(t, r);
                    }),
                !0
            );
        };
    if (null != o && o.type === m.I.SOCIAL_LAYER_STOREFRONT)
        return (e) => {
            e?.preventDefault();
            let [t, r] = o.code.split("-");
            return (
                Promise.resolve()
                    .then(n.bind(n, 44724))
                    .then((e) => {
                        let { default: n } = e;
                        n({ guildId: r, skuId: t });
                    }),
                !0
            );
        };
    if (null != o && o.type === m.I.QUESTS_EMBED && (0, S.s)({ location: F.rE.EMBED_MOBILE }) && !(0, k.I)())
        return (e) => {
            let t, r;
            e?.preventDefault();
            let { search: a } = U.A.toURLSafe(o.url) ?? {};
            if (null != a) {
                let e = new URLSearchParams(a);
                (t = e.get("sort") ?? void 0), (r = e.get("filter") ?? void 0);
            }
            return (
                Promise.resolve()
                    .then(n.bind(n, 545986))
                    .then((e) => {
                        let { navigateToQuestHome: t } = e;
                        t({ fromContent: i.u.QUEST_SHARE_LINK, questId: o.code });
                    }),
                !0
            );
        };
    let { host: O, hostname: D, pathname: L, search: P, hash: B } = U.A.toURLSafe(e) ?? {},
        Y = U.A.isDiscordHostname(D ?? null) || U.A.isDiscordLocalhost(O ?? null, D ?? null);
    if (Y && (L?.startsWith("/application-directory") || L?.startsWith("/discovery/applications"))) {
        let e = L.split("/"),
            t = L?.startsWith("/discovery/applications"),
            r = e[t ? 3 : 2],
            i = "search" === r,
            a = t && "categories" === r && "featured" !== e[4];
        return (t) => {
            let r, s, o;
            if ((t?.preventDefault(), i)) {
                let e = new URLSearchParams(P);
                (r = e.get("q") ?? void 0), (s = e.get("category_id") ?? void 0), (o = e.get("page") ?? void 0);
            } else a && (s = e[4]);
            return (
                n
                    .e("38042")
                    .then(n.bind(n, 837057))
                    .then((e) => {
                        let { transitionToGlobalDiscovery: t } = e,
                            { GlobalDiscoveryTab: i } = n(488995);
                        t({
                            tab: i.APPS,
                            categoryId: null == s || Number.isNaN(s) ? void 0 : Number(s),
                            page: null == o || Number.isNaN(o) ? void 0 : Number(o),
                            query: r,
                        });
                    }),
                !0
            );
        };
    }
    if (null != L && Y && U.A.isAppRoute(L)) {
        let e = { navigationReplace: !1, openChannel: !0 };
        return null != P && (e.search = P), null != B && (e.hash = B), (t) => (t?.preventDefault(), (0, y.A)(L, e), !0);
    }
    if (null != L && Y) {
        let { getOAuth2AuthorizeProps: t, openOAuth2ModalWithCreateGuildModal: r } = n(200330),
            i = t(e);
        if (null != i)
            return (e) => (
                e?.preventDefault(),
                null != o &&
                    o.type === m.I.APP_OAUTH2_LINK &&
                    _.Ay.trackWithMetadata(V.HAw.APP_OAUTH2_LINK_EMBED_URL_CLICKED, { application_id: i.clientId }),
                r(i),
                !0
            );
    }
    let W = (0, T.SK)(L);
    if (null != L && Y && null != W)
        return (e) => {
            e?.preventDefault();
            let t = x.A.getGuildId();
            null != W.guildId && "" !== W.guildId && W.guildId !== t && (0, y.A)(V.BVt.CHANNEL(W.guildId));
            let n = A.Ay.getGuildScheduledEvent(W.guildEventId);
            return null != n && (0, E.uR)({ eventId: n.id }), !0;
        };
    if (Y && L?.startsWith("/settings/")) {
        let { default: e } = n(849823),
            { openUserSettingsFromParsedUrl: t } = n(840065),
            r = e(L, P);
        if (null != r)
            return (e) => (e?.preventDefault(), t({ match: r, analyticsLocations: a, urlOrigin: "link_click" }), !0);
    }
    if (Y && L?.startsWith("/playground")) {
        let { parsePlaygroundUrl: e } = n(682492),
            { openPlayground: t } = n(965042),
            { collection: r, story: i } = e(L);
        return (e) => (e?.preventDefault(), t(r, i), !0);
    }
    return Y && L?.startsWith("/discovery/quests")
        ? (e) => (e?.preventDefault(), (0, C.navigateToQuestHome)({ fromContent: i.u.QUEST_BADGE }), !0)
        : Y && L?.startsWith("/quest-home")
          ? (e) => (e?.preventDefault(), (0, b.pX)(V.BVt.QUEST_HOME_V2 + (P ?? "")), !0)
          : Y && L?.startsWith("/quest-preview")
            ? (e) => {
                  if ((e?.preventDefault(), (0, v.U)({ location: F.rE.NAVIGATE_TO_QUEST_HOME_UTIL })))
                      return (0, b.pX)(V.BVt.QUEST_HOME_V2 + `?tab=preview_tool&quest_id=${L.split("/").at(-1)}`), !0;
              }
            : Y && L?.startsWith("/discovery/servers")
              ? (e) => (
                    e?.preventDefault(),
                    n
                        .e("38042")
                        .then(n.bind(n, 837057))
                        .then((e) => {
                            let { transitionToGlobalDiscovery: t } = e,
                                { GlobalDiscoveryTab: r } = n(488995);
                            t({ tab: r.SERVERS });
                        }),
                    !0
                )
              : t || null == (0, R.m)(e)
                ? void 0
                : (t) => (t?.preventDefault(), N.A.show(e), !0);
}
