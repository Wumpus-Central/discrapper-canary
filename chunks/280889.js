...
class w extends O.Ay {
    ...
    async upload() {
            ...
            t =
                ...
                null != this.item.file
                    ? (0, y.E5)(this.item.file)
                        ? "heic"
                        : (0, y.Tn)(this.item.file)
                          ? "jxr"
                          : null
                    : null;
        if (null != t && this.item.platform === O.xz.WEB && null != this.item.file) {
            (null == this.mimeType || "" === this.mimeType) &&
                (this.mimeType = "heic" === t ? (0, y.II)(this.item.file) : (0, y.NX)(this.item.file));
            let n =
                "heic" === t
        ...
        let n = await i.getUploadPayload(this),
            r = (0, R.B)(this.item.target);
        if (null == n.filename || "" === n.filename) {
            (v.error("File does not have a filename.", JSON.stringify(n)), this.handleError(D.t02.INVALID_FILE_ASSET));
            return;
        }
        ...
        let a = r.getMaxFileSize(this.channelId);
    ...
    static async tryConvertToJpeg(e) {
        ...
        try {
            ...
            return (
                v.warn(`${i} conversion threw for ${a}:`, e),
                {
                    convertedFile: null,
