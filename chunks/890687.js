...
function er(e, t, n, r) {
    ...
    let o = !(0, j.Ic)(e),
        ...
        f = r === P.rE.RANK_DOWN_ALL_QUESTS_AFTER_30_MINUTES ? 30 * v.A.Millis.MINUTE : 20 * v.A.Millis.MINUTE,
        p = tt(e, f),
        h = tt(t, f);
    if (o) {
...
function eD(e) {
    ...
    return r.useCallback(() => {
        t.id === Z.Fw
            ? window.open(N.A.getArticleURL(X.MVz.VIRTUAL_CURRENCY_LEARN_MORE))
...
let ej = (e) => {
    ...
    eW = () => {
        let { xboxAccounts: e, playstationAccounts: t } = eH(),
            ...
            r = t.length > 0,
            i = (n && !r) || (!n && r),
            s = N.A.getArticleURL(X.MVz.QUEST_HOW_TO_PLAYSTATION),
            a = N.A.getArticleURL(X.MVz.QUEST_HOW_TO_XBOX),
...
function tn() {
    let { enabled: e } = P.sn.useConfig({ location: Z.rE.QUEST_HOME_DESKTOP }),
    ...
    return (
        ...
        r.useEffect(() => {
            !(async function () {
                try {
                    if (null == f) return;
                    await Promise.all([(0, C.NN)(f.assetHeroImage.url), (0, C.NN)(f.assetSponsorImage.url)]);
                } catch (e) {
                    c(!0);
                } finally {
                    a(!1);
