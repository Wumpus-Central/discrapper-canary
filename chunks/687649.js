...
let y = (0, n(945810).mj)({
    kind: "user",
    name: "2026-06-friendship-anniversary-backend-persistence",
...
class L extends c.A {
    ...
    maybeReconcileServerDismissals() {
        !this.isReconcileEligible("PremiumGiftingIntentManager maybeReconcileServerDismissals") ||
            this.reconcileBackoff.pending ||
            this.attemptReconcileFetch();
    }
    retryReconcileServerDismissals = () => {
        this.isReconcileEligible("PremiumGiftingIntentManager retryReconcileServerDismissals") &&
            this.attemptReconcileFetch();
    };
    isReconcileEligible(e) {
        return 0 !== N.Ay.getFriendAnniversaries().length && y.getConfig({ location: e }).enabled;
    }
    ...
    shouldHoldGiftingPromptSystemMessageForServerReconcile(e) {
        return (
            !!y.getConfig({ location: e }).enabled &&
            N.Ay.getLastKnownGiftIntentDismissedAtMs() < this.getServerDismissalTimestampMs()
        );
    }
    isChannelEligible(e) {
        ...
        N.Ay.isGiftIntentMessageInCooldown(n) ||
            ...
            (s
                ? this.trySendGiftingPromptSystemMessage(
                      ...
                      n,
                      i,
                      "PremiumGiftingIntentManager maybeSendGiftingPromptSystemMessage",
                  ) && (0, C.xs)(n)
                : A.A.whenReady(e, () => {
    ...
    sendGiftPromptMessageInSelectedChannelIfEligible(e) {
        let { enabled: t } = S.u.getConfig({ location: "PremiumGiftingIntentManager handleChannelSelect" }),
            n = g.A.getChannel(e);
    ...
    async sendGiftingNotificationIfEligible() {
        let { enabled: e, showDmPrompts: t } = S.u.getConfig({
            ...
            try {
                ...
                if (
                    ...
                    !this.trySendGiftingPromptSystemMessage(
                        ...
                        n,
                        v.l1.SEND_MESSAGE,
                        "PremiumGiftingIntentManager sendGiftingNotificationIfEligible",
                    )
                )
                    return;
