...
e.exports = function (e, t, n, _, m, h, g, E) {
    ...
    if ((x > w && (x = w), 0 === w)) return (m[h++] = 20971520), (m[h++] = 20971520), (E.bits = 1), 0;
    for (D = 1; D < w && 0 === B[D]; D++);
    for (x < D && (x = D), M = 1, P = 1; P <= i; P++) if (((M <<= 1), (M -= B[P]) < 0)) return -1;
    if (M > 0 && (e === s || 1 !== w)) return -1;
    ...
    for (;;) {
        ...
        if (P > x && (U & v) !== O) {
            for (0 === j && (j = x), S += D, M = 1 << (L = P - j); L + j < w && !((M -= B[L + j]) <= 0); )
                L++, (M <<= 1);
