...
class c extends s.PureComponent {
    ...
    updateAnimation = (e) => {
        ...
        let u = Math.max(Math.min((e - this.dataChangedAt) / l, 1), 0),
            p = i.getContext("2d"),
            A = this.maxValue;
        (p.strokeStyle = r), (p.lineWidth = c);
        let m = i.height - p.lineWidth,
            g = p.createLinearGradient(0, 0, 0, m);
        if (null != h) g.addColorStop(0, h);
        else {
            let { r: e, g: t, b: n } = a.E2(r);
            g.addColorStop(0, `rgba(${e}, ${t}, ${n}, 0)`);
        }
        g.addColorStop(1, this.fillColor), (p.fillStyle = g);
        let f = s.length >= o ? o : s.length;
        p.setTransform(1, 0, 0, -1, 0, i.height),
            p.clearRect(0, 0, i.width, i.height),
            p.translate(0, 0.5 * p.lineWidth);
        let _ = Math.floor(i.width / (f - 3)),
            x = 0.5 * _;
        p.translate(_ - _ * u, 0), p.beginPath();
        let y = -_;
        s.forEach((e, i) => {
            (t = { x: y, y: (m * e) / A }),
                0 === i ? p.moveTo(t.x, t.y) : p.bezierCurveTo(n.x + x, n.y, t.x - x, t.y, t.x, t.y),
                (n = t),
                (y += _);
        }),
            p.stroke(),
            ...
            p.lineTo(0, 0),
            p.fill(),
            d && u < 1 && (this.animationFrameRequestId = window.requestAnimationFrame(this.updateAnimation));
    };
    render() {
