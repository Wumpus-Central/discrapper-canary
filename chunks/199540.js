n.d(t, { Z: () => E }), n(47120);
var i = n(200651),
    l = n(192379),
    r = n(120356),
    a = n.n(r),
    o = n(642128),
    s = n(866442),
    c = n(442837),
    d = n(477690),
    u = n(481060),
    h = n(377171),
    p = n(540059),
    m = n(565138),
    g = n(430824),
    f = n(624138),
    _ = n(674552),
    v = n(981631),
    C = n(388032),
    x = n(895134);
let I = (0, f.Mg)(d.Z.FOLDER_ITEM_ANIMATION_DURATION),
    Z = (0, f.Mg)(d.Z.FOLDER_ITEM_GUILD_ICON_SIZE),
    b = ['13px 4px 4px 4px', '4px 13px 4px 4px', '4px 4px 4px 13px', '4px 4px 13px 4px'];
function S(e) {
    let { guildId: t, animate: n, index: l } = e,
        r = (0, c.e7)([g.Z], () => g.Z.getGuild(t), [t]),
        a = (0, p.Q3)('GuildIcon'),
        o = (0, u.dQu)(u.TVs.modules.guildbar.AVATAR_SIZE);
    return null == r
        ? (0, i.jsx)('div', {
              className: x.guildIconUnavailable,
              children: '!'
          })
        : (0, i.jsx)(m.Z, {
              guild: r,
              animate: n,
              size: a ? m.Z.Sizes.MINI : m.Z.Sizes.SMOL,
              iconSize: a ? o : void 0,
              lossless: a,
              className: x.guildIcon,
              tabIndex: -1,
              style: a ? { borderRadius: b[l] } : void 0
          });
}
function N(e) {
    let t,
        n,
        { folderNode: r, hovered: a, expanded: c } = e,
        d = (0, p.Q3)('FolderIconContent'),
        { color: h, children: m } = r,
        g = null != h ? h : v.Wyy,
        f = g === v.Wyy,
        _ = m.map((e) => e.id),
        [C, b] = l.useState(!1),
        [N, E] = l.useState(c),
        j = d ? (c ? 22 : -11) : c ? 0 : -Z,
        y = (0, u.q_F)(
            {
                transform: 'translate3d(0, '.concat(j, 'px, 0)'),
                config: { duration: I },
                onStart() {
                    b(!0), E(c);
                },
                onRest() {
                    b(!1), E(c);
                }
            },
            'animate-always'
        ),
        T = C ? y : void 0;
    (C || N) &&
        (t = (0, i.jsx)(o.animated.div, {
            style: T,
            className: x.expandedFolderIconWrapper,
            children: (0, i.jsx)(u.ROc, {
                size: d ? 'sm' : 'md',
                color: 'currentColor',
                style: { color: (0, s.Rf)(g) }
            })
        })),
        (C || !N) &&
            (n = (0, i.jsx)(o.animated.div, {
                style: T,
                className: x.closedFolderIconWrapper,
                children: _.slice(0, 4).map((e, t) =>
                    (0, i.jsx)(
                        S,
                        {
                            index: t,
                            guildId: e,
                            animate: a
                        },
                        e
                    )
                )
            }));
    let A = { backgroundColor: d && f ? u.TVs.colors.BG_MOD_SUBTLE.css : (0, s.br)(g, 0.4) };
    return (0, i.jsxs)('div', {
        'aria-hidden': !0,
        style: c ? void 0 : A,
        className: x.folderIconWrapper,
        children: [t, n]
    });
}
function E(e) {
    let {
            folderNode: t,
            forceCircular: n,
            expanded: r,
            sorting: o,
            mediaState: s,
            mentionCount: c = 0,
            isMentionLowImportance: d,
            tooltipName: m,
            folderGroupId: g,
            folderIconContentClassName: f,
            folderIconContent: v,
            onClick: I,
            onContextMenu: Z,
            onHoverChange: b,
            onKeyDown: S,
            treeItemProps: { onFocus: E, ...j }
        } = e,
        [y, T] = l.useState(!1),
        A = l.useCallback(() => {
            o || T(!0), null == b || b(!0);
        }, [o, b]),
        P = l.useCallback(() => {
            o || T(!1), null == b || b(!1);
        }, [o, b]),
        R = (0, p.Q3)('FolderHeader'),
        w = r || null == s ? null : (0, _.Or)(s),
        L = !r && c > 0 ? (0, _.Ne)(c, d ? h.Z.BACKGROUND_ACCENT : h.Z.STATUS_DANGER) : null,
        M = (0, u.dQu)(u.TVs.modules.guildbar.FOLDER_SIZE);
    return (0, i.jsx)(u.aRk, {
        isFolder: !0,
        style: R
            ? {
                  width: M,
                  height: M
              }
            : void 0,
        selected: !!R || !n,
        upperBadge: w,
        lowerBadge: L,
        lowerBadgeSize: { width: (0, u.OVM)(c) },
        children: (0, i.jsx)(u.P3F, {
            className: a()(x.folder, { [x.hover]: y }),
            onClick: I,
            onContextMenu: Z,
            onMouseEnter: A,
            onMouseLeave: P,
            onKeyDown: S,
            onFocus: E,
            'aria-label': C.intl.formatToPlainString(C.t['90/DwM'], {
                folderName: m,
                mentions: c
            }),
            'aria-expanded': r,
            'aria-owns': g,
            focusProps: { enabled: !1 },
            ...j,
            role: 'treeitem',
            children:
                null != v
                    ? (0, i.jsx)('div', {
                          className: a()(x.expandedFolderIconWrapper, f),
                          children: v
                      })
                    : (0, i.jsx)(N, {
                          folderNode: t,
                          hovered: y,
                          expanded: r
                      })
        })
    });
}
