...
let tr = {
    ...
    [ef.kqX.GUILD_RAID_NOTIFICATION]: {
        predicate: () => (0, C.dj)().show && !tn(ef.kqX.GUILD_RAID_NOTIFICATION),
    ...
    [ef.kqX.PTT_NO_KEYBIND_WARNING]: {
        predicate: () =>
            !!eb.A.isConnected() &&
            ev.Ay.getMode() === ef.TBI.PUSH_TO_TALK &&
    ...
    [ef.kqX.PENDING_MEMBER]: {
        predicate: (e) => {
            ...
            return (
                (null != t &&
                    null != n &&
                    !eN.A.getGuild(t)?.features.has(ef.GuildFeatures.GUILD_ONBOARDING) &&
                    eS.Ay.getMember(t, n.id)?.isPending) ??
                !1
            );
        },
    },
    [ef.kqX.OUTBOUND_PROMOTION]: { predicate: () => (0, H.So)() },
    [ef.kqX.CORRUPT_INSTALLATION]: {
    ...
    [ef.kqX.VIDEO_UNSUPPORTED_BROWSER]: {
        predicate: (e) => {
            ...
            return (
                null != t && ek.A.hasVideo(t) && !ev.Ay.supports(eZ.O5.VIDEO) && !tn(ef.kqX.VIDEO_UNSUPPORTED_BROWSER)
            );
        },
    },
    [ef.kqX.DETECTED_OFF_PLATFORM_PREMIUM_PERK]: {
    ...
    [ef.kqX.DOWNLOAD_NAG]: { predicate: () => !eH.isPlatformEmbedded && !tn(ef.kqX.DOWNLOAD_NAG) },
    [ef.kqX.QUEST_APP_UPSELL]: {
        predicate: () =>
            (0, eH.isOculusWeb)() && !tn(ef.kqX.QUEST_APP_UPSELL) && W.A.getConfig({ location: "NoticeStore" }).enabled,
    },
    [ef.kqX.SCHEDULED_MAINTENANCE]: {
...
class to extends a.Ay.Store {
    ...
    initialize() {
        this.syncWith(
            [T.A, eP.Ay, eO.A, eT, eL.A, j.A, eF.A, ei.default, ea.A, Q.A, R.A, K.A, et.A, y.Ay, ee.A, T.A, E.A],
            ts,
        ),
            this.waitFor(
                T.A,
