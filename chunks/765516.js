n.d(t, { A: () => M });
var r = n(627968);
n(64700);
var i = n(503698),
    l = n.n(i),
    s = n(989349),
    a = n.n(s),
    o = n(17928),
    c = n(297264),
    u = n(531260),
    d = n(287809),
    m = n(166403),
    g = n(428262),
    x = n(526292),
    T = n(664504),
    f = n(422936),
    I = n(234419),
    h = n(410516),
    p = n(462887),
    E = n(289873),
    A = n(834730),
    P = n(736653),
    j = n(97352),
    v = n(202541),
    _ = n(375708),
    R = n(966522);
function y(e) {
    let { subscriptionTier: t, priceOptions: n } = e,
        i = (0, o.bG)([j.A], () => j.A.isLoadedForSKUs([t])),
        l = (0, p.q)((0, P.Ay)());
    if (!i) return (0, r.jsx)(E.y, { type: E.y.Type.PULSING_ELLIPSIS, className: R.xB });
    let s = j.A.getForSkuAndInterval((0, g.mH)(t), v.WT.MONTH),
        a = null != s ? (0, g.sS)(s, n, !1, !0) : null;
    return (0, r.jsxs)("div", {
        children: [
            (0, r.jsx)(A.E, {
                variant: "heading-xxl/extrabold",
                color: l ? "text-strong" : "text-overlay-light",
                tag: "span",
                children: (0, r.jsx)("span", { children: a }),
            }),
            (0, r.jsx)(A.E, { variant: "text-xs/medium", color: "text-muted", children: _.intl.string(_.t.lVQycU) }),
        ],
    });
}
var N = n(156601),
    S = n(86280);
function M(e) {
    let {
            isGift: t,
            premiumTier: n,
            offerTierMatchesCard: i,
            offerType: s,
            showYearlyPrice: p,
            priceOptions: E,
            textVariant: A,
            className: P,
            isApplicationHome: j,
            enablePremiumBrandRefresh: R,
            headerClassName: M,
            headingVariant: C = "heading-md/normal",
            headingColor: U,
        } = e,
        L = (0, o.bG)([m.A], () => m.A.getPremiumTypeSubscription()),
        b = (0, o.bG)([d.default], () => d.default.getCurrentUser()),
        G = (0, u.A)(),
        O = (0, x.k5)(),
        w = (0, x.nf)(),
        k = n === v.PremiumTypes.TIER_0 ? v.pe.TIER_0 : v.pe.TIER_2,
        D = L?.hasActiveTrial ? b?.premiumType : O ? v.PremiumTypes.TIER_2 : null,
        F = (0, I.V)(),
        H = (0, f.O)(),
        Z = n === v.PremiumTypes.TIER_0 ? v.gD.PREMIUM_MONTH_TIER_0 : v.gD.PREMIUM_MONTH_TIER_2,
        B = (0, h.N1)(Z),
        V = null != H && (0, h.YJ)(H) === Z,
        { useNewCopy: J } = T.A.useConfig({ location: `TierCardPrice${n}${t ? "" : " - DO NOT USE"}` }),
        W = F?.subscription_trial;
    if (!j && !t && null != D && n === D && null != L && null != L.planIdFromItems) {
        let e = null != L.trialEndsAt ? a()(L?.trialEndsAt).diff(a()(), "d") : 0,
            t = v.hd[L.planIdFromItems],
            n = g.Ay.formatPriceString(g.Ay.getDefaultPrice(t.id), t.interval),
            i = s === v.Vk.PREMIUM_TRIAL,
            o = L.planIdFromItems === v.gD.PREMIUM_YEAR_TIER_2;
        return (0, r.jsx)(c.D, {
            variant: C,
            color: U,
            className: l()((i || !o) && S.K, M),
            children:
                null == L
                    ? null
                    : i
                      ? _.intl.format(_.t["2CGBri"], { remainingTime: e, price: n })
                      : o
                        ? _.intl.format(_.t.z2oQtA, {
                              percent: w?.percentage ?? v.Cq,
                              regularPrice: n,
                              renewalDate: g.Ay.getExpectedRenewalDate(L, G),
                          })
                        : _.intl.formatToPlainString(_.t["3ZiutU"], {
                              percent: w?.percentage ?? v._$,
                              regularPrice: n,
                              numMonths: w?.duration ?? v.OJ,
                          }),
        });
    }
    if (!j && !t && i) {
        let e = g.Ay.formatPriceString(
            g.Ay.getDefaultPrice(n === v.PremiumTypes.TIER_0 ? v.gD.PREMIUM_MONTH_TIER_0 : v.gD.PREMIUM_MONTH_TIER_2),
            v.WT.MONTH,
        );
        if (s === v.Vk.PREMIUM_TRIAL)
            return (0, r.jsx)(c.D, {
                variant: C,
                color: U,
                className: l()(S.K, M),
                children: _.intl.format(_.t["9vyovu"], {
                    planName: (0, g.RH)(v.En[W?.sku_id ?? v.pe.NONE] ?? v.gD.PREMIUM_MONTH_TIER_2),
                    duration: (0, g.re)({
                        intervalType: W?.interval ?? v.WT.DAY,
                        intervalCount: W?.interval_count ?? 30,
                        capitalize: !1,
                    }),
                    price: e,
                }),
            });
        if (null != H && null != B && V)
            return (0, r.jsx)(c.D, {
                variant: C,
                color: U,
                className: l()(S.K, M),
                children: _.intl.format(_.t.sJTwHQ, {
                    numMonths: H.discount.userUsageLimit ?? v.OJ,
                    discountedPrice: B,
                    regularPrice: e,
                }),
            });
    }
    return t && J
        ? (0, r.jsx)(y, { subscriptionTier: k, priceOptions: E })
        : (0, r.jsxs)(r.Fragment, {
              children: [
                  (0, r.jsx)(N.A, {
                      subscriptionTier: k,
                      isGift: t,
                      className: null != P ? P : S.q,
                      priceOptions: E,
                      variant: A,
                      isApplicationHome: j,
                      enablePremiumBrandRefresh: R,
                  }),
                  p &&
                      (0, r.jsx)(N.A, {
                          subscriptionTier: k,
                          interval: v.WT.YEAR,
                          className: null != P ? P : S.q,
                          isGift: t,
                          priceOptions: E,
                          variant: A,
                          isApplicationHome: j,
                          enablePremiumBrandRefresh: R,
                      }),
              ],
          });
}
