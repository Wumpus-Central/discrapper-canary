l.r(t), l.d(t, { default: () => i6 });
var i,
    s = l(627968),
    a = l(64700),
    n = l(503698),
    r = l.n(n),
    o = l(735438),
    c = l.n(o),
    d = l(17928),
    u = l(323384),
    h = l(548411),
    m = l(534514),
    p = l(417098),
    g = l(111159),
    x = l(834730),
    A = l(711111),
    v = l(770472),
    j = l(737823),
    C = (l(749638), l(876622)),
    I = l(521974),
    _ = l(965660),
    E = l(976860),
    f = l(495544),
    N = l(287809),
    S = l(174459),
    b = l(993748),
    y = l(416730),
    R = l(212534),
    P = l(38181),
    L = l(939635);
l(323874), l(14289), l(35956);
var T = l(323125),
    D = l(310419),
    O = l(652215);
function k(e, t) {
    let l = D.h.getField("sessionId"),
        i = D.h.getField("guildId"),
        s = D.h.getField("entrypoint")?.name,
        a = N.default.getCurrentUser(),
        n = { ...t, guild_id: i, directory_session_id: l, source: s, user_id: a?.id };
    return S.default.track(e, n);
}
function G() {
    return (0, d.bG)([f.default], () => f.default.isAuthenticated());
}
function M() {
    let e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {},
        t = new URL(location.href);
    for (let l in e) {
        let i = e[l];
        t.searchParams.set(l, i);
    }
    let l = t.pathname + t.search,
        i = (0, T.T2)(l, !1);
    (0, E.pX)(i);
}
var U = l(435220),
    w = l(375708);
function V() {
    return { id: U.GLOBAL_DISCOVERY_APPS_FEATURED_CATEGORY_ID, name: w.intl.string(w.t.ijDDwz) };
}
function F() {
    let { query: e, categoryId: t, page: l } = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : {},
        i = { previousView: (0, D.R4)() },
        s = new URLSearchParams();
    null != e && "" !== e && s.set("q", e),
        null != t && t !== L.lj && s.set("category_id", t.toString()),
        null != l && l > 1 && s.set("page", l.toString()),
        (0, E.pX)(O.BVt.GLOBAL_DISCOVERY_APPS_SEARCH, { state: i, search: s.toString() });
}
function B(e) {
    let {
        location: { state: t },
    } = (0, E.JK)();
    (0, E.bG)(e, t);
}
var H = l(902119),
    Y = l(701508),
    z = l(979590),
    X = l(783531),
    K = l(624134);
let W = "/assets/3aa7e375b23dc56f.svg";
function Z(e) {
    let { heading: t = w.intl.string(w.t.NGw8v2), body: l = w.intl.string(w.t["1XZRY4"]), className: i } = e;
    return (0, s.jsxs)("div", {
        className: r()(K.kL, i),
        children: [
            (0, s.jsx)("div", {
                className: K.ZS,
                children: (0, s.jsx)("img", { className: K.Sl, src: W, alt: w.intl.string(w.t["/UJaI2"]) }),
            }),
            (0, s.jsx)(m.D, { variant: "heading-xl/semibold", className: K.wx, children: t }),
            (0, s.jsx)(x.E, { variant: "text-md/normal", color: "text-default", children: l }),
        ],
    });
}
var $ = l(935573),
    q = l(702841),
    Q = l(932793),
    J = l(919796),
    ee = l(111042),
    et = l(651753),
    el = l(289873),
    ei = l(566984);
function es(e) {
    let { loading: t, children: l } = e;
    return (0, s.jsxs)("div", {
        className: ei.kL,
        children: [
            t
                ? (0, s.jsx)("div", {
                      className: ei.dc,
                      children: (0, s.jsx)(el.y, { className: ei.u1, type: el.y.Type.SPINNING_CIRCLE }),
                  })
                : null,
            (0, s.jsx)("div", { className: r()({ [ei.Lq]: t }), children: l }),
        ],
    });
}
var ea = l(269115),
    en = l(735991),
    er = l(635350),
    eo = l(486020),
    ec = l(775602),
    ed = l(435582),
    eu = l(283488),
    eh = l(607470),
    em = l(654107),
    ep = l(564096);
let eg = 16 / 9,
    ex = 325 / 72;
function eA(e) {
    let { application: t, isHovered: l, botBannerUrl: i, botBannerUrlAnimated: n, iconURL: o } = e,
        c = (0, eu.A)({ applicationId: t.id, size: 600, names: ["embedded_cover"] }),
        d = a.useMemo(() => {
            let e = (0, en.Cx)(t);
            return null != e && null != e.activity_preview_video_asset_id
                ? (0, ed.A)(t.id, e.activity_preview_video_asset_id)
                : null;
        }, [t]),
        { imageUrl: u, imageUrlAnimated: h } = a.useMemo(
            () => ("fetched" === c.state && null != c.url ? { imageUrl: c.url } : { imageUrl: i, imageUrlAnimated: n }),
            [c.state, c.url, i, n],
        ),
        m = null != u,
        p = null != d;
    return "loading" === c.state
        ? null
        : p
          ? (0, s.jsxs)(s.Fragment, {
                children: [
                    m
                        ? (0, s.jsx)(ev, { isHovered: l, url: u, animatedUrl: void 0 })
                        : (0, s.jsx)(eh.A, { src: d, className: r()(ep._e, ep.IH), muted: !0 }),
                    (0, s.jsx)(ej, { isHovered: l, url: d, isVideo: !0 }),
                ],
            })
          : m
            ? (0, s.jsx)(ev, { isHovered: l, url: u, animatedUrl: h })
            : (0, s.jsx)(eC, { iconURL: o });
}
function ev(e) {
    let { isHovered: t, url: l, animatedUrl: i } = e,
        [n, o] = a.useState(!1);
    return (0, s.jsxs)(s.Fragment, {
        children: [
            (0, s.jsx)("img", { src: l, alt: "", className: r()(ep._e, ep.IH, { [ep.Kc]: n }), onLoad: () => o(!0) }),
            null != i ? (0, s.jsx)(ej, { isHovered: t, url: i, isVideo: !1 }) : null,
        ],
    });
}
function ej(e) {
    let { isHovered: t, url: l, isVideo: i } = e,
        [n, o] = a.useState(!1),
        [c, d] = a.useState(t);
    return (a.useEffect(() => {
        t && d(!0);
    }, [t]),
    c)
        ? (0, s.jsx)("div", {
              className: r()(ep.hY, { [ep.Q]: !t, [ep.nR]: t && n }),
              onAnimationEnd: () => (t ? null : d(!1)),
              children: i
                  ? (0, s.jsx)(eh.A, {
                        src: l,
                        loop: !0,
                        autoPlay: !0,
                        muted: !0,
                        className: ep.IH,
                        onLoadedData: () => o(!0),
                    })
                  : (0, s.jsx)("img", { src: l, alt: "", className: ep.IH, onLoad: () => o(!0) }),
          })
        : null;
}
function eC(e) {
    let { iconURL: t } = e,
        { backgroundGradient: l, hasFetchedColors: i } = (function (e) {
            let [t, l, i] = (0, em.rh)(e, "transparent");
            return { backgroundGradient: `linear-gradient(45deg, ${t}, ${i ?? l})`, hasFetchedColors: (0, em.D1)(e) };
        })(t);
    return (0, s.jsx)("div", { className: r()(ep.IH, ep.nY, { [ep.Kc]: i }), style: { backgroundImage: l } });
}
let eI = function (e) {
    let { application: t, bannerType: l, iconURL: i } = e,
        n = (0, d.bG)([ec.Ay], () => ec.Ay.useReducedMotion),
        [r, o] = a.useState(!1),
        c = a.useCallback(() => {
            o(!0);
        }, []),
        u = a.useCallback(() => o(!1), []),
        h = (0, en.Ag)(t),
        m = t.bot,
        { botBannerUrl: p, botBannerUrlAnimated: g } = a.useMemo(
            () =>
                m?.id == null || m?.banner == null
                    ? {}
                    : {
                          botBannerUrl: (0, eo.z)({ id: m.id, banner: m.banner, canAnimate: !1, size: 600 }),
                          botBannerUrlAnimated: n
                              ? void 0
                              : (0, eo.z)({ id: m.id, banner: m.banner, canAnimate: !0, size: 600 }),
                      },
            [m?.id, m?.banner, n],
        );
    return (0, s.jsx)("div", {
        className: ep.cI,
        style: { aspectRatio: "card" === l ? eg : ex },
        onMouseEnter: c,
        onFocus: c,
        onMouseLeave: u,
        onBlur: u,
        children:
            "card" === l && h
                ? (0, s.jsx)(eA, {
                      application: t,
                      isHovered: r,
                      botBannerUrl: null == p ? void 0 : p,
                      botBannerUrlAnimated: null == g ? void 0 : g,
                  })
                : null != p
                  ? (0, s.jsx)(ev, { isHovered: r, url: p, animatedUrl: g })
                  : (0, s.jsx)(eC, { iconURL: i }),
    });
};
var e_ = l(92400);
let eE = function (e) {
    let { application: t, onSelectApplication: l, showCategory: i = !1 } = e,
        n = a.useRef(null),
        r = t.categories?.[0],
        [o, c] = a.useState(!1),
        d = a.useCallback((e) => {
            e && c(!0);
        }, []),
        u = (0, en.NO)(t),
        h = a.useCallback(() => {
            l(t.id);
        }, [l, t.id]),
        p = a.useMemo(() => (o ? eo.Ay.getApplicationIconURL({ id: t.id, icon: t.icon, size: 48 }) : void 0), [o, t]),
        g = i && null != r;
    return (0, s.jsx)(ea.L, {
        innerRef: n,
        onChange: d,
        active: !o,
        children: (0, s.jsx)("div", {
            ref: n,
            className: e_.kL,
            children: (0, s.jsxs)(er.A, {
                className: e_.Nr,
                onClick: h,
                onContextMenu: () => {},
                "aria-label": w.intl.string(w.t.np1Wru),
                children: [
                    (0, s.jsxs)("div", {
                        className: e_.wx,
                        children: [
                            (0, s.jsx)("div", {
                                style: { width: "100%", display: "block", aspectRatio: 16 / 9 },
                                children: o ? (0, s.jsx)(eI, { application: t, bannerType: "card", iconURL: p }) : null,
                            }),
                            (0, s.jsx)("div", {
                                className: e_.H,
                                style: { height: 28 },
                                children: o
                                    ? (0, s.jsx)("img", {
                                          src: p,
                                          alt: "",
                                          className: e_.my,
                                          height: 48,
                                          width: 48,
                                          style: { borderWidth: 4, marginLeft: 12 },
                                      })
                                    : null,
                            }),
                        ],
                    }),
                    (0, s.jsxs)("div", {
                        className: e_.eV,
                        children: [
                            (0, s.jsx)("div", {
                                className: e_.gn,
                                children: (0, s.jsx)("div", {
                                    className: e_.DD,
                                    children: (0, s.jsx)(m.D, {
                                        variant: "heading-md/semibold",
                                        color: "text-strong",
                                        className: e_.JB,
                                        children: t.name,
                                    }),
                                }),
                            }),
                            g || u
                                ? (0, s.jsxs)("div", {
                                      className: e_.op,
                                      children: [
                                          g
                                              ? (0, s.jsx)(x.E, {
                                                    className: e_.m4,
                                                    variant: "text-xs/medium",
                                                    color: "text-default",
                                                    children: r.name,
                                                })
                                              : null,
                                          g && u
                                              ? (0, s.jsx)(x.E, {
                                                    variant: "text-xs/medium",
                                                    color: "text-subtle",
                                                    children: "\u2022",
                                                })
                                              : null,
                                          u
                                              ? (0, s.jsx)(x.E, {
                                                    variant: "text-xs/medium",
                                                    color: "text-subtle",
                                                    children: w.intl.string(w.t["/eVltv"]),
                                                })
                                              : null,
                                      ],
                                  })
                                : null,
                            (0, s.jsx)(x.E, {
                                className: e_.h_,
                                variant: "text-sm/medium",
                                color: "text-default",
                                lineClamp: g || u ? 2 : 3,
                                children: t.directory_entry?.short_description ?? t.description,
                            }),
                        ],
                    }),
                ],
            }),
        }),
    });
};
var ef = l(392896);
let eN = { results: [], totalPages: 0, loadId: "" },
    eS = function (e) {
        let { categoryId: t, onSelectApplication: l, resetScroll: i } = e,
            [n, r] = a.useState(1),
            o = a.useCallback((e) => {
                r(e);
            }, []);
        a.useEffect(() => {
            r(1);
        }, [t]);
        let c = a.useMemo(() => ({ query: U.EMPTY_QUERY, page: n, pageSize: U.PAGE_SIZE, categoryId: t }), [n, t]),
            d = (0, q.bG)([et.A], () =>
                et.A.getFetchState({ query: U.EMPTY_QUERY, page: n, pageSize: U.PAGE_SIZE, categoryId: t }),
            ),
            u = (0, q.cf)([et.A], () => et.A.getSearchResults(c) ?? eN),
            h = (0, J.A)(u) ?? eN,
            { results: m, totalPages: p, loadId: g } = a.useMemo(() => (d === ee.e.FETCHING ? h : u), [d, h, u]),
            x = a.useMemo(() => m?.filter((e) => e.type === $.j.APPLICATION), [m]),
            A = a.useCallback((e) => {
                let { page: t, activeCategoryId: l, onSuccessCallback: i, guildId: s, fetchCounts: a } = e;
                a && b.$P({ query: U.EMPTY_QUERY, guildId: s }),
                    b.$P({
                        query: U.EMPTY_QUERY,
                        guildId: s,
                        options: { page: t, pageSize: U.PAGE_SIZE, categoryId: l },
                        onSuccessCallback: i,
                    });
            }, []);
        a.useEffect(() => {
            i(), A({ page: n, activeCategoryId: t, onSuccessCallback: () => {} });
        }, [t, i, A, n]);
        let v = a.useCallback(
            (e, t) => {
                k(O.HAw.APP_DIRECTORY_SEARCH_RESULT_CLICKED, {
                    current_page: D.ev.SEARCH,
                    application_id: e,
                    load_id: g,
                    position: t,
                }),
                    l(e);
            },
            [g, l],
        );
        return d === ee.e.ERROR
            ? (0, s.jsx)("div", { className: ef.Un, children: (0, s.jsx)(Z, { className: ef.z3 }) })
            : (0, s.jsxs)(es, {
                  loading: d === ee.e.FETCHING,
                  children: [
                      (0, s.jsx)("div", {
                          className: ef.Qs,
                          children: x?.map((e, t) => {
                              if (e.type !== $.j.APPLICATION) return e.type, null;
                              {
                                  let l = e.data;
                                  return (0, s.jsx)(eE, { application: l, onSelectApplication: (e) => v(e, t) }, l.id);
                              }
                          }),
                      }),
                      (0, s.jsx)(Q.m, {
                          className: ef.JV,
                          totalCount: Math.min(p * U.PAGE_SIZE, U.MAX_PAGES * U.PAGE_SIZE),
                          pageSize: U.PAGE_SIZE,
                          disablePaginationGap: !0,
                          hideMaxPage: !0,
                          currentPage: n,
                          onPageChange: o,
                      }),
                  ],
              });
    };
var eb = l(504797);
let ey = function (e) {
    let { tabId: t, onSelectApplication: l, onScroll: i } = e,
        n = a.useRef(null),
        r = a.useCallback(() => {
            n.current?.scrollTo({ to: 0 });
        }, []),
        o = (0, d.bG)([P.A], () => P.A.getCategories()),
        c = a.useMemo(() => o.find((e) => e.id === t), [o, t]),
        u = a.useMemo(
            () =>
                null != c
                    ? (function (e) {
                          switch (e.id) {
                              case 4:
                                  return w.intl.string(w.t["3m9Egb"]);
                              case 6:
                                  return w.intl.string(w.t.VJRr3Y);
                              case 8:
                                  return w.intl.string(w.t.Uv7mFd);
                              case 9:
                                  return w.intl.string(w.t.TEK9Ax);
                              case 10:
                                  return w.intl.string(w.t.lfBt2T);
                          }
                          return "";
                      })(c)
                    : "",
            [c],
        );
    return (0, s.jsxs)(X.A, {
        onScroll: i,
        ref: n,
        children: [
            (0, s.jsx)(Y.A, { title: c?.name ?? "", description: u, children: (0, s.jsx)(H.A, {}) }),
            (0, s.jsx)(z.A, {
                children:
                    null == c
                        ? (0, s.jsx)("div", { className: eb.U, children: (0, s.jsx)(Z, { className: eb.z }) })
                        : (0, s.jsx)(eS, { categoryId: c.id, onSelectApplication: l, resetScroll: r }),
            }),
        ],
    });
};
var eR = l(123791),
    eP = l(761508),
    eL = l(759057);
let eT = function (e) {
    let { tabs: t, onTabSelect: l, selectedTab: i, className: a } = e;
    return (0, s.jsx)(eP.V, {
        type: "top-pill",
        selectedItem: i,
        onItemSelect: l,
        className: a,
        children: t.map((e, t) => {
            let { id: l, label: i, count: a } = e;
            return (0, s.jsx)(
                eP.V.Item,
                {
                    id: l,
                    className: r()(eL.V, { [eL.r]: 0 === t }),
                    "aria-label": i,
                    children: null != a && a > 0 ? `${i} (${a})` : i,
                },
                l,
            );
        }),
    });
};
var eD = l(82293),
    eO = l(403362),
    ek = l(939249),
    eG = l(175671),
    eM = l(259407),
    eU = l(451988),
    ew = l(408278),
    eV = l(921853),
    eF = l(477155),
    eB = l(320448),
    eH = l(935286),
    eY = l(228366),
    ez = l(725951),
    eX = l(625494),
    eK = l(332412);
let eW = 16 / 9;
class eZ extends a.PureComponent {
    static defaultProps = { includeHitboxPadding: !0 };
    componentDidMount() {
        eX._.subscribe(O.jej.CAROUSEL_PREV, this.handlePrevious), eX._.subscribe(O.jej.CAROUSEL_NEXT, this.handleNext);
    }
    componentWillUnmount() {
        eX._.unsubscribe(O.jej.CAROUSEL_PREV, this.handlePrevious),
            eX._.unsubscribe(O.jej.CAROUSEL_NEXT, this.handleNext);
    }
    render() {
        let {
                current: e,
                count: t,
                includeHitboxPadding: l,
                arrowClassName: i,
                paginationDotClassName: a,
                paginationDotSelectedClassName: n,
                paginationArrowIconType: o = "arrow",
            } = this.props,
            d = r()(eK.sb, { [eK.JU]: l }, i);
        return (0, s.jsxs)("div", {
            className: r()(eK.ne, this.props.className),
            children: [
                (0, s.jsx)("div", {
                    className: d,
                    children: (0, s.jsx)(ew.K, {
                        variant: "icon-only",
                        icon: "caret" === o ? eV.n : eF.r,
                        "aria-label": w.intl.string(w.t.vgfxaA),
                        onClick: this.handlePrevious,
                    }),
                }),
                (0, s.jsx)("div", {
                    className: eK.r$,
                    children: c().times(t, (t) =>
                        (0, s.jsx)(
                            ek.D,
                            {
                                onClick: () => this.handleDotClick(t),
                                className: t === e ? r()(eK.NU, n) : r()(eK.Lw, a),
                                "aria-label": w.intl.formatToPlainString(w.t["2SXOrM"], { pageNumber: t + 1 }),
                            },
                            `dot-${t}`,
                        ),
                    ),
                }),
                (0, s.jsx)("div", {
                    className: d,
                    children: (0, s.jsx)(ew.K, {
                        variant: "icon-only",
                        icon: "caret" === o ? eB._ : eH.E,
                        "aria-label": w.intl.string(w.t.XiOHRX),
                        onClick: this.handleNext,
                    }),
                }),
            ],
        });
    }
    handleDotClick = (e) => {
        let { onSetItem: t, onIntentionalChange: l, current: i } = this.props;
        l?.(i, e, "jump"), t(e);
    };
    handleNext = () => {
        let { onIntentionalChange: e, current: t, onChangePage: l } = this.props,
            i = l(1);
        e?.(t, i, "next");
    };
    handlePrevious = () => {
        let { onIntentionalChange: e, current: t, onChangePage: l } = this.props,
            i = l(-1);
        e?.(t, i, "previous");
    };
}
class e$ extends a.PureComponent {
    timer = new eU.IX();
    static defaultProps = { aspectRatio: eW };
    constructor(e) {
        super(e),
            (this.state = {
                visibleIndex: !0 === e.randomize ? c().random(0, e.items.length - 1) : 0,
                direction: ez.f.LEFT,
                paused: !1,
            });
    }
    handleWindowFocusChange = (e) => {
        let { focused: t } = e;
        this.setState({ paused: !t });
    };
    componentDidMount() {
        eY.h.subscribe("WINDOW_FOCUS", this.handleWindowFocusChange),
            this.props.initialPaused || this.state.paused || this.startTimer();
    }
    componentWillUnmount() {
        this.stopTimer(), eY.h.unsubscribe("WINDOW_FOCUS", this.handleWindowFocusChange);
    }
    componentDidUpdate(e, t) {
        function l(e, t) {
            return !e.initialPaused && !t.paused;
        }
        let i = l(this.props, this.state),
            s = l(e, t);
        i && !s ? this.startTimer() : !i && s && this.stopTimer();
        let { items: a } = this.props,
            { visibleIndex: n } = this.state;
        null == a[n] && this.changeItem(a, 1);
    }
    startTimer() {
        null != this.props.delay && this.timer.start(this.props.delay, this.nextItem);
    }
    stopTimer() {
        null != this.props.delay && this.timer.stop();
    }
    changeItem(e, t) {
        let l = this.state.visibleIndex + t;
        l < 0 ? (l = e.length - 1) : l > e.length - 1 && (l = 0);
        let { onChangeItem: i } = this.props;
        i?.(e[l], this.state.visibleIndex, l);
        let s = t > 0 ? ez.f.LEFT : ez.f.RIGHT;
        return this.setState({ visibleIndex: l, direction: s }), l;
    }
    getCurrentDirection = () => this.state.direction;
    nextItem = () => {
        let { items: e } = this.props;
        this.changeItem(e, 1);
    };
    previousItem = () => {
        let { items: e } = this.props;
        this.changeItem(e, -1);
    };
    render() {
        let {
                items: e,
                renderItem: t,
                className: l,
                slideAnimatorClassName: i,
                slideAnimatorSpringSettings: a,
                slideAnimatorFadeInOut: n,
                paginationClassName: o,
                paginationArrowClassName: c,
                paginationArrowIconType: d,
                paginationDotClassName: u,
                paginationDotSelectedClassName: h,
                themedPagination: m,
                includeHitboxPadding: p,
                style: g,
                aspectRatio: x,
                children: A,
            } = this.props,
            { visibleIndex: v } = this.state;
        return (0, s.jsxs)("div", {
            className: eK.zr,
            children: [
                (0, s.jsxs)("div", {
                    className: r()(eK.Cf, l),
                    style: g,
                    onMouseEnter: this.handleMouseEnter,
                    onMouseLeave: this.handleMouseLeave,
                    children: [
                        (0, s.jsx)("div", {
                            style: { aspectRatio: x },
                            children: (0, s.jsx)(ez.A, {
                                className: r()(eK.Dk, i),
                                step: v,
                                direction: this.getCurrentDirection(),
                                springSettings: a,
                                fadeInOut: n,
                                children: t(e[v], v),
                            }),
                        }),
                        e.length > 1 &&
                            (0, s.jsx)(eZ, {
                                className: r()(o, m ? eK.S$ : eK.X$),
                                arrowClassName: c,
                                includeHitboxPadding: p,
                                current: v,
                                count: e.length,
                                onChangePage: (t) => this.changeItem(e, t),
                                onSetItem: this.handleSetItem,
                                onIntentionalChange: this.handleIntentionalChange,
                                paginationArrowIconType: d,
                                paginationDotClassName: u,
                                paginationDotSelectedClassName: h,
                            }),
                    ],
                }),
                null != A && A({ step: v, direction: this.getCurrentDirection() }),
            ],
        });
    }
    handleSetItem = (e) => {
        let { visibleIndex: t } = this.state,
            { items: l } = this.props;
        this.changeItem(l, e - t);
    };
    handleMouseEnter = () => {
        this.setState({ paused: !0 });
    };
    handleMouseLeave = () => {
        this.setState({ paused: !1 });
    };
    handleIntentionalChange = (e, t, l) => {
        let { items: i, onIntentionalChange: s } = this.props;
        return s?.(i[t], e, t, l);
    };
}
var eq = l(619517),
    eQ = l(248643),
    eJ = l(34337),
    e0 = l(615300),
    e1 = l(456412),
    e7 = l(133296),
    e3 = l(689175),
    e9 = l(532197),
    e6 = l(937751);
let e8 = { CENTER: e6.Hu, LEFT: e6.Vl };
class e2 extends a.PureComponent {
    static Align = e8;
    static defaultProps = { scrollToPadding: { top: 0, left: 0, bottom: 0, right: 0 }, align: e8.CENTER };
    _scrollerRef = null;
    _paginationItemRefs = [];
    componentDidUpdate(e) {
        let { selectedIndex: t } = this.props;
        e.selectedIndex !== t && this.handleSelectedIndexChange(t);
    }
    handleSetScrollerRef = (e) => {
        this._scrollerRef = e;
    };
    handleSelectedIndexChange = (e) => {
        let t = this._scrollerRef;
        if (null == t) return;
        let l = this._paginationItemRefs[e];
        null != l && t.scrollIntoViewNode({ node: l, animate: !0, padding: this.props.scrollToPadding });
    };
    handlePageClick = (e) => {
        let { onSetItem: t } = this.props;
        t(e);
    };
    render() {
        let { renderItem: e, items: t, vertical: l, paginationContainerClass: i, align: n } = this.props,
            o = l ? e6.XA : e6.BU;
        return (0, s.jsx)(e3.Gt, {
            orientation: l ? "vertical" : "horizontal",
            className: r()(o, i, n),
            ref: this.handleSetScrollerRef,
            children: t.map((t, l) =>
                a.cloneElement(e(t, l), {
                    onClick: () => this.handlePageClick(l),
                    key: l,
                    ref: (e) => {
                        this._paginationItemRefs[l] = e;
                    },
                }),
            ),
        });
    }
}
class e5 extends a.PureComponent {
    handlePrevClick = (e) => {
        e.stopPropagation(), e.preventDefault();
        let { onClick: t } = this.props;
        t?.(e);
    };
    render() {
        let { className: e } = this.props;
        return (0, s.jsx)(ek.D, {
            className: r()(e6.t1, e),
            onClick: this.handlePrevClick,
            children: (0, s.jsx)(e9.A, { className: e6.UE, direction: e9.A.Directions.LEFT }),
        });
    }
}
class e4 extends a.PureComponent {
    handleNextClick = (e) => {
        e.stopPropagation(), e.preventDefault();
        let { onClick: t } = this.props;
        t?.(e);
    };
    render() {
        let { className: e } = this.props;
        return (0, s.jsx)(ek.D, {
            className: r()(e6.XS, e),
            onClick: this.handleNextClick,
            children: (0, s.jsx)(e9.A, { className: e6.UE, direction: e9.A.Directions.RIGHT }),
        });
    }
}
var te = l(452282),
    tt = l(821209),
    tl = l(795574);
let ti = `rel=0&iv_load_policy=3&showinfo=0&modestbranding=1&pageType=${O.NzX}`,
    ts = { width: 640, height: 360, margin: 15 },
    ta = { width: 880, height: 495, margin: 20 },
    tn = { width: 1920, height: 1080 },
    tr = { width: 1280, height: 720 };
function to(e) {
    return (0, eJ.$o)({ ...e, className: tl.tN, mediaPlayerClassName: tl.yf });
}
function tc(e) {
    return (0, s.jsx)(eq.Ay, { ...e });
}
let td = (e) => {
    let t,
        l,
        i,
        {
            item: a,
            size: n,
            onPlay: r,
            onEnded: o,
            onClick: c,
            playable: d,
            volume: u,
            isMuted: h,
            onVolumeChange: m,
            onMute: p,
        } = e;
    if (a.type === O.geh.YOUTUBE_VIDEO)
        (t = { url: (0, O.PE$)(a.youtubeVideoId), ...tn }),
            (l = { url: `${(0, O.uJ2)(a.youtubeVideoId)}?${ti}`, ...tr }),
            (i = eM.m.YOUTUBE);
    else {
        let e = null != a.width ? a.width : 0,
            i = null != a.height ? a.height : 0;
        (t = { url: a.thumbnailSrc, width: e, height: i }), (l = { url: a.src, proxyURL: a.src, width: e, height: i });
    }
    let g = (0, s.jsx)(eG.rr, {
        href: null,
        thumbnail: t,
        video: l,
        provider: i,
        allowFullScreen: !0,
        maxHeight: n.height,
        maxWidth: n.width,
        onPlay: r,
        onEnded: o,
        playable: d,
        className: tl.Ki,
        volume: u,
        onVolumeChange: m,
        autoMute: h,
        onMute: p,
        autoPlay: d,
        renderVideoComponent: to,
        renderImageComponent: tc,
        renderLinkComponent: eJ.bU,
    });
    return d ? g : (0, s.jsx)(ek.D, { className: tl.AU, onClick: c, children: g });
};
function tu() {
    return (0, s.jsx)("div", {
        className: tl.eM,
        children: (0, s.jsx)(tt.A, { inactive: !0, className: tl.J6, renderLinkComponent: eJ.bU }),
    });
}
class th extends a.PureComponent {
    static defaultProps = { autoplayInterval: 8e3, paused: !1 };
    state = {
        currentIndex: 0,
        animate: !0,
        playingVideo: !1,
        hasInteracted: !1,
        hoveringPreviousItem: !1,
        hoveringNextItem: !1,
        volume: 1,
        isMuted: !0,
    };
    componentDidMount() {
        eX._.subscribe(O.jej.CAROUSEL_PREV, this.manualPrevious), eX._.subscribe(O.jej.CAROUSEL_NEXT, this.manualNext);
    }
    componentWillUnmount() {
        eX._.unsubscribe(O.jej.CAROUSEL_PREV, this.manualPrevious),
            eX._.unsubscribe(O.jej.CAROUSEL_NEXT, this.manualNext);
    }
    getItemSize = () => {
        let { width: e } = this.props,
            t = ta;
        return null != e && e < t.width && (t = ts), t;
    };
    getStyle(e, t) {
        let l = t.interpolate({ inputRange: [0, 1], outputRange: [1, 0], extrapolate: e0.A.Extrapolate.CLAMP });
        return {
            opacity: t.interpolate({ inputRange: [0, 1], outputRange: [0.3, 1], extrapolate: e0.A.Extrapolate.CLAMP }),
            filter: e0.A.template`grayscale(${l})`,
        };
    }
    setItem = (e, t) => {
        let {
                props: { items: l, onIntentionalChange: i },
                state: { currentIndex: s },
            } = this,
            a = (0, te.U3)(s, l.length),
            n = (0, te.U3)(e, l.length),
            r = 1 === Math.abs(e - a) || (e === l.length - 1 && 0 === a) || (0 === e && a === l.length - 1);
        this.setState({ playingVideo: !1, currentIndex: n, animate: r, hasInteracted: t }),
            t && null != i && i(l[n], a, n);
    };
    autoNext = () => {
        this.setItem(this.state.currentIndex + 1, !1);
    };
    manualNext = () => {
        this.setItem(this.state.currentIndex + 1, !0);
    };
    manualPrevious = () => {
        this.setItem(this.state.currentIndex - 1, !0);
    };
    handlePlay = (e) => this.setState({ playingVideo: !0, hasInteracted: !e });
    handleEnded = () => {
        this.state.hasInteracted || this.autoNext();
    };
    handleVolumeChange = (e) => {
        this.setState({ volume: e, hasInteracted: !0 });
    };
    handleMute = (e) => {
        this.setState({ isMuted: e, hasInteracted: !0 });
    };
    handleVideoClick = (e) => {
        this.setItem(e, !0);
    };
    handleCurrentItemClick(e, t) {
        let { onCurrentItemClick: l } = this.props;
        null != l && l(e, t), this.setState({ hasInteracted: !0 });
    }
    handleEdgeItemMouseEnter(e) {
        let { currentIndex: t } = this.state;
        e < t ? this.setState({ hoveringPreviousItem: !0 }) : e > t && this.setState({ hoveringNextItem: !0 });
    }
    handleEdgeItemMouseLeave = () => {
        this.setState({ hoveringPreviousItem: !1, hoveringNextItem: !1 });
    };
    renderCarouselImage = (e, t, l) =>
        l
            ? (0, s.jsx)(ek.D, {
                  onClick: () => this.handleCurrentItemClick(e, t),
                  className: tl.AU,
                  children: (0, s.jsx)("img", { src: e.src, alt: "", className: tl.NP }),
              })
            : (0, s.jsx)(ek.D, {
                  onClick: () => this.setItem(t, !0),
                  className: tl.AU,
                  children: (0, s.jsx)("img", { alt: "", className: tl.tv, src: e.src, draggable: !1 }),
              });
    renderItem = (e, t, l) => {
        let { currentIndex: i, volume: a, isMuted: n } = this.state,
            o = t === (0, te.U3)(i, this.props.items.length);
        return (0, s.jsx)(e0.A.div, {
            className: r()(tl.AS, { [tl.Xt]: o }),
            style: null != l ? this.getStyle(t, l) : null,
            onMouseEnter: o ? null : () => this.handleEdgeItemMouseEnter(t),
            onMouseLeave: o ? null : this.handleEdgeItemMouseLeave,
            children:
                e.type === O.geh.VIDEO || e.type === O.geh.YOUTUBE_VIDEO
                    ? (0, s.jsx)(td, {
                          item: e,
                          size: this.getItemSize(),
                          onPlay: this.handlePlay,
                          onEnded: this.handleEnded,
                          playable: o,
                          onClick: () => this.handleVideoClick(t),
                          volume: a,
                          onVolumeChange: this.handleVolumeChange,
                          isMuted: n,
                          onMute: this.handleMute,
                      })
                    : this.renderCarouselImage(e, t, o),
        });
    };
    renderCarousel() {
        let { items: e } = this.props,
            { currentIndex: t, animate: l, hoveringPreviousItem: i, hoveringNextItem: a } = this.state,
            n = (0, s.jsx)(te.Ay, {
                items: e,
                itemSize: this.getItemSize(),
                renderItem: this.renderItem,
                currentIndex: t,
                animate: l,
                edgeItems: 2,
            });
        return e.length <= 1
            ? n
            : (0, s.jsxs)("div", {
                  className: tl.HY,
                  children: [
                      n,
                      (0, s.jsx)(e5, { onClick: this.manualPrevious, className: r()(tl.UE, { [tl.h_]: i }) }),
                      (0, s.jsx)(e4, { onClick: this.manualNext, className: r()(tl.UE, { [tl.h_]: a }) }),
                  ],
              });
    }
    getPaginationSrc(e) {
        switch (e.type) {
            case O.geh.YOUTUBE_VIDEO:
                return (0, O.PE$)(e.youtubeVideoId);
            case O.geh.IMG:
                return e.src;
            case O.geh.VIDEO:
                return e.thumbnailSrc;
            default:
                return null;
        }
    }
    renderPaginationItem = (e, t) => {
        let { currentIndex: l } = this.state,
            i = l === t;
        return (0, s.jsxs)(
            "div",
            {
                className: r()(tl.Yw, i ? tl.sM : tl.N7),
                children: [
                    (0, s.jsx)("img", {
                        alt: null == e.alt ? t : e.alt,
                        className: tl.Pr,
                        src: this.getPaginationSrc(e),
                        draggable: !1,
                    }),
                    e.type === O.geh.YOUTUBE_VIDEO || e.type === O.geh.VIDEO ? (0, s.jsx)(tu, {}) : null,
                    i ? (0, s.jsx)("div", { className: tl.SD }) : null,
                ],
            },
            `page-${t}`,
        );
    };
    render() {
        let { items: e, className: t, autoplayInterval: l, paused: i } = this.props,
            { currentIndex: a, playingVideo: n, hasInteracted: r } = this.state;
        return (0, s.jsxs)(e7.A, {
            pauseOnHover: !0,
            onInterval: this.autoNext,
            interval: l,
            className: t,
            disable: n || r || i,
            children: [
                this.renderCarousel(),
                (0, s.jsx)("div", {
                    className: tl.X$,
                    children: (0, s.jsx)(e2, {
                        renderItem: this.renderPaginationItem,
                        scrollToPadding: 85,
                        items: e,
                        selectedIndex: a,
                        onSetItem: (e) => this.setItem(e, !0),
                        paginationContainerClass: tl.XG,
                        align: e.length < 6 ? e2.Align.LEFT : e2.Align.CENTER,
                    }),
                }),
            ],
        });
    }
}
(0, e1.A)(th);
var tm = l(665513);
function tp(e) {
    let { className: t, ...l } = e;
    return (0, s.jsx)(eQ.A, {
        ...l,
        className: r()(tm.Zy, t),
        mediaPlayerClassName: tm.Zy,
        renderLinkComponent: eJ.bU,
    });
}
function tg(e) {
    let { containerClassName: t, className: l, ...i } = e;
    return (0, s.jsx)(eq.Ay, { ...i, className: r()(l, t) });
}
let tx = { width: 1280, height: 720 },
    tA = { width: 640, height: 360 },
    tv = `rel=0&iv_load_policy=3&showinfo=0&modestbranding=1&pageType=${O.NzX}`,
    tj = (e) => {
        let t,
            l,
            i,
            { item: a, onPlay: n, volume: r, onVolumeChange: o, onMute: c, isMuted: d, autoPlay: u } = e;
        if (a.type === O.geh.YOUTUBE_VIDEO)
            (t = { url: (0, O.PE$)(a.youtubeVideoId), ...tx }),
                (l = { url: `${(0, O.uJ2)(a.youtubeVideoId)}?${tv}`, ...tA }),
                (i = eM.m.YOUTUBE);
        else {
            let e = null != a.width ? a.width : 0,
                i = null != a.height ? a.height : 0;
            (t = { url: a.thumbnailSrc, width: e, height: i }),
                (l = { url: a.src, proxyURL: a.src, width: e, height: i });
        }
        return (0, s.jsx)("div", {
            className: tm.x,
            children: (0, s.jsx)(eG.rr, {
                className: tm.Zy,
                iframeWrapperClassName: tm.Zy,
                href: null,
                thumbnail: t,
                video: l,
                provider: i,
                maxHeight: ts.height,
                maxWidth: "small",
                onPlay: n,
                playable: !0,
                volume: r,
                onVolumeChange: o,
                autoMute: d,
                onMute: c,
                autoPlay: u,
                renderImageComponent: tg,
                renderVideoComponent: tp,
                renderLinkComponent: eJ.bU,
            }),
        });
    };
class tC extends a.PureComponent {
    static defaultProps = { paused: !1, videoAutoPlay: !0 };
    state = {
        hasInteracted: !1,
        isVideoPlaying: !1,
        isMuted: !0,
        volume: 1,
        status: new Map(),
        imageLoadingStartTime: new Map(),
    };
    componentDidMount() {
        let { items: e } = this.props;
        e.length > 0 &&
            e[0].type === O.geh.IMG &&
            this.setState({
                imageLoadingStartTime: new Map(this.state.imageLoadingStartTime).set(e[0].src, Date.now()),
            });
    }
    handlePlay = (e) => {
        this.setState({ isVideoPlaying: !0, hasInteracted: !e });
    };
    handleVolumeChange = (e) => {
        this.setState({ volume: e, hasInteracted: !0 });
    };
    handleMute = (e) => {
        this.setState({ isMuted: e, hasInteracted: !0 });
    };
    handleChangeItem = (e) => {
        this.setState({ isVideoPlaying: !1 }),
            e.type === O.geh.IMG &&
                this.setState({
                    imageLoadingStartTime: new Map(this.state.imageLoadingStartTime).set(e.src, Date.now()),
                });
    };
    handleIntentionalChange = (e, t, l, i) => {
        this.setState({ hasInteracted: !0 });
        let { onIntentionalChange: s } = this.props;
        null != s && s(e, t, l, i);
    };
    handleCurrentItemClick(e, t) {
        let { onCurrentItemClick: l } = this.props;
        null != l && l(e, t), this.setState({ hasInteracted: !0 });
    }
    handleOnErrorImg = (e) => {
        this.setState({ status: new Map(this.state.status).set(e.target.src, "errored") });
    };
    handleOnLoadImg = (e) => {
        let { onImageLoad: t } = this.props;
        this.setState({ status: new Map(this.state.status).set(e.target.src, "loaded") });
        let l = this.state.imageLoadingStartTime.get(e.target.src),
            i = null != l ? Date.now() - l : void 0;
        t?.({ src: e.target.src, loadTimeMs: i });
    };
    renderItem = (e, t) => {
        let l,
            { isMuted: i, volume: a } = this.state,
            { videoAutoPlay: n } = this.props;
        switch (e.type) {
            case O.geh.YOUTUBE_VIDEO:
            case O.geh.VIDEO:
                l = (0, s.jsx)(tj, {
                    item: e,
                    onPlay: this.handlePlay,
                    isMuted: i,
                    volume: a,
                    onVolumeChange: this.handleVolumeChange,
                    onMute: this.handleMute,
                    autoPlay: n,
                });
                break;
            case O.geh.IMG:
                if ("errored" === this.state.status.get(e.src)) {
                    let e = (0, s.jsx)(m.D, { variant: "heading-xl/semibold", children: w.intl.string(w.t.UvDfMz) });
                    return (0, s.jsx)("div", {
                        className: tm.Un,
                        children: void 0 !== this.props.errorComponent ? this.props.errorComponent : e,
                    });
                }
                {
                    let t = e.alt ?? w.intl.string(w.t.X4IxWL);
                    l = (0, s.jsxs)(s.Fragment, {
                        children: [
                            (0, s.jsx)(el.y, {
                                className: r()(tm.u1, { [tm.R]: void 0 !== this.state.status.get(e.src) }),
                                type: el.y.Type.SPINNING_CIRCLE,
                            }),
                            (0, s.jsx)("img", {
                                onError: this.handleOnErrorImg,
                                onLoad: this.handleOnLoadImg,
                                className: r()(tm.JD, { [tm.R]: "loaded" !== this.state.status.get(e.src) }),
                                src: e.src,
                                alt: t,
                            }),
                        ],
                    });
                }
                break;
            default:
                throw Error("Unexpected MediaType");
        }
        return (0, s.jsx)(ek.D, { className: tm.Pj, onClick: () => this.handleCurrentItemClick(e, t), children: l });
    };
    render() {
        let { items: e, autoplayInterval: t, className: l, paused: i, themedPagination: a } = this.props,
            { hasInteracted: n } = this.state;
        return (0, s.jsx)(e$, {
            items: e.slice(0, 12),
            delay: t,
            initialPaused: i || n,
            renderItem: this.renderItem,
            className: r()(tm.Gt, l),
            themedPagination: a,
            onIntentionalChange: this.handleIntentionalChange,
            onChangeItem: this.handleChangeItem,
        });
    }
}
var tI = l(256905),
    t_ = l(531685),
    tE = l(427019);
function tf(e) {
    let { application: t, carouselItems: l } = e,
        i = (0, d.bG)([t_.A], () => t_.A.isFocused()),
        n = (0, s.jsxs)(s.Fragment, {
            children: [
                (0, s.jsx)("img", { className: tE.M, src: W, "aria-hidden": !0, alt: "" }),
                (0, s.jsx)(m.D, { variant: "heading-xl/semibold", children: w.intl.string(w.t.UvDfMz) }),
            ],
        });
    l.forEach((e, i) => {
        e.alt = w.intl.formatToPlainString(w.t.sSEhHb, { index: i + 1, totalImages: l.length, name: t.name });
    });
    let r = a.useCallback(
        (e, t) => {
            if (e.type === O.geh.IMG) {
                let t = l.filter((e) => e.type === O.geh.IMG),
                    i = t.findIndex((t) => t === e);
                if (i < 0) return;
                let s = t.map((e) => {
                    var t;
                    return {
                        url: ((t = e.src), `${t.split("?")[0]}?size=2048`),
                        original: e.src,
                        width: e.width,
                        height: e.height,
                        type: "IMAGE",
                    };
                });
                (0, tI.R)({
                    items: s,
                    startingIndex: i,
                    shouldHideMediaOptions: !0,
                    location: "GlobalDiscoveryAppsDetailCarousel",
                });
            }
        },
        [l],
    );
    return 0 === l.length
        ? null
        : (0, s.jsx)(tC, {
              className: tE.D,
              themedPagination: !0,
              items: l,
              autoplayInterval: 8e3,
              paused: !i,
              videoAutoPlay: !0,
              onCurrentItemClick: r,
              errorComponent: n,
          });
}
var tN = l(168186);
l(827669);
var tS = l(733975);
function tb(e) {
    let { application: t } = e,
        l = a.useMemo(() => {
            let e = t.directory_entry?.popular_application_commands;
            return null != e
                ? e.map((e) => (0, tN.Oe)({ rootCommand: e, command: e, applicationId: e.application_id }))
                : [];
        }, [t]);
    return 0 === l.length
        ? null
        : (0, s.jsxs)("div", {
              className: tS.hd,
              children: [
                  (0, s.jsx)(m.D, {
                      variant: "heading-lg/semibold",
                      color: "text-strong",
                      children: w.intl.string(w.t.swIgTL),
                  }),
                  (0, s.jsx)("div", {
                      className: tS.hQ,
                      children: l.map((e) =>
                          (0, s.jsxs)(
                              "div",
                              {
                                  className: tS.dO,
                                  children: [
                                      (0, s.jsxs)(x.E, {
                                          className: tS.p6,
                                          variant: "code",
                                          color: "text-strong",
                                          children: ["/", e.displayName],
                                      }),
                                      (0, s.jsx)(x.E, {
                                          variant: "text-md/normal",
                                          color: "text-default",
                                          children: e.displayDescription,
                                      }),
                                  ],
                              },
                              e.id,
                          ),
                      ),
                  }),
              ],
          });
}
var ty = l(534890),
    tR = l(812991),
    tP = l(177953),
    tL = l(203363),
    tT = l(975571),
    tD = l(123917),
    tO = l(169795);
let tk = tT.A.getArticleURL(O.MVz.BOT_DATA_ACCESS);
function tG(e) {
    let { application: t } = e,
        { hasMessageContent: l, hasGuildPresences: i, hasGuildMembers: n, hasIntents: r } = (0, tL.Z)(t),
        o = a.useCallback(() => {
            null != t.privacy_policy_url && (0, tD.h)({ href: t.privacy_policy_url });
        }, [t.privacy_policy_url]);
    return (0, s.jsxs)("div", {
        className: tO.hd,
        children: [
            (0, s.jsx)(m.D, {
                variant: "heading-lg/semibold",
                color: "text-strong",
                children: w.intl.string(w.t.QzDgMq),
            }),
            (0, s.jsxs)("div", {
                className: tO.hQ,
                children: [
                    r &&
                        (0, s.jsxs)("ul", {
                            className: tO.In,
                            children: [
                                (0, s.jsx)(m.D, {
                                    variant: "heading-md/semibold",
                                    color: "text-default",
                                    children: w.intl.string(w.t.U6KH5S),
                                }),
                                l &&
                                    (0, s.jsx)(tM, {
                                        icon: ty.o,
                                        heading: w.intl.string(w.t.gJpBO2),
                                        body: w.intl.string(w.t["L+QVbh"]),
                                    }),
                                i &&
                                    (0, s.jsx)(tM, {
                                        icon: tR.n,
                                        heading: w.intl.string(w.t["jo0oj/"]),
                                        body: w.intl.string(w.t.Dm0jqx),
                                    }),
                                n &&
                                    (0, s.jsx)(tM, {
                                        icon: tP.n,
                                        heading: w.intl.string(w.t.QZql7O),
                                        body: w.intl.string(w.t["ez/N/R"]),
                                    }),
                            ],
                        }),
                    (0, s.jsx)(x.E, {
                        variant: "text-md/normal",
                        color: "text-default",
                        children: w.intl.format(w.t.b6nqk0, { helpCenterUrl: tk }),
                    }),
                    null != t.privacy_policy_url &&
                        (0, s.jsx)(x.E, {
                            variant: "text-md/normal",
                            color: "text-default",
                            className: tO.hM,
                            children: w.intl.format(w.t.agYVY5, { onClick: o }),
                        }),
                ],
            }),
        ],
    });
}
function tM(e) {
    let { icon: t, heading: l, body: i } = e;
    return (0, s.jsxs)("li", {
        className: tO.KT,
        children: [
            (0, s.jsx)(t, { color: "currentColor", size: "md" }),
            (0, s.jsxs)("div", {
                className: tO.D6,
                children: [
                    (0, s.jsx)(m.D, { variant: "heading-md/semibold", color: "text-default", children: l }),
                    (0, s.jsx)(x.E, { variant: "text-md/normal", color: "text-default", children: i }),
                ],
            }),
        ],
    });
}
l(321073);
var tU = l(990078),
    tw = l(319060),
    tV = l(664121),
    tF = l(573435),
    tB = l(773669),
    tH = l(240248),
    tY = l(805936),
    tz =
        (((i = {})[(i.SMALL = (0, tH.xI)(tw.A.GUILD_COUNT_SMALL_ICON_SIZE))] = "SMALL"),
        (i[(i.LARGE = (0, tH.xI)(tw.A.GUILD_COUNT_LARGE_ICON_SIZE))] = "LARGE"),
        i);
let tX = { [tz.SMALL]: tY.Ps, [tz.LARGE]: tY.XY },
    tK = { [tz.SMALL]: tY.Bx, [tz.LARGE]: tY.J3 };
function tW(e) {
    let {
            application: t,
            mutualGuilds: l,
            mutualGuildShownMax: i = 4,
            className: n,
            textVariant: o = "text-sm/normal",
            compact: c,
            guildIconSize: u = tz.LARGE,
            guildsClassName: h,
            children: m,
        } = e,
        p = (0, d.bG)([tB.default], () => tB.default.locale),
        g = new Intl.ListFormat(p),
        A = t.directory_entry?.guild_count ?? 0,
        v = l?.length ?? 0,
        j = Math.max(0, A - v),
        { shownMutualGuilds: C, hiddenMutualGuilds: I } = a.useMemo(() => {
            let e = [],
                t = [];
            return (
                l?.forEach((l) => {
                    e.length < i && null != l.icon ? e.push(l) : t.push(l);
                }),
                e.length === i && t.length > 0 && (t.push(e[i - 1]), (e = e.slice(0, i - 1))),
                { shownMutualGuilds: e, hiddenMutualGuilds: t }
            );
        }, [l, i]),
        _ = I.length,
        E = (function (e, t, l, i, s) {
            if (0 === t && 0 === e) return null;
            if (t > 0 && 0 === l) return w.intl.formatToPlainString(w.t.pnzE1t, { mutualGuildCount: t });
            let a = t > 0 ? w.t.YR8PSL : w.t.GQjq6c,
                n = new Intl.NumberFormat(i, { notation: s ? "compact" : "standard", compactDisplay: "short" });
            return w.intl.formatToPlainString(a, {
                guildCount: n.format(e),
                mutualGuildCount: t,
                nonMutualGuildCount: n.format(l),
            });
        })(A, v, j, p, c);
    return 0 === C.length && null == E
        ? null
        : (0, s.jsxs)(s.Fragment, {
              children: [
                  m,
                  (0, s.jsxs)("div", {
                      className: r()(n, tY.iE),
                      children: [
                          (0, s.jsx)("div", {
                              className: r()(tY.Pt, h),
                              children:
                                  C.length > 0
                                      ? (0, s.jsxs)(s.Fragment, {
                                            children: [
                                                C.map((e, t) => {
                                                    let l = t === C.length - 1 && 0 === _,
                                                        i = eo.Ay.getGuildIconURL({
                                                            id: e.id,
                                                            icon: e.icon,
                                                            size: u,
                                                            canAnimate: !1,
                                                        }),
                                                        n = (0, s.jsx)(tU.m, {
                                                            text: e.name,
                                                            position: "top",
                                                            children: (0, s.jsx)("img", {
                                                                className: r()(tY.Kk, tK[u]),
                                                                src: i,
                                                                alt: "",
                                                            }),
                                                        });
                                                    return l
                                                        ? (0, s.jsx)(a.Fragment, { children: n }, e.id)
                                                        : (0, s.jsx)(
                                                              tF.Ay,
                                                              {
                                                                  className: tY.SA,
                                                                  height: u,
                                                                  width: u,
                                                                  mask: tF.Ay.Masks.VOICE_USER_SUMMARY_ITEM,
                                                                  children: n,
                                                              },
                                                              e.id,
                                                          );
                                                }),
                                                _ > 0
                                                    ? (0, s.jsx)(tU.m, {
                                                          text: w.intl.formatToPlainString(w.t.m6oRrA, {
                                                              appNames: g.format(I.map((e) => e.name)),
                                                          }),
                                                          position: "top",
                                                          children: (0, s.jsxs)("div", {
                                                              className: r()(tY.S_, tX[u]),
                                                              children: ["+", _],
                                                          }),
                                                      })
                                                    : null,
                                            ],
                                        })
                                      : (0, s.jsx)(tV.R, {
                                            size: "custom",
                                            color: "currentColor",
                                            width: u,
                                            height: u,
                                            className: tY.DE,
                                        }),
                          }),
                          null != E ? (0, s.jsx)(x.E, { variant: o, color: "text-default", children: E }) : null,
                      ],
                  }),
              ],
          });
}
var tZ = l(973172),
    t$ = l(788021);
function tq(e) {
    let { application: t, className: l } = e,
        i = a.useCallback((e) => {
            k(O.HAw.APP_DIRECTORY_CATEGORY_CLICKED, {
                category: e.name,
                category_id: e.id,
                current_page: D.ev.APPLICATION,
            }),
                F({ categoryId: e.id });
        }, []);
    return null == t.categories || 0 === t.categories.length
        ? null
        : (0, s.jsxs)("div", {
              className: l,
              children: [
                  (0, s.jsx)(m.D, {
                      variant: "heading-sm/semibold",
                      color: "text-strong",
                      children: w.intl.string(w.t.s7iald),
                  }),
                  (0, s.jsx)("div", {
                      className: t$.L,
                      children: t.categories.map((e) =>
                          (0, s.jsx)(
                              ek.D,
                              {
                                  className: t$.c,
                                  onClick: () => i(e),
                                  children: (0, s.jsx)(x.E, {
                                      variant: "text-sm/semibold",
                                      color: "redesign-button-tertiary-text",
                                      children: e.name,
                                  }),
                              },
                              e.id,
                          ),
                      ),
                  }),
              ],
          });
}
var tQ = l(462887),
    tJ = l(514042),
    t0 = l(194261),
    t1 = l(173936),
    t7 = l(9578),
    t3 = l(736653),
    t9 = l(573648),
    t6 = l(355584);
function t8(e) {
    let { application: t, className: l, innerClassName: i } = e,
        a = t.directory_entry?.external_urls ?? [];
    return null != t.terms_of_service_url || null != t.privacy_policy_url || a.length > 0
        ? (0, s.jsxs)("div", {
              className: r()(t6.kL, l),
              children: [
                  (0, s.jsx)(m.D, {
                      variant: "heading-sm/semibold",
                      color: "text-strong",
                      children: w.intl.string(w.t.l6DP2n),
                  }),
                  (0, s.jsxs)("div", {
                      className: r()(t6.p_, i),
                      children: [
                          a.map((e, t) => (0, s.jsx)(t2, { url: e.url, children: e.name }, t)),
                          null != t.terms_of_service_url
                              ? (0, s.jsx)(t5, {
                                    icon: tJ.o,
                                    url: t.terms_of_service_url,
                                    children: w.intl.string(w.t.s7STcY),
                                })
                              : null,
                          null != t.privacy_policy_url
                              ? (0, s.jsx)(t5, {
                                    icon: t0.X,
                                    url: t.privacy_policy_url,
                                    children: w.intl.string(w.t.kH3JR5),
                                })
                              : null,
                      ],
                  }),
              ],
          })
        : null;
}
function t2(e) {
    let { url: t, children: l } = e,
        i = (0, t3.Ay)(),
        a = t9.A.getByUrl(t),
        n = (0, tQ.M)(i) ? a?.icon?.darkSVG : a?.icon?.lightSVG;
    return (0, s.jsx)(t5, { icon: void 0 === n ? t1.q : void 0, imageSrc: n, url: t, children: l });
}
function t5(e) {
    let { icon: t, imageSrc: l, iconColor: i, url: a, children: n } = e,
        o = G(),
        c = null;
    null != t
        ? (c = (0, s.jsx)(t, { className: t6.GU, color: i ?? "currentColor", width: 20, height: 20, size: "custom" }))
        : null != l && (c = (0, s.jsx)("img", { className: t6.GR, src: l, alt: "" }));
    let d = (0, s.jsxs)(s.Fragment, {
        children: [c, (0, s.jsx)(x.E, { className: t6.so, variant: "text-sm/medium", selectable: !0, children: n })],
    });
    return (0, s.jsx)(t7.A, {
        href: a,
        className: r()(t6.Aw, t6.SF),
        trusted: !o,
        useDefaultUnderlineStyles: !1,
        children: d,
    });
}
var t4 = l(385648),
    le = l(781696),
    lt = l(950305),
    ll = l(548118),
    li = l(714991),
    ls = l(776231),
    la = l(860689),
    ln = l(728968),
    lr = l(937495),
    lo = l(517061);
function lc(e) {
    let { guild: t, className: l, onClick: i } = e,
        a = (0, t3.Ay)(),
        n = (0, la.dangerouslyConstructGuildRecordFromUntypedObject)({ name: t.name, icon: t.icon }),
        o = eo.Ay.getGuildDiscoverySplashURL({ id: t.id, splash: t.discovery_splash, size: 192 * (0, ls.mZ)() }),
        c =
            null != o
                ? o
                : (function (e) {
                      switch (e) {
                          case O.NJ8.DARK:
                              return lr;
                          case O.NJ8.LIGHT:
                              return lo;
                      }
                  })(a),
        d = eo.Ay.getGuildIconURL({ id: t.id, icon: t.icon, size: 50 });
    return (0, s.jsxs)(ek.D, {
        className: r()(ln.Nr, l),
        onClick: i,
        "aria-label": w.intl.string(w.t.RB4L2x),
        children: [
            (0, s.jsxs)("div", {
                className: ln.MY,
                children: [
                    (0, s.jsx)("img", { src: c, alt: "", className: ln.j0 }),
                    (0, s.jsx)("div", {
                        className: ln.$f,
                        children: (0, s.jsx)(tF.Ay, {
                            mask: tF.Ay.Masks.AVATAR_DEFAULT,
                            width: 58,
                            height: 58,
                            children: (0, s.jsx)("div", {
                                className: ln.SA,
                                children: (0, s.jsx)(ll.Ay, {
                                    className: ln.__invalid_icon,
                                    iconSrc: d,
                                    guild: n,
                                    size: ll.Ay.Sizes.LARGE,
                                }),
                            }),
                        }),
                    }),
                ],
            }),
            (0, s.jsxs)("div", {
                className: ln.RN,
                children: [
                    (0, s.jsxs)("div", {
                        className: ln.Y_,
                        children: [
                            (0, s.jsx)(li.A, { className: ln.n2, guild: t, tooltipColor: le.ST.Colors.PRIMARY }),
                            (0, s.jsx)(m.D, { variant: "heading-md/semibold", className: ln.J5, children: t.name }),
                        ],
                    }),
                    (0, s.jsx)(x.E, {
                        className: ln.eF,
                        variant: "text-sm/normal",
                        color: "text-default",
                        lineClamp: 2,
                        children: t.description,
                    }),
                    (0, s.jsx)("div", {
                        className: ln.Fj,
                        children: (0, s.jsxs)("div", {
                            className: ln.FG,
                            children: [
                                (0, s.jsx)(lt.n, { color: "currentColor", className: ln.NP, size: "xs" }),
                                (0, s.jsx)(x.E, {
                                    variant: "text-xs/normal",
                                    color: "text-default",
                                    children: w.intl.format(w.t.zRl6XR, { count: t.approximate_member_count }),
                                }),
                            ],
                        }),
                    }),
                ],
            }),
        ],
    });
}
let ld = "redirect_to_support_server";
function lu(e) {
    var t;
    let l,
        i,
        n,
        { application: r, className: o } = e,
        { onGoToSupportServer: c } =
            ((t = r),
            (l = G()),
            (i = (0, d.bG)([f.default], () => f.default.getSessionId())),
            (n = a.useCallback(async () => {
                try {
                    let e = t?.guild?.id;
                    if (null != e) {
                        k(O.HAw.APP_DIRECTORY_SUPPORT_SERVER_JOINED, { application_id: t.id, support_guild_id: e });
                        let l = { page: O.liQ.APPLICATION_DIRECTORY };
                        await (0, t4.Z2)(e, l);
                    }
                } catch {}
            }, [t])),
            a.useEffect(() => {
                let e = new URL(location.href);
                if (null != t && l && null != i && "true" === e.searchParams.get(ld)) {
                    e.searchParams.delete(ld);
                    let t = e.pathname + e.search;
                    (0, E.bG)(t), n();
                }
            }, [l, n, i, t]),
            {
                onGoToSupportServer: a.useCallback(() => {
                    null != t?.guild?.id && (l ? n() : M({ [ld]: "true" }));
                }, [t, n, l]),
            });
    return null != r.guild && r.guild.features.includes(O.GuildFeatures.DISCOVERABLE)
        ? (0, s.jsxs)("div", {
              className: o,
              children: [
                  (0, s.jsx)(m.D, {
                      variant: "heading-sm/semibold",
                      color: "text-strong",
                      children: w.intl.string(w.t.KJEO27),
                  }),
                  (0, s.jsx)(lc, { guild: r.guild, onClick: c }),
              ],
          })
        : null;
}
var lh = l(820764);
let lm = (0, w.getAvailableLocales)();
function lp(e) {
    let { application: t, className: l } = e,
        i = (0, d.bG)([tB.default], () => tB.default.locale),
        { supportedLanguages: n, usersLanguageIsFirst: o } = a.useMemo(() => {
            if (t.directory_entry?.supported_locales != null) {
                let e = new Set(t.directory_entry.supported_locales),
                    l = e.has(i),
                    s = lm.filter((t) => e.has(t.value));
                return (
                    l && s.sort((e, t) => (e.value === i ? -1 : +(t.value === i))),
                    { supportedLanguages: s.map((e) => w.intl.string(e.localizedName)), usersLanguageIsFirst: l }
                );
            }
            return { supportedLanguages: [], usersLanguageIsFirst: !1 };
        }, [t.directory_entry, i]);
    return (0, s.jsxs)("div", {
        className: l,
        children: [
            (0, s.jsx)(m.D, {
                variant: "heading-sm/semibold",
                color: "text-strong",
                children: w.intl.string(w.t.Fbjlu3),
            }),
            (0, s.jsx)("ul", {
                className: lh.nw,
                children: n.map((e, t) =>
                    (0, s.jsx)(
                        "li",
                        {
                            className: r()(lh.WH, lh.O6),
                            children: (0, s.jsx)(x.E, {
                                variant: 0 === t && o ? "text-sm/bold" : "text-sm/normal",
                                children: e,
                            }),
                        },
                        e,
                    ),
                ),
            }),
            " ",
        ],
    });
}
var lg = l(78655);
function lx(e) {
    let { className: t, view: l, application: i } = e,
        a = "embedded" === l,
        n = (0, tZ.A)({ application: i });
    return (0, s.jsxs)("div", {
        className: r()(t, { [lg.PD]: !a, [lg.UT]: a }),
        children: [
            (0, s.jsxs)("div", {
                className: r()(lg.hd, { [lg.d5]: a }),
                children: [
                    (0, s.jsx)("div", {
                        className: lg.iY,
                        children: (0, s.jsx)(tW, {
                            application: i,
                            mutualGuilds: n,
                            mutualGuildShownMax: 3,
                            className: lg.wu,
                            guildIconSize: tz.SMALL,
                            guildsClassName: lg.HG,
                            compact: !0,
                            children: (0, s.jsx)(m.D, {
                                variant: "heading-sm/semibold",
                                color: "text-strong",
                                children: w.intl.string(w.t["3NxHC4"]),
                            }),
                        }),
                    }),
                    (0, s.jsx)(tq, { application: i, className: lg.iY }),
                    (0, s.jsx)(lp, { application: i, className: lg.iY }),
                ],
            }),
            (0, s.jsxs)("div", {
                className: r()(lg.hd, { [lg.d5]: a }),
                children: [
                    (0, s.jsx)(t8, { application: i, className: lg.iY, innerClassName: a ? lg.RU : void 0 }),
                    (0, s.jsx)(lu, { application: i, className: lg.iY }),
                ],
            }),
        ],
    });
}
var lA = l(791332),
    lv = l.n(lA),
    lj = l(900797),
    lC = l(847374),
    lI = l(558179),
    l_ = l(349288),
    lE = l(376728),
    lf = l(167189),
    lN = l(833291),
    lS = l(481859),
    lb = l(143145);
let ly = /^discord.gg\/[a-zA-Z0-9-]+/,
    lR = {
        ...lI.A.rules,
        heading: {
            ...lI.A.rules.heading,
            react(e, t, l) {
                let i = `h${Math.min(Math.max(2, e.level + 1), 4)}`;
                return (0, s.jsx)(lS.A, { tag: i, children: t(e.content, l) }, l.key);
            },
        },
        image: { ...lv().defaultRules.image },
        inviteLink: {
            order: lI.A.rules.link.order + 1,
            match: (e, t) => (t.inline ? ly.exec(e) : null),
            parse: (e, t, l) => {
                let i = (0, lN.br)(e[0]);
                return null == i || i.type !== lf.I.INVITE
                    ? { type: "text", content: e[0] }
                    : {
                          type: "inviteLink",
                          content: [{ type: "text", content: e[0] }],
                          onClick: () => {
                              lE.Ay.acceptInviteAndTransitionToInviteChannel({
                                  inviteKey: i.code,
                                  context: { location: "Application Directory" },
                              }),
                                  l.closeModal?.();
                          },
                      };
            },
            react: (e, t, l) =>
                (0, s.jsx)(
                    l_.Anchor,
                    {
                        onClick: (t) => {
                            t.preventDefault(), e.onClick();
                        },
                        children: (0, lb.t)(e, t, l),
                    },
                    l.key,
                ),
        },
    };
var lP = l(349202);
let lL = lv().parserFor(lR),
    lT = lv().reactFor(lv().ruleOutput(lR, "react"));
function lD(e) {
    let { description: t, supportsMarkdown: l } = e,
        [i, n] = a.useState(!1),
        o = a.useRef(null);
    a.useEffect(() => {
        if (null == o.current) return;
        let e = new ResizeObserver(() => {
            n((o.current?.scrollHeight ?? 0) - (o.current?.clientHeight ?? 0) > 1);
        });
        return e.observe(o.current), () => e.disconnect();
    }, []);
    let [c, d] = a.useState(!1),
        u = a.useCallback(() => {
            d((e) => !e);
        }, []);
    return (0, s.jsxs)(s.Fragment, {
        children: [
            (0, s.jsx)("div", {
                ref: o,
                className: r()({ [lP.mA]: !c, [lP.ii]: !c && "Safari" === platform.name }),
                children: l
                    ? (0, s.jsx)(lI.A, {
                          className: lP.R4,
                          parser: lL,
                          output: lT,
                          state: { allowLinks: !0 },
                          children: t,
                      })
                    : (0, s.jsx)(x.E, { variant: "text-md/normal", color: "text-default", children: t }),
            }),
            i || c ? (0, s.jsx)(lO, { isShowingMore: c, onToggle: u }) : null,
        ],
    });
}
function lO(e) {
    let { onToggle: t, isShowingMore: l } = e,
        i = a.useMemo(
            () =>
                (0, s.jsxs)("div", {
                    className: lP.eR,
                    children: [
                        (0, s.jsx)(x.E, {
                            variant: "text-md/semibold",
                            color: "interactive-text-default",
                            children: l ? w.intl.string(w.t["vtfc4+"]) : w.intl.string(w.t.ZDRyuq),
                        }),
                        l
                            ? (0, s.jsx)(lj.t, { size: "md", color: "currentColor", className: lP.ys })
                            : (0, s.jsx)(lC.a, { size: "md", color: "currentColor", className: lP.ys }),
                    ],
                }),
            [l],
        );
    return (0, s.jsxs)(ek.D, {
        className: lP.dO,
        onClick: t,
        children: [(0, s.jsx)("div", { className: lP.yF }), i, (0, s.jsx)("div", { className: lP.yF })],
    });
}
let lk = function (e) {
    let { application: t } = e,
        l = t.directory_entry?.detailed_description,
        i = t.directory_entry?.short_description,
        n = a.useMemo(
            () =>
                null != l && l.length > 0
                    ? (0, s.jsx)(lD, { description: l, supportsMarkdown: !0 })
                    : null != i && i.length > 0
                      ? (0, s.jsx)(lD, { description: i, supportsMarkdown: !1 })
                      : null,
            [l, i],
        );
    return null == n
        ? null
        : (0, s.jsxs)("div", {
              className: lP.l4,
              children: [
                  (0, s.jsx)(m.D, {
                      variant: "heading-lg/semibold",
                      color: "text-strong",
                      children: w.intl.string(w.t.txraKS),
                  }),
                  n,
              ],
          });
};
var lG = l(136722),
    lM = l(595738),
    lU = l(376092),
    lw = l(925317);
let lV = function (e) {
    let { application: t } = e,
        l = a.useMemo(() => {
            let e = t?.install_params?.permissions;
            if (null != e) return lU.Q.filter((t) => lG.zy(lG.iu(e), t));
        }, [t?.install_params?.permissions]);
    return null != l && l.length > 0
        ? (0, s.jsxs)("div", {
              className: lw.A,
              children: [
                  (0, s.jsx)(m.D, {
                      variant: "heading-lg/semibold",
                      color: "text-strong",
                      children: w.intl.string(w.t.runNF8),
                  }),
                  (0, s.jsx)(lM.A, {
                      className: lw.h,
                      grantedPermissions: l,
                      grantedPermissionsHeader: w.intl.string(w.t.SLgiNH),
                  }),
              ],
          })
        : null;
};
var lF = l(534740);
function lB(e) {
    let { application: t } = e,
        l = a.useMemo(() => (t.directory_entry?.carousel_items ?? []).map(L.xb).filter(eO.Vq) ?? [], [t]);
    return (0, s.jsxs)(s.Fragment, {
        children: [
            (0, s.jsxs)("div", {
                className: lF.Xb,
                children: [
                    (0, s.jsx)(tf, { application: t, carouselItems: l }),
                    (0, s.jsx)(lk, { application: t }),
                    (0, s.jsx)(tb, { application: t }),
                    (0, s.jsx)(lV, { application: t }),
                    (0, s.jsx)(tG, { application: t }),
                ],
            }),
            (0, s.jsxs)("div", {
                className: lF.Q9,
                children: [
                    (0, s.jsx)("div", { className: lF.yF }),
                    (0, s.jsx)(m.D, {
                        variant: "heading-lg/semibold",
                        color: "text-strong",
                        children: w.intl.string(w.t["mD+J/i"]),
                    }),
                    (0, s.jsx)(lx, { application: t, view: "embedded" }),
                ],
            }),
        ],
    });
}
var lH = l(607399),
    lY = l(691540),
    lz = l(857250),
    lX = l(97483),
    lK = l(980707),
    lW = l(477782),
    lZ = l(138134),
    l$ = l(922016),
    lq = l(365199),
    lQ = l(663341),
    lJ = l(821609),
    l0 = l(782134),
    l1 = l(787389),
    l7 = l(688810),
    l3 = l(361926),
    l9 = l(204776),
    l6 = l(50268),
    l8 = l(207371),
    l2 = l(928658),
    l5 = l(395671),
    l4 = l(957565),
    ie = l(692848),
    it = l(440970);
let il = "start_application_install",
    ii = "launch_activity";
function is(e) {
    let { renderDropdown: t, size: l } = e,
        i = a.useRef(null);
    return (0, s.jsx)(l$.Y, {
        targetElementRef: i,
        renderPopout: (e) => {
            let { closePopout: l } = e;
            return t(l);
        },
        position: "left",
        align: "top",
        animation: l$.Y.Animation.NONE,
        children: (e) =>
            (0, s.jsx)(ew.K, {
                buttonRef: i,
                variant: "secondary",
                size: l,
                icon: lq.j,
                "aria-label": w.intl.string(w.t["UKOtz+"]),
                ...e,
            }),
    });
}
function ia(e) {
    var t;
    let l,
        i,
        { application: n, variant: r, size: o, contentWidth: c } = e,
        { onClickInstallApplication: d, canInstall: u } =
            ((t = n),
            (l = G()),
            (i = a.useCallback(() => {
                let e = D.h.getField("guildId") ?? void 0;
                (0, ie.o)({
                    applicationId: t.id,
                    customInstallUrl: t.custom_install_url,
                    installParams: t.install_params,
                    integrationTypesConfig: t.integration_types_config,
                    guildId: e,
                    source: "product_page",
                });
            }, [t])),
            a.useEffect(() => {
                let e = new URL(location.href);
                if (l && "true" === e.searchParams.get(il)) {
                    e.searchParams.delete(il);
                    let t = e.pathname + e.search;
                    (0, E.bG)(t), i();
                }
            }, [l, i]),
            {
                onClickInstallApplication: a.useCallback(() => {
                    if (
                        (k(O.HAw.APP_DIRECTORY_INSTALL_CLICKED, { application_id: t.id }),
                        (lH.Fr || lH.v1) && null == t.custom_install_url)
                    ) {
                        let e = (0, ie.Z)({
                            applicationId: t.id,
                            customInstallUrl: t.custom_install_url,
                            installParams: t.install_params,
                            integrationTypesConfig: t.integration_types_config,
                        });
                        if (null != e) return void window.open(e, "_blank");
                    }
                    if (l) i();
                    else {
                        let e = D.h.getField("guildId");
                        S.default.track(O.HAw.APPLICATION_ADD_TO_SERVER_CLICKED, {
                            application_id: t.id,
                            guild_id: e,
                            auth_type: null != t.custom_install_url ? "custom_url" : "in_app",
                            source: "product_page",
                            device_platform: lH.Fr ? "mobile_web" : "desktop_web",
                        }),
                            M({ [il]: "true" });
                    }
                }, [t, i, l]),
                canInstall: a.useMemo(
                    () =>
                        (0, l9.Ie)({
                            customInstallUrl: t.custom_install_url,
                            installParams: t.install_params,
                            integrationTypesConfig: t.integration_types_config,
                        }),
                    [t],
                ),
            });
    return "icon" === c
        ? (0, s.jsx)(ew.K, {
              variant: r,
              icon: lQ.p,
              size: o,
              onClick: d,
              "aria-label": w.intl.string(w.t.NgXl3C),
              disabled: !u,
          })
        : (0, s.jsx)(lJ.$, {
              variant: r,
              size: o,
              text: "small" === c ? w.intl.string(w.t.qRZ35t) : w.intl.string(w.t.NgXl3C),
              onClick: d,
              disabled: !u,
          });
}
function ir(e) {
    let { size: t, contentWidth: l, onClick: i, isSubmitting: a } = e;
    return "icon" === l
        ? (0, s.jsx)(ew.K, {
              variant: "primary",
              icon: l0.u,
              size: t,
              onClick: i,
              loading: a,
              "aria-label": w.intl.string(w.t.QO7rO6),
          })
        : (0, s.jsx)(lJ.$, { variant: "primary", size: t, text: w.intl.string(w.t.QO7rO6), onClick: i, loading: a });
}
let io = a.forwardRef(function (e, t) {
    let { application: l, size: i } = e,
        { analyticsLocations: n } = (0, l7.Ay)(),
        o = a.useCallback(() => {
            var e;
            k(O.HAw.APP_DIRECTORY_APPLICATION_LINK_COPIED, { application_id: l.id }),
                (0, l4.C)(
                    ((e = l.id), `${location.protocol}//${location.host}${O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE(e)}`),
                    () => (0, lY.P0)((0, lz.o)(w.intl.string(w.t["L/PwZf"]), lX.Ck.SUCCESS)),
                );
        }, [l.id]),
        c = a.useCallback(() => {
            let e = l5.Ay.createFromServer(l);
            (0, l2.r3)({ application: e, entrypoint: "app_directory" });
        }, [l]),
        u = (0, l6.A)({
            id: l.id,
            label: w.intl.string(w.t["FfCL+6"]),
            onSuccess: () => (0, lY.P0)((0, lz.o)(w.intl.string(w.t.eNjAah), lX.Ck.SUCCESS)),
        }),
        h = a.useCallback(
            (e, t) =>
                (0, s.jsxs)(lK.W, {
                    "data-menu-migrated-auto": !0,
                    navId: "application-directory-profile",
                    "aria-label": w.intl.string(w.t["/7I4/C"]),
                    onClose: t,
                    onSelect: void 0,
                    children: [
                        e
                            ? (0, s.jsx)(lW.rX, {
                                  children: (0, s.jsx)(lW.Dr, {
                                      id: "copy",
                                      label: w.intl.string(w.t.z4sP5J),
                                      action: o,
                                      icon: t1.q,
                                      leadingAccessory: { type: "icon", icon: t1.q },
                                  }),
                              })
                            : null,
                        (0, s.jsx)(lW.rX, {
                            children: (0, s.jsx)(lW.Dr, {
                                id: "report",
                                label: w.intl.string(w.t.NgA5vp),
                                color: "danger",
                                leadingAccessory: { type: "icon", icon: lZ.i },
                                action: c,
                            }),
                        }),
                        null != u ? (0, s.jsx)(lW.rX, { children: u }) : null,
                    ],
                }),
            [u, c, o],
        ),
        { onClickLaunchActivity: m, isSubmitting: p } = (function (e, t) {
            let l = e.id,
                i = e?.bot?.id,
                s = (0, l8.x)(e),
                n = (0, l3.Vr)({ context: { type: "contextless" }, applicationId: l, botUserId: i }),
                r = null != i && (0, en.$B)(e) && (0, en.Ag)(e) && (s || n),
                [o, c] = a.useState(!1),
                u = G(),
                h = (0, d.bG)([N.default], () => N.default.getCurrentUser()),
                m = (0, l1.A)({
                    applicationId: l,
                    analyticsLocations: t,
                    runBeforeLaunchAttempt: () => c(!0),
                    runAfterLaunchAttempt: () => c(!1),
                });
            a.useEffect(() => {
                let e = new URL(location.href),
                    t = "true" === e.searchParams.get(ii);
                if (u && r && t && null != i && null != h) {
                    e.searchParams.delete(ii);
                    let t = e.pathname + e.search;
                    (0, E.bG)(t), m();
                }
            }, [u, r, i, h, m]);
            let p = a.useCallback(async () => {
                if (null != i) {
                    if ((k(O.HAw.APP_DIRECTORY_LAUNCH_CLICKED, { application_id: l }), !u))
                        return void M({ [ii]: "true" });
                    await m();
                }
            }, [l, i, u, m]);
            return { onClickLaunchActivity: r ? p : void 0, isSubmitting: o };
        })(l, n),
        g = null != m,
        x = g ? "secondary" : "primary";
    return (0, s.jsxs)("div", {
        ref: t,
        children: [
            (0, s.jsxs)("div", {
                className: r()(it.Ik, it.U),
                children: [
                    g && (0, s.jsx)(ir, { size: i, contentWidth: "normal", onClick: m, isSubmitting: p }),
                    (0, s.jsx)(ia, { application: l, size: i, contentWidth: "normal", variant: x }),
                    l4.p5
                        ? (0, s.jsx)(ew.K, {
                              variant: "secondary",
                              size: i,
                              icon: t1.q,
                              onClick: o,
                              "aria-label": w.intl.string(w.t.z4sP5J),
                          })
                        : null,
                    (0, s.jsx)(is, { renderDropdown: (e) => h(!1, e), size: i }),
                ],
            }),
            (0, s.jsxs)("div", {
                className: r()(it.Ik, it.EX),
                children: [
                    g && (0, s.jsx)(ir, { size: i, contentWidth: "small", onClick: m, isSubmitting: p }),
                    (0, s.jsx)(ia, { application: l, size: i, contentWidth: "small", variant: x }),
                    (0, s.jsx)(is, { renderDropdown: (e) => h(l4.p5, e), size: i }),
                ],
            }),
            (0, s.jsxs)("div", {
                className: r()(it.Ik, it.nU),
                children: [
                    g && (0, s.jsx)(ir, { size: i, contentWidth: "icon", onClick: m, isSubmitting: p }),
                    (0, s.jsx)(ia, { application: l, size: i, contentWidth: "icon", variant: x }),
                    (0, s.jsx)(is, { renderDropdown: (e) => h(l4.p5, e), size: i }),
                ],
            }),
        ],
    });
});
var ic = l(811308);
let id = function (e) {
    let { application: t, onButtonsVisibilityChange: l } = e,
        i = a.useRef(null),
        n = a.useMemo(() => eo.Ay.getApplicationIconURL({ id: t.id, icon: t.icon, size: 96 }), [t]),
        r = a.useMemo(() => {
            let e = [];
            return (
                (0, en.b7)(t) && e.push(w.intl.string(w.t.LO4f0P)),
                (0, en.K4)(t) && (0 !== e.length && e.push("\u2022"), e.push(w.intl.string(w.t["8z5B2U"]))),
                (0, en.ME)(t) && (0 !== e.length && e.push("\u2022"), e.push(w.intl.string(w.t["5khEk8"]))),
                e
            );
        }, [t]);
    return (0, s.jsxs)(s.Fragment, {
        children: [
            (0, s.jsx)(eI, { application: t, bannerType: "detail", iconURL: n }),
            (0, s.jsx)("div", {
                className: ic.H,
                style: { height: 52 },
                children: (0, s.jsx)("img", {
                    src: n,
                    alt: "",
                    className: ic.my,
                    height: 96,
                    width: 96,
                    style: { borderWidth: 4, marginLeft: 28 },
                }),
            }),
            (0, s.jsxs)("div", {
                className: ic.op,
                children: [
                    (0, s.jsxs)("div", {
                        className: ic.tJ,
                        children: [
                            (0, s.jsx)(m.D, {
                                variant: "heading-xxl/bold",
                                color: "text-strong",
                                lineClamp: 1,
                                children: t.name,
                            }),
                            (0, s.jsx)("div", {
                                className: ic.A7,
                                children: r.map((e, t) =>
                                    (0, s.jsx)(
                                        x.E,
                                        {
                                            className: ic.MO,
                                            variant: "text-xs/normal",
                                            color: "text-muted",
                                            children: e,
                                        },
                                        t,
                                    ),
                                ),
                            }),
                        ],
                    }),
                    (0, s.jsx)(ea.L, {
                        innerRef: i,
                        onChange: l,
                        active: !0,
                        children: (0, s.jsx)(io, { ref: i, application: t, size: "md" }),
                    }),
                ],
            }),
        ],
    });
};
var iu = l(172218),
    ih = l(668168);
function im(e) {
    let { applicationId: t, similarApplications: l, onSelectApplication: i, similarLoadId: n } = e,
        r = (function (e) {
            let { applicationId: t, similarAppIds: l } = e,
                [i, s] = a.useState(),
                n = (0, iu.K)(function (e) {
                    e && s(t);
                });
            return (
                a.useEffect(() => {
                    i === t &&
                        k(O.HAw.APP_DIRECTORY_RECOMMENDATIONS_VIEWED, {
                            application_id: t,
                            suggested_application_ids: l,
                        });
                }, [i, t, l]),
                n
            );
        })({ applicationId: t, similarAppIds: a.useMemo(() => l.map((e) => e.id), [l]) });
    return (0, s.jsx)("div", {
        className: ih.hQ,
        ref: r,
        children: l.map((e, l) =>
            (0, s.jsx)(
                ip,
                { applicationId: t, similarApplication: e, onSelectApplication: i, similarLoadId: n, position: l },
                e.id,
            ),
        ),
    });
}
function ip(e) {
    let { applicationId: t, similarApplication: l, onSelectApplication: i, position: n, similarLoadId: r } = e,
        o = (0, tZ.A)({ application: l }),
        c = l.categories?.[0],
        d = a.useMemo(() => eo.Ay.getApplicationIconURL({ id: l.id, icon: l.icon, size: 48 }), [l]),
        u = a.useCallback(() => {
            k(O.HAw.APP_DIRECTORY_RECOMMENDATION_CLICKED, {
                current_page: "product",
                application_id: t,
                suggested_application_id: l.id,
                position: n,
                load_id: r,
                shown_mutual_guilds_count: o.length,
            }),
                i(l.id);
        }, [i, l.id, t, o.length, r, n]);
    return (0, s.jsxs)(ek.D, {
        onClick: u,
        className: ih.vU,
        children: [
            (0, s.jsxs)("div", {
                className: ih.nT,
                children: [
                    (0, s.jsx)("img", { src: d, alt: "", className: ih.my, height: 48, width: 48 }),
                    (0, s.jsxs)("div", {
                        className: ih.gn,
                        children: [
                            (0, s.jsx)("div", {
                                style: { flex: 1, minWidth: 0, overflow: "hidden" },
                                children: (0, s.jsx)(m.D, {
                                    variant: "heading-md/semibold",
                                    color: "text-strong",
                                    className: ih.JB,
                                    children: l.name,
                                }),
                            }),
                            null != c
                                ? (0, s.jsx)(x.E, {
                                      className: ih.m4,
                                      variant: "text-xs/medium",
                                      color: "text-muted",
                                      children: c.name,
                                  })
                                : null,
                        ],
                    }),
                ],
            }),
            (0, s.jsx)(x.E, {
                className: ih.i3,
                variant: "text-sm/medium",
                color: "text-default",
                lineClamp: 2,
                children: l.directory_entry?.short_description ?? l.description,
            }),
            (0, s.jsx)(tW, {
                application: l,
                textVariant: "text-xs/medium",
                mutualGuilds: o,
                mutualGuildShownMax: 3,
                guildIconSize: tz.SMALL,
                compact: !0,
            }),
        ],
    });
}
let ig = function (e) {
    let { applicationId: t, fetchState: l, similarApplications: i, onSelectApplication: a, similarLoadId: n } = e;
    return l !== eD.e.FETCHING && (null == i || 0 === i.length)
        ? null
        : (0, s.jsxs)("div", {
              className: ih.hd,
              children: [
                  (0, s.jsx)("div", { className: ih.yF }),
                  (0, s.jsx)(m.D, {
                      variant: "heading-sm/semibold",
                      color: "text-strong",
                      className: ih.bV,
                      children: w.intl.string(w.t.E8wCnk),
                  }),
                  (0, s.jsx)(es, {
                      loading: l === eD.e.FETCHING,
                      children:
                          null != i
                              ? (0, s.jsx)(im, {
                                    applicationId: t,
                                    similarApplications: i,
                                    onSelectApplication: a,
                                    similarLoadId: n,
                                })
                              : null,
                  }),
              ],
          });
};
l(938796);
var ix = l(192308),
    iA = l(549711),
    iv = l(925435),
    ij = l(311321),
    iC = l(163437),
    iI = l(67480),
    i_ = l(785936);
function iE(e) {
    let { application: t } = e;
    return (0, s.jsxs)("div", {
        className: i_.ZM,
        children: [(0, s.jsx)(iN, { application: t }), (0, s.jsx)(ib, { application: t })],
    });
}
function iN(e) {
    let { application: t } = e,
        { subscriptions: l, otps: i } = (0, eR.C)(t.id);
    return (0, s.jsxs)("div", {
        className: i_.ZM,
        children: [
            l.length > 0 &&
                (0, s.jsxs)("div", {
                    className: i_.dD,
                    children: [
                        (0, s.jsx)(ij.A, { subscriptions: l, color: "text-strong" }),
                        (0, s.jsx)("div", {
                            className: i_.ZE,
                            children: l.map((e) => {
                                let l = (0, iC.bg)(e.skuFlags) ? "user" : "guild";
                                return (0, s.jsx)(
                                    iv.B4,
                                    {
                                        appId: t.id,
                                        listing: e,
                                        subscriptionType: l,
                                        onDetails: () =>
                                            B(O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_STORE_SKU(t.id, e.skuId)),
                                    },
                                    e.id,
                                );
                            }),
                        }),
                    ],
                }),
            i.length > 0 &&
                (0, s.jsxs)("div", {
                    className: i_.dD,
                    children: [
                        (0, s.jsx)(m.D, {
                            variant: "heading-lg/semibold",
                            color: "text-strong",
                            children: w.intl.string(w.t.yUGTs8),
                        }),
                        (0, s.jsx)("div", {
                            className: i_.ZE,
                            children: i.map((e) =>
                                (0, s.jsx)(
                                    iv.B8,
                                    {
                                        skuId: e.skuId,
                                        appId: t.id,
                                        onDetails: () =>
                                            B(O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_STORE_SKU(t.id, e.skuId)),
                                    },
                                    e.id,
                                ),
                            ),
                        }),
                    ],
                }),
            (0, s.jsx)(iA.A, {
                path: O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_STORE_SKU(":applicationId", ":skuId"),
                exact: !0,
                render: (e) => (0, s.jsx)(iS, { ...e }),
            }),
        ],
    });
}
function iS(e) {
    let {
            match: {
                params: { applicationId: t, skuId: i },
            },
        } = e,
        n = (0, d.bG)([iI.A], () => iI.A.get(i), [i]),
        r = (0, d.bG)([iI.A], () => (null != i ? iI.A.getParentSKU(i) : void 0), [i]),
        o = a.useId(),
        c = a.useCallback(
            () => B(O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_SECTION(t, U.GlobalDiscoveryAppsSections.STORE)),
            [t],
        );
    return (
        a.useLayoutEffect(() => {
            var e;
            switch (n?.type) {
                case O.Puh.CONSUMABLE:
                case O.Puh.DURABLE:
                    return void (0, ix.openModalLazy)(
                        async () => {
                            let { ItemDetailsModal: e } = await Promise.all([
                                l.e("45976"),
                                l.e("60654"),
                                l.e("70179"),
                                l.e("53917"),
                                l.e("15186"),
                                l.e("80830"),
                                l.e("70104"),
                            ]).then(l.bind(l, 963179));
                            return (l) => (0, s.jsx)(e, { appId: t, skuId: i, ...l });
                        },
                        {
                            modalKey: o,
                            onCloseCallback: () => {
                                iy() ||
                                    B(
                                        O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_SECTION(
                                            t,
                                            U.GlobalDiscoveryAppsSections.STORE,
                                        ),
                                    );
                            },
                        },
                    );
                case O.Puh.SUBSCRIPTION:
                    if (r?.flags == null) return;
                    return (
                        (e = (0, iC.bg)(r.flags) ? "user" : "guild"),
                        void (0, ix.openModalLazy)(
                            async () => {
                                let { SubscriptionDetailsModal: a } = await Promise.all([
                                    l.e("45976"),
                                    l.e("60654"),
                                    l.e("70179"),
                                    l.e("53917"),
                                    l.e("15186"),
                                    l.e("80830"),
                                    l.e("64850"),
                                ]).then(l.bind(l, 168393));
                                return (l) =>
                                    (0, s.jsx)(a, { appId: t, subscriptionType: e, skuId: i, guildId: void 0, ...l });
                            },
                            {
                                modalKey: o,
                                onCloseCallback: () => {
                                    iy() ||
                                        B(
                                            O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_SECTION(
                                                t,
                                                U.GlobalDiscoveryAppsSections.STORE,
                                            ),
                                        );
                                },
                            },
                        )
                    );
            }
        }, [t, o, c, r?.flags, n?.flags, n?.type, i]),
        a.useLayoutEffect(
            () => () => {
                (0, ix.closeModal)(o);
            },
            [o],
        ),
        null
    );
}
function ib(e) {
    let { application: t } = e,
        l = G(),
        i = t.terms_of_service_url,
        a = t.privacy_policy_url;
    if (null == i && null == a) return null;
    function n(e, t) {
        return (0, s.jsx)(t7.A, { href: t, trusted: !l, children: e });
    }
    return (0, s.jsx)("div", {
        className: i_.V6,
        children: (0, s.jsx)(x.E, {
            color: "text-strong",
            variant: "text-sm/normal",
            children:
                null != i && null != a
                    ? w.intl.format(w.t.nylPOT, { termsHook: (e) => n(e, i), privacyHook: (e) => n(e, a) })
                    : null != i
                      ? w.intl.format(w.t["0cPyDz"], { termsHook: (e) => n(e, i) })
                      : null != a
                        ? w.intl.format(w.t["loYGC/"], { privacyHook: (e) => n(e, a) })
                        : null,
        }),
    });
}
function iy() {
    return window.location.pathname.startsWith("/login");
}
var iR = l(798385);
let iP = function (e) {
    let { applicationId: t, onSelectApplication: l, onScroll: i, initialTab: n, onButtonsVisibilityChange: r } = e,
        o = a.useRef(null),
        c = (0, d.bG)([R.A], () => R.A.getApplication(t)),
        u = (0, d.bG)([R.A], () => R.A.getApplicationFetchState(t));
    a.useEffect(() => {
        if (null != t && null == c) {
            let e = "true" === new URLSearchParams(location.search).get("preview") || void 0;
            b.Di(t, { noCache: e });
        }
    }, [t, c]),
        a.useEffect(() => {
            b.xZ({ applicationId: t });
        }, [t]),
        a.useEffect(() => () => r(!0), [r]);
    let h = c?.storefront_available,
        [m, p] = a.useState(n ?? U.GlobalDiscoveryAppsSections.ABOUT),
        g = a.useCallback(
            (e) => {
                o.current?.scrollTo({ to: 0 }), p(U.GlobalDiscoveryAppsSections.ABOUT), l(e);
            },
            [l],
        );
    a.useEffect(() => {
        c?.storefront_available && null != t && (0, eR.F)(t);
    }, [c?.storefront_available, t]);
    let x = (0, D.Bn)(),
        A = (0, d.bG)([eD.A], () => eD.A.getFetchState({ applicationId: c?.id })),
        v = (0, d.bG)([eD.A], () => eD.A.getSimilarApplications({ applicationId: c?.id })),
        j = a.useMemo(() => v?.applications.slice(0, 3), [v]),
        C = { displayedSimilarApplications: j, previousView: x },
        I = a.useRef(C);
    a.useEffect(() => {
        I.current = C;
    }),
        a.useEffect(() => {
            let { displayedSimilarApplications: e, previousView: t } = I.current;
            c?.id != null &&
                A !== eD.e.FETCHING &&
                k(O.HAw.APP_DIRECTORY_PAGE_VIEWED, {
                    current_page: D.ev.APPLICATION,
                    previous_page: t?.type,
                    application_id: c.id,
                    referrer: document.referrer,
                    suggested_application_ids: e?.map((e) => e.id),
                });
        }, [c?.id, A]);
    let _ = a.useMemo(
            () => [
                { id: U.GlobalDiscoveryAppsSections.ABOUT, label: w.intl.string(w.t.DkyHMK) },
                { id: U.GlobalDiscoveryAppsSections.STORE, label: w.intl.string(w.t.jgEXGB) },
            ],
            [],
        ),
        E = a.useMemo(() => {
            if (null == c) return null;
            switch (m) {
                case U.GlobalDiscoveryAppsSections.ABOUT:
                    return (0, s.jsx)(lB, { application: c });
                case U.GlobalDiscoveryAppsSections.STORE:
                    return (0, s.jsx)(iE, { application: c });
            }
        }, [m, c]),
        f = a.useCallback(
            (e) => {
                k(O.HAw.APP_DIRECTORY_PROFILE_TAB_CLICKED, { application_id: t, tab_name: e }),
                    p(e),
                    e === U.GlobalDiscoveryAppsSections.ABOUT
                        ? B(O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE(t))
                        : B(O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE_SECTION(t, e));
            },
            [t],
        ),
        N = m === U.GlobalDiscoveryAppsSections.ABOUT;
    return (0, s.jsx)(X.A, {
        onScroll: i,
        ref: o,
        children: (0, s.jsx)(z.A, {
            children:
                null == c
                    ? null == u || u === R.e.FETCHING
                        ? (0, s.jsx)("div", { className: iR.Dg, children: (0, s.jsx)(es, { loading: !0 }) })
                        : (0, s.jsx)("div", { className: iR.Dg, children: (0, s.jsx)(Z, { className: iR.z3 }) })
                    : (0, s.jsxs)("div", {
                          className: iR.nM,
                          children: [
                              (0, s.jsx)(id, { application: c, onButtonsVisibilityChange: r }),
                              (0, s.jsxs)("div", {
                                  className: iR.hQ,
                                  children: [
                                      (0, s.jsxs)("div", {
                                          className: iR.Ne,
                                          children: [
                                              h &&
                                                  (0, s.jsx)("div", {
                                                      className: iR.OF,
                                                      children: (0, s.jsx)(eT, {
                                                          tabs: _,
                                                          onTabSelect: f,
                                                          selectedTab: m,
                                                      }),
                                                  }),
                                              E,
                                              (0, s.jsx)(ig, {
                                                  applicationId: t,
                                                  fetchState: A,
                                                  similarApplications: j,
                                                  onSelectApplication: g,
                                                  similarLoadId: v?.loadId,
                                              }),
                                          ],
                                      }),
                                      N && (0, s.jsx)(lx, { className: iR.pz, application: c, view: "side" }),
                                  ],
                              }),
                          ],
                      }),
        }),
    });
};
var iL = l(984516),
    iT = l(724002),
    iD = l(287174),
    iO = l(487899),
    ik = l(239314),
    iG = l(111162),
    iM = l(242552);
let iU = iD.K.APPLICATION_DIRECTORY;
function iw(e) {
    let { collection: t, index: l, onSelectApplication: i } = e,
        n = (function (e) {
            let { collectionId: t, index: l } = e,
                [i, s] = a.useState(!1),
                n = (0, iu.K)(function (e) {
                    e && s(!0);
                });
            return (
                a.useEffect(() => {
                    i && k(O.HAw.APP_DIRECTORY_COLLECTION_VIEWED, { collection_id: t, collection_position: l });
                }, [i, t, l]),
                n
            );
        })({ collectionId: t.id, index: l }),
        o = a.useCallback(
            (e, s) => {
                k(O.HAw.APP_DIRECTORY_COLLECTION_ITEM_CLICKED, {
                    collection_id: t.id,
                    item_position: s,
                    collection_position: l,
                    application_id: e,
                }),
                    i(e);
            },
            [t.id, l, i],
        );
    return (0, s.jsxs)("div", {
        ref: n,
        children: [
            (0, s.jsx)(m.D, {
                className: r()(iM.Gf, { [iM.SH]: 0 === l }),
                variant: "heading-lg/semibold",
                color: "text-strong",
                children: t.title,
            }),
            (0, s.jsx)("div", {
                className: iM.Qs,
                children: t.application_directory_collection_items.map((e, t) =>
                    e.type !== iO.L.APPLICATION || null == e.application
                        ? null
                        : (0, s.jsx)(
                              eE,
                              {
                                  application: e.application,
                                  onSelectApplication: () => o(e.application.id, t),
                                  showCategory: !0,
                              },
                              e.id,
                          ),
                ),
            }),
        ],
    });
}
let iV = function (e) {
    let { onSelectApplication: t } = e,
        l = (0, d.bG)([iG.default], () => iG.default.onlyShowPreviewAppCollections) ? iT.W.PREVIEW : iT.W.ACTIVE,
        i = (0, d.bG)([ee.A], () => ee.A.getFetchState({ surface: iU, activeState: l })),
        n = (0, d.bG)([ee.A], () => ee.A.getCollections({ surface: iU, activeState: l }));
    a.useEffect(() => {
        b.An({ surface: iU, activeState: l });
    }, [l]);
    let r = a.useMemo(() => n?.filter((e) => e.type !== ik.Y.GALLERY), [n]);
    return i === ee.e.ERROR
        ? (0, s.jsx)("div", { className: iM.Un, children: (0, s.jsx)(Z, { className: iM.z3 }) })
        : (0, s.jsx)(es, {
              loading: i === ee.e.FETCHING,
              children: r?.map((e, l) => (0, s.jsx)(iw, { collection: e, index: l, onSelectApplication: t }, l)),
          });
};
var iF = l(210349);
let iB = function (e) {
    let { onScroll: t, onSelectApplication: l } = e;
    return (0, s.jsxs)(X.A, {
        onScroll: t,
        children: [
            (0, s.jsx)(Y.A, {
                title: w.intl.format(w.t.kEcZ97, {}),
                description: w.intl.string(w.t.XxFts9),
                children: (0, s.jsx)(H.A, {}),
            }),
            (0, s.jsx)(z.A, {
                children: (0, s.jsxs)("div", {
                    className: iF.h,
                    children: [(0, s.jsx)(iV, { onSelectApplication: l }), (0, s.jsx)(iL.A, { padding: !0 })],
                }),
            }),
        ],
    });
};
var iH = l(873263),
    iY = l(994369),
    iz = l(475743),
    iX = l(191711),
    iK = l(666168);
function iW(e) {
    let { category: t, count: l, onSelectCategory: i, selected: n } = e,
        o = a.useCallback(() => {
            i(t.id);
        }, [t.id, i]),
        c = l.toLocaleString(tB.default.locale);
    return (0, s.jsxs)(ek.D, {
        onClick: o,
        className: r()(iK.L1, { [iK.wH]: n }),
        children: [
            (0, s.jsx)(x.E, { className: iK.UU, variant: "text-sm/medium", color: "text-strong", children: t.name }),
            (0, s.jsx)(x.E, { className: iK.U9, variant: "text-sm/normal", color: "text-muted", children: c }),
        ],
    });
}
function iZ(e) {
    let { countsByCategory: t, onSelectCategory: l, selectedCategoryId: i } = e,
        n = (0, d.bG)([P.A], () => P.A.getCategories()),
        r = a.useMemo(() => [(0, L.AU)(), ...n.filter((e) => (t[e.id] ?? 0) !== 0)], [n, t]);
    return 0 === r.length
        ? null
        : (0, s.jsx)("div", {
              className: iK.LZ,
              children: r.map((e) => {
                  let a = e.id,
                      n = t[a] ?? 0;
                  return (0, s.jsx)(iW, { category: e, count: n, selected: a === i, onSelectCategory: () => l(e) }, a);
              }),
          });
}
function i$(e) {
    let { countsByCategory: t, onSelectCategory: l, selectedCategoryId: i } = e,
        n = (0, d.bG)([P.A], () => P.A.getCategories()),
        r = a.useMemo(() => [(0, L.AU)(), ...n.filter((e) => (t[e.id] ?? 0) !== 0)], [n, t]),
        o = a.useMemo(() => r.map((e) => ({ id: `${e.id}`, name: e.name, count: t[e.id] ?? 0 })), [r, t]),
        c = a.useCallback(
            (e) => {
                let t = r.find((t) => t.id === Number(e));
                null != t && l(t);
            },
            [l, r],
        ),
        u = a.useMemo(() => r.find((e) => e.id === i), [i, r]);
    return 0 === o.length
        ? null
        : (0, s.jsxs)("div", {
              className: iK.kL,
              children: [
                  (0, s.jsx)(x.E, {
                      variant: "text-sm/medium",
                      color: "text-subtle",
                      children: w.intl.string(w.t.f09BQJ),
                  }),
                  (0, s.jsx)(iX.A, {
                      items: o,
                      title: u?.name ?? "",
                      onSelect: c,
                      selected: `${i}`,
                      "aria-label": w.intl.string(w.t.WHdCwR),
                      variant: iX.H.FILLED,
                  }),
              ],
          });
}
var iq = l(678871),
    iQ = l(842503);
let iJ = function (e) {
    let { selectedCategoryId: t, searchAllCategories: l } = e,
        i = (0, d.bG)([P.A], () => P.A.getCategories()),
        n = a.useMemo(() => i.find((e) => e.id === t)?.name, [i, t]),
        r =
            t === L.lj || null == n
                ? w.intl.string(w.t["FVJt+8"])
                : w.intl.formatToPlainString(w.t.FjCaNB, { categoryName: n }),
        o = t === L.lj ? w.intl.string(w.t.GXiMta) : w.intl.format(w.t.JaLoFF, { onClick: l });
    return (0, s.jsxs)("div", {
        className: iq.kL,
        children: [
            (0, s.jsx)("img", { alt: "", src: iQ, className: iq.Sl }),
            (0, s.jsx)(m.D, { variant: "heading-xl/semibold", className: iq.wx, children: r }),
            (0, s.jsx)(x.E, { variant: "text-md/normal", color: "text-default", children: o }),
        ],
    });
};
var i0 = l(798387);
function i1(e) {
    let { onSelectApplication: t, onScroll: l } = e,
        i = (0, d.bG)([f.default], () => f.default.isAuthenticated()),
        n = a.useRef(null),
        r = (0, iH.zy)(),
        o = a.useRef(null),
        {
            query: c,
            page: u,
            categoryId: h,
        } = a.useMemo(() => {
            let e = new URLSearchParams(r.search),
                t = e.get("page"),
                l = null != t ? Number(t) : 1;
            return {
                query: e.get("q") ?? "",
                page: l,
                categoryId: Number(e.get("category_id")) ?? U.GLOBAL_DISCOVERY_APPS_FEATURED_CATEGORY_ID,
            };
        }, [r.search]),
        m = (0, d.bG)([P.A], () => P.A.getCategories()),
        p = a.useMemo(() => m?.find((e) => e.id === h), [m, h]),
        g = a.useCallback(
            (e) => {
                k(O.HAw.APP_DIRECTORY_SEARCHED, {
                    search_term: c,
                    num_results: e,
                    current_page: D.ev.SEARCH,
                    result_page: u,
                    category: p?.name,
                    category_id: p?.id,
                });
            },
            [c, u, p?.name, p?.id],
        );
    a.useEffect(() => {
        n.current?.scrollTo({ to: 0 }),
            c !== o.current && ((o.current = c), b.$P({ query: c, options: { source: iY.V.APP_DIRECTORY } })),
            b.$P({
                query: c,
                options: { categoryId: h, page: u, pageSize: U.PAGE_SIZE, source: iY.V.APP_DIRECTORY },
                onSuccessCallback: g,
            });
    }, [h, u, c, g]);
    let { fetchState: x, searchResults: A } = (0, d.cf)([et.A], () => ({
            fetchState: et.A.getFetchState({
                query: c,
                categoryId: h,
                page: u,
                pageSize: U.PAGE_SIZE,
                source: iY.V.APP_DIRECTORY,
            }),
            searchResults: et.A.getSearchResults({
                query: c,
                categoryId: h,
                page: u,
                pageSize: U.PAGE_SIZE,
                source: iY.V.APP_DIRECTORY,
            }),
        })),
        v = (0, d.cf)([et.A], () => {
            let e = et.A.getSearchResults({ query: c, source: iY.V.APP_DIRECTORY });
            return null != e ? { [L.lj]: e.totalCount, ...e.countsByCategory } : {};
        }),
        j = (0, iz.A)(A),
        C = a.useMemo(() => (x === R.e.FETCHING ? j : A), [x, j, A]),
        I = a.useCallback(
            (e) => {
                F({ query: c, categoryId: h, page: e });
            },
            [h, c],
        ),
        _ = a.useCallback(
            (e) => {
                k(O.HAw.APP_DIRECTORY_CATEGORY_CLICKED, {
                    category: e.name,
                    category_id: e.id,
                    current_page: D.ev.SEARCH,
                }),
                    F({ query: c, categoryId: e.id });
            },
            [c],
        ),
        E = a.useCallback(
            (e, l) => {
                k(O.HAw.APP_DIRECTORY_SEARCH_RESULT_CLICKED, {
                    current_page: D.ev.SEARCH,
                    application_id: e,
                    load_id: C?.loadId,
                    search_term: c,
                    position: l,
                }),
                    t(e);
            },
            [C?.loadId, t, c],
        ),
        N = V();
    return (0, s.jsxs)("div", {
        className: i0.kL,
        children: [
            (0, s.jsx)(X.A, {
                ref: n,
                onScroll: l,
                children: (0, s.jsxs)("div", {
                    className: i0.WH,
                    children: [
                        i ? null : (0, s.jsx)("div", { className: i0.AW }),
                        (0, s.jsx)(z.A, {
                            children: (0, s.jsxs)("div", {
                                className: i0.hQ,
                                children: [
                                    (0, s.jsx)("div", {
                                        className: i0.$B,
                                        children: (0, s.jsx)(i$, {
                                            countsByCategory: v,
                                            selectedCategoryId: h,
                                            onSelectCategory: _,
                                        }),
                                    }),
                                    (0, s.jsx)(es, {
                                        loading: x === R.e.FETCHING,
                                        children:
                                            x === R.e.FETCHED && (null == C || C?.results.length === 0)
                                                ? (0, s.jsx)(iJ, {
                                                      selectedCategoryId: h ?? N.id,
                                                      searchAllCategories: () => _(N),
                                                  })
                                                : (0, s.jsx)("div", {
                                                      className: i0.Qs,
                                                      children: C?.results.map((e, t) => {
                                                          if (e.type === $.j.APPLICATION) {
                                                              let l = e.data;
                                                              return (0, s.jsx)(
                                                                  eE,
                                                                  {
                                                                      application: l,
                                                                      onSelectApplication: (e) => E(e, t),
                                                                      showCategory: !0,
                                                                  },
                                                                  l.id,
                                                              );
                                                          }
                                                          return null;
                                                      }),
                                                  }),
                                    }),
                                    (0, s.jsx)(Q.m, {
                                        className: i0.JV,
                                        totalCount: Math.min(
                                            (C?.totalPages ?? 0) * U.PAGE_SIZE,
                                            U.MAX_PAGES * U.PAGE_SIZE,
                                        ),
                                        pageSize: U.PAGE_SIZE,
                                        disablePaginationGap: !0,
                                        hideMaxPage: !0,
                                        currentPage: u,
                                        onPageChange: I,
                                    }),
                                ],
                            }),
                        }),
                    ],
                }),
            }),
            (0, s.jsx)("div", {
                className: i0.pn,
                children: (0, s.jsx)("div", {
                    className: i0.jv,
                    children: (0, s.jsx)(iZ, { countsByCategory: v, selectedCategoryId: h, onSelectCategory: _ }),
                }),
            }),
        ],
    });
}
var i7 = l(386607);
function i3(e) {
    let { applicationId: t, hideDetailHeaderButtons: l, onClickBack: i } = e,
        n = (0, d.bG)([R.A], () => R.A.getApplication(t)),
        [o, c] = a.useState(l),
        u = a.useCallback(() => {
            c(l);
        }, [l]);
    return (0, s.jsxs)("div", {
        className: i7._u,
        children: [
            (0, s.jsx)("div", { className: i7.w$, children: (0, s.jsx)(j.XQ, { icon: h.Z, onClick: i }) }),
            (0, s.jsx)(m.D, {
                variant: "heading-lg/semibold",
                color: "text-strong",
                className: i7.Rm,
                lineClamp: 1,
                children: n?.name,
            }),
            (0, s.jsx)("div", {
                className: r()(i7.w$, i7.xY, { [i7.RK]: !l, [i7.jD]: l && !o, [i7.R]: l && o }),
                onTransitionEnd: u,
                children: null != n ? (0, s.jsx)(io, { application: n, size: "sm" }) : null,
            }),
        ],
    });
}
function i9() {
    let e = a.useCallback(() => {
            let e, t;
            (e = new URL(location.href)),
                (t = `?redirect_to=${encodeURIComponent(e.pathname + e.search)}`),
                (0, E.pX)(O.BVt.REGISTER + t);
        }, []),
        t = a.useCallback(() => {
            M();
        }, []);
    return (0, s.jsxs)(p.$T, {
        color: p.Hv.BRAND,
        className: i7.aL,
        children: [
            (0, s.jsx)(g.p, { size: "custom", color: "white", className: i7.wm }),
            (0, s.jsx)(x.E, {
                tag: "span",
                color: "always-white",
                variant: "text-md/normal",
                children: w.intl.string(w.t.Ol3MIt),
            }),
            (0, s.jsxs)("div", {
                className: i7.ll,
                children: [
                    (0, s.jsx)(p.Z_, { onClick: e, noticeType: O.kqX.LOGIN, children: w.intl.string(w.t["825cFy"]) }),
                    (0, s.jsx)(p.Z_, {
                        onClick: t,
                        noticeType: O.kqX.REGISTER,
                        minor: !0,
                        children: w.intl.string(w.t.HAkXzo),
                    }),
                ],
            }),
        ],
    });
}
let i6 = function () {
    var e;
    let t,
        l,
        i,
        n,
        c,
        p,
        g = (0, d.bG)([f.default], () => !f.default.isAuthenticated()),
        { onScroll: x, scrollPosition: R, resetScrollPosition: T } = (0, A.G)(),
        M = (0, D.R4)(),
        B = G(),
        H = M?.type === D.ev.CATEGORY ? M.categoryId : void 0,
        { applicationId: Y, section: z } = M?.type === D.ev.APPLICATION ? M : {},
        { query: X, categoryId: K } = M?.type === D.ev.SEARCH ? M : {},
        W = (0, d.bG)([P.A], () => P.A.getCategories()),
        Z = a.useMemo(() => [(0, L.AU)(), ...W], [W]),
        $ = a.useMemo(() => Z.find((e) => e.id === Number(K)), [Z, K]),
        {
            tabs: q,
            selectedTab: Q,
            onSelectTab: J,
        } = ((e = H?.toString()),
        (t = (0, d.bG)([P.A], () => P.A.getCategories())),
        (l = V()),
        (i = a.useMemo(
            () => [{ id: l.id.toString(), label: l.name }, ...t.map((e) => ({ id: e.id.toString(), label: e.name }))],
            [t, l],
        )),
        (n = (0, D.R4)()),
        (c = a.useCallback(
            (e) => {
                let t = i.find((t) => t.id === e);
                null != t &&
                    (k(O.HAw.APP_DIRECTORY_CATEGORY_CLICKED, {
                        category: t.label,
                        category_id: Number(t.id),
                        current_page: n?.type,
                    }),
                    (function (e) {
                        let { categoryId: t } = e,
                            l = { previousView: (0, D.R4)() };
                        (0, E.pX)(O.BVt.GLOBAL_DISCOVERY_APPS_CATEGORY(t), { state: l });
                    })({ categoryId: e }));
            },
            [n?.type, i],
        )),
        (p = null != e && i.some((t) => t.id === e) ? e : i[0].id),
        { tabs: i, selectedTab: p, onSelectTab: c }),
        [ee, et] = a.useState(!0),
        el = D.h.useField("trackedOpenedFromExternalEntrypoint"),
        ei = D.h.useField("sessionId"),
        es = (0, d.bG)([N.default], () => N.default.getCurrentUser());
    a.useEffect(() => {
        if (!el && null == ei) {
            let e = (0, _.YP)();
            S.default.track(O.HAw.APP_DIRECTORY_OPENED, { source: D.sW.EXTERNAL, session_id: e, user_id: es?.id }),
                D.h.setState({
                    trackedOpenedFromExternalEntrypoint: !0,
                    sessionId: e,
                    entrypoint: { name: D.sW.EXTERNAL },
                    guildId: null,
                });
        }
    }, [ei, el, es?.id]),
        a.useEffect(() => {
            b.bW();
        }, []),
        a.useEffect(() => {
            B && b.wD();
        }, [B]),
        a.useEffect(() => D.tS.setState({ lastItem: M }), [M]);
    let ea = null != Y,
        en = M?.type === D.ev.SEARCH,
        {
            searchQuery: er,
            onSearchTextChange: eo,
            onClearSearch: ec,
            onSearchSubmit: ed,
        } = (function (e) {
            let { initialQuery: t = "" } = e,
                l = y.A.useField("query");
            a.useEffect(() => y.A.setState({ query: t }), [t]);
            let i = a.useCallback((e) => {
                    y.A.setState({ query: e });
                }, []),
                s = a.useCallback(() => {
                    y.A.setState({ query: "" });
                }, []),
                n = a.useCallback(() => {
                    let e = V();
                    k(O.HAw.APP_DIRECTORY_SEARCH_STARTED, {
                        search_term: l,
                        category: e.name,
                        current_page: D.ev.HOME,
                        category_id: e.id,
                    }),
                        F({ query: l });
                }, [l]);
            return { searchQuery: l, onSearchTextChange: i, onClearSearch: s, onSearchSubmit: n };
        })({ initialQuery: X ?? "" }),
        eu = y.A.useField("searchBarState"),
        {
            onTabsAvailableWidthChange: eh,
            onCollapsedSearchBarClick: em,
            onSearchBarBlur: ep,
            tabsClassName: eg,
        } = (0, v.p)({
            isSearchBarVisible: !0,
            isSearchBarEmpty: "" === er.trim(),
            searchBarState: eu,
            setSearchBarState: (e) => y.A.setState({ searchBarState: e }),
        }),
        ex = a.useCallback(
            (e) => {
                J(e), en && ec(), T();
            },
            [en, ec, J, T],
        ),
        eA = a.useCallback(
            (e) =>
                (function (e) {
                    let { applicationId: t } = e,
                        l = { previousView: (0, D.R4)() };
                    (0, E.pX)(O.BVt.GLOBAL_DISCOVERY_APPS_PROFILE(t), { state: l });
                })({ applicationId: e }),
            [],
        ),
        ev = ea || en,
        ej = a.useCallback(() => {
            if (null != (0, D.Bn)()) (0, E.aX)();
            else {
                let e;
                (e = { previousView: (0, D.R4)() }), (0, E.pX)(O.BVt.GLOBAL_DISCOVERY_APPS, { state: e });
            }
        }, []),
        eC = a.useMemo(
            () =>
                (0, o.debounce)((e) => {
                    let { scrollTop: t, offsetHeight: l, scrollHeight: i, location: s } = e;
                    t > 0 &&
                        k(O.HAw.APP_DIRECTORY_PAGE_SCROLLED, {
                            scroll_visible_percentile: (t + l) / i,
                            current_page: s,
                        });
                }, 200),
            [],
        ),
        eI = a.useCallback(
            (e, t) => {
                x(e),
                    eC({
                        scrollTop: e.currentTarget.scrollTop,
                        offsetHeight: e.currentTarget.offsetHeight,
                        scrollHeight: e.currentTarget.scrollHeight,
                        location: t,
                    });
            },
            [eC, x],
        );
    return (0, s.jsxs)("div", {
        className: r()(i7.nw, { [i7.Yz]: g }),
        children: [
            g ? (0, s.jsx)(i9, {}) : null,
            (0, s.jsxs)("div", {
                className: i7.WH,
                children: [
                    (0, s.jsxs)(j.Ay, {
                        variant: ev ? j.Z5.RELATIVE : j.Z5.OVERLAY,
                        children: [
                            !ev && (0, s.jsx)(j.ns, { scrollPosition: R }),
                            ea
                                ? (0, s.jsx)(i3, { applicationId: Y, hideDetailHeaderButtons: ee, onClickBack: ej })
                                : (0, s.jsxs)(s.Fragment, {
                                      children: [
                                          en
                                              ? (0, s.jsx)(j.XQ, { icon: h.Z, onClick: ej })
                                              : (0, s.jsx)(j.T4, { icon: u.k }),
                                          en
                                              ? (0, s.jsx)(m.D, {
                                                    variant: "heading-lg/semibold",
                                                    color: "text-strong",
                                                    className: i7.wL,
                                                    children:
                                                        null != X && "" !== X
                                                            ? w.intl.formatToPlainString(w.t.zHdzqW, { query: X })
                                                            : w.intl.formatToPlainString(w.t.Qhj5Br, {
                                                                  categoryName: $?.name ?? w.intl.string(w.t.E407b7),
                                                              }),
                                                })
                                              : (0, s.jsx)(C.A, {
                                                    className: eg,
                                                    tabs: q,
                                                    selectedTab: Q,
                                                    onTabSelect: ex,
                                                    onAvailableWidthChange: eh,
                                                }),
                                          (0, s.jsx)(I.A, {
                                              query: er,
                                              placeholder: w.intl.string(w.t["5h0QOP"]),
                                              onTextChange: eo,
                                              onClear: ec,
                                              onSubmit: ed,
                                              onCollapsedClick: em,
                                              state: eu,
                                              onBlur: ep,
                                          }),
                                      ],
                                  }),
                        ],
                    }),
                    ea
                        ? (0, s.jsx)(iP, {
                              onScroll: (e) => eI(e, D.ev.APPLICATION),
                              onSelectApplication: eA,
                              applicationId: Y,
                              initialTab: z,
                              onButtonsVisibilityChange: et,
                          })
                        : en
                          ? (0, s.jsx)(i1, { onSelectApplication: eA, onScroll: (e) => eI(e, D.ev.SEARCH) })
                          : Q === U.GLOBAL_DISCOVERY_APPS_FEATURED_CATEGORY_ID.toString()
                            ? (0, s.jsx)(iB, { onScroll: (e) => eI(e, D.ev.HOME), onSelectApplication: eA })
                            : (0, s.jsx)(ey, {
                                  tabId: Number(Q),
                                  onScroll: (e) => eI(e, D.ev.CATEGORY),
                                  onSelectApplication: eA,
                              }),
                ],
            }),
        ],
    });
};
