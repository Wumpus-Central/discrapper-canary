        ...
        s.ZP.useNewNotifications &&
            o.Z.getGuildsArray().forEach((e) => {
                if (
                    ...
                    s.ZP.getMessageNotifications(e.id) === f.bL.ALL_MESSAGES
                )
                    return;
                for (let t of Object.values(s.ZP.getChannelOverrides(e.id)))
                    if (
                        (null != t.message_notifications && t.message_notifications !== f.bL.NULL) ||
                ...
                let n = {};
                for (let r of t)
                    n[r.id] = { flags: (0, d.pq)(s.ZP.getChannelIdFlags(e.id, r.id), _.ic.UNREADS_ALL_MESSAGES) };
