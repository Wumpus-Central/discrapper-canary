    ...
    let {
        ...
        R = r.useMemo(() => {
            let e = [
                {
                    icon: o.T39,
                    text: O.intl.formatToPlainString(O.t.gcdJ8P, { timestamp: g.default.extractTimestamp(w.id) }),
                },
            ];
            return (
                null != w.user &&
                    e.push({
                        icon: o.tBG,
                        text: O.intl.formatToPlainString(O.t.qE7oqq, { user: f.ZP.getUserTag(w.user) }),
                    }),
                e
            );
        }, [w.id, w.user]),
