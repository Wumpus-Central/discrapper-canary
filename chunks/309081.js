n.d(t, { Z: () => C }), n(388685);
var r = n(951288),
    l = n(647438),
    a = n(120356),
    i = n.n(a),
    o = n(442837),
    c = n(481060),
    s = n(596454),
    u = n(907040),
    d = n(906411),
    f = n(592125),
    m = n(358085),
    p = n(79390),
    b = n(35463),
    g = n(489887),
    j = n(185923),
    O = n(957825),
    h = n(388032),
    y = n(514271);
function v(e, t) {
    if (null == e) return {};
    var n,
        r,
        l = (function (e, t) {
            if (null == e) return {};
            var n,
                r,
                l = {},
                a = Object.keys(e);
            for (r = 0; r < a.length; r++) (n = a[r]), t.indexOf(n) >= 0 || (l[n] = e[n]);
            return l;
        })(e, t);
    if (Object.getOwnPropertySymbols) {
        var a = Object.getOwnPropertySymbols(e);
        for (r = 0; r < a.length; r++)
            (n = a[r]), !(t.indexOf(n) >= 0) && Object.prototype.propertyIsEnumerable.call(e, n) && (l[n] = e[n]);
    }
    return l;
}
function P(e) {
    let {
        hasUpload: t,
        mediaUrl: n,
        mediaFilename: l,
        imageClassName: a,
        emoji: i,
        emojiClassName: o,
        fallback: c,
    } = e;
    return t
        ? (0, r.jsx)("img", {
              src: n,
              alt: l,
              className: a,
          })
        : null != i
          ? (0, r.jsx)(s.Z, {
                className: o,
                emojiId: i.id,
                emojiName: i.type === d.B.UNICODE ? i.optionallyDiverseSequence : i.name,
                animated: i.animated,
            })
          : c;
}
function w(e) {
    let { onSelect: t, onEditMedia: n, onDeleteMedia: l, closePopout: a } = e;
    return (0, r.jsx)("div", {
        className: y.menuContainer,
        children: (0, r.jsx)(c.v2r, {
            navId: "poll-media-edit-menu",
            onClose: a,
            "aria-label": h.intl.string(h.t["cV+h7O"]),
            onSelect: t,
            children: (0, r.jsxs)(c.kSQ, {
                children: [
                    (0, r.jsx)(c.sNh, {
                        id: "poll-media-replace",
                        label: h.intl.string(h.t.CZeRhY),
                        action: n,
                    }),
                    (0, r.jsx)(c.sNh, {
                        id: "poll-media-delete",
                        label: h.intl.string(h.t.IhMxgo),
                        action: l,
                        color: "danger",
                    }),
                ],
            }),
        }),
    });
}
function x(e) {
    let {
            channelId: t,
            localCreationAnswerId: n,
            buttonImage: a,
            onEmojiSelect: s,
            onEmojiRemove: d,
            answerIndex: m,
            shouldShowEmojiPicker: g,
            toggleEmojiPicker: x,
            containerRef: C,
        } = e,
        k = l.useRef(null),
        R = (0, o.e7)([f.Z], () => f.Z.getChannel(t)),
        {
            emoji: E,
            isLoadingMedia: _,
            hasUpload: S,
            mediaUrl: D,
            mediaFilename: A,
        } = (0, b.Z)({
            channelId: t,
            localCreationAnswerId: n,
            image: a,
        }),
        I = S || null != E,
        N = l.useCallback(() => {
            d(m);
        }, [d, m]),
        L = l.useMemo(
            () =>
                S
                    ? h.intl.formatToPlainString(h.t.vcC7Qk, {
                          imageName: (0, p.fw)(A),
                          answerNumber: m + 1,
                      })
                    : null != E
                      ? h.intl.formatToPlainString(h.t.ncOAhY, {
                            emojiName: E.name,
                            answerNumber: m + 1,
                        })
                      : h.intl.formatToPlainString(h.t.emdpNj, { answerNumber: m + 1 }),
            [S, E, m, A],
        ),
        T = l.useCallback(
            (e) => {
                let { closePopout: t } = e;
                return (0, r.jsx)("div", {
                    className: y.emojiPicker,
                    children: (0, r.jsx)(u.Z, {
                        channel: R,
                        pickerIntention: j.Hz.POLLS,
                        closePopout: t,
                        onNavigateAway: t,
                        onSelectEmoji: (e) => {
                            let { emoji: n, willClose: r } = e;
                            null != n && s(n, m), r && t();
                        },
                    }),
                });
            },
            [R, s, m],
        ),
        U = l.useCallback(
            (e) => {
                let { closePopout: t } = e;
                return (0, r.jsx)(w, {
                    onSelect: t,
                    onEditMedia: x,
                    onDeleteMedia: N,
                    closePopout: t,
                });
            },
            [N, x],
        ),
        Z = _
            ? (0, r.jsx)(c.$jN, { className: y.spinnerWrapperDefault })
            : (0, r.jsx)(P, {
                  hasUpload: S,
                  mediaUrl: D,
                  mediaFilename: A,
                  imageClassName: i()(y.media, y.gifDefault),
                  emoji: E,
                  emojiClassName: i()(y.media, y.emojiDefault),
                  fallback: (0, r.jsx)(c.EO4, {
                      size: "sm",
                      color: "currentColor",
                      className: y.expressionPickerIconDefault,
                  }),
              }),
        B = i()(O.CT, y.expressionPickerButtonDefault, { [y.canEditMedia]: I });
    return (0, r.jsx)(c.yRy, {
        targetElementRef: C,
        renderPopout: T,
        shouldShow: g,
        onRequestClose: x,
        animation: c.yRy.Animation.NONE,
        position: "bottom",
        spacing: 2,
        children: (e) => {
            var { "aria-controls": t, "aria-expanded": n } = e;
            return (
                v(e, ["aria-controls", "aria-expanded"]),
                (0, r.jsx)(c.yRy, {
                    targetElementRef: k,
                    animation: c.yRy.Animation.NONE,
                    position: "bottom",
                    renderPopout: U,
                    children: (e) => {
                        var l,
                            a,
                            { onClick: i, "aria-controls": o, "aria-expanded": s } = e,
                            u = v(e, ["onClick", "aria-controls", "aria-expanded"]);
                        return (0, r.jsxs)(
                            c.P3F,
                            ((l = (function (e) {
                                for (var t = 1; t < arguments.length; t++) {
                                    var n = null != arguments[t] ? arguments[t] : {},
                                        r = Object.keys(n);
                                    "function" == typeof Object.getOwnPropertySymbols &&
                                        (r = r.concat(
                                            Object.getOwnPropertySymbols(n).filter(function (e) {
                                                return Object.getOwnPropertyDescriptor(n, e).enumerable;
                                            }),
                                        )),
                                        r.forEach(function (t) {
                                            var r;
                                            (r = n[t]),
                                                t in e
                                                    ? Object.defineProperty(e, t, {
                                                          value: r,
                                                          enumerable: !0,
                                                          configurable: !0,
                                                          writable: !0,
                                                      })
                                                    : (e[t] = r);
                                        });
                                }
                                return e;
                            })({ innerRef: k }, u)),
                            (a = a =
                                {
                                    className: B,
                                    onClick: I && !g ? i : x,
                                    "aria-label": L,
                                    "aria-controls": null != t ? t : o,
                                    "aria-expanded": n || s,
                                    children: [
                                        Z,
                                        I &&
                                            (0, r.jsx)(c.vdY, {
                                                size: "md",
                                                color: "currentColor",
                                                className: y.editIcon,
                                                "aria-hidden": !0,
                                            }),
                                    ],
                                }),
                            Object.getOwnPropertyDescriptors
                                ? Object.defineProperties(l, Object.getOwnPropertyDescriptors(a))
                                : (function (e, t) {
                                      var n = Object.keys(e);
                                      if (Object.getOwnPropertySymbols) {
                                          var r = Object.getOwnPropertySymbols(e);
                                          n.push.apply(n, r);
                                      }
                                      return n;
                                  })(Object(a)).forEach(function (e) {
                                      Object.defineProperty(l, e, Object.getOwnPropertyDescriptor(a, e));
                                  }),
                            l),
                        );
                    },
                })
            );
        },
    });
}
function C(e) {
    let {
            channelId: t,
            answer: n,
            index: a,
            isLastAnswer: i,
            onAnswerTextChange: o,
            onEmojiSelect: s,
            onEmojiRemove: u,
            canRemoveAnswer: d,
            onRemoveAnswer: f,
            addAnswer: b,
            submitPoll: j,
            error: O,
            ref: y,
            answerRowRefs: v,
        } = e,
        P = l.useRef(null),
        w = l.useRef(null),
        C = l.useRef(null);
    l.useImperativeHandle(y, () => ({
        focusDeleteButton: () => {
            var e;
            return null == (e = w.current) ? void 0 : e.focus();
        },
        focusInput: () => {
            var e;
            return null == (e = C.current) ? void 0 : e.focus();
        },
    }));
    let [k, R] = l.useState(!1),
        E = l.useCallback(() => {
            R((e) => !e);
        }, []),
        _ = l.useCallback(
            (e) => {
                let t = e.ctrlKey && !(e.altKey || e.metaKey || e.shiftKey),
                    r = e.metaKey && !(e.altKey || e.ctrlKey || e.shiftKey);
                switch (e.key.toLowerCase()) {
                    case "enter":
                        var l;
                        i &&
                            (e.preventDefault(),
                            ((0, p.cS)(n) || "macos" !== (0, m.getOS)() ? t : r) ? (e.stopPropagation(), j()) : b()),
                            null == (l = v.current[a + 1]) || l.focusInput();
                        break;
                    case "e":
                        ("macos" === (0, m.getOS)() ? r : t) && (e.preventDefault(), e.stopPropagation(), E());
                }
            },
            [b, n, v, a, i, j, E],
        ),
        S = (0, r.jsx)(x, {
            containerRef: P,
            channelId: t,
            buttonImage: n.image,
            onEmojiSelect: s,
            onEmojiRemove: u,
            localCreationAnswerId: n.localCreationAnswerId,
            answerIndex: a,
            shouldShowEmojiPicker: k,
            toggleEmojiPicker: E,
        });
    return (0, r.jsx)("div", {
        ref: P,
        children: (0, r.jsx)(c.oil, {
            error: O,
            leading: {
                type: "emoji",
                button: S,
            },
            trailing: d
                ? {
                      "aria-label": h.intl.formatToPlainString(h.t["22fjER"], { answerNumber: a + 1 }),
                      icon: c.XHJ,
                      onClick: () => f(a),
                      buttonRef: w,
                  }
                : void 0,
            "aria-label": h.intl.formatToPlainString(h.t["3+V8Gx"], { answerNumber: a + 1 }),
            placeholder: h.intl.string(h.t.NNHVlp),
            value: n.text,
            onChange: (e) =>
                o({
                    text: e,
                    index: a,
                    localCreationAnswerId: n.localCreationAnswerId,
                }),
            onKeyDown: _,
            maxLength: g.WA,
            inputRef: C,
        }),
    });
}
