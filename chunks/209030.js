n.d(t, { A: () => et, L: () => ee });
var l = n(627968),
    a = n(64700),
    i = n(503698),
    s = n.n(i),
    r = n(997101),
    o = n(17928),
    u = n(661531),
    c = n(289873),
    d = n(885574),
    p = n(834730),
    m = n(683071),
    h = n(534514),
    A = n(457287),
    C = n(821891),
    E = n(953689),
    y = n(546605),
    P = n(531260),
    S = n(364995),
    _ = n(284009),
    T = n.n(_),
    f = n(937008),
    N = n(156312),
    x = n(566980),
    I = n(800471),
    g = n(543767),
    v = n(299301),
    M = n(97352),
    b = n(94420),
    R = n(666646),
    j = n(699595),
    L = n(788868),
    O = n(71867),
    D = n(222707),
    w = n(421094),
    U = n(216641),
    k = n(615405),
    Y = n(79387),
    G = n(67480),
    F = n(45938),
    B = n(927578),
    W = n(242874),
    H = n(735164),
    V = n(778307),
    K = n(75825),
    Z = n(218075),
    q = n(337092),
    z = n(344159),
    $ = n(285719),
    J = n(818348),
    X = n(985018),
    Q = n(814304);
let ee = (e) => {
    let {
            selectedPlanId: t,
            paymentSources: n,
            priceOptions: l,
            trialId: i,
            metadata: s,
            currencies: r,
            onCurrencyChange: u,
            isTrial: c = !1,
        } = e,
        d = (0, b.t4)((e) => e.selectedSkuId),
        {
            isEmbeddedIAP: p,
            activeSubscription: m,
            defaultPlanId: h,
            isPremium: A,
            startedPaymentFlowWithPaymentSourcesRef: C,
            inReverseTrial: E,
            hasPaymentSources: _,
            enablePremiumBrandRefresh: O,
            isPremiumGroupPurchase: D,
            isEligibleForDiscount: w,
            displayCurrency: k,
        } = (0, N.P5)(),
        { checkoutPaymentSources: Y } = (0, S.t6)(),
        { isGift: W, giftRecipient: H, selectedGiftStyle: V } = (0, f.Pv)(),
        K = d ?? "",
        Z = (0, o.bG)([G.A], () => G.A.get(K), [K]),
        q = Z?.eligiblePaymentGateways,
        $ = (0, I.vT)({ isTrial: c, isGift: W, selectedSkuId: d, startedPaymentFlowWithPaymentSources: C.current }),
        {
            subscriptionPlan: J,
            purchaseDisabled: Q,
            checkoutInvoicePreview: ee,
            invoiceSummaryTypeWithPreview: et,
            proratedInvoicePreview: en,
            renewalInvoicePreview: el,
            discountInvoicePreview: ea,
            subscriptionPeriodEnd: ei,
        } = ((e) => {
            let {
                    selectedPlanId: t,
                    priceOptions: n,
                    trialId: l,
                    metadata: i,
                    isInOneStepSubscriptionCheckout: s = !1,
                    isTrial: r = !1,
                } = e,
                u = (0, b.t4)((e) => e.selectedSkuId),
                {
                    activeSubscription: c,
                    contextMetadata: d,
                    setPurchasePreviewError: p,
                    isEligibleForDiscount: m,
                } = (0, N.P5)(),
                { isGift: h } = (0, f.Pv)(),
                {
                    subscriptionPlan: A,
                    purchaseDisabled: C,
                    newItems: E,
                } = ((e) => {
                    let { selectedPlanId: t } = e,
                        n = (0, b.t4)((e) => e.purchaseState),
                        l = (0, o.bG)([M.A], () => M.A.get(t));
                    T()(null != l, "Missing plan");
                    let a = [{ planId: l.id, quantity: 1 }],
                        i = n === x.h.PURCHASING || n === x.h.COMPLETED;
                    return { subscriptionPlan: l, purchaseState: n, purchaseDisabled: i, newItems: a };
                })({ selectedPlanId: t }),
                [y, P] = (0, g.Kq)({
                    items: E,
                    renewal: !1,
                    preventFetch: h || C,
                    applyEntitlements: !0,
                    paymentSourceId: n.paymentSourceId,
                    currency: n.currency,
                    trialId: l,
                    metadata: i,
                }),
                S = a.useMemo(() => {
                    if (r && null != y) return y;
                }, [r, y]),
                [_, O] = (0, g.Kq)({
                    subscriptionId: c?.id,
                    items: E,
                    renewal: !0,
                    preventFetch: h || C,
                    trialId: l,
                    paymentSourceId: n.paymentSourceId,
                    currency: n.currency,
                    metadata: i,
                }),
                [D, w] = (0, g.FP)({
                    paymentSourceId: n.paymentSourceId,
                    skuId: u,
                    subscriptionPlanId: t,
                    currency: n.currency,
                    preventFetch: !h || C,
                    loadId: d.loadId,
                }),
                { checkoutInvoicePreview: U, checkoutInvoiceError: k } = a.useMemo(
                    () =>
                        h
                            ? { checkoutInvoicePreview: D, checkoutInvoiceError: w }
                            : { checkoutInvoicePreview: y, checkoutInvoiceError: P },
                    [h, D, y, w, P],
                ),
                { primaryInvoiceError: Y } = (0, R.OQ)({
                    checkoutInvoicePreview: U,
                    checkoutInvoiceError: k,
                    renewalInvoicePreview: _,
                    renewalInvoiceError: O,
                }),
                [G, F] = (0, g.Kq)({
                    items: [{ planId: L.gD.PREMIUM_MONTH_TIER_2, quantity: 1 }],
                    renewal: !1,
                    preventFetch: !m,
                    trialId: l,
                    paymentSourceId: n.paymentSourceId,
                    currency: n.currency,
                    metadata: i,
                });
            (0, j.c)(G, m);
            let B = a.useMemo(() => Y ?? F, [Y, F]);
            a.useEffect(() => {
                p(B);
            }, [B, p]);
            let W = a.useMemo(() => (null != y ? y.subscriptionPeriodEnd : void 0), [y]),
                H = a.useMemo(() => (0, I.UB)(s, y, A), [s, y, A]),
                V = a.useMemo(
                    () =>
                        (0, v.yf)({
                            error: B,
                            isGift: h,
                            oneTimePurchaseNitroGiftInvoicePreview: D,
                            trialInvoicePreview: S,
                            proratedInvoicePreview: y,
                            renewalInvoicePreview: _,
                            planSwitchLoading: H,
                        }),
                    [B, h, D, S, y, _, H],
                );
            return {
                checkoutInvoicePreview: U,
                invoiceSummaryTypeWithPreview: V,
                discountInvoicePreview: G,
                oneTimePurchaseNitroGiftInvoicePreview: D,
                trialInvoicePreview: S,
                proratedInvoicePreview: y,
                renewalInvoicePreview: _,
                purchaseDisabled: C,
                subscriptionPlan: A,
                invoiceError: B,
                subscriptionPeriodEnd: W,
            };
        })({
            selectedPlanId: t,
            priceOptions: l,
            trialId: i,
            metadata: s,
            isInOneStepSubscriptionCheckout: $,
            isTrial: c,
        }),
        es = W && (0, F.Ik)(H),
        er = l.paymentSourceId,
        eo = (0, U.g)(n, er),
        eu = (0, y.vg)("PremiumSubscriptionReview") ? (Y?.find((e) => e.id === er)?.relocationCountry ?? null) : null,
        { hasEntitlements: ec, entitlements: ed } = (0, z.X)(J.id, W),
        ep = (0, B.J$)(l.paymentSourceId),
        em = (0, P.A)({ forceFetch: !1, excludeReverseTrial: !1, excludeReverseTrialFromCountdown: !0 }),
        eh = !$ && (em.isFractionalPremiumActive || E) && L.JM.has(t),
        eA = a.useMemo(() => (0, B.Tm)({ skuId: d, isPremium: A, defaultPlanId: h }), [d, h, A]),
        eC = a.useMemo(
            () => ({
                label: X.intl.string(X.t["/AAR02"]),
                selectedCurrency: l.currency ?? k,
                currencies: r,
                onChange: u,
            }),
            [l.currency, k, r, u],
        ),
        eE = a.useMemo(
            () => (ec && null == er ? X.intl.format(X.t["2wPRSF"], { months: ed.length }) : null),
            [ec, er, ed],
        );
    return {
        disabled: Q,
        activeSubscription: m,
        subscriptionPeriodEnd: ei,
        plan: J,
        premiumPlanOptions: eA,
        checkoutInvoicePreview: ee,
        invoiceSummaryTypeWithPreview: et,
        discountInvoicePreview: ea,
        renewalInvoicePreview: el,
        proratedInvoicePreview: en,
        isGift: W,
        isEmbeddedIAP: p,
        paymentSourceType: eo,
        giftRecipient: H,
        selectedGiftStyle: V,
        isInOneStepSubscriptionCheckout: $,
        shouldAllowPlanSelect: $ && !D,
        hasPaymentSources: _,
        paymentSourceId: er,
        eligiblePaymentGateways: q,
        inReverseTrial: E,
        fractionalPremiumInfo: em,
        showFractionalPremiumBannerInInvoiceSummary: eh,
        isPrepaid: ep,
        isCustomGift: es,
        enablePremiumBrandRefresh: O,
        isPremiumGroupPurchase: D,
        isEligibleForDiscount: w,
        hasEntitlements: ec,
        entitlements: ed,
        currencyDropdownProps: eC,
        paymentSourceOptionalWarningCopy: eE,
        selectedSourceRelocationCountry: eu,
    };
};
function et(e) {
    let {
            selectedPlanId: t,
            planGroup: n,
            paymentSources: i,
            priceOptions: o,
            currencies: y,
            onCurrencyChange: P,
            handlePaymentSourceAdd: S,
            setHasAcceptedTerms: _,
            legalTermsNodeRef: T,
            hasLegalTermsFlash: f,
            trialId: N,
            trialFooterMessageOverride: x,
            reviewWarningMessage: I,
            metadata: g,
            hideSubscriptionDetails: M,
            referralTrialOfferId: b,
            isTrial: R = !1,
            isDiscount: j = !1,
            handleClose: L,
        } = e,
        {
            disabled: U,
            isEmbeddedIAP: G,
            paymentSourceType: F,
            giftRecipient: z,
            selectedGiftStyle: et,
            subscriptionPeriodEnd: en,
            premiumPlanOptions: el,
            shouldAllowPlanSelect: ea,
            hasPaymentSources: ei,
            paymentSourceId: es,
            eligiblePaymentGateways: er,
            inReverseTrial: eo,
            fractionalPremiumInfo: eu,
            plan: ec,
            isGift: ed,
            invoiceSummaryTypeWithPreview: ep,
            checkoutInvoicePreview: em,
            renewalInvoicePreview: eh,
            showFractionalPremiumBannerInInvoiceSummary: eA,
            isPrepaid: eC,
            isCustomGift: eE,
            enablePremiumBrandRefresh: ey,
            isPremiumGroupPurchase: eP,
            hasEntitlements: eS,
            currencyDropdownProps: e_,
            paymentSourceOptionalWarningCopy: eT,
            selectedSourceRelocationCountry: ef,
        } = ee({
            selectedPlanId: t,
            paymentSources: i,
            priceOptions: o,
            trialId: N,
            metadata: g,
            isTrial: R,
            currencies: y,
            onCurrencyChange: P,
        }),
        eN = a.useMemo(() => (0, B.l6)(o, em?.checkoutContext?.available_plans), [o, em]);
    if (null != ep && ep.type === v.N$.LOADING)
        return (0, l.jsx)("div", { className: Q.zp, children: (0, l.jsx)(c.y, {}) });
    let ex = (0, l.jsx)(v.eb, {
            invoiceSummaryTypeWithPreview: ep,
            priceOptions: o,
            trialFooterMessageOverride: x,
            hideSubscriptionDetails: M,
            referralTrialOfferId: b,
            isTrial: R,
            inReverseTrial: eo,
            fractionalPremiumInfo: eu,
            plan: ec,
            showFractionalPremiumBanner: eA,
            isPrepaid: eC,
            isCustomGift: eE,
            enablePremiumBrandRefresh: ey,
        }),
        eI = r.M.EEA_COUNTRIES.has(k.A.ipCountryCodeWithFallback),
        eg = X.intl.formatToPlainString(X.t["sBpy9/"], { planName: ec.name });
    ed && !eE
        ? (eg = X.intl.string(X.t.J5a0eb))
        : ed && eE
          ? (eg = "")
          : (0, B.ys)(ec.id) && (eg = B.Ay.getBillingReviewSubheader(null, ec));
    let ev = null != er && er.length > 0 && (es === Y.B || null === F) && ei ? Z.fU.SELECT_PAYMENT_METHOD : void 0;
    return (0, l.jsxs)("div", {
        children: [
            (0, l.jsx)(Z.je, { paymentRestrictionBannerType: ev }),
            null != I &&
                (0, l.jsxs)("div", {
                    className: Q.Je,
                    children: [
                        (0, l.jsx)(d.m, {
                            size: "custom",
                            color: u.A.unsafe_rawColors.YELLOW_300.css,
                            width: 20,
                            height: 20,
                        }),
                        (0, l.jsx)(p.E, { className: Q.CJ, variant: "text-sm/normal", children: I }),
                    ],
                }),
            eP &&
                (0, l.jsx)("div", {
                    className: Q.Ni,
                    children: (0, l.jsx)(m.w, {
                        type: "info",
                        children: (0, l.jsx)(p.E, { variant: "text-sm/medium", children: (0, D.Nn)() }),
                    }),
                }),
            ea &&
                (0, l.jsxs)("div", {
                    children: [
                        (0, l.jsx)(O.P, { planSkuId: ec?.skuId, referralTrialOfferId: b }),
                        (0, l.jsx)(q.$p, {
                            disabled: U,
                            planOptions: el,
                            selectedPlanId: t,
                            planGroup: n,
                            subscriptionPeriodEnd: en,
                            showTotal: !1,
                            priceOptions: eN,
                            handleClose: L,
                        }),
                        (0, l.jsx)(H.pK, {}),
                    ],
                }),
            !R && "" !== eg && (0, l.jsx)(h.D, { variant: "heading-md/semibold", className: Q.wx, children: eg }),
            eE &&
                null != et &&
                (0, l.jsxs)(l.Fragment, {
                    children: [
                        (0, l.jsx)(K.A, {
                            defaultAnimationState: W.oA.LOOP,
                            giftStyle: et,
                            shouldAnimate: !0,
                            className: Q.Os,
                        }),
                        (0, l.jsx)($.Z, { giftRecipient: z }),
                    ],
                }),
            ex,
            (0, l.jsxs)("div", {
                className: Q.LC,
                children: [
                    (0, l.jsx)(w.K, {
                        label: X.intl.string(R ? X.t["YH7B+D"] : X.t["mmDvV+"]),
                        handlePaymentSourceAdd: S,
                        isTrial: R,
                        isDiscount: j,
                        disabled: U,
                        hasEntitlements: eS,
                        eligiblePaymentGateways: er,
                        shouldUseUnifiedCheckoutUI: !1,
                        paymentSourceDropdownClassName: s()({ [Q.E4]: ey }),
                        paymentSourceDropdownPrependOption:
                            eS && !R ? { label: X.intl.string(X.t.IGU7El), value: null } : null,
                    }),
                    null != eT ? (0, l.jsx)("div", { className: Q.QN, children: eT }) : null,
                    j
                        ? null
                        : (0, l.jsx)(A.f, {
                              currencies: e_.currencies,
                              className: Q.p2,
                              children: (0, l.jsx)(A.A, { ...e_ }),
                          }),
                ],
            }),
            (0, l.jsx)(V.A, {
                isActive: f,
                ref: T,
                children: (0, l.jsx)(E.A, {
                    onChange: _,
                    forceShow: !0,
                    finePrint:
                        x ??
                        (0, l.jsx)(C.A, {
                            hide: R || j,
                            subscriptionPlan: ec,
                            renewalInvoice: eh,
                            isGift: ed,
                            paymentSourceType: F,
                            isEmbeddedIAP: G,
                            basePrice: (0, B.y8)(ec.id, !1, ed, eN),
                            willRelocateStoreCountry: null != ef,
                        }),
                    showPricingLink: ec.currency !== J.Yr.USD,
                    showWithdrawalWaiver: eI,
                    disabled: U,
                    isTrial: R && null == x,
                    isDiscount: j,
                    subscriptionPlan: ec,
                    isGift: ed,
                }),
            }),
        ],
    });
}
