...
let g = (e, t, n, r) => {
    ...
    E = function (e) {
        let t,
            ...
            s,
            E,
            { subscriptionPlan: b, isGift: y, isTrial: v, isOrbCheckout: O, isEmbeddedIAP: I, renewalInvoice: S, paymentSourceType: T, hide: A, purchaseType: N, productLine: C, basePrice: R, currentSubscription: P, skuId: w } = e,
        ...
        let x = (0, a.K)({
            ...
            U = '';
        if (O) {
            j = h.intl.format(h.t.xlJyOD, {
                virtualGoodsURL: f.EYA.PAID_TERMS_VIRTUAL_GOODS,
                paidURL: f.EYA.PAID_TERMS
            });
            let e = '';
            (e = w === _.Vt.ORB_PROFILE_BADGE ? h.intl.string(h.t.APcKRk) : w === _.Vt.FRACTIONAL_PREMIUM ? h.intl.string(h.t.FhJ74u) : h.intl.string(h.t.jCRYDw)), Array.isArray(j) ? (j = [...j, ' '.concat(e)]) : (j += ' '.concat(e));
        } else if (I)
            if (null != s && (null == S ? void 0 : S.subscriptionPeriodEnd) != null) j = h.intl.format(h.t['2VPTa2'], { subtotalRate: E });
            else
                switch (t) {
                    case p.rV.MONTH:
                        j = 1 === n ? h.intl.format(h.t['+ywPmZ'], {}) : h.intl.format(h.t.oBwZfn, { intervalCount: n });
                        break;
                    case p.rV.YEAR:
                        j = h.intl.format(h.t.y9gsX1, {});
                        break;
                    case void 0:
                        j = h.intl.format(h.t.UxGFHx, {});
                        break;
                    default:
            ...
            switch (C) {
                case f.POd.COLLECTIBLES:
                    (U = M), (j = y ? h.intl.format(h.t.ofqyUl, { paidURL: f.EYA.PAID_TERMS }) : h.intl.format(h.t.G3l0s7, { paidURL: f.EYA.PAID_TERMS }));
                    break;
                case f.POd.GUILD_PRODUCT:
                    (U = M), (j = h.intl.format(h.t['GEAQ+v'], { paidURL: f.EYA.PAID_TERMS }));
                    break;
                default:
                    (U = k), (j = h.intl.string(h.t['9/siSU']));
            }
        else if (null == b || y)
            switch ((y && (U = M), t)) {
                case p.rV.MONTH:
                    (j = y ? h.intl.string(h.t.IjNapq) : h.intl.string(h.t['/sGXPj'])), (j = y ? h.intl.string(h.t.IjNapq) : 1 === n ? h.intl.string(h.t['/sGXPj']) : h.intl.formatToPlainString(h.t.Fqjiho, { intervalCount: n }));
                    break;
                case p.rV.YEAR:
                    j = y ? h.intl.string(h.t.vAfbGx) : h.intl.string(h.t.IGzFc3);
                    break;
                case void 0:
                    j = '';
                    break;
