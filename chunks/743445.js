...
class b extends m.Ay {
    ...
    retryOpts() {
        return this.item.platform === m.xz.REACT_NATIVE
            ? {
                  ...
                  backoff: new s.A(0.5 * g.A.Millis.SECOND, 30 * g.A.Millis.MINUTE),
                  retries: 12,
              }
    ...
    createAttachmentUrlRetryOpts() {
        return this.item.platform === m.xz.REACT_NATIVE
            ? {
                  ...
                  backoff: new s.A(0.5 * g.A.Millis.SECOND, 60 * g.A.Millis.SECOND),
                  retries: 8,
              }
            : this.retryOpts();
    }
    ...
    async upload() {
        ...
        if ((this.currentSize ?? 0) > r) return void this.handleError(N.t02.ENTITY_TOO_LARGE);
    ...
    static async tryConvertToWebP(e, t, i) {
        ...
        if (!a.enabled) return y.warn(`webp conversion skipped for ${i}: not enabled`), null;
    ...
    async ensureFreshResponseUrl() {
        ...
        let e = await i.getUploadPayload(this),
            t = (0, O.B)(this.item.target).getCreateAttachmentURL(this.channelId),
            n = await o.Bo.post({
                url: t,
                body: { files: [e] },
                ...this.createAttachmentUrlRetryOpts(),
