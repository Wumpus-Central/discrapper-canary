...
class L extends Error {
    ...
    constructor(e, t = {}) {
        const { cause: n, response: i } = t,
            ...
            a = "server_error" === l || "client_error" === l ? `${e}:${l}:status_${i?.status ?? 0}` : `${e}:${l}`;
        super(a, { cause: n }),
    ...
    static getErrorKind(e, t) {
        ...
        return i ? "server_error" : l ? "client_error" : a ? "network_error" : "unknown";
    }
...
class U extends m.Ay {
    ...
    constructor(e, t, n, i) {
        super(e),
            ...
            null != this.origin &&
                (this.uploadAnalytics.origin = "string" == typeof this.origin ? this.origin : m.Cj[this.origin]),
    ...
    supportsResume() {
        return this._libdiscoreEnabled || this.item.platform !== m.xz.REACT_NATIVE;
    }
    ...
    async startOrResumeUpload(e) {
        ...
        if (
            (y.log(`Attempting to upload attachment with resumeFrom: ${t} and attempts: ${this.uploadAttempts}`), t > 0)
        )
        ...
        e.onRequestProgress = r().throttle(this.createResumeAwareProgressFn(t), 50);
        let n = await this._uploadHttpClient
            .doUpload(e, { fileByteRange: { start: t } })
            .catch(L.rejectionHandler("upload"));
        if (200 !== n.status && 201 !== n.status) throw new L("upload", { response: n });
        return n;
    }
    async uploadFileWithResumption(e, t, n) {
        let { timeout: i, backoff: l, retries: a } = this.retryOpts(),
            s = {
                ...
                signal: this._abortController.signal,
                onRequestProgress: r().throttle(this.createResumeAwareProgressFn(0), 50),
                retries: 0,
                rejectWithError: !0,
                timeout: i,
            },
        ...
        for (; this.uploadAttempts <= a && !this._aborted; ) {
            ...
            try {
                ...
                if (e instanceof L && e.canRetry()) {
                    y.warn(`Error uploading ${this.id}: ${e.message}, attempting resumption`),
                        (this.uploadAnalytics.uploadResumptionReason = e.messageShort),
                        await I.A.awaitOnline();
                    let t = l.fail();
                    y.log(`Waiting ${t}ms before attachment upload attempt ${this.uploadAttempts + 1}`),
                        await new Promise((e) => setTimeout(e, t));
                } else throw (y.warn(`Unrecoverable error uploading ${this.id}: ${e.message}`), e);
            }
        }
    ...
    async upload() {
        ...
        if (this.allowOptimization && this.item.platform === m.xz.WEB) {
            let e = await U.tryConvertToWebP(this.item.file, () => this._aborted, this.id);
            null != e &&
        ...
        if (null == e.filename || "" === e.filename) {
            y.error("File does not have a filename.", JSON.stringify(e)), this.handleError(N.t02.INVALID_FILE_ASSET);
            return;
        }
        ...
        let n = (0, E.R8)({ location: "CloudUpload.upload.postCompressionCheck" }),
            l = (0, E.Jy)(n, t.getMaxFileSize(this.channelId));
        if ((this.currentSize ?? 0) > l) return void this.handleError(N.t02.ENTITY_TOO_LARGE);
        ...
        try {
            ...
            e !== N.t02.ENTITY_TOO_LARGE &&
                (y.error(`Requesting upload url failed with code ${e ?? JSON.stringify(t.body)} for ${this.id}`),
        ...
        try {
            ...
            this.isCancelled()
                ...
                : (y.info(`Error: status ${e.status} for ${this.id}`), this.handleError(e));
        }
    }
    async reactNativeCompressAndExtractData() {
        if (!(0, C.B)(this.item.target).shouldReactNativeCompressUploads)
            return (
                ...
                y.log("reactNativeCompressAndExtractData() disabled by upload target"),
                this
            );
        if (!0 === this.reactNativeFilePrepped)
            return (
                (this.uploadAnalytics.fileAlreadyPrepped = !0),
                y.log(`reactNativeCompressAndExtractData() file already prepped - ${this.id}`),
                this
            );
        ...
        if (null == e || null == e.file) return y.error(`Failed to get compressed file for ${this.id}`), this;
        let t = e.uri,
            n = e.file.name;
        if (
            ((0, m.Sm)(e.file) &&
            ...
            throw (
                (y.error(`Insufficient file data: ${{ filename: n, uri: t, type: e.file.type }} for ${this.id}`),
        ...
        if (((this.postCompressionSize = a), (this.currentSize = a), null == a))
            throw (y.error(`Size missing from file data for ${this.id}`), Error("Size missing from file data"));
        return (
            y.log(
    ...
    static async tryConvertToWebP(e, t, i) {
        ...
        if (!r.enabled) return y.warn(`webp conversion skipped for ${i}: not enabled`), null;
        if (null == e) return y.warn(`webp conversion skipped for ${i}: no file`), null;
        ...
        let s = performance.now(),
            o = { compressTimeMs: 0 };
        try {
            let r = await Promise.all([n.e("96904"), n.e("87096")]).then(n.bind(n, 989707));
            if (((a = r.ConversionFailureReason), (l = await r.maybeConvertToWebP(e)), t())) return null;
            if (l.success && null != l.convertedBlob)
                ...
                y.log(`webp conversion skipped for ${i}: ${e}`), (o.conversionFailureReason = e);
            }
        } catch (e) {
    ...
    async delete() {
        ...
        let e = (0, C.B)(this.item.target).getDeleteUploadURL(this.uploadedFilename);
        try {
            await o.Bo.del(e);
        } catch {}
    }
    ...
    async ensureFreshResponseUrl() {
        ...
        let e = await i.getUploadPayload(this),
            t = (0, C.B)(this.item.target).getCreateAttachmentURL(this.channelId),
            n = await o.Bo.post({
                url: t,
                body: { files: [e] },
                ...this.createAttachmentUrlRetryOpts(),
        ...
        if (!n.ok || n.body?.attachments?.[0] == null) throw new L("upload", { response: n });
        this.setResponseUrl(n.body.attachments[0].upload_url),
            this.setUploadedFilename(n.body.attachments[0].upload_filename);
    }
    setStatus(e) {
